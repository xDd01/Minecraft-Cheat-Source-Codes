...
public abstract class Entity implements ICommandSender
{
    ...
    public boolean isMovingXZ(final double abs) {
        return Math.abs(this.motionX) > abs || Math.abs(this.motionZ) > abs;
    }
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            double var12 = z;
            final EventSafeWalk eventSF = new EventSafeWalk();
            Base.INSTANCE.getEventManager().emit(eventSF);
            final boolean var13 = (this.onGround && this.isSneaking() && this instanceof EntityPlayer) || eventSF.isCancelled();
            if (var13) {
    ...
    public Vec3 getLook(final float p_70676_1_) {
        if (p_70676_1_ == 1.0f) {
            return this.getRotationVec(this.rotationPitch, this.rotationYaw);
        }
    ...
    public Vec3 getPositionEyes(final float p_174824_1_) {
        if (p_174824_1_ == 1.0f) {
    ...
    public MovingObjectPosition rayTrace(final double p_174822_1_, final float p_174822_3_) {
        final Vec3 var4 = this.getPositionEyes(p_174822_3_);
    ...
    public Entity rayTraceEntity(final double distance, final float yaw, final float pitch) {
        final Vec3 RotVec = Minecraft.getMinecraft().thePlayer.getRotationVec(yaw, pitch);
        for (double dist = 0.0; dist < distance; dist += 0.2) {
            RotVec.rotateYaw(yaw);
            RotVec.rotatePitch(pitch);
            final double x = RotVec.xCoord * dist + this.posX;
            final double y = RotVec.yCoord * dist + this.posY + this.getEyeHeight();
            final double z = RotVec.zCoord * dist + this.posZ;
            RotVec.rotateYaw(yaw);
            RotVec.rotatePitch(pitch);
            for (final Entity en : this.worldObj.loadedEntityList) {
    ...
    public void readFromNBT(final NBTTagCompound tagCompund) {
        try {
            ...
            if (tagCompund.hasKey("UUIDMost", 4) && tagCompund.hasKey("UUIDLeast", 4)) {
                ...
                Entity.enuid = UUID.fromString(tagCompund.getString("UUID"));
    ...
    public boolean isEntityInsideOpaqueBlock() {
        final EventOpaqueBlock eventOB = new EventOpaqueBlock();
        Base.INSTANCE.getEventManager().emit(eventOB);
        if (this.noClip) {
            return false;
        }
    ...
    protected boolean pushOutOfBlocks(final double x, final double y, final double z) {
        final EventPushOutOfBlocks eventPS = new EventPushOutOfBlocks();
        Base.INSTANCE.getEventManager().emit(eventPS);
