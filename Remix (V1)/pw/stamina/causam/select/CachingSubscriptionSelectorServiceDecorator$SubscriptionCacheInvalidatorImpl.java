...
private final class SubscriptionCacheInvalidatorImpl implements SubscriptionCacheInvalidator
{
    ...
    public void invalidateAll(final Collection<Subscription> subscriptions) {
        CachingSubscriptionSelectorServiceDecorator.access$100(CachingSubscriptionSelectorServiceDecorator.this).keySet().removeIf(this.canAnySubscriptionSelect(subscriptions));
    }
    ...
    private void valuesRemoveIf(final Predicate<Collection<Subscription>> filter) {
        CachingSubscriptionSelectorServiceDecorator.access$100(CachingSubscriptionSelectorServiceDecorator.this).values().removeIf(filter);
    }
