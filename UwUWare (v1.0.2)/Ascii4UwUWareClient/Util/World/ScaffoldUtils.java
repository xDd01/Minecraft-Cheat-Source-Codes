...
public class ScaffoldUtils {
    ...
    public static float[] getRotations(final BlockPos block, final EnumFacing face) {
        final double x = block.getX() + 0.5 - Minecraft.thePlayer.posX + face.getFrontOffsetX() / 2.0;
        final double z = block.getZ() + 0.5 - Minecraft.thePlayer.posZ + face.getFrontOffsetZ() / 2.0;
        double y = block.getY() + 0.5;
        final double d1 = Minecraft.thePlayer.posY + Minecraft.thePlayer.getEyeHeight() - y;
    ...
    public static int grabBlockSlot() {
        ...
        for (int i = 0; i < 36; ++i) {
            final ItemStack itemStack = Minecraft.thePlayer.inventory.mainInventory[i];
            if (itemStack != null && itemStack.getItem() instanceof ItemBlock && canItemBePlaced(itemStack) && itemStack.stackSize > 0) {
                ...
                if (i > 8 && !didGetHotbar) {
                    ...
                    if (hotbarNum != -1 && Minecraft.thePlayer.inventory.mainInventory[i].stackSize > highestStack) {
        ...
        if (slot > 8) {
            int hotbarNum = getFreeHotbarSlot();
            if (hotbarNum != -1) {
                Minecraft.getMinecraft().playerController.windowClick(Minecraft.thePlayer.inventoryContainer.windowId, slot, hotbarNum, 2, Minecraft.thePlayer);
            } else {
                return -1;
            }
        }
    ...
    public static float clampRotation() {
        ...
        if (Minecraft.thePlayer.movementInput.moveForward < 0.0f) {
            rotationYaw += 180.0f;
            n = -0.5f;
        } else if (Minecraft.thePlayer.movementInput.moveForward > 0.0f) {
            n = 0.5f;
        }
