...
public class PlayerProfileCache {
	...
	public PlayerProfileCache(MinecraftServer server, File cacheFile) {
		...
		gsonbuilder.registerTypeHierarchyAdapter(ProfileEntry.class,
				new Serializer());
	...
	private void addEntry(GameProfile gameProfile, Date expirationDate) {
		...
		ProfileEntry playerprofilecache$profileentry = new ProfileEntry(
				gameProfile, expirationDate);

		if (this.uuidToProfileEntryMap.containsKey(uuid)) {
			ProfileEntry playerprofilecache$profileentry1 = (ProfileEntry) this.uuidToProfileEntryMap
					.get(uuid);
	...
	public GameProfile getGameProfileForUsername(String username) {
		...
		ProfileEntry playerprofilecache$profileentry = (ProfileEntry) this.usernameToProfileEntryMap
				.get(s);
		...
		if (playerprofilecache$profileentry != null) {
			...
			if (gameprofile1 != null) {
				...
				playerprofilecache$profileentry = (ProfileEntry) this.usernameToProfileEntryMap
						.get(s);
			}
		}
	...
	public GameProfile getProfileByUUID(UUID uuid) {
		ProfileEntry playerprofilecache$profileentry = (ProfileEntry) this.uuidToProfileEntryMap
				.get(uuid);
		return playerprofilecache$profileentry == null ? null : playerprofilecache$profileentry.getGameProfile();
	}
	...
	class Serializer implements JsonDeserializer<ProfileEntry>,
			JsonSerializer<ProfileEntry> {
		private Serializer() {
		}

		public JsonElement serialize(ProfileEntry p_serialize_1_, Type p_serialize_2_,
		...
		public ProfileEntry deserialize(JsonElement p_deserialize_1_, Type p_deserialize_2_,
			...
			if (p_deserialize_1_.isJsonObject()) {
				...
				if (jsonelement != null && jsonelement1 != null) {
					...
					if (s1 != null && s != null) {
						...
						ProfileEntry playerprofilecache$profileentry = PlayerProfileCache.this.new ProfileEntry(
