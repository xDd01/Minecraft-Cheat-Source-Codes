...
public class PlayerManager {
	...
	public void updatePlayerInstances() {
		...
		if (i - this.previousTotalWorldTime > 8000L) {
			...
			for (int j = 0; j < this.playerInstanceList.size(); ++j) {
				PlayerInstance playermanager$playerinstance = (PlayerInstance) this.playerInstanceList
						.get(j);
			...
			for (int k = 0; k < this.playerInstancesToUpdate.size(); ++k) {
				PlayerInstance playermanager$playerinstance1 = (PlayerInstance) this.playerInstancesToUpdate
						.get(k);
	...
	private PlayerInstance getPlayerInstance(int chunkX, int chunkZ, boolean createIfAbsent) {
		long i = (long) chunkX + 2147483647L | (long) chunkZ + 2147483647L << 32;
		PlayerInstance playermanager$playerinstance = (PlayerInstance) this.playerInstances
				.getValueByKey(i);
				...
				for (int l1 = i - i1; l1 <= i + i1; ++l1) {
					for (int i2 = j - i1; i2 <= j + i1; ++i2) {
						...
						if (!this.overlaps(l1 - j1, i2 - k1, i, j, i1)) {
							PlayerInstance playermanager$playerinstance = this.getPlayerInstance(l1 - j1,
									i2 - k1, false);
				...
				if (i > 0) {
					for (int j1 = j - radius; j1 <= j + radius; ++j1) {
						for (int k1 = k - radius; k1 <= k + radius; ++k1) {
							PlayerInstance playermanager$playerinstance = this.getPlayerInstance(j1, k1,
									true);
