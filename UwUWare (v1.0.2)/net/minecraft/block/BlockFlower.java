...
public abstract class BlockFlower extends BlockBush {
	...
	protected BlockFlower() {
		this.setDefaultState(this.blockState.getBaseState().withProperty(this.getTypeProperty(),
				this.getBlockType() == EnumFlowerColor.RED ? EnumFlowerType.POPPY
						: EnumFlowerType.DANDELION));
	}
	...
	public void getSubBlocks(Item itemIn, CreativeTabs tab, List<ItemStack> list) {
		for (EnumFlowerType blockflower$enumflowertype : EnumFlowerType
	...
	public IBlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(this.getTypeProperty(),
				EnumFlowerType.getType(this.getBlockType(), meta));
	}
	...
	public static enum EnumFlowerType implements IStringSerializable {
		...
		private static final EnumFlowerType[][] TYPES_FOR_BLOCK = new EnumFlowerType[EnumFlowerColor
				.values().length][];
		...
		static {
			for (final EnumFlowerColor blockflower$enumflowercolor : EnumFlowerColor.values()) {
				Collection<EnumFlowerType> collection = Collections2.<EnumFlowerType>filter(
						Lists.newArrayList(values()), new Predicate<EnumFlowerType>() {
							public boolean apply(EnumFlowerType p_apply_1_) {
								return p_apply_1_.getBlockType() == blockflower$enumflowercolor;
							}
						});
				TYPES_FOR_BLOCK[blockflower$enumflowercolor.ordinal()] = (EnumFlowerType[]) collection
