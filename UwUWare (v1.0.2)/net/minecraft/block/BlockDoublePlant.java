...
public class BlockDoublePlant extends BlockBush implements IGrowable {
	public static final PropertyEnum<EnumPlantType> VARIANT = PropertyEnum.<EnumPlantType>create(
			"variant", EnumPlantType.class);
	public static final PropertyEnum<EnumBlockHalf> HALF = PropertyEnum.<EnumBlockHalf>create(
			"half", EnumBlockHalf.class);
	public static final PropertyEnum<EnumFacing> field_181084_N = BlockDirectional.FACING;

	public BlockDoublePlant() {
		super(Material.vine);
		this.setDefaultState(
				this.blockState.getBaseState().withProperty(VARIANT, EnumPlantType.SUNFLOWER)
	...
	public boolean isReplaceable(World worldIn, BlockPos pos) {
		...
		if (iblockstate.getBlock() != this) {
			...
			EnumPlantType blockdoubleplant$enumplanttype = (EnumPlantType) this
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		if (state.getValue(HALF) == EnumBlockHalf.UPPER) {
			...
			EnumPlantType blockdoubleplant$enumplanttype = (EnumPlantType) state
					.getValue(VARIANT);
	...
	public int damageDropped(IBlockState state) {
		return state.getValue(HALF) != EnumBlockHalf.UPPER
				&& state.getValue(VARIANT) != EnumPlantType.GRASS
						? ((EnumPlantType) state.getValue(VARIANT)).getMeta()
						: 0;
	}
	...
	public int colorMultiplier(IBlockAccess worldIn, BlockPos pos, int renderPass) {
		...
		return blockdoubleplant$enumplanttype != EnumPlantType.GRASS
				&& blockdoubleplant$enumplanttype != EnumPlantType.FERN ? 16777215
	...
	public void placeAt(World worldIn, BlockPos lowerPos, EnumPlantType variant, int flags) {
		worldIn.setBlockState(lowerPos, this.getDefaultState().withProperty(HALF, EnumBlockHalf.LOWER)
				.withProperty(VARIANT, variant), flags);
		worldIn.setBlockState(lowerPos.up(),
				this.getDefaultState().withProperty(HALF, EnumBlockHalf.UPPER), flags);
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer,
			ItemStack stack) {
		worldIn.setBlockState(pos.up(), this.getDefaultState().withProperty(HALF, EnumBlockHalf.UPPER),
				2);
	}
	...
	public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, TileEntity te) {
		if (worldIn.isRemote || player.getCurrentEquippedItem() == null
				...
				|| state.getValue(HALF) != EnumBlockHalf.LOWER
		...
		if (state.getValue(HALF) == EnumBlockHalf.UPPER) {
			if (worldIn.getBlockState(pos.down()).getBlock() == this) {
				if (!player.capabilities.isCreativeMode) {
					...
					EnumPlantType blockdoubleplant$enumplanttype = (EnumPlantType) iblockstate
							.getValue(VARIANT);
	...
	private boolean onHarvest(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player) {
		...
		if (blockdoubleplant$enumplanttype != EnumPlantType.FERN
			...
			int i = (blockdoubleplant$enumplanttype == EnumPlantType.GRASS
