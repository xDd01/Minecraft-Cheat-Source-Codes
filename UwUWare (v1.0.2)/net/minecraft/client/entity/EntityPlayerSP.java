...
import Ascii4UwUWareClient.API.Events.Misc.EventLastDistance;
...
import Ascii4UwUWareClient.API.Events.World.EventPushBlock;
...
import Ascii4UwUWareClient.Module.Modules.Move.TargetStrafe;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        ...
        EventBus.getInstance().register(eventPushBlock);
        if (eventPushBlock.isCancelled()) return false;
        if (this.noClip) {
            return false;
        } else {
    ...
    public void setMoveSpeed(final EventMove event, double speed) {
        ...
        if (TargetStrafe.canStrafe()) {
            if (mc.thePlayer.getDistanceToEntity(Killaura.target) <= 3) moveForward = 0;
        }
        double yaw = TargetStrafe.canStrafe() ? RotationUtil.getRotations(Killaura.target)[0] : mc.thePlayer.rotationYaw;
