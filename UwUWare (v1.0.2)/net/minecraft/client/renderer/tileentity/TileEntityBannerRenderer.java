...
public class TileEntityBannerRenderer extends TileEntitySpecialRenderer<TileEntityBanner> {
	...
	private ResourceLocation func_178463_a(TileEntityBanner bannerObj) {
		...
		if (s.isEmpty()) {
			...
			TimedBannerTexture tileentitybannerrenderer$timedbannertexture = (TimedBannerTexture) DESIGNS
					.get(s);
			...
			if (tileentitybannerrenderer$timedbannertexture == null) {
				if (DESIGNS.size() >= 256) {
					...
					while (iterator.hasNext()) {
						...
						TimedBannerTexture tileentitybannerrenderer$timedbannertexture1 = (TimedBannerTexture) DESIGNS
								.get(s1);
