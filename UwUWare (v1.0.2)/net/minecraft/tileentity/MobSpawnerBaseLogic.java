...
public abstract class MobSpawnerBaseLogic {
	...
	private void resetTimer() {
		...
		if (this.minecartToSpawn.size() > 0) {
			this.setRandomEntity((WeightedRandomMinecart) WeightedRandom
		...
		if (nbt.hasKey("SpawnPotentials", 9)) {
			...
			for (int i = 0; i < nbttaglist.tagCount(); ++i) {
				this.minecartToSpawn
						.add(new WeightedRandomMinecart(nbttaglist.getCompoundTagAt(i)));
			}
		}

		if (nbt.hasKey("SpawnData", 10)) {
			this.setRandomEntity(
					new WeightedRandomMinecart(nbt.getCompoundTag("SpawnData"), this.mobID));
		} else {
