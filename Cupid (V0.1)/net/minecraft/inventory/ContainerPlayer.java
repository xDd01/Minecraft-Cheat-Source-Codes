...
public class ContainerPlayer extends Container {
  ...
  public ContainerPlayer(InventoryPlayer playerInventory, boolean localWorld, EntityPlayer player) {
    ...
    addSlotToContainer(new SlotCrafting(playerInventory.player, this.craftMatrix, this.craftResult, 0, 144, 36));
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++)
        addSlotToContainer(new Slot(this.craftMatrix, j + i * 2, 88 + j * 18, 26 + i * 18)); 
    } 
    for (int k = 0; k < 4; k++) {
      final int k_f = k;
      addSlotToContainer(new Slot((IInventory)playerInventory, playerInventory.getSizeInventory() - 1 - k, 8, 8 + k * 18) {
