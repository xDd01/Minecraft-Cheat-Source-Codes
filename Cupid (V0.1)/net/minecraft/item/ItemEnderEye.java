...
public class ItemEnderEye extends Item {
  ...
  public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
    ...
    if (playerIn.canPlayerEdit(pos.offset(side), side, stack) && iblockstate.getBlock() == Blocks.end_portal_frame && !((Boolean)iblockstate.getValue((IProperty)BlockEndPortalFrame.EYE)).booleanValue()) {
      if (worldIn.isRemote)
        return true; 
      worldIn.setBlockState(pos, iblockstate.withProperty((IProperty)BlockEndPortalFrame.EYE, Boolean.valueOf(true)), 2);
      ...
      EnumFacing enumfacing = (EnumFacing)iblockstate.getValue((IProperty)BlockEndPortalFrame.FACING);
      int l = 0;
      int j = 0;
      boolean flag1 = false;
      boolean flag = true;
      ...
      for (int k = -2; k <= 2; k++) {
        ...
        if (iblockstate1.getBlock() == Blocks.end_portal_frame) {
          if (!((Boolean)iblockstate1.getValue((IProperty)BlockEndPortalFrame.EYE)).booleanValue()) {
            flag = false;
            break;
          } 
          j = k;
      ...
      if (flag && j == l + 2) {
        ...
        for (int i1 = l; i1 <= j; i1++) {
          ...
          if (iblockstate3.getBlock() != Blocks.end_portal_frame || !((Boolean)iblockstate3.getValue((IProperty)BlockEndPortalFrame.EYE)).booleanValue()) {
            flag = false;
            break;
          } 
        } 
        for (int j1 = l - 1; j1 <= j + 1; j1 += 4) {
          ...
          for (int l1 = 1; l1 <= 3; l1++) {
            ...
            if (iblockstate2.getBlock() != Blocks.end_portal_frame || !((Boolean)iblockstate2.getValue((IProperty)BlockEndPortalFrame.EYE)).booleanValue()) {
              flag = false;
              break;
            } 
          } 
  ...
  public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn) {
    ...
    if (movingobjectposition != null && movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && worldIn.getBlockState(movingobjectposition.getBlockPos()).getBlock() == Blocks.end_portal_frame)
      return itemStackIn; 
    if (!worldIn.isRemote) {
      BlockPos blockpos = worldIn.getStrongholdPos("Stronghold", new BlockPos((Entity)playerIn));
      if (blockpos != null) {
        EntityEnderEye entityendereye = new EntityEnderEye(worldIn, playerIn.posX, playerIn.posY, playerIn.posZ);
        entityendereye.moveTowards(blockpos);
        worldIn.spawnEntityInWorld((Entity)entityendereye);
        worldIn.playSoundAtEntity((Entity)playerIn, "random.bow", 0.5F, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));
        worldIn.playAuxSFXAtEntity((EntityPlayer)null, 1002, new BlockPos((Entity)playerIn), 0);
