...
public class ItemMap extends ItemMapBase {
  ...
  public void updateMapData(World worldIn, Entity viewer, MapData data) {
    if (worldIn.provider.getDimensionId() == data.dimension && viewer instanceof EntityPlayer) {
      ...
      for (int k1 = l - j1 + 1; k1 < l + j1; k1++) {
        if ((k1 & 0xF) == (mapdata$mapinfo.field_82569_d & 0xF) || flag) {
          flag = false;
          double d0 = 0.0D;
            ...
            if (k1 >= 0 && l1 >= -1 && k1 < 128 && l1 < 128) {
              ...
              if (!chunk.isEmpty()) {
                ...
                if (worldIn.provider.getHasNoSky()) {
                  ...
                  if ((l3 >> 20 & 0x1) == 0) {
                    hashMultiset.add(Blocks.dirt.getMapColor(Blocks.dirt.getDefaultState().withProperty((IProperty)BlockDirt.VARIANT, (Comparable)BlockDirt.DirtType.DIRT)), 10);
                  } else {
                    hashMultiset.add(Blocks.stone.getMapColor(Blocks.stone.getDefaultState().withProperty((IProperty)BlockStone.VARIANT, (Comparable)BlockStone.EnumType.STONE)), 100);
                  } 
                  d1 = 100.0D;
                } else {
                  ...
                  for (int i4 = 0; i4 < i; i4++) {
                    for (int j4 = 0; j4 < i; j4++) {
                      ...
                      if (k4 > 1) {
                        do {
                          k4--;
                          iblockstate = chunk.getBlockState((BlockPos)blockpos$mutableblockpos.func_181079_c(i4 + i3, k4, j4 + j3));
                ...
                if (d2 < -0.6D)
                  i5 = 0; 
                MapColor mapcolor = (MapColor)Iterables.getFirst((Iterable)Multisets.copyHighestCountFirst((Multiset)hashMultiset), MapColor.airColor);
  ...
  public void onCreated(ItemStack stack, World worldIn, EntityPlayer playerIn) {
    if (stack.hasTagCompound() && stack.getTagCompound().getBoolean("map_is_scaling")) {
      ...
      mapdata1.markDirty();
      worldIn.setItemData("map_" + stack.getMetadata(), (WorldSavedData)mapdata1);
    } 
  }
  ...
  public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced) {
    ...
    if (advanced)
      if (mapdata == null) {
        ...
        tooltip.add("(Level " + mapdata.scale + "/" + '\004' + ")");
      }  
  }
}
