  ...
  public boolean matches(InventoryCrafting inv, World worldIn) {
    ...
    if (j <= 3 && i <= 1) {
        ...
        if (l > 0) {
          ...
          for (int k2 = 0; k2 < inv.getSizeInventory(); k2++) {
            ...
            if (itemstack3 != null && itemstack3.getItem() == Items.firework_charge && itemstack3.hasTagCompound() && itemstack3.getTagCompound().hasKey("Explosion", 10))
              nbttaglist.appendTag((NBTBase)itemstack3.getTagCompound().getCompoundTag("Explosion")); 
          } 
          nbttagcompound3.setTag("Explosions", (NBTBase)nbttaglist);
          nbttagcompound3.setByte("Flight", (byte)j);
          nbttagcompound1.setTag("Fireworks", (NBTBase)nbttagcompound3);
      ...
      if (j == 1 && i == 0 && l == 0 && k > 0 && j1 <= 1) {
        ...
        for (int l1 = 0; l1 < inv.getSizeInventory(); l1++) {
          ...
          if (itemstack2 != null)
            if (itemstack2.getItem() == Items.dye) {
              list.add(Integer.valueOf(ItemDye.dyeColors[itemstack2.getMetadata() & 0xF]));
        ...
        nbttagcompound.setTag("Explosion", (NBTBase)nbttagcompound2);
        ...
        for (int i2 = 0; i2 < inv.getSizeInventory(); i2++) {
          ...
          if (itemstack1 != null)
            if (itemstack1.getItem() == Items.dye) {
              list1.add(Integer.valueOf(ItemDye.dyeColors[itemstack1.getMetadata() & 0xF]));
