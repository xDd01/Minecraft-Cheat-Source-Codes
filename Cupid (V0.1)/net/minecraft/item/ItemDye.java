...
public class ItemDye extends Item {
  ...
  public String getUnlocalizedName(ItemStack stack) {
    ...
    return getUnlocalizedName() + "." + EnumDyeColor.byDyeDamage(i).getUnlocalizedName();
  }
  ...
  public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
    ...
    if (enumdyecolor == EnumDyeColor.WHITE) {
      ...
      if (block == Blocks.log && iblockstate.getValue((IProperty)BlockPlanks.VARIANT) == BlockPlanks.EnumType.JUNGLE) {
        ...
        if (worldIn.isAirBlock(pos)) {
          IBlockState iblockstate1 = Blocks.cocoa.onBlockPlaced(worldIn, pos, side, hitX, hitY, hitZ, 0, (EntityLivingBase)playerIn);
  ...
  public static void spawnBonemealParticles(World worldIn, BlockPos pos, int amount) {
    ...
    if (block.getMaterial() != Material.air) {
      block.setBlockBoundsBasedOnState((IBlockAccess)worldIn, pos);
