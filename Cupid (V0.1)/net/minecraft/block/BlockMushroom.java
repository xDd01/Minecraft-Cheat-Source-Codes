...
public class BlockMushroom extends BlockBush implements IGrowable {
  ...
  public boolean canBlockStay(World worldIn, BlockPos pos, IBlockState state) {
    if (pos.getY() >= 0 && pos.getY() < 256) {
      ...
      return (iblockstate.getBlock() == Blocks.mycelium) ? true : ((iblockstate.getBlock() == Blocks.dirt && iblockstate.getValue((IProperty)BlockDirt.VARIANT) == BlockDirt.DirtType.PODZOL) ? true : ((worldIn.getLight(pos) < 13 && canPlaceBlockOn(iblockstate.getBlock()))));
    } 
    return false;
  }
