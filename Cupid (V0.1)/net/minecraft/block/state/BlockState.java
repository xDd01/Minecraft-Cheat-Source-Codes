...
public class BlockState {
  ...
  public BlockState(Block blockIn, IProperty... properties) {
    ...
    for (List<Comparable> list1 : (Iterable<List<Comparable>>)Cartesian.cartesianProduct(getAllowedValues())) {
      Map<IProperty, Comparable> map1 = MapPopulator.createMap((Iterable)this.properties, list1);
  ...
  public String toString() {
    return Objects.toStringHelper(this).add("block", Block.blockRegistry.getNameForObject(this.block)).add("properties", Iterables.transform((Iterable)this.properties, GET_NAME_FUNC)).toString();
  }
  ...
  static class StateImplementation extends BlockStateBase {
    ...
    public void buildPropertyValueTable(Map<Map<IProperty, Comparable>, StateImplementation> map) {
      ...
      for (UnmodifiableIterator<IProperty<? extends Comparable>> unmodifiableIterator = this.properties.keySet().iterator(); unmodifiableIterator.hasNext(); ) {
        IProperty<? extends Comparable> iproperty = unmodifiableIterator.next();
        for (Comparable comparable : iproperty.getAllowedValues()) {
          if (comparable != this.properties.get(iproperty))
            hashBasedTable.put(iproperty, comparable, map.get(getPropertiesWithValue(iproperty, comparable))); 
        } 
      } 
    ...
    private Map<IProperty, Comparable> getPropertiesWithValue(IProperty property, Comparable value) {
      Map<IProperty, Comparable> map = Maps.newHashMap((Map)this.properties);
