...
public class BlockPistonStructureHelper {
  ...
  public boolean canMove() {
    ...
    if (!func_177251_a(this.blockToMove))
      return false; 
    for (int i = 0; i < this.toMove.size(); i++) {
      BlockPos blockpos = this.toMove.get(i);
      if (this.world.getBlockState(blockpos).getBlock() == Blocks.slime_block && !func_177250_b(blockpos))
        return false; 
    } 
    return true;
  }
    ...
    while (true) {
      ...
      if (k > -1) {
        ...
        for (int l = 0; l <= k + i1; l++) {
          ...
          if (this.world.getBlockState(blockpos2).getBlock() == Blocks.slime_block && !func_177250_b(blockpos2))
            return false; 
        } 
        return true;
      } 
  ...
  private boolean func_177250_b(BlockPos p_177250_1_) {
    for (EnumFacing enumfacing : EnumFacing.values()) {
      if (enumfacing.getAxis() != this.moveDirection.getAxis() && !func_177251_a(p_177250_1_.offset(enumfacing)))
        return false; 
    } 
    return true;
  }
