...
public class BlockCake extends Block {
  ...
  public void setBlockBoundsBasedOnState(IBlockAccess worldIn, BlockPos pos) {
    float f = 0.0625F;
    float f1 = (1 + ((Integer)worldIn.getBlockState(pos).getValue((IProperty)BITES)).intValue() * 2) / 16.0F;
    float f2 = 0.5F;
  ...
  public AxisAlignedBB getCollisionBoundingBox(World worldIn, BlockPos pos, IBlockState state) {
    float f = 0.0625F;
    float f1 = (1 + ((Integer)state.getValue((IProperty)BITES)).intValue() * 2) / 16.0F;
    float f2 = 0.5F;
  ...
  protected BlockState createBlockState() {
    return new BlockState(this, new IProperty[] { (IProperty)BITES });
  }
  
  public int getComparatorInputOverride(World worldIn, BlockPos pos) {
    return (7 - ((Integer)worldIn.getBlockState(pos).getValue((IProperty)BITES)).intValue()) * 2;
  }
