...
public class BlockCrops extends BlockBush implements IGrowable {
  ...
  public void grow(World worldIn, BlockPos pos, IBlockState state) {
    ...
    if (i > 7)
      i = 7; 
    worldIn.setBlockState(pos, state.withProperty((IProperty)AGE, Integer.valueOf(i)), 2);
  }
    ...
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        ...
        if (iblockstate.getBlock() == Blocks.farmland) {
          f1 = 1.0F;
          if (((Integer)iblockstate.getValue((IProperty)BlockFarmland.MOISTURE)).intValue() > 0)
            f1 = 3.0F; 
        } 
        if (i != 0 || j != 0)
          f1 /= 4.0F; 
  ...
  public Item getItemDropped(IBlockState state, Random rand, int fortune) {
    return (((Integer)state.getValue((IProperty)AGE)).intValue() == 7) ? getCrop() : getSeed();
  }
