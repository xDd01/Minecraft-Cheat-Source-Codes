...
public abstract class BlockLiquid extends Block {
  ...
  protected int getLevel(IBlockAccess worldIn, BlockPos pos) {
    return (worldIn.getBlockState(pos).getBlock().getMaterial() == this.blockMaterial) ? ((Integer)worldIn.getBlockState(pos).getValue((IProperty)LEVEL)).intValue() : -1;
  }
  ...
  protected Vec3 getFlowVector(IBlockAccess worldIn, BlockPos pos) {
    ...
    if (((Integer)worldIn.getBlockState(pos).getValue((IProperty)LEVEL)).intValue() >= 8)
  ...
  public Vec3 modifyAcceleration(World worldIn, BlockPos pos, Entity entityIn, Vec3 motion) {
    return motion.add(getFlowVector((IBlockAccess)worldIn, pos));
  }
  ...
  public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
    ...
    if (rand.nextInt(10) == 0 && World.doesBlockHaveSolidTopSurface((IBlockAccess)worldIn, pos.down())) {
