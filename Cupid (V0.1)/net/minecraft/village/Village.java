...
public class Village {
  ...
  public void tick(int p_75560_1_) {
    ...
    if (this.numIronGolems < i && this.villageDoorInfoList.size() > 20 && this.worldObj.rand.nextInt(7000) == 0) {
      Vec3 vec3 = func_179862_a(this.center, 2, 4, 2);
      if (vec3 != null) {
        ...
        this.worldObj.spawnEntityInWorld((Entity)entityirongolem);
        this.numIronGolems++;
      } 
    } 
  }
  ...
  private boolean func_179861_a(BlockPos p_179861_1_, BlockPos p_179861_2_) {
    if (!World.doesBlockHaveSolidTopSurface((IBlockAccess)this.worldObj, p_179861_2_.down()))
      return false; 
  ...
  public boolean func_179866_a(BlockPos pos) {
    return (this.center.distanceSq((Vec3i)pos) < (this.villageRadius * this.villageRadius));
  }
  ...
  public VillageDoorInfo getExistedDoor(BlockPos doorBlock) {
    if (this.center.distanceSq((Vec3i)doorBlock) > (this.villageRadius * this.villageRadius))
      return null; 
  ...
  public EntityLivingBase findNearestVillageAggressor(EntityLivingBase entitylivingbaseIn) {
    ...
    for (int i = 0; i < this.villageAgressors.size(); i++) {
      ...
      double d1 = village$villageaggressor1.agressor.getDistanceSqToEntity((Entity)entitylivingbaseIn);
      if (d1 <= d0) {
    ...
    for (String s : this.playerReputation.keySet()) {
      if (isPlayerReputationTooLow(s)) {
        ...
        if (entityplayer1 != null) {
          double d1 = entityplayer1.getDistanceSqToEntity((Entity)villageDefender);
          if (d1 <= d0) {
  ...
  private void removeDeadAndOutOfRangeDoors() {
    ...
    while (iterator.hasNext()) {
      ...
      if (!isWoodDoor(villagedoorinfo.getDoorBlockPos()) || Math.abs(this.tickCounter - villagedoorinfo.getInsidePosY()) > 1200) {
  ...
  private boolean isWoodDoor(BlockPos pos) {
    ...
    return (block instanceof net.minecraft.block.BlockDoor) ? ((block.getMaterial() == Material.wood)) : false;
  }
  ...
  public void writeVillageDataToNBT(NBTTagCompound p_82689_1_) {
    ...
    p_82689_1_.setTag("Doors", (NBTBase)nbttaglist);
    ...
    p_82689_1_.setTag("Players", (NBTBase)nbttaglist1);
  }
