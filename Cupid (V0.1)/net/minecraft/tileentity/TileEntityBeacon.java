...
public class TileEntityBeacon extends TileEntityLockable implements ITickable, IInventory {
  ...
  private void updateSegmentColors() {
    ...
    while (true) {
      float[] afloat;
      if (i1 < 256) {
        IBlockState iblockstate = this.worldObj.getBlockState((BlockPos)blockpos$mutableblockpos.func_181079_c(j, i1, l));
        if (iblockstate.getBlock() == Blocks.stained_glass) {
          afloat = EntitySheep.func_175513_a((EnumDyeColor)iblockstate.getValue((IProperty)BlockStainedGlass.COLOR));
        } else {
          if (iblockstate.getBlock() != Blocks.stained_glass_pane) {
            ...
            afloat = EntitySheep.func_175513_a((EnumDyeColor)iblockstate.getValue((IProperty)BlockStainedGlassPane.COLOR));
            if (!flag)
