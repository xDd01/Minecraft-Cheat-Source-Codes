...
public class TwitchStream implements BroadcastController.BroadcastListener, ChatController.ChatListener, IngestServerTester.IngestTestListener, IStream {
  ...
  private final IChatComponent twitchComponent = (IChatComponent)new ChatComponentText("Twitch");
  ...
  public void func_152922_k() {
    if (this.broadcastController.isBroadcasting() && !this.broadcastController.isBroadcastPaused()) {
      ...
      if (flag) {
        ...
        float f2 = framebuffer1.framebufferWidth / framebuffer1.framebufferTextureWidth;
        float f3 = framebuffer1.framebufferHeight / framebuffer1.framebufferTextureHeight;
  ...
  public void func_152911_a(Metadata p_152911_1_, long p_152911_2_) {
    if (isBroadcasting() && this.field_152957_i) {
  ...
  public void updateStreamVolume() {
    if (isBroadcasting()) {
      ...
      this.broadcastController.setRecordingDeviceVolume(func_152929_G() ? 0.0F : this.mc.gameSettings.streamMicVolume);
    } 
  }
  
  public void func_152930_t() {
    GameSettings gamesettings = this.mc.gameSettings;
    VideoParams videoparams = this.broadcastController.func_152834_a(formatStreamKbps(gamesettings.streamKbps), formatStreamFps(gamesettings.streamFps), formatStreamBps(gamesettings.streamBytesPerPixel), this.mc.displayWidth / this.mc.displayHeight);
  ...
  public void func_152893_b(ErrorCode p_152893_1_) {
    ...
    this.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion((IChatComponent)new ChatComponentText("Issue streaming frame: " + p_152893_1_ + " (" + ErrorCode.getString(p_152893_1_) + ")"), 2);
  }
  ...
  public void func_152892_c(ErrorCode p_152892_1_) {
    if (p_152892_1_ == ErrorCode.TTV_EC_SOUNDFLOWER_NOT_INSTALLED) {
      ChatComponentTranslation chatComponentTranslation1 = new ChatComponentTranslation("stream.unavailable.soundflower.chat.link", new Object[0]);
      chatComponentTranslation1.getChatStyle().setChatClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://help.mojang.com/customer/portal/articles/1374877-configuring-soundflower-for-streaming-on-apple-computers"));
      chatComponentTranslation1.getChatStyle().setUnderlined(Boolean.valueOf(true));
      ChatComponentTranslation chatComponentTranslation2 = new ChatComponentTranslation("stream.unavailable.soundflower.chat", new Object[] { chatComponentTranslation1 });
      chatComponentTranslation2.getChatStyle().setColor(EnumChatFormatting.DARK_RED);
      this.mc.ingameGUI.getChatGUI().printChatMessage((IChatComponent)chatComponentTranslation2);
    } else {
      ChatComponentTranslation chatComponentTranslation = new ChatComponentTranslation("stream.unavailable.unknown.chat", new Object[] { ErrorCode.getString(p_152892_1_) });
      chatComponentTranslation.getChatStyle().setColor(EnumChatFormatting.DARK_RED);
      this.mc.ingameGUI.getChatGUI().printChatMessage((IChatComponent)chatComponentTranslation);
    } 
  }
  ...
  public int func_152920_A() {
    return isBroadcasting() ? (this.broadcastController.getStreamInfo()).viewers : 0;
  }
  ...
  public void func_180605_a(String p_180605_1_, ChatRawMessage[] p_180605_2_) {
    for (ChatRawMessage chatrawmessage : p_180605_2_) {
      ...
      if (func_176028_a(chatrawmessage.modes, chatrawmessage.subscriptions, this.mc.gameSettings.streamChatUserFilter)) {
        ChatComponentText chatComponentText1 = new ChatComponentText(chatrawmessage.userName);
        ChatComponentTranslation chatComponentTranslation = new ChatComponentTranslation("chat.stream." + (chatrawmessage.action ? "emote" : "text"), new Object[] { this.twitchComponent, chatComponentText1, EnumChatFormatting.getTextWithoutFormattingCodes(chatrawmessage.message) });
        ...
        chatComponentText2.appendSibling((IChatComponent)new ChatComponentTranslation("stream.userinfo.chatTooltip", new Object[0]));
        for (IChatComponent ichatcomponent3 : GuiTwitchUserMode.func_152328_a(chatrawmessage.modes, chatrawmessage.subscriptions, (IStream)null)) {
          ...
          chatComponentText2.appendSibling(ichatcomponent3);
        } 
        chatComponentText1.getChatStyle().setChatHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (IChatComponent)chatComponentText2));
        chatComponentText1.getChatStyle().setChatClickEvent(new ClickEvent(ClickEvent.Action.TWITCH_USER_INFO, chatrawmessage.userName));
        this.mc.ingameGUI.getChatGUI().printChatMessage((IChatComponent)chatComponentTranslation);
      } 
    } 
  }
