...
public class EntityRenderer implements IResourceManagerReloadListener {
  ...
  public void getMouseOver(float partialTicks) {
    ...
    if (entity != null && this.mc.theWorld != null) {
      ...
      if (this.mc.playerController.extendedReach()) {
        ...
        if (d0 > (Reach.enabled ? Reach.reach : 3.0D))
          flag = true; 
        d0 = d0;
      } 
      ...
      Vec3 vec33 = null;
      float f = 1.0F;
      List<Entity> list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand(f, f, f), Predicates.and(EntitySelectors.NOT_SPECTATING, new EntityRendererOne(this)));
      double d2 = d1;
      ...
      if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > (Reach.enabled ? Reach.reach : 3.0D)) {
        this.pointedEntity = null;
  ...
  private void hurtCameraEffect(float partialTicks) {
    if (Lite.MODULE_FACTORY.findByClass(NoHurtCam.class).getData().isEnabled())
      return; 
  ...
  private void updateLightmap(float partialTicks) {
    if (this.lightmapUpdateNeeded) {
      ...
      if (worldclient != null) {
        if (Config.isCustomColors() && CustomColors.updateLightmap((World)worldclient, this.torchFlickerX, this.lightmapColors, this.mc.thePlayer.isPotionActive(Potion.nightVision))) {
    ...
    if (!this.mc.skipRenderWorld) {
      ...
      if (this.mc.currentScreen != null) {
        ...
        try {
          ...
          crashreportcategory.addCrashSectionCallable("Screen name", new EntityRendererTWo(this));
          crashreportcategory.addCrashSectionCallable("Mouse location", new Callable() {
                private static final String __OBFID = "CL_00000950";
                
                public String call() throws Exception {
                  return String.format("Scaled: (%d, %d). Absolute: (%d, %d)", new Object[] { Integer.valueOf(this.val$j1), Integer.valueOf(this.val$k1), Integer.valueOf(Mouse.getX()), Integer.valueOf(Mouse.getY()) });
                }
              });
  ...
  private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
    ...
    if (!this.debugView) {
      ...
      GlStateManager.matrixMode(5888);
      GlStateManager.popMatrix();
      Lite.EVENT_BUS.call((Event)new Render3DEvent(partialTicks));
      GlStateManager.pushMatrix();
      ...
      for (int i1 = k - b0; i1 <= k + b0; i1++) {
        for (int j1 = i - b0; j1 <= i + b0; j1++) {
          ...
          if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow()) {
            int l1 = worldclient.getPrecipitationHeight((BlockPos)blockpos$mutableblockpos).getY();
            int i2 = j - b0;
            int j2 = j + b0;
            if (i2 < l1)
              i2 = l1; 
            ...
            if (i2 != j2) {
              ...
              if (worldclient.getWorldChunkManager().getTemperatureAtHeight(f1, l1) >= 0.15F) {
                ...
                double d5 = ((this.rendererUpdateCount + j1 * j1 * 3121 + j1 * 45238971 + i1 * i1 * 418711 + i1 * 13761 & 0x1F) + partialTicks) / 32.0D * (3.0D + this.random.nextDouble());
                ...
                int l2 = worldclient.getCombinedLight((BlockPos)blockpos$mutableblockpos, 0);
                int i3 = l2 >> 16 & 0xFFFF;
                int j3 = l2 & 0xFFFF;
                ...
                blockpos$mutableblockpos.func_181079_c(j1, k2, i1);
                int k3 = (worldclient.getCombinedLight((BlockPos)blockpos$mutableblockpos, 0) * 3 + 15728880) / 4;
                int l3 = k3 >> 16 & 0xFFFF;
                int i4 = k3 & 0xFFFF;
  ...
  private void updateFogColor(float partialTicks) {
    ...
    if (this.cloudFog) {
      ...
      Vec3 vec34 = CustomColors.getUnderwaterColor((IBlockAccess)this.mc.theWorld, (this.mc.getRenderViewEntity()).posX, (this.mc.getRenderViewEntity()).posY + 1.0D, (this.mc.getRenderViewEntity()).posZ);
      if (vec34 != null) {
  ...
  private void frameInit() {
    ...
    if (this.updatedWorld != worldClient) {
      RandomMobs.worldChanged(this.updatedWorld, (World)worldClient);
