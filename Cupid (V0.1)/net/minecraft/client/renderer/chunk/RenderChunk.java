...
public class RenderChunk {
  ...
  public void rebuildChunk(float x, float y, float z, ChunkCompileTaskGenerator generator) {
    ...
    if (!regionrendercache.extendedLevelsInChunkCache()) {
      ...
      Iterator<BlockPosM> iterator = BlockPosM.getAllInBoxMutable(blockpos, blockpos1).iterator();
      ...
      while (iterator.hasNext()) {
        ...
        IBlockState iblockstate = regionrendercache.getBlockState((BlockPos)blockposm);
        Block block = iblockstate.getBlock();
        if (block.isOpaqueCube())
          var10.func_178606_a((BlockPos)blockposm); 
        if (ReflectorForge.blockHasTileEntity(iblockstate)) {
          TileEntity tileentity = regionrendercache.getTileEntity(new BlockPos((Vec3i)blockposm));
        ...
        for (int i = 0; i < aenumworldblocklayer.length; i++) {
          ...
          if (block.getRenderType() != -1) {
            ...
            aboolean[j] = aboolean[j] | blockrendererdispatcher.renderBlock(iblockstate, (BlockPos)blockposm, (IBlockAccess)regionrendercache, worldrenderer);
          } 
          continue;
        } 
      } 
