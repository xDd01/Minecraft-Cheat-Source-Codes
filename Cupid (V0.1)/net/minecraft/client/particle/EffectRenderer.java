...
public class EffectRenderer {
  ...
  public void addEffect(EntityFX effect) {
    if (effect != null)
      if (!(effect instanceof EntityFirework.SparkFX) || Config.isFireworkParticles()) {
        ...
        if (!(effect instanceof Barrier) || !reuseBarrierParticle(effect, this.fxLayers[i][j]))
  ...
  private void updateEffectLayer(int p_178922_1_) {
    for (int i = 0; i < 2; i++)
      updateEffectAlphaLayer(this.fxLayers[p_178922_1_][i]); 
  }
      ...
      for (int i = 0; i < b0; i++) {
        for (int j = 0; j < b0; j++) {
          for (int k = 0; k < b0; k++) {
            ...
            addEffect((new EntityDiggingFX(this.worldObj, d0, d1, d2, d0 - pos.getX() - 0.5D, d1 - pos.getY() - 0.5D, d2 - pos.getZ() - 0.5D, state)).func_174846_a(pos));
          } 
        } 
      } 
    } 
  ...
  public void addBlockHitEffects(BlockPos pos, EnumFacing side) {
    ...
    if (block.getRenderType() != -1) {
      ...
      addEffect((new EntityDiggingFX(this.worldObj, d0, d1, d2, 0.0D, 0.0D, 0.0D, iblockstate)).func_174846_a(pos).multiplyVelocity(0.2F).multipleParticleScaleBy(0.6F));
    } 
  }
