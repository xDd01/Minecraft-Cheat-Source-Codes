...
public class EntityFirework {
  ...
  public static class OverlayFX extends EntityFX {
    ...
    public void renderParticle(WorldRenderer worldRendererIn, Entity entityIn, float partialTicks, float p_180434_4_, float p_180434_5_, float p_180434_6_, float p_180434_7_, float p_180434_8_) {
      ...
      float f2 = 0.125F;
      float f3 = 0.375F;
      float f4 = 7.1F * MathHelper.sin((this.particleAge + partialTicks - 1.0F) * 0.25F * 3.1415927F);
  ...
  public static class SparkFX extends EntityFX {
    ...
    public void setColour(int colour) {
      float f = ((colour & 0xFF0000) >> 16) / 255.0F;
      float f1 = ((colour & 0xFF00) >> 8) / 255.0F;
      float f2 = ((colour & 0xFF) >> 0) / 255.0F;
      float f3 = 1.0F;
  ...
  public static class StarterFX extends EntityFX {
      ...
      if (this.fireworkAge % 2 == 0 && this.fireworkExplosions != null && this.fireworkAge / 2 < this.fireworkExplosions.tagCount()) {
        ...
        if (l == 1) {
          ...
          createShaped(0.5D, new double[][] { 
                ...
                { 0.2D, -0.4D }, , { 0.0D, -0.4D },  }, aint, aint1, flag4, flag2, true);
        } else if (l == 4) {
    ...
    private void createShaped(double speed, double[][] shape, int[] colours, int[] fadeColours, boolean trail, boolean twinkleIn, boolean p_92038_8_) {
      double d0 = shape[0][0];
      double d1 = shape[0][1];
      createParticle(this.posX, this.posY, this.posZ, d0 * speed, d1 * speed, 0.0D, colours, fadeColours, trail, twinkleIn);
      float f = this.rand.nextFloat() * 3.1415927F;
      double d2 = p_92038_8_ ? 0.034D : 0.34D;
      for (int i = 0; i < 3; i++) {
        double d3 = f + (i * 3.1415927F) * d2;
        double d4 = d0;
        double d5 = d1;
        for (int j = 1; j < shape.length; j++) {
          ...
          double d7 = shape[j][1];
          double d8;
          for (d8 = 0.25D; d8 <= 1.0D; d8 += 0.25D) {
            double d9 = (d4 + (d6 - d4) * d8) * speed;
            double d10 = (d5 + (d7 - d5) * d8) * speed;
            double d11 = d9 * Math.sin(d3);
            d9 *= Math.cos(d3);
            double d12;
            for (d12 = -1.0D; d12 <= 1.0D; d12 += 2.0D)
              createParticle(this.posX, this.posY, this.posZ, d9 * d12, d10, d11 * d12, colours, fadeColours, trail, twinkleIn); 
          } 
          d4 = d6;
          d5 = d7;
        } 
    ...
    private void createBurst(int[] colours, int[] fadeColours, boolean trail, boolean twinkleIn) {
      ...
      for (int i = 0; i < 70; i++) {
        ...
        createParticle(this.posX, this.posY, this.posZ, d2, d4, d3, colours, fadeColours, trail, twinkleIn);
      } 
    }
