...
public class WorldGenTrees extends WorldGenAbstractTree {
  private static final IBlockState field_181653_a = Blocks.log.getDefaultState().withProperty((IProperty)BlockOldLog.VARIANT, (Comparable)BlockPlanks.EnumType.OAK);
  
  private static final IBlockState field_181654_b = Blocks.leaves.getDefaultState().withProperty((IProperty)BlockOldLeaf.VARIANT, (Comparable)BlockPlanks.EnumType.OAK).withProperty((IProperty)BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
      ...
      for (int j = position.getY(); j <= position.getY() + 1 + i; j++) {
        ...
        for (int l = position.getX() - k; l <= position.getX() + k && flag; l++) {
          for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; i1++) {
            if (j >= 0 && j < 256) {
              if (!func_150523_a(worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.func_181079_c(l, j, i1)).getBlock()))
                flag = false; 
            } else {
              flag = false;
            } 
          ...
          for (int k3 = position.getY() - 3 + i; k3 <= position.getY() + i; k3++) {
            ...
            for (int l4 = position.getX() - k4; l4 <= position.getX() + k4; l4++) {
              for (int i5 = position.getZ() - k4; i5 <= position.getZ() + k4; i5++) {
                ...
                if (worldIn.getBlockState((BlockPos)blockpos$mutableblockpos1).getBlock().getMaterial() == Material.leaves) {
            ...
            for (int l3 = 0; l3 < 2; l3++) {
              for (Object enumfacing : EnumFacing.Plane.HORIZONTAL) {
                if (rand.nextInt(4 - l3) == 0) {
                  ...
                  func_181652_a(worldIn, rand.nextInt(3), position.add(enumfacing1.getFrontOffsetX(), i - 5 + l3, enumfacing1.getFrontOffsetZ()), (EnumFacing)enumfacing);
                } 
              } 
            }  
        } 
  ...
  private void func_181652_a(World p_181652_1_, int p_181652_2_, BlockPos p_181652_3_, EnumFacing p_181652_4_) {
    setBlockAndNotifyAdequately(p_181652_1_, p_181652_3_, Blocks.cocoa.getDefaultState().withProperty((IProperty)BlockCocoa.AGE, Integer.valueOf(p_181652_2_)).withProperty((IProperty)BlockCocoa.FACING, (Comparable)p_181652_4_));
  }
  
  private void func_181651_a(World p_181651_1_, BlockPos p_181651_2_, PropertyBool p_181651_3_) {
    setBlockAndNotifyAdequately(p_181651_1_, p_181651_2_, Blocks.vine.getDefaultState().withProperty((IProperty)p_181651_3_, Boolean.valueOf(true)));
  }
