    ...
    if (position.getY() >= 1 && position.getY() + i + 1 < 256) {
          ...
          for (int i2 = j1; i2 <= k1; i2++) {
            ...
            if (position.getY() >= position.getY() + i - 1 || blockhugemushroom$enumtype != BlockHugeMushroom.EnumType.ALL_INSIDE) {
              ...
              if (!worldIn.getBlockState(blockpos).getBlock().isFullBlock())
                setBlockAndNotifyAdequately(worldIn, blockpos, this.mushroomType.getDefaultState().withProperty((IProperty)BlockHugeMushroom.VARIANT, (Comparable)blockhugemushroom$enumtype)); 
            } 
            continue;
          } 
        } 
      ...
      for (int i3 = 0; i3 < i; i3++) {
        ...
        if (!block2.isFullBlock())
          setBlockAndNotifyAdequately(worldIn, position.up(i3), this.mushroomType.getDefaultState().withProperty((IProperty)BlockHugeMushroom.VARIANT, (Comparable)BlockHugeMushroom.EnumType.STEM)); 
      } 
      return true;
    } 
    return false;
