...
public class WorldGenSwamp extends WorldGenAbstractTree {
  private static final IBlockState field_181648_a = Blocks.log.getDefaultState().withProperty((IProperty)BlockOldLog.VARIANT, (Comparable)BlockPlanks.EnumType.OAK);
  
  private static final IBlockState field_181649_b = Blocks.leaves.getDefaultState().withProperty((IProperty)BlockOldLeaf.VARIANT, (Comparable)BlockPlanks.EnumType.OAK).withProperty((IProperty)BlockOldLeaf.CHECK_DECAY, Boolean.valueOf(false));
      ...
      for (int j = position.getY(); j <= position.getY() + 1 + i; j++) {
        ...
        for (int l = position.getX() - k; l <= position.getX() + k && flag; l++) {
          for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; i1++) {
            if (j >= 0 && j < 256) {
              Block block = worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.func_181079_c(l, j, i1)).getBlock();
  ...
  private void func_181647_a(World p_181647_1_, BlockPos p_181647_2_, PropertyBool p_181647_3_) {
    IBlockState iblockstate = Blocks.vine.getDefaultState().withProperty((IProperty)p_181647_3_, Boolean.valueOf(true));
