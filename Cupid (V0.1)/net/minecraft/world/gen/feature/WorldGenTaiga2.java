...
public class WorldGenTaiga2 extends WorldGenAbstractTree {
  private static final IBlockState field_181645_a = Blocks.log.getDefaultState().withProperty((IProperty)BlockOldLog.VARIANT, (Comparable)BlockPlanks.EnumType.SPRUCE);
  
  private static final IBlockState field_181646_b = Blocks.leaves.getDefaultState().withProperty((IProperty)BlockOldLeaf.VARIANT, (Comparable)BlockPlanks.EnumType.SPRUCE).withProperty((IProperty)BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
      ...
      for (int i1 = position.getY(); i1 <= position.getY() + 1 + i && flag; i1++) {
        ...
        for (int k1 = position.getX() - j1; k1 <= position.getX() + j1 && flag; k1++) {
          for (int l1 = position.getZ() - j1; l1 <= position.getZ() + j1 && flag; l1++) {
            if (i1 >= 0 && i1 < 256) {
              Block block = worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.func_181079_c(k1, i1, l1)).getBlock();
