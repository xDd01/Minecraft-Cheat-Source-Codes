...
float getColorDirection() {
    return verticalShade ? gl_TexCoord[0].st.y : gl_TexCoord[0].st.x;
}
...
void main() {
    ...
    float round = roundedRect(pixel - center, center, radius);
    ...
    gl_FragColor = vec4(shade, alphaColor * smoothstep(1.0, 0, round));
}