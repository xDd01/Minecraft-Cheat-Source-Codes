...
final class CachedDateFormat extends DateFormat {
  ...
  public static int findMillisecondStart(long time, String formatted, DateFormat formatter) {
    ...
    for (int i = 0; i < formatted.length(); i++) {
      if (formatted.charAt(i) != plusMagic.charAt(i)) {
        ...
        if (plusZero
          .length() == formatted.length() && magicString
          .regionMatches(0, plusMagic, i, magicString
            .length()) && formattedMillis
          .toString().regionMatches(0, formatted, i, magicString
            .length()) && "000"
          .regionMatches(0, plusZero, i, "000"
            .length()))
          return i; 
        return -1;
      } 
  ...
  public StringBuffer format(long now, StringBuffer buf) {
    ...
    if (this.millisecondStart >= 0)
      this
        .millisecondStart = findMillisecondStart(now, this.cache.toString(), this.formatter); 
    return buf;
  }
...
/* Location:              C:\Users\Joona\Downloads\Cupid.jar!\org\apache\logging\log4j\core\pattern\CachedDateFormat.class
