...
public class HttpPipelineReceiver extends Thread {
  ...
  private HttpResponse readResponse(InputStream p_readResponse_1_) throws IOException {
    String s = readLine(p_readResponse_1_);
    ...
    while (true) {
      String s3 = readLine(p_readResponse_1_);
      if (s3.length() <= 0) {
        byte[] abyte = null;
        ...
        if (s6 != null) {
          ...
          if (k > 0) {
            abyte = new byte[k];
            readFull(abyte, p_readResponse_1_);
          } 
        } else {
          String s7 = map.get("Transfer-Encoding");
          if (Config.equals(s7, "chunked"))
            abyte = readContentChunked(p_readResponse_1_); 
        } 
  ...
  private byte[] readContentChunked(InputStream p_readContentChunked_1_) throws IOException {
    ...
    do {
      String s = readLine(p_readContentChunked_1_);
      String[] astring = Config.tokenize(s, "; ");
      i = Integer.parseInt(astring[0], 16);
      byte[] abyte = new byte[i];
      readFull(abyte, p_readContentChunked_1_);
      bytearrayoutputstream.write(abyte);
      readLine(p_readContentChunked_1_);
    } while (i != 0);
  ...
  private void readFull(byte[] p_readFull_1_, InputStream p_readFull_2_) throws IOException {
    int i;
    for (i = 0; i < p_readFull_1_.length; i += j) {
...
/* Location:              C:\Users\Joona\Downloads\Cupid.jar!\optifine\HttpPipelineReceiver.class
