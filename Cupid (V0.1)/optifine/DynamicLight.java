...
public class DynamicLight {
  ...
  public void update(RenderGlobal p_update_1_) {
    ...
    if (Math.abs(d2) > d5 || Math.abs(d3) > d5 || Math.abs(d4) > d5 || this.lastLightLevel != j) {
      ...
      WorldClient worldClient = p_update_1_.getWorld();
      if (worldClient != null) {
        this.blockPosMutable.func_181079_c(MathHelper.floor_double(d6), MathHelper.floor_double(d0), MathHelper.floor_double(d1));
        IBlockState iblockstate = worldClient.getBlockState((BlockPos)this.blockPosMutable);
      ...
      if (j > 0) {
        ...
        updateChunkLight(renderchunk, this.setLitChunkPos, set);
        updateChunkLight(renderchunk1, this.setLitChunkPos, set);
        updateChunkLight(renderchunk2, this.setLitChunkPos, set);
        updateChunkLight(renderchunk3, this.setLitChunkPos, set);
        updateChunkLight(renderchunk4, this.setLitChunkPos, set);
        updateChunkLight(renderchunk5, this.setLitChunkPos, set);
        updateChunkLight(renderchunk6, this.setLitChunkPos, set);
        updateChunkLight(renderchunk7, this.setLitChunkPos, set);
      } 
...
/* Location:              C:\Users\Joona\Downloads\Cupid.jar!\optifine\DynamicLight.class
