...
public class TooltipManager {
  ...
  public void drawTooltips(int p_drawTooltips_1_, int p_drawTooltips_2_, List p_drawTooltips_3_) {
    if (Math.abs(p_drawTooltips_1_ - this.lastMouseX) <= 5 && Math.abs(p_drawTooltips_2_ - this.lastMouseY) <= 5) {
      ...
      if (System.currentTimeMillis() >= this.mouseStillTime + i) {
        ...
        int l = j + 150 + 150;
        int i1 = k + 84 + 10;
        GuiButton guibutton = getSelectedButton(p_drawTooltips_1_, p_drawTooltips_2_, p_drawTooltips_3_);
  ...
  private GuiButton getSelectedButton(int p_getSelectedButton_1_, int p_getSelectedButton_2_, List<GuiButton> p_getSelectedButton_3_) {
    for (int i = 0; i < p_getSelectedButton_3_.size(); i++) {
      GuiButton guibutton = p_getSelectedButton_3_.get(i);
...
/* Location:              C:\Users\Joona\Downloads\Cupid.jar!\optifine\TooltipManager.class
