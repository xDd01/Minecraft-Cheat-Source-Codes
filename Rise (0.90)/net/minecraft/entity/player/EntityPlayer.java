...
import dev.rise.module.impl.ghost.KeepSprint;
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag2) {
                        if (i > 0) {
                            ...
                            handle:
                            {
                                final Module keepSprint = this.getModule(KeepSprint.class);
                                final Setting auraKeepSprint = Rise.INSTANCE.getModuleManager().getSetting("Aura", "Keep Sprint");
                                final Module aura = this.getModule(Aura.class);

                                if (keepSprint == null || auraKeepSprint == null) break handle;
                                if (keepSprint.isEnabled() || (((BooleanSetting) auraKeepSprint).isEnabled() && Objects.requireNonNull(aura).isEnabled())) {
                                    break handle;
                                }
