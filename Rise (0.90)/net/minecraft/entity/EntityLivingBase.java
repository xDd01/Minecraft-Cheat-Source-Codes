...
public abstract class EntityLivingBase extends Entity {
    ...
    public boolean isPotionActive(Potion potionIn) {
        /* Makes experience smoother because Hypixel usually false flags you when you run out of Speed, this fixes it */
        if (potionIn == Potion.moveSpeed && PlayerUtil.isOnServer("Hypixel") && this.activePotionsMap.containsKey(potionIn.id))
            return this.getActivePotionEffect(Potion.moveSpeed).getDuration() > 20;
                ...
                if (flag) {
                    ...
                    if (entity != null) {
                        ...
                        if (this == Minecraft.getMinecraft().thePlayer) {
                            BooleanSetting setting = (BooleanSetting) Rise.INSTANCE.getModuleManager().getSetting("Aura", "Strafe");

                            assert setting != null;
                            if (setting.isEnabled() && Aura.target != null && Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getModule("Aura")).isEnabled()) {
                                this.attackedAtYaw = (float) (MathHelper.func_181159_b(d0, d1) * 180.0D / Math.PI - (double) Aura.yaw);
                            }
                        }
    ...
    private int getArmSwingAnimationEnd() {
        if (this == Minecraft.getMinecraft().thePlayer)
            if (Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getModule("Animations")).isEnabled()) {
                return (int) ((NumberSetting) Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getSetting("Animations", "Slow"))).getValue();
            }
    ...
    protected void jump() {
        ...
        if (this.isSprinting()) {
            ...
            if (mc.thePlayer.allowMultiDirSprint) {
                if (mc.gameSettings.keyBindForward.isKeyDown()) f = mc.thePlayer.rotationYaw;
                else if (mc.gameSettings.keyBindLeft.isKeyDown()) f = mc.thePlayer.rotationYaw - 90;
                else if (mc.gameSettings.keyBindRight.isKeyDown()) f = mc.thePlayer.rotationYaw + 90;
                else if (mc.gameSettings.keyBindBack.isKeyDown()) f = mc.thePlayer.rotationYaw - 180;
                else f = mc.thePlayer.rotationYaw + 180;
            ...
            if (this == mc.thePlayer) {
                if (EntityPlayer.movementYaw != null) {
                    f = EntityPlayer.movementYaw * 0.017453292F;
                }
            }
    ...
    public void onLivingUpdate() {
        ...
        double nigger = 0.005D;

        if (Math.abs(this.motionX) < nigger) {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < nigger) {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < nigger) {
            this.motionZ = 0.0D;
        }
     ...
     * returns true if the entity provided in the argument can be seen. (Raytrace)
     */
    public boolean canEntityBeSeen(Vec3 position) {
        return this.worldObj.rayTraceBlocks(new Vec3(this.posX, this.posY + (double) this.getEyeHeight(), this.posZ), new Vec3(position.xCoord, position.yCoord, position.zCoord)) == null;
    }
