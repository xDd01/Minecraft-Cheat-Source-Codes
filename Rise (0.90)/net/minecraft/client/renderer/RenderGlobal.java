...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener {
    ...
    private static final Set<? extends EnumFacing> SET_ALL_FACINGS = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(EnumFacing.VALUES)));
    ...
    public void renderEntities(final Entity renderViewEntity, final ICamera camera, final float partialTicks) {
        ...
        if (this.renderEntitiesStartupCounter > 0) {
            ...
            for (final ContainerLocalRenderInformation e : this.renderInfosEntities) {
                final Chunk chunk = e.renderChunk.getChunk();
                final ClassInheritanceMultiMap<Entity> classinheritancemultimap = chunk.getEntityLists()[e.renderChunk.getPosition().getY() / 16];

                if (!classinheritancemultimap.isEmpty()) {
                    final Iterator<Entity> iterator = classinheritancemultimap.iterator();
            ...
            for (final ContainerLocalRenderInformation e : this.renderInfosTileEntities) {
                final List<TileEntity> list1 = e.renderChunk.getCompiledChunk().getTileEntities();

                if (!list1.isEmpty()) {
                    final Iterator<TileEntity> iterator1 = list1.iterator();
    ...
    public void setupTerrain(final Entity viewEntity, final double partialTicks, ICamera camera, final int frameCount, final boolean playerSpectator) {
        ...
        if (!flag && this.displayListEntitiesDirty && !Shaders.isShadowPass) {
            ...
            final Deque<ContainerLocalRenderInformation> deque = this.visibilityDeque;
            ...
            if (renderchunk != null && renderchunk.getPosition().getY() <= j) {
                ...
                final Set<EnumFacing> set1 = new HashSet<>(SET_ALL_FACINGS);
            ...
            while (!deque.isEmpty()) {
                final RenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation5 = deque.poll();
    ...
    public int renderBlockLayer(final EnumWorldBlockLayer blockLayerIn, final Entity entityIn) {
        RenderHelper.disableStandardItemLighting();
    ...
    public void renderSky(final float partialTicks, final int pass) {
        ...
        if (this.mc.theWorld.provider.getDimensionId() == 1) {
            ...
            if (Config.isSunTexture()) {
                ...
                worldrenderer.pos(-f16, 100.0D, -f16).func_181673_a(0.0D, 0.0D).endVertex();
                worldrenderer.pos(f16, 100.0D, -f16).func_181673_a(1.0D, 0.0D).endVertex();
                worldrenderer.pos(f16, 100.0D, f16).func_181673_a(1.0D, 1.0D).endVertex();
                worldrenderer.pos(-f16, 100.0D, f16).func_181673_a(0.0D, 1.0D).endVertex();
                tessellator.draw();
            }
            ...
            if (Config.isMoonTexture()) {
                ...
                worldrenderer.begin(7, DefaultVertexFormats.field_181707_g);
                worldrenderer.pos(-f16, -100.0D, f16).func_181673_a(f23, f14).endVertex();
                worldrenderer.pos(f16, -100.0D, f16).func_181673_a(f19, f14).endVertex();
                worldrenderer.pos(f16, -100.0D, -f16).func_181673_a(f19, f21).endVertex();
                worldrenderer.pos(-f16, -100.0D, -f16).func_181673_a(f23, f21).endVertex();
                tessellator.draw();
            }
    ...
    public void updateChunks(long finishTimeNano) {
        ...
        if (this.chunksToUpdateForced.size() > 0) {
            final Iterator<RenderChunk> iterator = this.chunksToUpdateForced.iterator();
        ...
        if (this.chunksToResortTransparency.size() > 0) {
            final Iterator<RenderChunk> iterator2 = this.chunksToResortTransparency.iterator();
        ...
        if (p_180449_1_.posX >= worldborder.maxX() - d0 || p_180449_1_.posX <= worldborder.minX() + d0 || p_180449_1_.posZ >= worldborder.maxZ() - d0 || p_180449_1_.posZ <= worldborder.minZ() + d0) {
            ...
            if (d2 > worldborder.maxX() - d0) {
                ...
                for (double d7 = d5; d7 < d6; f7 += 0.5F) {
                    ...
                    worldrenderer.pos(worldborder.maxX(), 256.0D, d7).func_181673_a(f3 + f7, f3 + 0.0F).endVertex();
                    worldrenderer.pos(worldborder.maxX(), 256.0D, d7 + d8).func_181673_a(f3 + f8 + f7, f3 + 0.0F).endVertex();
                    worldrenderer.pos(worldborder.maxX(), 0.0D, d7 + d8).func_181673_a(f3 + f8 + f7, f3 + 128.0F).endVertex();
                    worldrenderer.pos(worldborder.maxX(), 0.0D, d7).func_181673_a(f3 + f7, f3 + 128.0F).endVertex();
                    ++d7;
                }
            }
            ...
            if (d2 < worldborder.minX() + d0) {
                ...
                for (double d9 = d5; d9 < d6; f9 += 0.5F) {
                    ...
                    worldrenderer.pos(worldborder.minX(), 256.0D, d9).func_181673_a(f3 + f9, f3 + 0.0F).endVertex();
                    worldrenderer.pos(worldborder.minX(), 256.0D, d9 + d12).func_181673_a(f3 + f12 + f9, f3 + 0.0F).endVertex();
                    worldrenderer.pos(worldborder.minX(), 0.0D, d9 + d12).func_181673_a(f3 + f12 + f9, f3 + 128.0F).endVertex();
                    worldrenderer.pos(worldborder.minX(), 0.0D, d9).func_181673_a(f3 + f9, f3 + 128.0F).endVertex();
                    ++d9;
                }
            }
            ...
            if (d4 > worldborder.maxZ() - d0) {
                ...
                for (double d10 = d5; d10 < d6; f10 += 0.5F) {
                    ...
                    worldrenderer.pos(d10, 256.0D, worldborder.maxZ()).func_181673_a(f3 + f10, f3 + 0.0F).endVertex();
                    worldrenderer.pos(d10 + d13, 256.0D, worldborder.maxZ()).func_181673_a(f3 + f13 + f10, f3 + 0.0F).endVertex();
                    worldrenderer.pos(d10 + d13, 0.0D, worldborder.maxZ()).func_181673_a(f3 + f13 + f10, f3 + 128.0F).endVertex();
                    worldrenderer.pos(d10, 0.0D, worldborder.maxZ()).func_181673_a(f3 + f10, f3 + 128.0F).endVertex();
                    ++d10;
                }
            }
            ...
            if (d4 < worldborder.minZ() + d0) {
                ...
                for (double d11 = d5; d11 < d6; f11 += 0.5F) {
                    ...
                    worldrenderer.pos(d11, 256.0D, worldborder.minZ()).func_181673_a(f3 + f11, f3 + 0.0F).endVertex();
                    worldrenderer.pos(d11 + d14, 256.0D, worldborder.minZ()).func_181673_a(f3 + f14 + f11, f3 + 0.0F).endVertex();
                    worldrenderer.pos(d11 + d14, 0.0D, worldborder.minZ()).func_181673_a(f3 + f14 + f11, f3 + 128.0F).endVertex();
                    worldrenderer.pos(d11, 0.0D, worldborder.minZ()).func_181673_a(f3 + f11, f3 + 128.0F).endVertex();
                    ++d11;
                }
            }
