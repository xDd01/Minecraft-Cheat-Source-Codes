...
public class RenderDragon extends RenderLiving<EntityDragon> {
    ...
    protected void rotateCorpse(final EntityDragon bat, final float p_77043_2_, final float p_77043_3_, final float partialTicks) {
        ...
        final float f1 = (float) (bat.getMovementOffsets(5, partialTicks)[1] - bat.getMovementOffsets(10, partialTicks)[1]);
    ...
    protected void drawRechargeRay(final EntityDragon dragon, final double p_180574_2_, final double p_180574_4_, final double p_180574_6_, final float p_180574_8_) {
        final float f = (float) dragon.healingEnderCrystal.innerRotation + p_180574_8_;
        float f1 = MathHelper.sin(f * 0.2F) / 2.0F + 0.5F;
        f1 = (f1 * f1 + f1) * 0.2F;
        final float f2 = (float) (dragon.healingEnderCrystal.posX - dragon.posX - (dragon.prevPosX - dragon.posX) * (double) (1.0F - p_180574_8_));
        final float f3 = (float) ((double) f1 + dragon.healingEnderCrystal.posY - 1.0D - dragon.posY - (dragon.prevPosY - dragon.posY) * (double) (1.0F - p_180574_8_));
        final float f4 = (float) (dragon.healingEnderCrystal.posZ - dragon.posZ - (dragon.prevPosZ - dragon.posZ) * (double) (1.0F - p_180574_8_));
        final float f5 = MathHelper.sqrt_float(f2 * f2 + f4 * f4);
        final float f6 = MathHelper.sqrt_float(f2 * f2 + f3 * f3 + f4 * f4);
        ...
        final float f7 = 0.0F - ((float) dragon.ticksExisted + p_180574_8_) * 0.01F;
        final float f8 = MathHelper.sqrt_float(f2 * f2 + f3 * f3 + f4 * f4) / 32.0F - ((float) dragon.ticksExisted + p_180574_8_) * 0.01F;
        worldrenderer.begin(5, DefaultVertexFormats.field_181709_i);
        final int i = 8;

        for (int j = 0; j <= 8; ++j) {
            final float f9 = MathHelper.sin((float) (j % 8) * (float) Math.PI * 2.0F / 8.0F) * 0.75F;
            final float f10 = MathHelper.cos((float) (j % 8) * (float) Math.PI * 2.0F / 8.0F) * 0.75F;
            final float f11 = (float) (j % 8) * 1.0F / 8.0F;
            worldrenderer.pos(f9 * 0.2F, f10 * 0.2F, 0.0D).func_181673_a(f11, f8).color(0, 0, 0, 255).endVertex();
            worldrenderer.pos(f9, f10, f6).func_181673_a(f11, f7).color(255, 255, 255, 255).endVertex();
        }
