...
public class RenderItem implements IResourceManagerReloadListener, InstanceAccess {
    ...
    private void renderModel(final IBakedModel model, final ItemStack stack) {
        this.renderModel(model, -1, stack, 1);
    }

    public void renderModel(final IBakedModel model, final int color) {
        this.renderModel(model, color, null, 1);
    }

    private void renderModel(final IBakedModel model, final int color, final ItemStack stack, double size) {

        GlStateManager.enableAlpha();

        GlStateManager.scale(size, size, size);
        double translate = (size - 1) * 0.125 * -1;
...
//                if (this.getModule(KillGlow.class).isEnabled() && mc.thePlayer.getHeldItem() != null && stack == mc.thePlayer.getHeldItem()) {
//                    this.renderModel(model, new Color(255, 255, 255, 255).hashCode(), stack, 1);
//
//                    for (int i = 1; i <= 4; ++i) {
//                        this.renderModel(model, new Color(255, 0, 0, 50).hashCode(), stack, 1 + i / 150f);
//                    }
//                } else {
...
//                }
    ...
    private void renderEffect(final IBakedModel model) {
        if (!Config.isCustomItems() || CustomItems.isUseGlint()) {
            if (!Config.isShaders() || !Shaders.isShadowPass) {
                ...
                final boolean enabled = Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getModule("Enchant")).isEnabled();

                if (enabled)
                    if (((BooleanSetting) Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getSetting("Enchant", "Rainbow"))).isEnabled())
                        color = ColorUtil.getRainbow();
                    else
                        color = Color.HSBtoRGB((float) ((NumberSetting) Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getSetting("Enchant", "Hue"))).getValue() / 255, 0.6f, 1);

                final int s = (int) ((NumberSetting) Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getSetting("Enchant", "Strength"))).getValue();
    ...
    public void renderItemIntoGUI(final ItemStack stack, final float x, final float y) {
    ...
    private void setupGuiTransform(final float xPosition, final float yPosition, final boolean isGui3d) {
    ...
    public void renderItemAndEffectIntoGUI(final ItemStack stack, final float xPosition, final float yPosition) {
