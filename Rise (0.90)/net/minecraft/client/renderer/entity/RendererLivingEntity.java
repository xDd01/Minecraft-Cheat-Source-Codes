...
import dev.rise.module.impl.render.ESP;
import dev.rise.module.impl.render.ShaderESP;
import dev.rise.module.manager.ModuleManager;
import dev.rise.setting.impl.BooleanSetting;
import dev.rise.setting.impl.ModeSetting;
import dev.rise.setting.impl.NumberSetting;
import dev.rise.util.InstanceAccess;
import dev.rise.util.render.OutlineUtil;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> implements InstanceAccess {
    public static final boolean animateModelLiving = Boolean.getBoolean("animate.model.living");
    protected final static FloatBuffer shaderBrightnessBuffer = GLAllocation.createDirectFloatBuffer(4);
    ...
    public static float dmgRed, dmgGreen, dmgBlue, dmgAlpha;
    ...
    public void doRender(final T entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z))) {
            ...
            final float distanceToPlayer = entity.getDistanceToEntity(mc.thePlayer);
            ...
            try {
                ...
                double size = ((NumberSetting) Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getSetting("Giants", "Size"))).getValue();

                if (Rise.INSTANCE.getModuleManager().getModule("Giants").isEnabled())
                ...
                //Not sure what it does but doesn't seem to have any effect generally
                ...
                if (this.renderOutlines) {
                    ...
                    if (flag && !(this.getModule(Animations.class).isEnabled() && this.getBoolean(Animations.class, "Old Damage"))) {
            ...
            if (!this.renderOutlines) {
                if (distanceToPlayer < 30 || mc.gameSettings.ofKeyBindZoom.isKeyDown())
                    super.doRender(entity, x, y, z, entityYaw, partialTicks);
            }

            //No clue what this does but it has a heavy performance impact so im removing it
    ...
    protected void renderModel(final T entitylivingbaseIn, final float p_77036_2_, final float p_77036_3_, final float p_77036_4_, final float p_77036_5_, final float p_77036_6_, final float p_77036_7_) {
        ...
        if (flag || flag1) {
            ...
            //RotationUtil.getrotation
            if (entitylivingbaseIn instanceof EntityPlayer && entitylivingbaseIn != mc.thePlayer && !entitylivingbaseIn.bot && this.getModule(ESP.class).isEnabled() && mc.thePlayer.getDistanceToEntity(entitylivingbaseIn) < maxDistance) {
            ...
            // Hidden.
            if (Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getModule("Chams")).isEnabled() && Chams.entity(entitylivingbaseIn) && entitylivingbaseIn.hurtTime >= 0 && modes.equals("CSGO")) {
                GL11.glPushMatrix();
                GL11.glEnable(10754);
                ...
                final Color chamsColorHidden = color2;
                GL11.glColor4f(chamsColorHidden.getRed() / 255F, chamsColorHidden.getGreen() / 255F, chamsColorHidden.getBlue() / 255F, chamsColorHidden.getAlpha() / 255F);
                GL11.glDisable(2929);
            ...
            if (Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getModule("Chams")).isEnabled() && Chams.entity(entitylivingbaseIn) && entitylivingbaseIn.hurtTime >= 0 && modes.equals("CSGO")) {
                ...
                final Color chamsColorVisible = color;
                GL11.glColor4f(chamsColorVisible.getRed() / 255F, chamsColorVisible.getGreen() / 255F, chamsColorVisible.getBlue() / 255F, chamsColorVisible.getAlpha() / 255F);
                this.mainModel.render(entitylivingbaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_, p_77036_7_);
                GL11.glEnable(2896);
    ...
    protected boolean setBrightness(final T entitylivingbaseIn, final float partialTicks, final boolean combineTextures) {
        if (ShaderESP.runningShader) return false;
        final String modes = ((ModeSetting) Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getSetting("Chams", "Mode"))).getMode();
        ...
        if (isModuleEnabled)
            isModuleEnabled = this.getModule(Chams.class).isEnabled() && modes.equals("Normal");
        ...
        if (!flag && !flag1 && !isModuleEnabled) {
            ...
            if (flag1) {
                final ModuleManager moduleManager = Rise.INSTANCE.getModuleManager();

                if (Objects.requireNonNull(moduleManager.getModule("DamageColor")).isEnabled()) {
                    this.brightnessBuffer.put(dmgRed);
                    this.brightnessBuffer.put(dmgGreen);
                    this.brightnessBuffer.put(dmgBlue);
                    this.brightnessBuffer.put(dmgAlpha);

                    if (Config.isShaders()) {
                        Shaders.setEntityColor(dmgRed, dmgGreen, dmgBlue, dmgAlpha);
                    }
                } else {
                ...
                if (isModuleEnabled && entitylivingbaseIn.hurtTime == 0) {
                    ...
                    if (lightning) GlStateManager.enableLighting();
                } else {
