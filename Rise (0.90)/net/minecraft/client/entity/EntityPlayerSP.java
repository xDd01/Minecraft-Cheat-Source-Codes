...
import dev.rise.module.impl.movement.No003;
import dev.rise.module.impl.movement.NoSlow;
import dev.rise.module.impl.movement.Phase;
...
public class EntityPlayerSP extends AbstractClientPlayer implements InstanceAccess {
    ...
    public boolean allowMultiDirSprint;
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            if (this == entityPlayer) new UpdateEvent().call();
    ...
    public void onUpdateWalkingPlayer() {
        ...
        RotationUtil.serverPitch = preMotionEvent.getPitch();
        RotationUtil.serverYaw = preMotionEvent.getYaw();
        ...
        if (flag1 != this.serverSneakState) {
            ...
            } else if (!(Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getModule("Sneak")).isEnabled() && ((ModeSetting) Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getSetting("Sneak", "Mode"))).is("Packet"))) {
        ...
        if (this.isCurrentViewEntity()) {
            ...
            boolean flag2 = d0 * d0 + d1 * d1 + d2 * d2 > (this.getModule(No003.class).isEnabled() ? 0.0 : 9.0E-4D) || this.positionUpdateTicks >= 20;
    ...
    public void sendChatMessage(String message) {
        if (Rise.INSTANCE.onSendChatMessage(message)) {
    ...
    public void closeScreen() {
        final Module stealerModule = Rise.INSTANCE.getModuleManager().getModule("Stealer");
        if (Stealer.closeAfterContainer && stealerModule.isEnabled()) stealerModule.toggleModule();
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        if (!(this.noClip || (this.getModule(Freecam.class).isEnabled() && this.getBoolean(Freecam.class, "No Clip") && this.getBoolean(Freecam.class, "Fly")) || (this.getModule(Phase.class).isEnabled() && !this.getMode(Phase.class, "Mode").equals("Hypixel")))) {
    ...
    public void onLivingUpdate() {
        ...
        boolean noslow = this.getModule(NoSlow.class).isEnabled();
        ...
        if (this.onGround && !flag1 && !flag2 && (this.allowMultiDirSprint || this.movementInput.moveForward >= f) && !this.isSprinting() && flag3 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness)) {
        ...
        if (!this.isSprinting() && (this.allowMultiDirSprint || this.movementInput.moveForward >= f) && flag3 && (!this.isUsingItem() || noslow) && !this.isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.isKeyDown()) {
            this.setSprinting(true);
        }

        if (this.isSprinting() && (!this.allowMultiDirSprint && (this.movementInput.moveForward < f || this.isCollidedHorizontally || !flag3))) {
            this.setSprinting(false);
        }
