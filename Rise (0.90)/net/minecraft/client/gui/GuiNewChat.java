                ...
                for (i1 = 0; i1 + this.scrollPos < this.field_146253_i.size() && i1 < i; ++i1) {
                    ...
                    if (chatline != null) {
                        ...
                        if (j1 < 200 || flag) {
                            ...
                            if (l1 > 3) {
                                final int height = Interface.customChat ? (int) Math.round(comfortaa.getHeight()) : 9;

                                final int i2 = 0;
                                final int j2 = -i1 * height;

                                final boolean clear = ((BooleanSetting) Objects.requireNonNull(Rise.INSTANCE.getModuleManager().getSetting("Interface", "Transparent Chat"))).isEnabled();

                                if (!clear) {
                                    drawRect(i2, j2 - height - height, i2 + l + 4, j2 - height, l1 / 2 << 24);
                                }
                                ...
                                if (Interface.customChat) {
                                    comfortaa.drawStringWithColorCode(s, (float) i2, (float) (j2 - height) - height + 1.5f, 16777215 + (l1 << 24));
                                } else {
                                    this.mc.fontRendererObj.drawStringWithShadow(s, (float) i2, (float) (j2 - 8) - height, 16777215 + (l1 << 24));
                                }
