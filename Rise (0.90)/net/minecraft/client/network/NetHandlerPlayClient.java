...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handleSpawnPlayer(final S0CPacketSpawnPlayer packetIn) {
        ...
        Rise.INSTANCE.getAntiCheat().getRegistrationListener().handleSpawn(entityotherplayermp);
    ...
    public void handleEntityMovement(final S14PacketEntity packetIn) {
        ...
        if (entity != null) {
            ...
            final double d1 = (double) entity.serverPosY / 32.0D;
            final double d2 = (double) entity.serverPosZ / 32.0D;
            final float f = packetIn.func_149060_h() ? (float) (packetIn.func_149066_f() * 360) / 256.0F : entity.rotationYaw;
            final float f1 = packetIn.func_149060_h() ? (float) (packetIn.func_149063_g() * 360) / 256.0F : entity.rotationPitch;
    ...
    public void handleDestroyEntities(final S13PacketDestroyEntities packetIn) {
        ...
        for (int i = 0; i < packetIn.getEntityIDs().length; ++i) {
            final Entity entity = this.clientWorldController.getEntityByID(packetIn.getEntityIDs()[i]);

            if (entity instanceof EntityOtherPlayerMP)
                Rise.INSTANCE.getAntiCheat().getRegistrationListener().handleDestroy(entity.getUniqueID());
    ...
    public void handlePlayerPosLook(final S08PacketPlayerPosLook packetIn) {
        ...
        final ModuleManager manager = Rise.INSTANCE.getModuleManager();
        if (Objects.requireNonNull(manager.getModule("NoRot")).isEnabled()) {
            switch (((ModeSetting) Objects.requireNonNull(manager.getSetting("NoRot", "Mode"))).getMode()) {
                case "Edit":
                ...
                case "Hypixel":
                    entityplayer.setPosition(d0, d1, d2);
                    this.netManager.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, f, f1, false));
                    break;
    ...
    public void handleExplosion(final S27PacketExplosion packetIn) {
        // If any false flags are detected just change the multiplier.
        // Keep in mind the velocity is actually (velocity / 8000D)
        ...
        if (packetIn.func_149149_c() >= threshold
            ...
            Rise.INSTANCE.getNotificationManager().registerNotification(
                    "This server attempted to crash your game with a large explosion. This was prevented by Rise."
            );
    ...
    public boolean validateResourcePackUrl(String url, final String hash) {
        try {
            ...
            if (isLevelProtocol && (url.contains("..") || !url.endsWith("/resources.zip"))) {
                ...
                if (player != null) {
                    Rise.addChatMessage("Server attempted to check if a file directory exists but was blocked by Rise.");
                    Rise.addChatMessage("Directory: " + url.substring(3));
                }
