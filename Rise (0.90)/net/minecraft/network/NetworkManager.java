...
import dev.rise.viamcp.handler.CommonTransformer;
import dev.rise.viamcp.handler.MCPDecodeHandler;
import dev.rise.viamcp.handler.MCPEncodeHandler;
import dev.rise.viamcp.utils.NettyUtil;
        ...
        ((((new Bootstrap()).group(lazyloadbase.getValue())).handler(new ChannelInitializer<Channel>() {
            protected void initChannel(Channel p_initChannel_1_) throws Exception {
                try {
                    ...
                    if (ProxyGUI.isProxyAvailable()) {
                        switch (ProxyGUI.getProxyType().toUpperCase()) {
                            case "HTTP":
                                p_initChannel_1_.pipeline().addFirst(new HttpProxyHandler(new InetSocketAddress(ProxyGUI.ip, ProxyGUI.port)));
                                break;
                            case "SOCKS5":
                                p_initChannel_1_.pipeline().addFirst(new Socks5ProxyHandler(new InetSocketAddress(ProxyGUI.ip, ProxyGUI.port)));
                                break;
                            case "SOCKS4":
                            default:
                                p_initChannel_1_.pipeline().addFirst(new Socks4ProxyHandler(new InetSocketAddress(ProxyGUI.ip, ProxyGUI.port)));
                                break;
                        }
                    }
                ...
                if (Rise.INSTANCE.versionSwitcher.getEnabled()) {
