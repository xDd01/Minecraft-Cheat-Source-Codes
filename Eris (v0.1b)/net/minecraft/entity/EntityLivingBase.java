...
public abstract class EntityLivingBase extends Entity {
    ...
    protected void jump() {
        if (this == Minecraft.getMinecraft().thePlayer) {
            double toSet = (double) this.getJumpUpwardsMotion();

            if (this.isPotionActive(Potion.jump)) {
                toSet += (double) ((float) (this.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1F);
            }

            EventJump ej = new EventJump(toSet);
            ej.call();
            if (ej.isCancelled()) {
                return;
            }
