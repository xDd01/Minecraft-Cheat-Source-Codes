...
public class RenderChunk
{
    ...
    public void rebuildChunk(float x, float y, float z, ChunkCompileTaskGenerator generator)
    {
        ...
        if (!this.isChunkRegionEmpty(blockpos))
        {
            ...
            for (Object blockposm : BlockPosM.getAllInBoxMutable(blockpos, blockpos1))
            {
                IBlockState iblockstate = chunkcacheof.getBlockState((BlockPosM)blockposm);
                Block block = iblockstate.getBlock();

                if (block.isOpaqueCube())
                {
                    lvt_10_1_.func_178606_a((BlockPosM)blockposm);
                }

                if (ReflectorForge.blockHasTileEntity(iblockstate))
                {
                    TileEntity tileentity = chunkcacheof.getTileEntity(new BlockPos((BlockPosM)blockposm));
                ...
                for (int j = 0; j < aenumworldblocklayer.length; ++j)
                {
                    ...
                    if (block.getRenderType() != -1)
                    {
                        ...
                        RenderEnv renderenv = worldrenderer.getRenderEnv(iblockstate, (BlockPosM)blockposm);
                        ...
                        aboolean[k] |= blockrendererdispatcher.renderBlock(iblockstate, (BlockPosM)blockposm, chunkcacheof, worldrenderer);
