...
//    private BooleanProperty prop_chests = new BooleanProperty("Chests", "Enables chests for ESP", () -> false, false);
	...
	public ESP() {
		...
		registerProperties(prop_chestColor, prop_elements, prop_healthStyle, prop_entites, prop_avoid, prop_boxColor,
				prop_boxTargetColor, prop_boxOutlineColor, prop_nameColor, prop_healthColor);
	}
	...
	public void onEnable() {
		...
		if (prop_elements.getValue().get("Chests")) {
			if (cheat_chestStealer == null) {
				cheat_chestStealer = (ChestStealer) Cloverhook.instance.cheatManager.getCheatRegistry()
						.get("ChestStealer");
			}
		}
	}
	...
	public void onRenderOverlay(RenderOverlayEvent renderOverlayEvent) {
		...
		entityLoop: for (EntityLivingBase entity : collectEntities()) {
			...
			currentEntityPoints[0] = project2D(
					boundingBox.minX - Minecraft.getMinecraft().getRenderManager().renderPosX,
					boundingBox.minY - Minecraft.getMinecraft().getRenderManager().renderPosY,
					boundingBox.minZ - Minecraft.getMinecraft().getRenderManager().renderPosZ);
			currentEntityPoints[1] = project2D(
					boundingBox.minX - Minecraft.getMinecraft().getRenderManager().renderPosX,
					boundingBox.minY - Minecraft.getMinecraft().getRenderManager().renderPosY,
					boundingBox.maxZ - Minecraft.getMinecraft().getRenderManager().renderPosZ);
			currentEntityPoints[2] = project2D(
					boundingBox.maxX - Minecraft.getMinecraft().getRenderManager().renderPosX,
					boundingBox.minY - Minecraft.getMinecraft().getRenderManager().renderPosY,
					boundingBox.minZ - Minecraft.getMinecraft().getRenderManager().renderPosZ);
			...
			currentEntityPoints[4] = project2D(
					boundingBox.minX - Minecraft.getMinecraft().getRenderManager().renderPosX,
					boundingBox.maxY - Minecraft.getMinecraft().getRenderManager().renderPosY,
					boundingBox.minZ - Minecraft.getMinecraft().getRenderManager().renderPosZ);
			...
			if (cheat_killAura == null) {
				cheat_killAura = (Aura) Cloverhook.instance.cheatManager.getCheatRegistry().get("KillAura");
			}
			if (prop_elements.getValue().get("Box")) {
				// LEFT
				if (cheat_killAura != null && cheat_killAura.getCurrentTarget() != null) {
					Draw.drawRectangle(renderPosX1, renderPosY1, renderPosX1 + 0.5, renderPosY2,
					...
					// TOP
					Draw.drawRectangle(renderPosX1, renderPosY1, renderPosX2, renderPosY1 + 0.5,
							cheat_killAura.getCurrentTarget() != null && cheat_killAura.getCurrentTarget() == entity
									|| entity.getName().toLowerCase().contains("bobicraft")
											? prop_boxTargetColor.getValue().getRGB()
											: prop_boxColor.getValue().getRGB());
					// RIGHT
					Draw.drawRectangle(renderPosX2 - 0.5, renderPosY1, renderPosX2, renderPosY2,
							cheat_killAura.getCurrentTarget() != null && cheat_killAura.getCurrentTarget() == entity
									|| entity.getName().toLowerCase().contains("bobicraft")
											? prop_boxTargetColor.getValue().getRGB()
											: prop_boxColor.getValue().getRGB());
					// BOTTOM
					Draw.drawRectangle(renderPosX1, renderPosY2 - 0.5, renderPosX2, renderPosY2,
							cheat_killAura.getCurrentTarget() != null && cheat_killAura.getCurrentTarget() == entity
									|| entity.getName().toLowerCase().contains("bobicraft")
											? prop_boxTargetColor.getValue().getRGB()
											: prop_boxColor.getValue().getRGB());
				...
				// LEFT
				Draw.drawRectangle(renderPosX1 + 0.5, renderPosY1 + 0.5, renderPosX1 + 1.0, renderPosY2 - 0.5,
						prop_boxOutlineColor.getValue().getRGB());
				// TOP
				Draw.drawRectangle(renderPosX1 + 0.5, renderPosY1 + 0.5, renderPosX2 - 0.5, renderPosY1 + 1.0,
						prop_boxOutlineColor.getValue().getRGB());
				// RIGHT
				Draw.drawRectangle(renderPosX2 - 1.0, renderPosY1 + 0.5, renderPosX2 - 0.5, renderPosY2 - 0.5,
						prop_boxOutlineColor.getValue().getRGB());
				// BOTTOM
				Draw.drawRectangle(renderPosX1 + 0.5, renderPosY2 - 1.0, renderPosX2 - 0.5, renderPosY2 - 0.5,
						prop_boxOutlineColor.getValue().getRGB());

				// LEFT
				Draw.drawRectangle(renderPosX1, renderPosY1 - 0.5, renderPosX1 - 0.5, renderPosY2 + 0.5,
						prop_boxOutlineColor.getValue().getRGB());
				// TOP
				Draw.drawRectangle(renderPosX1 - 0.5, renderPosY1, renderPosX2 + 0.5, renderPosY1 - 0.5,
						prop_boxOutlineColor.getValue().getRGB());
				// RIGHT
				Draw.drawRectangle(renderPosX2 + 0.5, renderPosY1 - 0.5, renderPosX2, renderPosY2 + 0.5,
						prop_boxOutlineColor.getValue().getRGB());
				// BOTTOM
				Draw.drawRectangle(renderPosX1 - 0.5, renderPosY2 + 0.5, renderPosX2 + 0.5, renderPosY2,
						prop_boxOutlineColor.getValue().getRGB());
			}
			...
			if (prop_elements.getValue().get("Name")) {
				...
				Fonts.verdana3.drawCenteredStringWithShadow(entity.getName(),
						(renderPosX1 + ((renderPosX2 - renderPosX1) / 2)) / scale, ((renderPosY1) / scale) - 16,
						prop_nameColor.getValue().getRGB());
			...
			if (prop_elements.getValue().get("Health")) {
				if (prop_healthStyle.getValue().get("Bar")) {
					...
					Draw.drawBorderedRectangle(renderPosX1 - 3, renderPosY2 - height, renderPosX1 - 1.5, renderPosY2,
							0.5F, ColorCreator.create(r, g, b, 255), ColorCreator.create(0,0,0),true);
				}
						...
						Fonts.f12.drawStringWithShadow(String.valueOf((int) (entity.getHealth() / 20 * 100)) + "%",
								renderPosX1 / scale - 4
										- Fonts.f12.getStringWidth(
												String.valueOf((int) (entity.getHealth() / 20 * 100)) + "%"),
