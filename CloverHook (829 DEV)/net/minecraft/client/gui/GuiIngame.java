...
import club.cloverhook.cheat.impl.misc.AutoPlay;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        renderOverlayEvent.setStage(Stage.PRE);
        Cloverhook.eventBus.publish(renderOverlayEvent);
        ...
        if (this.field_175195_w > 0) {
            ...
            if (l1 > 8) {
                ...
                mc.fontRendererObj.drawString(this.field_175201_x, (float) (-this.getFontRenderer().getStringWidth(this.field_175201_x) / 2), -10.0F, 16777215 | j2, true);
                GlStateManager.popMatrix();
                GlStateManager.pushMatrix();
                GlStateManager.scale(2.0F, 2.0F, 2.0F);
                mc.fontRendererObj.drawString(this.field_175200_y, (float) (-this.getFontRenderer().getStringWidth(this.field_175200_y) / 2), 5.0F, 16777215 | j2, true);
                GlStateManager.popMatrix();
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
        ...
        renderOverlayEvent.setStage(Stage.POST);
        Cloverhook.eventBus.publish(renderOverlayEvent);
    }
    ...
    public void func_181551_a(ScaledResolution p_181551_1_) {
        ...
        if (this.remainingHighlightTicks > 0 && this.highlightingItemStack != null) {
            ...
            if (k > 0) {
                ...
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                Fonts.verdanaChat.drawStringWithShadow(s, (float) i, (float) j, 16777215 + (k << 24));
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
        }
    ...
    public void renderDemo(ScaledResolution p_175185_1_) {
        ...
        int i = Fonts.verdanaChat.getStringWidth(s);
        Fonts.verdanaChat.drawStringWithShadow(s, (float) (p_175185_1_.getScaledWidth() - i - 10), 5.0F, 16777215);
        this.mc.mcProfiler.endSection();
    }

    protected boolean showCrosshair() {
        if (Cloverhook.instance.cheatManager.isCheatEnabled("Crosshair"))
            return false;
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        int i = Fonts.verdanaChat.getStringWidth(p_180475_1_.getDisplayName());

        for (Object score0 : arraylist1) {
            Score score = (Score) score0;
            ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(score.getPlayerName());
            String s = ScorePlayerTeam.formatPlayerName(scoreplayerteam, score.getPlayerName()) + ": " + EnumChatFormatting.RED + score.getScorePoints();
            i = Math.max(i, Fonts.verdanaChat.getStringWidth(s));
        }

        int j1 = arraylist1.size() * 10;
        ...
        for (Object score10 : arraylist1) {
            ...
            Fonts.verdanaChat.drawStringWithShadow(s1, j, l, 553648127);
            Fonts.verdanaChat.drawStringWithShadow(s2, i1 - this.getFontRenderer().getStringWidth(s2), l, 553648127);
            insane = s1.contains("Insane") || s2.contains("Insane");
            if (!(mc.currentScreen instanceof GuiDownloadTerrain) || Objects.nonNull(mc.thePlayer)) {
                Stopwatch timer = new Stopwatch();
                boolean autoplay = Cloverhook.instance.cheatManager.isCheatEnabled("AutoPlay");
                boolean solo = AutoPlay.mode.getValue().get("Solo");
                if (autoplay && timer.hasPassed(Mafs.getRandomInRange(2000, 3000)) && mc.thePlayer.isSpectator()) {
                    if (solo) {
                        mc.thePlayer.sendChatMessage("/play solo_" + (insane ? "insane" : "normal"));
                    } else {
                        mc.thePlayer.sendChatMessage("/play doubles_" + (insane ? "insane" : "normal"));
                    }
                }
            }
            ...
            if (k == arraylist1.size()) {
                ...
                drawRect(j - 2, l - 10 - 1, i1, l - 1, 1610612736);
                drawRect(j - 2, l - 1, i1, l, 1342177280);
                Fonts.verdanaChat.drawStringWithShadow(s3, j + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2, l - 10, 553648127);
            }
        }
    ...
    private void renderBossHealth() {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
            ...
            Fonts.verdanaChat.drawStringWithShadow(s, (float) (i / 2 - this.getFontRenderer().getStringWidth(s) / 2), (float) (b0 - 10), 16777215);
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            this.mc.getTextureManager().bindTexture(icons);
        }
    }
