...
import club.mega.event.impl.EventLook;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    {
        return Animations.getInstance().isToggled() ? Animations.getInstance().swingSlowness.getAsInt() : (this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6));
    }
    ...
    protected void jump()
    {
        ...
        if (this.isSprinting())
        {
            ...
            if (Mega.INSTANCE.getModuleManager().isToggled(KillAura.class) && AuraUtil.getTarget() != null && Mega.INSTANCE.getModuleManager().getModule(KillAura.class).moveFix.get())
                yaw = AuraUtil.getRotations()[0];
