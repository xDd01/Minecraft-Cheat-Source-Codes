...
import club.mega.module.impl.visual.Animations;
...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            {
                if (!KillAura.getInstance().isToggled() || AuraUtil.getTarget() == null || !KillAura.getInstance().fakeBlock.get())
                ...
                else if (abstractclientplayer.getItemInUseCount() > 0)
                {
                    ...
                    switch (enumaction)
                    {
                        ...
                        case BLOCK:
                            if (Animations.getInstance().isToggled()) animate(f, f1); else {
                                this.transformFirstPersonItem(f, 0.0F);
    ...
    private void animate(final float f, final float f1) {
        float equipProgress;
        if(Animations.getInstance().equipProgress.get())
            equipProgress = f * Animations.getInstance().equipProgressMultiplier.getAsFloat();
         else
            equipProgress = 0;


        float f2 = MathHelper.sin(f1 * f1 * (float)Math.PI);
        float f3 = MathHelper.sin(MathHelper.sqrt_float(f1) * (float)Math.PI);

        current = AnimationUtil.animate(current, f1, 1);
        current2 = AnimationUtil.animate(current2, f2, 0.02);
        switch (Animations.getInstance().mode.getCurrent())
        {
            case "Mega":
                GlStateManager.translate(0.56F, -0.52F, -00.71999997F);
                GlStateManager.translate(0.0F, equipProgress * -0.6F + 0.1, -.1F);
                GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f2 * -30.0F, 1.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f3 * -30.0F, 1.0F, 0.0F, 1.0F);
                GlStateManager.rotate(f3 * -20.0F, 1.0F, 0.0F, 0.0F);
                GlStateManager.scale(0.4F, 0.4F, 0.4F);
                break;
            case "Mega2":
                break;
            case "Mega3":
                ...
                GlStateManager.translate(0.0F, equipProgress * -0.6F + 0.1, -.1F);
                GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
                f2 = MathHelper.sin((float)current * (float)current * (float)Math.PI);
                f3 = MathHelper.sin(MathHelper.sqrt_float((float) current) * (float)Math.PI);
                GlStateManager.rotate((float)current2 * -10.0F, 1.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f3 * -30.0F, (float)current, 0.0F, f2* 2);
                GlStateManager.rotate((float)current2 * -20.0F, 1.0F, 0.0F, f);
                GlStateManager.scale(0.4F, 0.4F, 0.4F);
                break;
            case "Default":
