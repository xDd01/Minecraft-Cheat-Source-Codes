...
import alphentus.mod.mods.visuals.ChestESP;
import alphentus.mod.mods.visuals.ItemESP;
import alphentus.mod.mods.visuals.PlayerESP;
...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener {
    ...
    public void renderEntities(Entity renderViewEntity, ICamera camera, float partialTicks) {
        ...
        if (this.renderEntitiesStartupCounter > 0) {
            ...
            List chestList = this.theWorld.loadedTileEntityList;
            ...
            if (this.isRenderEntityOutlines()) {
                ...
                if (Init.getInstance().modManager.getModuleByClass(PlayerESP.class).getState()) {
                ...
                if (Init.getInstance().modManager.getModuleByClass(ChestESP.class).getState()) {
                    for (int j = 0; j < chestList.size(); j++) {
                        TileEntity chest = (TileEntity) chestList.get(j);
                        if (chest != null && chest instanceof TileEntityChest) {
                            GL11.glPushMatrix();
                            GL11.glColor4f(Init.getInstance().CLIENT_COLOR.getRed()/ 255F, Init.getInstance().CLIENT_COLOR.getGreen() / 255F, Init.getInstance().CLIENT_COLOR.getBlue() / 255F, 1);
                ...
                if (Init.getInstance().modManager.getModuleByClass(ItemESP.class).getState()) {
                    for (Object o : mc.theWorld.loadedEntityList) {
                        if (o instanceof EntityItem) {
                            EntityItem item = (EntityItem) o;
                            GL11.glPushMatrix();
                            GL11.glColor4f(Init.getInstance().CLIENT_COLOR.getRed()/ 255F, Init.getInstance().CLIENT_COLOR.getGreen() / 255F, Init.getInstance().CLIENT_COLOR.getBlue() / 255F, 1);
                            this.renderManager.renderEntitySimple(item, partialTicks);
                            GL11.glPopMatrix();
                        }
                    }
                }
