...
import alphentus.gui.customhud.hud.HUDManager;
import alphentus.init.Init;
import alphentus.mod.mods.hud.Crosshair;
import alphentus.mod.mods.hud.HUD;
import alphentus.mod.mods.hud.Hotbar;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.showCrosshair() && this.mc.gameSettings.thirdPersonView < 1 && !Init.getInstance().modManager.getModuleByClass(Crosshair.class).getState()) {
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            boolean hotbar = Init.getInstance().modManager.getModuleByClass(Hotbar.class).getState();


            if (hotbar) {
                if (Init.getInstance().modManager.getModuleByClass(HUD.class).isCustom()) {
                    if (Init.getInstance().modManager.getModuleByClass(Hotbar.class).blur.isState())
                        Init.getInstance().blurUtil.blur(i - 91, sr.getScaledHeight() - 24, 182, sr.getScaledHeight(), 30);

                    Gui.drawRect(i - 91, sr.getScaledHeight() - 24, i - 91 + 182, sr.getScaledHeight() - 2, 0x99000000);
                    Gui.drawRect(i - 91 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 24, i - 91 - 1 + entityplayer.inventory.currentItem * 20 + 23, sr.getScaledHeight() - 2, new Color(Init.getInstance().CLIENT_COLOR.getRed(), Init.getInstance().CLIENT_COLOR.getGreen(), Init.getInstance().CLIENT_COLOR.getBlue(), 125).brighter().getRGB());
                } else if (Init.getInstance().modManager.getModuleByClass(HUD.class).theme.getSelectedCombo().equals("Alphentus")) {
                    // Background
                    Gui.drawRect(i - 91, sr.getScaledHeight() - 24, i - 91 + 182, sr.getScaledHeight(), 0xFF202020);

                    Gui.drawRect(0, sr.getScaledHeight() - 24, 100, sr.getScaledHeight(), 0xFF202020);
                    Gui.drawRect(sr.getScaledWidth() - 100, sr.getScaledHeight() - 24, sr.getScaledWidth(), sr.getScaledHeight(), 0xFF202020);

                    int playerPing = 0;

                    if (mc.thePlayer != null && mc.thePlayer.isServerWorld() && mc.theWorld != null) {
                    ...
                    // Text
                    Init.getInstance().fontManager.stem20.drawStringWithShadow("FPS: " + Minecraft.getDebugFPS(), 2, sr.getScaledHeight() - 22, 0xFFFFFFFF, false);
                    Init.getInstance().fontManager.stem20.drawStringWithShadow("Ping: " + playerPing, 2, sr.getScaledHeight() - 11, 0xFFFFFFFF, false);

                    Init.getInstance().fontManager.stem20.drawStringWithShadow("Y: " + Math.round(mc.thePlayer.posY), sr.getScaledWidth() - 98, sr.getScaledHeight() - 22, 0xFFFFFFFF, false);
                    Init.getInstance().fontManager.stem20.drawStringWithShadow("Facing: " + mc.thePlayer.getHorizontalFacing().toString().toUpperCase(), sr.getScaledWidth() - 98, sr.getScaledHeight() - 11, 0xFFFFFFFF, false);

                    // Current Slot
                    Gui.drawRect(i - 91 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 24, i + entityplayer.inventory.currentItem * 20 - 69, sr.getScaledHeight(), 0xBBFFFFFF);
                }
                if (Init.getInstance().modManager.getModuleByClass(HUD.class).theme.getSelectedCombo().equals("Sigma")) {
                    if (Init.getInstance().modManager.getModuleByClass(Hotbar.class).blur.isState())
                        Init.getInstance().blurUtil.blur(i - 91, sr.getScaledHeight() - 24, 182, sr.getScaledHeight(), 30);
            ...
            for (int j = 0; j < 9; ++j) {
                ...
                int test = hotbar ? 5 : 3;
                int l = sr.getScaledHeight() - 16 - test;
        ...
        GL11.glPushMatrix();
        RenderUtils.setDeltaTime();
        EventManager.call(new Event(Type.RENDER2D));
        GL11.glPopMatrix();
    ...
    public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_) {
        ...
        if (i > 0) {
            ...
            int test = hotbar ? 1 : 3;
            int j = p_175176_1_.getScaledHeight() - 32 + test;
    ...
    private void renderPlayerStats(ScaledResolution p_180477_1_) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            int test = hotbar ? 42 : 39;
            int k1 = p_180477_1_.getScaledHeight() - test;
