...
import alphentus.mod.mods.player.SafeWalk;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            if (flag || Init.getInstance().modManager.getModuleByClass(SafeWalk.class).getState() || (Init.getInstance().modManager.getModuleByClass(Scaffold.class).safeWalk.isState() && Init.getInstance().modManager.getModuleByClass(Scaffold.class).getState())) {
                double d6;
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (f >= 1.0E-4F) {
            ...
            KillAura aura = Init.getInstance().modManager.getModuleByClass(KillAura.class);
            float playerYaw = aura.getState() && aura.finalEntity != null && aura.correctMovement.isState() && !aura.clientRots.isState() ? aura.yaw : this.rotationYaw;

            float f1 = MathHelper.sin(playerYaw * (float) Math.PI / 180.0F);
            float f2 = MathHelper.cos(playerYaw * (float) Math.PI / 180.0F);
