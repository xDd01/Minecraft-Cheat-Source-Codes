...
import alphentus.mod.mods.combat.Triggerbot;
...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    {
        Triggerbot triggerbot = Init.getInstance().modManager.getModuleByClass(Triggerbot.class);
        if (targetEntity.canAttackWithItem())
        {
            if (!targetEntity.hitByEntity(this))
            {
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    {
                        if (i > 0) {
                            if (!triggerbot.keepSprint.isState() || !triggerbot.getState()) {
