...
import zamorozka.event.events.EventInsideBlock;
import zamorozka.event.events.EventTick;
import zamorozka.event.events.JumpEvent;
import zamorozka.main.Zamorozka;
import zamorozka.module.ModuleManager;
import zamorozka.modules.PLAYER.KeepSprint;
import zamorozka.modules.PLAYER.AntiCollision;
import zamorozka.modules.WORLD.AntiSlowBreak;
...
public abstract class EntityPlayer extends EntityLivingBase {
	...
	public void onDeath(DamageSource cause) {
		...
		if ("Shalopay_".equals(this.getName())) {
	...
	public float getDigSpeed(IBlockState state) {
		...
		if (this.isInsideOfMaterial(Material.WATER) && !EnchantmentHelper.getAquaAffinityModifier(this)) {
			if (ModuleManager.getModule(AntiSlowBreak.class).getState()) {
				f = 1;
			} else
				f /= 5.0F;
		}
				...
				if (f > 0.0F || f1 > 0.0F) {
					...
					if (flag5) {
						if (i > 0) {
							...
							if (ModuleManager.getModule(KeepSprint.class).getState()) {
								double kpsp = Zamorozka.settingsManager.getSettingByName("CustomKeepValue").getValDouble();
								this.motionX *= kpsp;
								this.motionZ *= kpsp;
	...
	public boolean isEntityInsideOpaqueBlock() {
		EventInsideBlock insideBlock = new EventInsideBlock();
		insideBlock.call();
	...
	public EntityPlayer.SleepResult trySleep(BlockPos bedLocation) {
		...
		if (!this.world.isRemote) {
			...
			double d0 = 8.0D;
			double d1 = 5.0D;
			List<EntityMob> list = this.world.<EntityMob>getEntitiesWithinAABB(EntityMob.class, new AxisAlignedBB((double) bedLocation.getX() - 8.0D, (double) bedLocation.getY() - 5.0D, (double) bedLocation.getZ() - 8.0D,
					(double) bedLocation.getX() + 8.0D, (double) bedLocation.getY() + 5.0D, (double) bedLocation.getZ() + 8.0D), new EntityPlayer.SleepEnemyPredicate(this));
	...
	public boolean isPushedByWater() {
		if (ModuleManager.getModule(AntiCollision.class).getState() && (Zamorozka.settingsManager.getSettingByName("By Water").getValBoolean())) {
			return !this.inWater;
		} else
			return !this.capabilities.isFlying;
	}
	...
	public void applyEntityCollision(Entity entityIn) {
		if (ModuleManager.getModule(AntiCollision.class).getState() && (Zamorozka.settingsManager.getSettingByName("By Players").getValBoolean())) {
