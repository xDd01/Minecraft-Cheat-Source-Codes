...
import zamorozka.modules.PLAYER.NoArrowSound;
	...
	protected void onHit(RayTraceResult raytraceResultIn) {
		...
		if (entity != null) {
			...
			if (entity.attackEntityFrom(damagesource, (float) i)) {
				...
				if (ModuleManager.getModule(NoArrowSound.class).getState()) {

				} else
					this.playSound(SoundEvents.ENTITY_ARROW_HIT, 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
				if (mc.player.getHeldItemMainhand().getItem() instanceof ItemBow) {
					if (((EntityPlayer) entity).hurtTime > 0 && entity != mc.player && entity != null) {
						if (ModuleManager.getModule(AntiBot2.class).getState()
								&& Zamorozka.settingsManager.getSettingByName("HitBefore").getValBoolean()) {
							AntiBot2.nobotsTimolia.add((EntityPlayer) entity);
							ChatUtils.printChatprefix(ChatFormatting.RED + entity.getName() + ChatFormatting.WHITE
									+ " Already in AntiBot-List!");

							if (AntiBot2.nobotsTimolia.contains(entity)) {
								ChatUtils.printChatprefix(ChatFormatting.RED + entity.getName() + ChatFormatting.WHITE
