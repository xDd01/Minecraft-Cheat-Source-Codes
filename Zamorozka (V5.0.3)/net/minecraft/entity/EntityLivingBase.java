...
import zamorozka.modules.PLAYER.NoJumpDelay;
import zamorozka.modules.TRAFFIC.CustomJumpHeight;
import zamorozka.modules.TRAFFIC.ElytraFly;
import zamorozka.modules.VISUALLY.Animations;
...
public abstract class EntityLivingBase extends Entity {
	...
	protected void onDeathUpdate() {
		...
		if (this.deathTime == 20) {
			...
			for (int k = 0; k < 20; ++k) {
				...
				this.world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + (double) (this.rand.nextFloat() * this.width * 2.0F) - (double) this.width, this.posY + (double) (this.rand.nextFloat() * this.height),
	...
	protected void updatePotionEffects() {
		...
		if (i > 0) {
			...
			if (flag && i > 0) {
				...
				this.world.spawnParticle(flag1 ? EnumParticleTypes.SPELL_MOB_AMBIENT : EnumParticleTypes.SPELL_MOB, this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width, this.posY + this.rand.nextDouble() * (double) this.height,
	...
	public final void setArrowCountInEntity(int count) {
		if (ModuleManager.getModule(NoOverlay.class).getState() && (Zamorozka.settingsManager.getSettingByName("NoArrowInPlayer").getValBoolean())) {
	...
	private int getArmSwingAnimationEnd() {
		if (ModuleManager.getModule(Animations.class).getState()) {
			int speed = (int) (Zamorozka.settingsManager.getSettingByName("HandSpeed").getValDouble());
			return (this.isPotionActive(MobEffects.HASTE) ? speed - (1 + this.getActivePotionEffect(MobEffects.HASTE).getAmplifier()) * 1
					: (this.isPotionActive(MobEffects.MINING_FATIGUE) ? speed + (1 + this.getActivePotionEffect(MobEffects.MINING_FATIGUE).getAmplifier()) * 2 : speed));
		} else {
	...
	protected float getJumpUpwardsMotion() {
		...
		if (ModuleManager.getModule(CustomJumpHeight.class).getState()) {
			return height;
		} else
			return 0.42F;
	}
	 ...
	 */
	protected void jump() {
		EventMotionUp event = new EventMotionUp(this.getJumpUpwardsMotion());
		event.call();
	...
	public void func_191986_a(float p_191986_1_, float p_191986_2_, float p_191986_3_) {
		if (this.isServerWorld() || this.canPassengerSteer()) {
			if (!this.isInWater() || ModuleManager.getModule(zamorozka.modules.VISUALLY.Freecam.class).getState() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
				if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
					if (this.isElytraFlying()) {
						...
						if (ModuleManager.getModule(ElytraFly.class).getState()) {
							this.motionX *= 0.9999999999999999D;
							this.motionY *= 0.1D;
							this.motionZ *= 0.9999999999999999D;
						} else {
							this.motionX *= 0.9900000095367432D;
							this.motionY *= 0.9800000190734863D;
							this.motionZ *= 0.9900000095367432D;
