...
public abstract class Entity implements ICommandSender {
	...
	public void setPosition(double x, double y, double z) {
		...
		if (this instanceof EntityPlayer) {
			float f = (this.width + HitBox.getAmount(this)) / 2.0F;
			float f1 = this.height + HitBox.getAmount(this);
	...
	public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
		if (NoClip.isNoClip(this) || ModuleManager.getModule(Freecam.class).getState()) {
			...
			if (((x == MoverType.SELF || x == MoverType.PLAYER) && this.onGround && this.isSneaking() && this instanceof EntityPlayer && !Scaffold.isSneaking) || (event.isCancelled() && this instanceof EntityPlayer)
					|| ModuleManager.getModule(SafeWalk.class).getState()) {
			...
			if (eventStep.getStepHeight() > 0.0F && flag && (d2 != p_70091_2_ || d4 != p_70091_6_)) {
				...
				EventStep eventStep1 = new EventStep(false, stepHeight, 1D + p_70091_4_);
				if (this == Minecraft.getMinecraft().player) {
					eventStep1.call();
				}
			}
	...
	protected void createRunningParticles() {
		...
		if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE) {
			this.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((double) this.rand.nextFloat() - 0.5D) * (double) this.width, this.getEntityBoundingBox().minY + 0.1D,
	...
	public void func_191958_b(float p_191958_1_, float p_191958_2_, float p_191958_3_, float p_191958_4_) {
		EventStrafe eventStrafe = new EventStrafe(p_191958_1_, p_191958_2_, p_191958_3_);
		eventStrafe.call();
		...
		if (f >= 1.0E-4F) {
			...
			EventMoveFlying event = new EventMoveFlying(rotationYaw);
	...
	public Vec3d getLookCustom(float pitch, float yaw, float partialTicks) {
		if (partialTicks == 1.0F) {
			return this.getVectorForRotation(this.rotationPitch, this.rotationYaw);
		} else {
			float f = this.prevRotationPitch + (this.rotationPitch - this.prevRotationPitch) * partialTicks;
