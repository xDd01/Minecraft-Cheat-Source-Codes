...
import zamorozka.event.events.EventPlayerClickBlock;
import zamorozka.event.events.EventPostMotionUpdates;
import zamorozka.event.events.EventUpdate;
import zamorozka.main.Zamorozka;
import zamorozka.module.ModuleManager;
import zamorozka.modules.PLAYER.MultiTask;
import zamorozka.modules.TRAFFIC.Teleport;
import zamorozka.modules.WORLD.ReachBlock;
...
public class PlayerControllerMP {
	...
	public boolean onPlayerDestroyBlock(BlockPos pos) {
		DamageBlockEvent event3 = new DamageBlockEvent(pos, null);
		event3.call();

		EventBlockBreaking event = new EventBlockBreaking(EventBlockBreaking.EnumBlock.DESTROY, pos);
		event.call();
	...
	public boolean clickBlock(BlockPos loc, EnumFacing face) {
		...
		l_Event.call();
		final EventBlockBreaking event = new EventBlockBreaking(EventBlockBreaking.EnumBlock.CLICK, loc, face);
		event.call();
		if (this.currentGameType.isAdventure()) {
			if (this.currentGameType == GameType.SPECTATOR) {
				return false;
	...
	public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing) {
		DamageBlockEvent event2 = new DamageBlockEvent(posBlock, directionFacing);
		event2.call();
	...
	public float getBlockReachDistance() {
		if (ModuleManager.getModule(ReachBlock.class).getState()) {
			return this.currentGameType.isSurvivalOrAdventure() ? 20.0F : 1.0F;
		}
		if (ModuleManager.getModule(Teleport.class).getState()) {
			return this.currentGameType.isSurvivalOrAdventure() ? 10.0F : 1.0F;
		} else {
	...
	public EnumActionResult processRightClickBlock(EntityPlayerSP player, WorldClient worldIn, BlockPos stack,
		...
		EventPlaceBlock event = new EventPlaceBlock(stack, pos);
		event.call();
		this.syncCurrentPlayItem();
	...
	public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
		...
		if (this.currentGameType != GameType.SPECTATOR) {
			...
			if (Zamorozka.settingsManager.getSettingByName("AutoResetCooldown").getValBoolean()) {
