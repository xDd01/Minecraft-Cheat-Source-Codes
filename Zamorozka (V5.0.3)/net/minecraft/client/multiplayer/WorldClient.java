...
import zamorozka.event.events.EventAddedEntity;
...
public class WorldClient extends World
{
    ...
    {
    	EventAddedEntity event = new EventAddedEntity();
    	event.call();
    ...
    public float[] faceBlock(final double posX, final double posY, final double posZ) {
        final double diffX = posX - mc.player.posX;
        final double diffZ = posZ - mc.player.posZ;
        final double diffY = posY - (mc.player.posY + mc.player.getEyeHeight());
        final double dist = MathHelper.sqrt_double(diffX * diffX + diffZ * diffZ);
        final float yaw = (float)(Math.atan2(diffZ, diffX) * 180.0 / 3.141592653589793) - 90.0f;
        final float pitch = (float)(-(Math.atan2(diffY, dist) * 180.0 / 3.141592653589793));
		float lyaw = mc.player.rotationYaw;
		float lpitch = mc.player.rotationPitch;
