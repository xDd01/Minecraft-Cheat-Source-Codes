...
import zamorozka.event.events.RenderGameTick;
...
import zamorozka.modules.COMBAT.Reach;
import zamorozka.modules.EXPLOITS.XrayLite;
import zamorozka.modules.PLAYER.KeepSprint;
import zamorozka.modules.PLAYER.NoMiningTrace;
import zamorozka.modules.VISUALLY.BlockOutline;
import zamorozka.modules.VISUALLY.NoHurtCam;
import zamorozka.modules.VISUALLY.SeeUnderLiquid;
import zamorozka.modules.WORLD.CameraClip;
...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	public static void getMouseOver(float partialTicks) {
		...
		if (entity != null && mc.world != null) {
			...
			for (int j = 0; j < list.size(); ++j) {
				if (ModuleManager.getModule(NoMiningTrace.class).getState()) {
				...
				double reach;
				if (ModuleManager.getModule(Reach.class).getState()) {
					reach = Zamorozka.settingsManager.getSettingByName("Reach").getValDouble();
				} else {
					reach = 3.0D;
				}
	...
	private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
		if (this.debugView) {
			...
			if (flag) {
				...
				if (Config.zoomMode) {
					double zz = Zamorozka.settingsManager.getSettingByName("Zoom Speed").getValDouble();
					if (this.smooth < 5.0) {
						this.smooth += zz * mc.frameTime / 6;
					}
				}
			} else {
				if (this.smooth > 1.0F) {
	...
	private void hurtCameraEffect(float partialTicks) {
		NoHurtCam hurtcam = (NoHurtCam) ModuleManager.getModule(NoHurtCam.class);
		...
		if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
			...
			GlStateManager.rotate(hurtcam.getState()
					? -f * (float) Zamorozka.settingsManager.getSettingByName("HurtAngle").getValDouble()
					: -f * 14.0F, 0.0F, 0.0F, 1.0F);
			GlStateManager.rotate(f2, 0.0F, 1.0F, 0.0F);
		}
	}
	...
	private void renderHand(float partialTicks, int pass) {
		RenderGameTick event = new RenderGameTick();
		event.call();
	...
	private boolean isDrawBlockOutline() {
		if (ModuleManager.getModule(BlockOutline.class).getState()) {
			return false;
		} else {
	...
	private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
		...
		RenderEvent3D render3D = new RenderEvent3D(partialTicks, 0, 0, 0);
		render3D.call();
	...
	private void setupFog(int startCoords, float partialTicks) {
		...
		if (f >= 0.0F) {
			...
			if (ModuleManager.getModule(SeeUnderLiquid.class).getState()) {
