...
import zamorozka.modules.VISUALLY.CustomHand;
...
public class ItemRenderer {
	...
	private void transformFirstPerson(EnumHandSide p_187453_1_, float p_187453_2_) {
		...
		float g = (float) Zamorozka.settingsManager.getSettingByName("ItemSize").getValDouble();
		if (ModuleManager.getModule(Animations.class).getState()) {
			GlStateManager.translate(Zamorozka.settingsManager.getSettingByName("ItemDistance").getValDouble(), -0.37F, -0.41999997F);
		...
		if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_)) {
			...
			if (p_187457_6_.func_190926_b()) {
				if (flag && !p_187457_1_.isInvisible()) {
					if (ModuleManager.getModule(CustomHand.class).getState()) {
	...
	public void renderOverlays(float partialTicks) {
		...
		if (!this.mc.player.isSpectator()) {
			if (this.mc.player.isInsideOfMaterial(Material.WATER) && !Reflector.callBoolean(Reflector.ForgeEventFactory_renderWaterOverlay, this.mc.player, partialTicks)) {
				if (ModuleManager.getModule(NoOverlay.class).getState() && (Zamorozka.settingsManager.getSettingByName("NoElementOverlay").getValBoolean())) {
