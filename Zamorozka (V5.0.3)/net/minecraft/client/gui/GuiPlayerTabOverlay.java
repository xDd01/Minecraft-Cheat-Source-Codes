	...
	public void renderPlayerlist(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn) {
		...
		for (int k4 = 0; k4 < l3; ++k4) {
			...
			if (k4 < list.size()) {
				...
				if (ModuleManager.getModule(YoutuberMode.class).getState()) {
					if (Zamorozka.settingsManager.getSettingByName("GuiTabSpoof").getValBoolean() && !s4.contains(mc.player.getName())) {
						s4 = "\247aProtected";
					}
					if (ModuleManager.getModule(YoutuberMode.class).getState()) {
						if (Zamorozka.settingsManager.getSettingByName("NameProtect").getValBoolean()) {
							s4 = s4.replace(this.mc.player.getName(), "\247aYou");
						}
					}
				}
		...
		if (list2 != null) {
			// bottom
			k1 = k1 + i4 * 9 + 1;
			float bottom = k1 + list2.size() * this.mc.fontRendererObj.FONT_HEIGHT;
			tabHeight = bottom;
			drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, bottom, Integer.MIN_VALUE);
