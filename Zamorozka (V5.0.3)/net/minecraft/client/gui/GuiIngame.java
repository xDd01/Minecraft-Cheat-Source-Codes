...
import zamorozka.gui.GuiInHameHackPl;
...
import zamorozka.modules.VISUALLY.NoOverlay;
import zamorozka.modules.VISUALLY.NoScoreBoard;
import zamorozka.modules.VISUALLY.NoTitle;
import zamorozka.ui.AnimationUtil;
import zamorozka.ui.Color1;
...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float partialTicks) {
		...
		if (this.mc.gameSettings.thirdPersonView == 0 && itemstack.getItem() == Item.getItemFromBlock(Blocks.PUMPKIN)) {
			if (ModuleManager.getModule(NoOverlay.class).getState() && (Zamorozka.settingsManager.getSettingByName("NoPumpkinOverlay").getValBoolean())) {
		...
		if (this.mc.player.isRidingHorse()) {
			...
			if (ModuleManager.getModule(NoOverlay.class).getState() && Zamorozka.settingsManager.getSettingByName("NoExpBar").getValBoolean()) {
		...
		if (ModuleManager.getModule(NoOverlay.class).getState() && (Zamorozka.settingsManager.getSettingByName("NoPotionDebug").getValBoolean())) {
		} else
		...
		if (this.titlesTimer > 0) {
			...
			if (i2 > 8) {
				...
				if (!ModuleManager.getModule(NoTitle.class).getState() && Zamorozka.settingsManager.getSettingByName("MainTitle").getValBoolean())
				...
				if (!ModuleManager.getModule(NoTitle.class).getState() && Zamorozka.settingsManager.getSettingByName("SubTitle").getValBoolean())
		...
		if (scoreobjective1 != null) {
			NoScoreBoard scoreBoard = (NoScoreBoard) ModuleManager.getModule(NoScoreBoard.class);
			if (ModuleManager.getModule(NoScoreBoard.class).getState() && Zamorozka.settingsManager.getSettingByName("Remove").getValBoolean()) {
	...
	private void renderAttackIndicator(float p_184045_1_, ScaledResolution p_184045_2_) {
		...
		if (gamesettings.thirdPersonView == 0) {
			...
			if (gamesettings.showDebugInfo && !gamesettings.hideGUI && !this.mc.player.hasReducedDebug() && !gamesettings.reducedDebugInfo) {
				...
				// TODO Crosshair
				final EventCrosshair event = new EventCrosshair(p_184045_2_);
				event.call();
	...
	protected void renderPotionEffects(ScaledResolution resolution) {
		int d = ModuleManager.getModule(ArreyList.class).getState() ? 400 : 0;
			...
			while (true) {
				...
				if (flag && potioneffect.doesShowParticles()) {
					...
					if (potioneffect.getIsAmbient()) {
						this.drawTexturedModalRect(k - d, l, 165, 166, 24, 24);
					} else {
						this.drawTexturedModalRect(k - d, l, 141, 166, 24, 24);
					...
					if (Reflector.ForgePotion_renderHUDEffect.exists()) {
						if (potion.hasStatusIcon()) {
							this.drawTexturedModalRect(k + 3 - d, l + 3, i1 % 8 * 18, 198 + i1 / 8 * 18, 18, 18);
						}
	...
	protected void renderHotbar(ScaledResolution sr, float partialTicks) {
		final CFontRenderer font5 = Fonts.default20;

		if (Zamorozka.settingsManager.getSettingByName("BetterHotbar").getValBoolean() && ModuleManager.getModule(HUD.class).getState()) {
			if (Zamorozka.settingsManager.getSettingByName("BetterBackground").getValBoolean()) {
				drawRect(0, sr.getScaledHeight() - 23 - HUD.globalOffset, sr.getScaledWidth(), sr.getScaledHeight(), 0x99000000);
			}
			if (Zamorozka.settingsManager.getSettingByName("BackgroundRect").getValBoolean()) {
				RenderUtils2.drawGradientLine(0, sr.getScaledHeight() - 24 - HUD.globalOffset, sr.getScaledWidth(), -1, -1);
			}

			if (!Zamorozka.settingsManager.getSettingByName("BetterBackground").getValBoolean()) {
				RenderingUtils.drawRect(sr.getScaledWidth() / 2 - 91, sr.getScaledHeight() - 23 - HUD.globalOffset, sr.getScaledWidth() / 2 + (float) 91, sr.getScaledHeight() - HUD.globalOffset, new java.awt.Color(0, 0, 0, 180).hashCode());
			}

			double target = (sr.getScaledWidth() / 2) - 91 + mc.player.inventory.currentItem * 20;

			double delta = (target - last) / Math.max((float) mc.getDebugFPS(), 5) * Zamorozka.settingsManager.getSettingByName("BetterHotbar Speed").getValDouble();
			...
			String mode = Zamorozka.settingsManager.getSettingByName("HotbarSlot Mode").getValString();
			if (mode.equalsIgnoreCase("Custom")) {
				float g = (float) Zamorozka.settingsManager.getSettingByName("CustomHotbarPoints").getValDouble() < 3 ? 2f : 11.3f;
				float r = (float) Zamorozka.settingsManager.getSettingByName("CustomHotbarPoints").getValDouble() < 3 ? 6f : 10f;
				RenderingTools.drawCircle((float) last + 11, sr.getScaledHeight() - g - HUD.globalOffset, r, (int) Zamorozka.settingsManager.getSettingByName("CustomHotbarPoints").getValDouble(), Colors.getColor(255, 155));
			}
			if (mode.equalsIgnoreCase("Rect")) {
				RenderingUtils.drawRect((float) last, sr.getScaledHeight() - 23 - HUD.globalOffset, (float) last + (float) 182 - mc.player.inventory.currentItem * 20 - 20 * (8 - mc.player.inventory.currentItem), sr.getScaledHeight() - HUD.globalOffset,
						Integer.MAX_VALUE);
			}
			GlStateManager.disableBlend();
			...
			int k = 91;
			this.zLevel = -90.0F;
			this.drawTexturedModalRect(i - 91, sr.getScaledHeight() - 22 - (int) HUD.globalOffset, 0, 0, 182, 22);
			this.drawTexturedModalRect(i - 91 - 1 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 22 - 1 - (int) HUD.globalOffset, 0, 22, 24, 22);
		}
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			if (!(Zamorozka.settingsManager.getSettingByName("BetterHotbar").getValBoolean() && (ModuleManager.getModule(HUD.class).getState()))) {
				if (!itemstack.func_190926_b()) {
					if (enumhandside == EnumHandSide.LEFT) {
						this.drawTexturedModalRect(i - 91 - 29, sr.getScaledHeight() - 23 - (int) HUD.globalOffset, 24, 22, 29, 24);
					} else {
						this.drawTexturedModalRect(i + 91, sr.getScaledHeight() - 23 - (int) HUD.globalOffset, 53, 22, 29, 24);
					}
				}
			}
			...
			for (int l = 0; l < 9; ++l) {
				...
				this.renderHotbarItem(i1, j1 - (int) HUD.globalOffset, partialTicks, entityplayer, entityplayer.inventory.mainInventory.get(l));
			}

			if (!itemstack.func_190926_b()) {
				CustomItems.setRenderOffHand(true);
				int l1 = sr.getScaledHeight() - 16 - 3 - (int) HUD.globalOffset;
	...
	public void renderExpBar(ScaledResolution scaledRes, int x) {
		...
		if (i > 0) {
			...
			int gg = Zamorozka.settingsManager.getSettingByName("BackgroundRect").getValBoolean() ? 1 : 3;
			int l = scaledRes.getScaledHeight() - 32 + gg - (int) HUD.globalOffset;
		...
		if (this.mc.player.experienceLevel > 0) {
			...
			int dd = (int) HUD.globalOffset + 8;
			int i1 = scaledRes.getScaledHeight() - 31 - dd;
	...
	private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
		...
		int k1 = 3;
		int l1 = (int) (scaledRes.getScaledWidth() - i - Zamorozka.settingsManager.getSettingByName("ScorPositionX").getValDouble());
		int j = 0;
		...
		for (Score score1 : collection) {
			...
			int k = scoreBoard.getState() ? j1 - j * this.getFontRenderer().FONT_HEIGHT + (int) Zamorozka.settingsManager.getSettingByName("ScorPositionY").getValDouble() : j1 - j * this.getFontRenderer().FONT_HEIGHT;
			int l = (int) (scaledRes.getScaledWidth() - 5 - Zamorozka.settingsManager.getSettingByName("ScorPositionX").getValDouble());
			if (!(Zamorozka.settingsManager.getSettingByName("NoScoreBoardRect").getValBoolean() && ModuleManager.getModule(NoScoreBoard.class).getState())) {
				drawRect(l1 - 2, k, l, k + (getFontRenderer()).FONT_HEIGHT, 1342177280);
			}
			if (ModuleManager.getModule(NoScoreBoard.class).getState() && Zamorozka.settingsManager.getSettingByName("NameBoardSpoof").getValBoolean()) {
				this.getFontRenderer().drawString(s1.replace(Minecraft.getMinecraft().player.getName().substring(0, 2),
						"                                                                                                                                                                                                                                                                                                                                                                        "),
						l1, k, 553648127);
			} else {
				this.getFontRenderer().drawString(s1, l1, k, 553648127);
			}
			if (scoreBoard.getState() && !(Zamorozka.settingsManager.getSettingByName("NoPoints").getValBoolean()))
			...
			if (j == collection.size()) {
				...
				this.mc.fontRendererObj.drawString(s3, l1 + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2, k - this.getFontRenderer().FONT_HEIGHT, 553648127);
			}
		}
	}
	...
	public void renderPlayerStats(ScaledResolution scaledRes) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			int gg = Zamorozka.settingsManager.getSettingByName("BackgroundRect").getValBoolean() ? 41 : 39;
			int j1 = scaledRes.getScaledHeight() - gg - (int) HUD.globalOffset;
			...
			for (int j5 = MathHelper.ceil((f + (float) k1) / 2.0F) - 1; j5 >= 0; --j5) {
				...
				if (entityplayer.isPotionActive(MobEffects.POISON)) {
					if (ModuleManager.getModule(NoOverlay.class).getState() && (Zamorozka.settingsManager.getSettingByName("NoHeartOverlay").getValBoolean())) {
	...
	public void updateTick() {
		...
		if (this.mc.player != null) {
			...
			} else if (!this.highlightingItemStack.func_190926_b() && itemstack.getItem() == this.highlightingItemStack.getItem() && ItemStack.areItemStackTagsEqual(itemstack, this.highlightingItemStack)
