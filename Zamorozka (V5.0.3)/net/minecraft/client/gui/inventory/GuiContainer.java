...
public abstract class GuiContainer extends GuiScreen {
	...
	private static final double ANIMATION_SPEED = 0.07;
	...
	public void drawSlot(Slot slotIn) {
		...
		if (Zamorozka.settingsManager.getSettingByName("SmoothInventoryOpen").getValBoolean() && ModuleManager.getModule(HUD.class).getState()) {
			animationState += ANIMATION_SPEED * mc.frameTime / 8;
			if (j >= animationState) {
				j = (int) animationState;
			}
		}
	...
	protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
		...
		} else if (this.dragSplitting && slot != null && !itemstack.func_190926_b() && (itemstack.func_190916_E() > this.dragSplittingSlots.size() || this.dragSplittingLimit == 2) && Container.canAddItemToSlot(slot, itemstack, true)
