...
import zamorozka.modules.COMBAT.VelocityPercentage;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
	...
	public void sendPacketSlient(Packet<?> p_147297_1_) {
	...
	public void handleEntityVelocity(SPacketEntityVelocity packetIn) {
		...
		if (mode.equalsIgnoreCase("Cancel")
				&& (ModuleManager.getModule(VelocityPercentage.class).getState() && (AngleUtil.randomFloat(0,
						100) <= Zamorozka.settingsManager.getSettingByName("CancelChance").getValDouble())))
			return;
		PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
		Entity entity = this.clientWorldController.getEntityByID(packetIn.getEntityID());

		if (entity != null) {
			if (mode.equalsIgnoreCase("MatrixTest") && ModuleManager.getModule(VelocityPercentage.class).getState()) {
				entity.setVelocity(0.0D, (double) packetIn.getMotionY() / 8000.0D,
						(double) packetIn.getMotionZ() / 8000.0D / 2.5D);
			} else {
		...
		if (this.guiScreenServer != null) {
			if (this.guiScreenServer instanceof GuiScreenRealmsProxy) {
				this.gameController.displayGuiScreen(
						(new DisconnectedRealmsScreen(((GuiScreenRealmsProxy) this.guiScreenServer).getProxy(),
	...
	public void sendPacket(Packet<?> packetIn) {
		PacketEvent DICK = new PacketEvent(packetIn);
		DICK.call();
		if (DICK.isCancelled()) {
			return;
		}
		...
		if (entity != null) {
			if (packetIn.getOpCode() == 21) {
				...
				if (entity == this.gameController.player) {
					if (ModuleManager.getModule(NoOverlay.class).getState() && (Zamorozka.settingsManager.getSettingByName("AntiTotemAnimation").getValBoolean())) {
	...
	public void handleConfirmTransaction(SPacketConfirmTransaction packetIn) {
		...
		if (container != null && !packetIn.wasAccepted()) {
			if (!(ModuleManager.getModule(Disabler.class).getState()
					&& Zamorozka.settingsManager.getSettingByName("Transaction").getValBoolean())) {
				this.sendPacket(
						new CPacketConfirmTransaction(packetIn.getWindowId(), packetIn.getActionNumber(), true));
			} else {
				Zamorozka.settingsManager.getSettingByName("PingFlood").setValBoolean(false);
			}
		}
	}
	...
	public void handleChangeGameState(SPacketChangeGameState packetIn) {
		...
		if (i == 1) {
			...
			if (Minecraft.getMinecraft().player.getHeldItemMainhand().getItem() instanceof ItemBow) {
				if (((EntityPlayer) entityplayer).hurtTime < 9 && entityplayer != Minecraft.getMinecraft().player) {
					if (ModuleManager.getModule(AntiBot2.class).getState()
							&& Zamorozka.settingsManager.getSettingByName("HitBefore").getValBoolean()) {
						AntiBot2.nobotsTimolia.add((EntityPlayer) entityplayer);
						ChatUtils.printChatprefix(ChatFormatting.RED + entityplayer.getName() + ChatFormatting.WHITE
								+ " Was added in AntiBot-List!");
						if (AntiBot2.nobotsTimolia.contains(entityplayer)) {
							ChatUtils.printChatprefix(ChatFormatting.RED + entityplayer.getName() + ChatFormatting.WHITE
									+ " Already in AntiBot-List!");
						}
					}
				}
			}
