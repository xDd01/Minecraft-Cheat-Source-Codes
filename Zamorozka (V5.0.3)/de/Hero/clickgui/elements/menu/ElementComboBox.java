...
public class ElementComboBox extends Element {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
         ...
         */
        RenderingUtils.drawRoundedRect(x, y, x + width, y + height, 0xff1a1a1a, 0xff1a1a1a);
        String srg = "" + height;

        FontUtil.drawTotalCenteredString(setstrg, x + width / 2, y + 15 / 1.9, 0xffffffff);

        RenderingUtils.drawRoundedRect(x, y + 14, x + width, y + 15, Zamorozka.getClientColor(), Zamorozka.getClientColor());
        if (comboextended) {
            RenderingUtils.drawRoundedRect(x, y + 15, x + width, y + height, 0xaa121212, 0xaa121212);
            double ay = y + 15;
            for (String sld : set.getOptions()) {
                String elementtitle = sld.substring(0, 1).toUpperCase() + sld.substring(1);
                FontUtil.drawCenteredString(elementtitle, x + width / 2, ay + 2.5, 0xffffffff);
                ...
                if (sld.equalsIgnoreCase(set.getValString())) {
                    Gui.drawRect(x + 2.5, ay, x, ay + FontUtil.getFontHeight() + 2, Zamorozka.getClientColor());
                }
                /*
                ...
                if (mouseX >= x && mouseX <= x + width && mouseY >= ay && mouseY < ay + FontUtil.getFontHeight() + 2) {
                	Gui.drawRect(x + width - 1.2, ay, x + width, ay + FontUtil.getFontHeight() + 2, temp.getRGB());
                }
