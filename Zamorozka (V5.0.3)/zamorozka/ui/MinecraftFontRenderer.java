...
public class MinecraftFontRenderer extends CFont {
	...
	public float drawColoredStringWithShadow(ColoredString text, double x, double y) {
		float shadowWidth = drawString(text.getString(), x + 1.0D, y + 1.0D, 0, true);
		return Math.max(shadowWidth, drawColoredString(text, x, y));
	}
	...
	public float drawColoredString(ColoredString text, double x, double y) {
		x -= 1;
		...
		for (int i = 0; i < size; i++) {
			ColoredChar ch = text.charAt(i);
			char character = ch.getChar();

			GlStateManager.color(ch.getR(), ch.getG(), ch.getB());
	...
	public int getSortMethod(String displayName) {
		String sor = Zamorozka.settingsManager.getSettingByName("ArraySort Mode").getValString();
		if (sor.equalsIgnoreCase("Height")) {
			return Fonts.comfortaa17.getStringHeight(displayName);
		}
		if (sor.equalsIgnoreCase("Width")) {
			return Zamorozka.FONT_MANAGER.chat.getStringWidth(displayName);
		}
		return -1;
	}
}