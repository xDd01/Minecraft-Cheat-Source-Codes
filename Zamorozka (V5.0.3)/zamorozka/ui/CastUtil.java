...
public class CastUtil implements MCUtil {
    ...
    public static RayTraceResult rayTrace(final double blockReachDistance, final float yaw, final float pitch) {
        final Vec3d vec3 = Minecraft.player.getPositionEyes(1.0f);
        ...
        return Minecraft.player.world.rayTraceBlocks(vec3, vec5, false, false, true);
    }
    ...
    public static RayTraceResult rayTrace(final float n, final double n2, final double n3, final double n4, final float n5, final float n6) {
        ...
        if (func_175606_aa != null && minecraft.world != null) {
            ...
            CastUtil.result = var24(minecraft.playerController.getBlockReachDistance(), minecraft.timer.renderPartialTicks, vec3, vectorForRotation);
            double var2 = n7;
            final Vec3d vec4 = vec3;
            if (CastUtil.result != null) {
                var2 = CastUtil.result.hitVec.distanceTo(vec4);
            }
            ...
            Vec3d vec6 = null;
            final float n8 = 1.0f;
            final List<Entity> var3 = minecraft.world.getEntitiesWithinAABBExcludingEntity(func_175606_aa, func_175606_aa.getEntityBoundingBox().expand(vec5.xCoord * n7, vec5.yCoord * n7, vec5.zCoord * n7).contract(n8, n8, n8));
            double n9 = var2;
            for (int i = 0; i < var3.size(); ++i) {
                final Entity entity = var3.get(i);
                final float collisionBorderSize = entity.getCollisionBorderSize();
                final AxisAlignedBB sub = entity.getEntityBoundingBox().contract(collisionBorderSize, collisionBorderSize, collisionBorderSize);
                final RayTraceResult calculateIntercept = sub.calculateIntercept(vec4, addVector);
                if (sub.isVecInside(vec4)) {
                    if (n9 >= 0.0) {
                        CastUtil.entity = entity;
            ...
            if (CastUtil.entity != null && (n9 < var2 || CastUtil.result == null)) {
                CastUtil.result = new RayTraceResult(CastUtil.entity, vec6);
                if (CastUtil.entity instanceof EntityLivingBase || CastUtil.entity instanceof EntityItemFrame) {
                    CastUtil.entity = CastUtil.entity;
                }
            }
        }
        return CastUtil.result;
    }
    ...
    private static RayTraceResult var24(double n, final float n2, final Vec3d vec3, final Vec3d vec4) {
        ...
        return Minecraft.player.world.rayTraceBlocks(vec3, addVector, false, false, true);
    }

    public static Vec3d getLook(final float yaw, final float pitch) {
        return Minecraft.player.getVectorForRotation(pitch, yaw);
    }
}