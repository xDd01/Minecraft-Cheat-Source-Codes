...
public class RandomUtil {
    ...
    public static int getRandom(final int floor, final int cap) {
        return floor + RandomUtil.rng.nextInt(cap - floor + 1);
    }

    public static double getRandom(final double floor, final double cap) {
        return floor + RandomUtil.rng.nextInt((int) (cap - floor + 1));
    }
    ...
    public static double[] getExpandCoords(double x, double z, float forward, float strafe, double exp, int pos, boolean aircheck) {
        BlockPos underPos = new BlockPos(x, Minecraft.player.posY + pos, z);
        Block underBlock = Minecraft.getMinecraft().world.getBlockState(underPos).getBlock();
        double xCalc = -999, zCalc = -999;
        double dist = 0;
        double expandDist = exp;
        while (!(underBlock.getMaterial(null).isReplaceable()) || !aircheck) {
            xCalc = x;
            zCalc = z;
            dist++;
            if (dist > expandDist) {
            ...
            xCalc += (forward * 0.45 * Math.cos(Math.toRadians(Minecraft.player.rotationYaw + 90.0f)) + strafe * 0.45 * Math.sin(Math.toRadians(Minecraft.player.rotationYaw + 90.0f))) * dist;
            zCalc += (forward * 0.45 * Math.sin(Math.toRadians(Minecraft.player.rotationYaw + 90.0f)) - strafe * 0.45 * Math.cos(Math.toRadians(Minecraft.player.rotationYaw + 90.0f))) * dist;
            if (dist == expandDist) {
                break;
            }
            underPos = new BlockPos(xCalc, Minecraft.player.posY + pos, zCalc);
            underBlock = Minecraft.getMinecraft().world.getBlockState(underPos).getBlock();
        }
