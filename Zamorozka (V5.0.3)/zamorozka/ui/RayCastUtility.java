...
public class RayCastUtility implements MCUtil{
	...
	public static Entity rayCast(double range, float yaw, float pitch) {
        ...
        Vec3d vec33 = null;
        float f = 1.0F;
        List list = mc.world.getEntitiesInAABBexcluding(mc.getRenderViewEntity(), mc.getRenderViewEntity().getEntityBoundingBox().offset(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand((double)f, (double)f, (double)f), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith));
        double d2 = d1;
        ...
        for (int i = 0; i < list.size(); ++i)
        {
            ...
            if (((List) axisalignedbb).contains(vec3))
            {
                if (d2 >= 0.0D)
                {
            ...
            else if (movingobjectposition != null)
            {
                ...
                if (d3 < d2 || d2 == 0.0D)
                {
                     ...
                     */
                    if (entity1 == mc.getRenderViewEntity().getRidingEntity() && !flag2)
                    {
                        if (d2 == 0.0D)
                        {
                            pointedEntity = entity1;
