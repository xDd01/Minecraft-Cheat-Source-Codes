...
public class ShaderUniform {
   ...
   private void uploadInt() {
      ...
      case 3:
         ...
         break;
      default:
         logger.warn(String.valueOf((new StringBuilder("Uniform.upload called, but count value (")).append(this.uniformCount).append(") is ").append(" not in the range of 1 to 4. Ignoring.")));
      }
   ...
   public void upload() {
      ...
      if (this.uniformType <= 3) {
         ...
         if (this.uniformType > 10) {
            logger.warn(String.valueOf((new StringBuilder("Uniform.upload called, but type value (")).append(this.uniformType).append(") is not ").append("a valid type. Ignoring.")));
            return;
         }
   ...
   public void set(float[] var1) {
      if (var1.length < this.uniformCount) {
         logger.warn(String.valueOf((new StringBuilder("Uniform.set called with a too-small value array (expected ")).append(this.uniformCount).append(", got ").append(var1.length).append("). Ignoring.")));
      } else {
         this.uniformFloatBuffer.position(0);
         this.uniformFloatBuffer.put(var1);
         this.uniformFloatBuffer.position(0);
