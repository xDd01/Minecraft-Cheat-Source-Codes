...
import koks.module.player.OverArmor;
    ...
    public ContainerPlayer(final InventoryPlayer playerInventory, boolean localWorld, EntityPlayer player)
    {
        ...
        for (int k = 0; k < 4; ++k)
        {
            ...
            this.addSlotToContainer(new Slot(playerInventory, playerInventory.getSizeInventory() - 1 - k, 8, 8 + k * 18)
            {
                ...
                {
                    return !Minecraft.getMinecraft().developerMode && ModuleRegistry.getModule(OverArmor.class).isToggled() || (stack == null ? false : (stack.getItem() instanceof ItemArmor ? ((ItemArmor)stack.getItem()).armorType == k_f : (stack.getItem() != Item.getItemFromBlock(Blocks.pumpkin) && stack.getItem() != Items.skull ? false : k_f == 0)));
                }
