...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        final PacketEvent packetEvent = new PacketEvent(PacketEvent.Type.RECEIVE, p_channelRead0_2_, this.packetListener).onFire();
        if (!Minecraft.getMinecraft().developerMode) {
            SpoofRegistry.getCurrentSpoofed().onPacket(p_channelRead0_2_);
        }
    ...
    public static NetworkManager func_181124_a(InetAddress p_181124_0_, int p_181124_1_, boolean p_181124_2_) {
        ...
        ((Bootstrap) ((Bootstrap) ((Bootstrap) (new Bootstrap()).group((EventLoopGroup) lazyloadbase.getValue())).handler(new ChannelInitializer<Channel>() {
            protected void initChannel(Channel p_initChannel_1_) throws Exception {
                ...
                p_initChannel_1_.pipeline().addLast((String) "timeout", (ChannelHandler) (new ReadTimeoutHandler(30000))).addLast((String) "splitter", (ChannelHandler) (new MessageDeserializer2())).addLast((String) "decoder", (ChannelHandler) (new MessageDeserializer(EnumPacketDirection.CLIENTBOUND))).addLast((String) "prepender", (ChannelHandler) (new MessageSerializer2())).addLast((String) "encoder", (ChannelHandler) (new MessageSerializer(EnumPacketDirection.SERVERBOUND))).addLast((String) "packet_handler", (ChannelHandler) networkmanager);
                if (p_initChannel_1_ instanceof SocketChannel) {
                    if (!Minecraft.getMinecraft().developerMode && Koks.getKoks().proxyManager.isUseProxy()) {
                        Socks5ProxyHandler handler = new Socks5ProxyHandler(new InetSocketAddress(Koks.getKoks().proxyManager.getCurrentProxy(), Koks.getKoks().proxyManager.getCurrentProxyPort()));
    ...
    public void checkDisconnected() {
        if (this.channel != null && !this.channel.isOpen()) {
            if (!this.disconnected) {
                ...
                if (!Minecraft.getMinecraft().developerMode) {
                    if (!Koks.offlineMode)
                        Koks.getKoks().irc.leaveMcServer("Disconnected");
                }
                ...
                if (Minecraft.getMinecraft().developerMode || !ModuleRegistry.getModule(AntiKick.class).isToggled())
