...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    public Minecraft(GameConfiguration gameConfig, boolean developerMode) {
        theMinecraft = this;
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        if (hasIRC || this.developerMode) {
            displayGuiScreen((GuiScreen) new GuiMainMenu());
        } else {
            displayGuiScreen((GuiScreen) new GuiSetup.GuiSetupIRC());
        }
        ...
        /*if (!this.session.getProfile().isLegacy())
            Display.releaseContext();*/

        this.renderGlobal.makeEntityOutlineShader();
    }

    private void startAdvanced() throws Exception {
        /*String author = "";
        ...
        SwingUtilities.invokeLater(() -> {
            final JFrame frame = new JFrame(Koks.name + " v" + Koks.version + " | by " + finalAuthor + " | Minecraft 1.8.9 | Banned Edition");
            final JFXPanel jfxPanel = new JFXPanel();
            jfxPanel.setSize(width, height);
            frame.getContentPane().add(jfxPanel);
            PlatformImpl.startup(() -> {
                final WebView webView = new WebView();
                webView.getEngine().load("http://koksban.000webhostapp.com");
    ...
    private void setWindowIcon() {
        ...
        if (util$enumos != Util.EnumOS.OSX) {
            ...
            try {
                if (!developerMode) {
                    inputstream = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("koks/icons/koks16x16.png"));
                    inputstream1 = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("koks/icons/koks32x32.png"));
                } else {
    ...
    private void drawSplashScreen(TextureManager textureManagerInstance) throws LWJGLException {
        ...
        int i = resolution.getScale();
        Framebuffer framebuffer = new Framebuffer(resolution.getWidth() * i, resolution.getHeight() * i, true);
        framebuffer.bindFramebuffer(false);
        GlStateManager.matrixMode(5889);
        GlStateManager.loadIdentity();
        GlStateManager.ortho(0.0D, (double) resolution.getWidth(), (double) resolution.getHeight(), 0.0D, 1000.0D, 3000.0D);
        GlStateManager.matrixMode(5888);
        GlStateManager.loadIdentity();
        ...
        int j = 256;
        int k = 256;
        this.func_181536_a((resolution.getWidth() - j) / 2, (resolution.getHeight() - k) / 2, 0, 0, j, k, 255, 255, 255, 255);
        GlStateManager.disableLighting();
        GlStateManager.disableFog();
        framebuffer.unbindFramebuffer();
        framebuffer.framebufferRender(resolution.getWidth() * i, resolution.getHeight() * i);
        GlStateManager.enableAlpha();
        GlStateManager.alphaFunc(516, 0.1F);
        this.updateDisplay();
    }
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        new DisplayGuiScreenEvent(guiScreenIn).onFire();
    ...
    private void runGameLoop() throws IOException {
        ...
        if (this.thePlayer != null && this.thePlayer.isEntityInsideOpaqueBlock() && developerMode) {
        ...
        final RunGameLoopEvent runGameLoopEvent = new RunGameLoopEvent(1).onFire();

        while (getSystemTime() >= this.debugUpdateTime + 1000L) {
            debugFPS = (int) (this.fpsCounter * runGameLoopEvent.getFpsMultiplier());
    ...
    public int getLimitFramerate() {
        return this.theWorld == null && this.currentScreen != null ? 40 : this.gameSettings.limitFramerate;
    }
     ...
     */ public void rightClickMouse() {
        if (!this.playerController.func_181040_m()) {
            new RightClickEvent().onFire();
            this.rightClickDelayTimer = 4;
            boolean flag = true;
            ItemStack itemstack = this.thePlayer.inventory.getFakeItem();
                    ...
                    case BLOCK:
                        ...
                        if (this.theWorld.getBlockState(blockpos).getBlock().getMaterial() != Material.air) {
                            ...
                            if (itemstack.stackSize == 0) {
                                this.thePlayer.inventory.mainInventory[this.thePlayer.inventory.fakeItem] = null;
            ...
            if (flag) {
                ItemStack itemstack1 = this.thePlayer.inventory.getFakeItem();
    ...
    private void resize(int width, int height) {
        ...
        if (this.currentScreen != null) {
            final Resolution resolution = Resolution.getResolution();
            this.currentScreen.onResize(this, resolution.getWidth(), resolution.getHeight());
        }
    ...
    public void runTick() throws IOException {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Keyboard.next()) {
                ...
                final KeyPressEvent keyPressEvent = new KeyPressEvent(k).onFire();
                if (keyPressEvent.isCanceled()) return;
            ...
            for (int l = 0; l < 9; ++l) {
                if (this.gameSettings.keyBindsHotbar[l].isPressed()) {
                    if (this.thePlayer.isSpectator()) {
                        ...
                        if (thePlayer.inventory.fakeItem == playerController.currentPlayerItem)
                            this.thePlayer.inventory.fakeItem = l;
                    }
                }
            }
    ...
    private void middleClickMouse() {
        if (this.objectMouseOver != null) {
            ...
            if (tileentity == null) {
                ...
                inventoryplayer.setInventorySlotContents(inventoryplayer.fakeItem, itemstack1);
            }

            if (flag) {
                int j = this.thePlayer.inventoryContainer.inventorySlots.size() - 9 + inventoryplayer.fakeItem;
                this.playerController.sendSlotPacket(inventoryplayer.getStackInSlot(inventoryplayer.fakeItem), j);
            }
        }
    }
