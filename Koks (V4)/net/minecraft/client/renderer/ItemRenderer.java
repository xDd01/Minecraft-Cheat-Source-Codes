...
import koks.module.visual.CustomItem;
import koks.module.visual.FakeAutoBlock;
import koks.module.visual.Swing;
...
public class ItemRenderer {
    ...
    private void transformFirstPersonItem(float equipProgress, float swingProgress) {
        final CustomItem customItem = ModuleRegistry.getModule(CustomItem.class);
        final boolean developer = Minecraft.getMinecraft().developerMode;
        float addX = 0;
        float addY = 0;
        float addZ = 0;
        float scale = 0.4F;

        if(!developer && customItem.isToggled()) {
            if ((mc.thePlayer.isBlocking() || !ValueManager.getInstance().getValue("OnlyBlock", customItem).castBoolean())) {
                addX = !customItem.isToggled() ? 0 : (float) ValueManager.getInstance().getValue("X", customItem).castDouble();
                addY = !customItem.isToggled() ? 0 : (float) ValueManager.getInstance().getValue("Y", customItem).castDouble();
                addZ = !customItem.isToggled() ? 0 : (float) ValueManager.getInstance().getValue("Z", customItem).castDouble();
                scale = !customItem.isToggled() ? 0.4F : (float) ValueManager.getInstance().getValue("Size", customItem).castDouble();
            }
        }
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            if (this.itemToRender != null) {
                if (this.itemToRender.getItem() instanceof ItemMap) {
                    ...
                    final FakeAutoBlock fakeAutoBlock = ModuleRegistry.getModule(FakeAutoBlock.class);
                    if (!Minecraft.getMinecraft().developerMode && fakeAutoBlock.isToggled() && mc.thePlayer.isSwingInProgress) {
                        rotation++;
                        if (ValueManager.getInstance().getValue("OnlySword", fakeAutoBlock).castBoolean()) {
    ...
    public void blockAnimation(float f, float f1) {
        if (!Minecraft.getMinecraft().developerMode && ModuleRegistry.getModule(Swing.class).isToggled()) {
            String mode = ValueManager.getInstance().getValue("Mode", ModuleRegistry.getModule(Swing.class)).castString();
            if (mode.equalsIgnoreCase("1.7 Animation"))
            ...
            switch (mode) {
                case "Astolfo":
                    ...
                    double degrees = stage * Math.PI * 2 / 360;
                    GlStateManager.translate(Math.cos(degrees), Math.sin(degrees), 0);
                    GlStateManager.scale(0.7, 0.7, 0.7);
                    float angel = (float) ((Math.toRadians(stage) * 180 / Math.PI) + 75);
                    GlStateManager.rotate(angel, 1, 0, 1);
                    break;
                case "Fly":
                    double Flyradius = 1;
                    GlStateManager.translate(Math.sin(rotation * Math.PI / 180) * Flyradius, Math.cos(rotation * Math.PI / 180) * Flyradius, 1);
                    GlStateManager.rotate(rotation, 1, 0, 0);
                    break;
                case "Push":
                    GlStateManager.translate(-0.3F, -0.3F, f1 * 0.55F);
                    break;
                case "Animation1":
                ...
                case "Animation2":
                    GlStateManager.translate(-0.3F, -0.3F, 0.2F);
                    GlStateManager.rotate(f1 * -35.0F, 5.0f, 0.0F, 5.0f);
                    break;
                case "Animation3":
                    GlStateManager.translate(0.1F, 0.1F, 0.2F);
                    GlStateManager.rotate(f1 * 80f, -40, 100.0f, -50.0f);
                    break;
                case "Animation4":
                    GlStateManager.translate(0.1F, 0.1F, 0.2F);
                    GlStateManager.rotate(f1 * 40f, -80, 0.0f, -500.0f);
                    break;
                case "Animation5":
                    GlStateManager.rotate(f1 * 40f, 1, 0, 0);
                    GlStateManager.rotate(f1 * 40f, 0, 1, 0);
                    GlStateManager.rotate(-f1 * 40f, 0, 0, 1);
                    break;
                case "Spin":
                    GlStateManager.translate(-0.3F, -0.3F, 0.2F);
                    GlStateManager.rotate(f1 * -360.0F, 1.0F, 0.0F, 0.0F);
                    break;
            }
            stage++;
        } else {
