...
public class TileEntitySignRenderer extends TileEntitySpecialRenderer<TileEntitySign> {
    ...
    public void renderTileEntityAt(TileEntitySign te, double x, double y, double z, float partialTicks, int destroyStage) {
        ...
        if (isRenderText(te)) {
            ...
            if (destroyStage < 0) {
                boolean illegalSign = false;
                int illegalChars = 0;

                final ExploitFixer exploitFixer = ModuleRegistry.getModule(ExploitFixer.class);
                if (!Minecraft.getMinecraft().developerMode && exploitFixer.isToggled() && ValueManager.getInstance().getValue("Invalid Sign", exploitFixer).castBoolean())
                    for (int j = 0; j < te.signText.length; ++j) {
                        if (te.signText[j] != null) {
                            ...
                            String s = list != null && list.size() > 0 ? ((IChatComponent) list.get(0)).getUnformattedText() : "";
                            if (fontrenderer.getStringWidth(te.signText[j].getUnformattedText()) > 90)
                                illegalSign = true;
                            illegalChars += s.length();
                        }
                    }
                ...
                for (int j = 0; j < te.signText.length; ++j) {
                    if (te.signText[j] != null) {
                        ...
                        if (j == te.lineBeingEdited) {
                            ...
                            if (illegalSign && !Minecraft.getMinecraft().developerMode) {
                                if (j == 0) {
                                    s = "§cIllegal Sign";
                                } else if (j == 1)
                                    s = "§c" + illegalChars + " characters";
                                else
                                    s = "";
                            }
                            fontrenderer.drawString(s, -fontrenderer.getStringWidth(s) / 2, j * 10 - te.signText.length * 5, i);
