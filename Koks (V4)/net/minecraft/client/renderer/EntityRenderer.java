...
import koks.event.*;
import koks.module.visual.CameraClip;
import koks.module.visual.Fog;
import koks.module.visual.SmoothZoom;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.theWorld != null) {
            ...
            final MouseOverEvent mouseOverEvent = new MouseOverEvent(d1, flag, entity).onFire();
    ...
    private void updateFovModifierHand() {
        ...
        final FOVEvent fovEvent = new FOVEvent(fovModifierHand).onFire();
        fovModifierHand = fovEvent.getFov();
    }
    ...
    SmoothZoom smoothZoom = null;
    double zoomState = 1;

    private float getFOVModifier(float partialTicks, boolean p_78481_2_) {
        if(smoothZoom == null) {
            smoothZoom = ModuleRegistry.getModule(SmoothZoom.class);
        }
        double multiplier = ValueManager.getInstance().getValue("Multiplier", smoothZoom).castDouble() / 10;
        if (this.debugView) {
            return 90.0F;
        } else {
            Entity entity = this.mc.getRenderViewEntity();
            ...
            if (flag) {
                ...
                if (Config.zoomMode) {
                    if(zoomState < 4)
                        zoomState += multiplier;
                    if (!smoothZoom.isToggled())
                        f /= 4;
                }
    ...
    private void hurtCameraEffect(float partialTicks) {
        final HurtCameraEvent hurtCameraEvent = new HurtCameraEvent().onFire();
        if (hurtCameraEvent.isCanceled()) return;
    ...
    private void setupViewBobbing(float partialTicks) {
        final BobbingEvent bobbingEvent = new BobbingEvent(mc.thePlayer.distanceWalkedModified, false).onFire();
        if (bobbingEvent.isChangeBobbing())
            mc.thePlayer.distanceWalkedModified = bobbingEvent.getBobbing();
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (movingobjectposition != null) {
                        ...
                        if (d7 < d3) {
                            if (Minecraft.getMinecraft().developerMode || !ModuleRegistry.getModule(CameraClip.class).isToggled())
                                d3 = d7;
                        }
                    }
                }
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        if (!Minecraft.getMinecraft().developerMode) {
            final Fog fog = ModuleRegistry.getModule(Fog.class);
            if (fog != null && fog.isToggled())
                this.farPlaneDistance *= (float) ValueManager.getInstance().getValue("Distance", fog).castDouble();
        }
            ...
            if (this.mc.theWorld != null) {
                ...
                if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null) {
                    ...
                    if (this.mc.gameSettings.showDebugInfo) {
                        Lagometer.showLagometer(resolution);
                    }
                }
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    crashreportcategory.addCrashSectionCallable("Screen size", new Callable<String>() {
                        public String call() throws Exception {
                            return String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", new Object[]{Integer.valueOf(resolution.getWidth()), Integer.valueOf(resolution.getHeight()), Integer.valueOf(EntityRenderer.this.mc.displayWidth), Integer.valueOf(EntityRenderer.this.mc.displayHeight), Integer.valueOf(resolution.getScale())});
                        }
                    });
    ...
    public void renderStreamIndicator(float partialTicks) {
        this.setupOverlayRendering();
        this.mc.ingameGUI.renderStreamIndicator(Resolution.getResolution());
    }
    ...
    public void setupOverlayRendering() {
        ...
        GlStateManager.matrixMode(5889);
        GlStateManager.loadIdentity();
        GlStateManager.ortho(0.0D, resolution.getWidth(), resolution.getHeight(), 0.0D, 1000.0D, 3000.0D);
        GlStateManager.matrixMode(5888);
        GlStateManager.loadIdentity();
    ...
    private void updateFogColor(float partialTicks) {
        ...
        final Fog fog = ModuleRegistry.getModule(Fog.class);
        if (!Minecraft.getMinecraft().developerMode && fog != null && fog.isToggled()) {
            final Color color = new Color(ValueManager.getInstance().getValue("Color", fog).castInteger());
            this.fogColorRed = color.getRed() / 255F;
            this.fogColorGreen = color.getGreen() / 255F;
            this.fogColorBlue = color.getBlue() / 255F;
        }
        if (this.cloudFog) {
