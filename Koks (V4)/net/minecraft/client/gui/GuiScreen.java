...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (!Minecraft.getMinecraft().developerMode)
            if (init && !buttonList.isEmpty()) {
                int highestButton = Integer.MAX_VALUE;
                for (GuiButton button : buttonList) {
                    final int position = button.yPosition + button.moveY;
                    if (position < highestButton && button.visible)
                        highestButton = position;
                }
    ...
    public void drawWorldBackground(int tint) {
        if (mc != null && this.mc.theWorld != null) {
