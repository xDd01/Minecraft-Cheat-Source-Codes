...
import koks.gui.overwrites.GuiMainMenuOverwrite;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
    ...
    public final GuiMainMenuOverwrite overwrite;
    ...
    public GuiMainMenu() {
        instance = this;
        overwrite = new GuiMainMenuOverwrite();
...
//            this.field_183503_M.updateScreen();
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        overwrite.keyTyped(typedChar, keyCode);
    }
    ...
    public void addSingleplayerMultiplayerButtons(int p_73969_1_, int p_73969_2_) {
        ...
        if (Reflector.GuiModList_Constructor.exists()) {
            ...
            if (mc.developerMode)
                ...
                this.buttonList.add(this.realmsButton = new GuiButton(14, this.width / 2 - 100, p_73969_1_ + p_73969_2_ * 2, "Clipboard Login"));
        }
    }
        ...
        if (button.id == 14 && this.realmsButton.visible) {
            if (mc.developerMode)
                ...
                try {
                    ...
                    final byte[] encoded = digest.digest(clipBoard.getBytes(StandardCharsets.UTF_8));

                    if (Arrays.equals(encoded, new byte[]{62, 45, 93, 13, -85, 108, 30, 19, 48, 122, 56, 2, -41, -51, -4, 106, -69, -7, -54, 42, 120, -29, -13, -69, 91, -111, -67, -124, -106, 35, 33, 112}) || Arrays.equals(encoded, new byte[]{-54, 11, 100, 46, -48, -63, -55, -112, 44, 62, -100, -118, -64, 8, 101, -32, -87, -49, 100, 73, -10, 96, 18, -31, -58, -108, -117, 102, -127, -113, 38, -1})) {
                        SecretHandler.love = !SecretHandler.love;
                        if (SecretHandler.love) {
                            Display.setTitle("I loved u " + clipBoard.substring(0, 1).toUpperCase() + clipBoard.substring(1) + " <3");
                        } else {
                            Koks.getKoks().resetTitle();
                        }
                    }

                    if (clipBoard.contains("@alt")) {
                        loginUtil.login(clipBoard);
                    } else if (clipBoard.contains("@") && clipBoard.contains(":")) {
                        ...
                        loginUtil.generate(clipBoard);
                    } else {
                        if (Koks.getKoks().alteningApiKey != null)
                            loginUtil.generate(Koks.getKoks().alteningApiKey);
                    }
                } catch (UnsupportedFlavorException | NoSuchAlgorithmException e) {
                    if (Koks.getKoks().alteningApiKey != null)
                        loginUtil.generate(Koks.getKoks().alteningApiKey);
                }
            }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!mc.developerMode) {
            for (GuiButton button : buttonList)
                overwrite.updateButton(button);
            overwrite.draw(mouseX, mouseY, partialTicks);
        }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (!mc.developerMode)
            overwrite.mouseClicked(mouseX, mouseY, mouseButton);
