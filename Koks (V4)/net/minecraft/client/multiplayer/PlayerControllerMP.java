...
import koks.event.ItemSyncEvent;
...
public class PlayerControllerMP {
    ...
    public boolean onPlayerDestroyBlock(BlockPos pos, EnumFacing side) {
        ...
        if (this.currentGameType.isCreative() && this.mc.thePlayer.getFakeItem() != null && this.mc.thePlayer.getFakeItem().getItem() instanceof ItemSword) {
            return false;
        } else {
        ...
        if (!this.mc.theWorld.getWorldBorder().contains(loc)) {
            ...
            if (this.currentGameType.isCreative()) {
                ...
                if (flag && block1.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, loc) >= 1.0F) {
                    ...
                    this.currentItemHittingBlock = this.mc.thePlayer.getFakeItem();
    ...
    private boolean isHittingPosition(BlockPos pos) {
        ItemStack itemstack = this.mc.thePlayer.getFakeItem();
    ...
    public void syncCurrentPlayItem() {
        final ItemSyncEvent itemSyncEvent = new ItemSyncEvent().onFire();
        if (!itemSyncEvent.isCanceled()) {
            int i = this.mc.thePlayer.inventory.currentItem;

            if (i != this.currentPlayerItem && (mc.developerMode || (this.mc.thePlayer.inventory.fakeItem == mc.thePlayer.inventory.currentItem))) {
    ...
    public boolean onPlayerRightClick(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos, EnumFacing side, Vec3 hitVec) {
        ...
        if (!this.mc.theWorld.getWorldBorder().contains(hitPos)) {
            ...
            if (this.currentGameType != WorldSettings.GameType.SPECTATOR) {
                ...
                if ((!player.isSneaking()) && iblockstate.getBlock().onBlockActivated(worldIn, hitPos, iblockstate, player, side, f, f1, f2)) {
                    flag = true;
                }
            ...
            this.netClientHandler.addToSendQueue(new C08PacketPlayerBlockPlacement(hitPos, side.getIndex(), player.inventory.getFakeItem(), f, f1, f2));
    ...
    public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn) {
        return sendUseItem(playerIn, worldIn, itemStackIn, true);
    }

    public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn, boolean sync) {
        if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
            return false;
        } else {
            if (sync)
            ...
            if (itemstack != itemStackIn || itemstack != null && itemstack.stackSize != i) {
                playerIn.inventory.mainInventory[playerIn.inventory.fakeItem] = itemstack;

                if (itemstack.stackSize == 0) {
                    playerIn.inventory.mainInventory[playerIn.inventory.fakeItem] = null;
                }
