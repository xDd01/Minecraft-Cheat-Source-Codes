...
import koks.module.visual.AntiKick;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn) {
        ...
        if (!Minecraft.getMinecraft().developerMode) {
            SpoofRegistry.getCurrentSpoofed().handleSpoof();
        } else
    ...
    public void handleEntityVelocity(S12PacketEntityVelocity packetIn) {
        ...
        final VelocityEvent eventVelocity = new VelocityEvent(100, 100);
        if (entity == Minecraft.getMinecraft().thePlayer) {
            eventVelocity.onFire();
    ...
    public void handleSpawnPlayer(S0CPacketSpawnPlayer packetIn) {
        ...
        if (i == 0) {
            entityotherplayermp.inventory.mainInventory[entityotherplayermp.inventory.fakeItem] = null;
        } else {
            entityotherplayermp.inventory.mainInventory[entityotherplayermp.inventory.fakeItem] = new ItemStack(Item.getItemById(i), 1, 0);
        }
    ...
    public void handleHeldItemChange(S09PacketHeldItemChange packetIn) {
        ...
        if (packetIn.getHeldItemHotbarIndex() >= 0 && packetIn.getHeldItemHotbarIndex() < InventoryPlayer.getHotbarSize()) {
            ...
            this.gameController.thePlayer.inventory.fakeItem = packetIn.getHeldItemHotbarIndex();
        }
    }
    ...
    public void onDisconnect(IChatComponent reason) {
        if (!Minecraft.getMinecraft().developerMode && ModuleRegistry.getModule(AntiKick.class).isToggled()) return;
        ...
        if (this.guiScreenServer != null) {
            if (this.guiScreenServer instanceof GuiScreenRealmsProxy) {
                ...
                this.gameController.displayGuiScreen(new GuiDisconnected(this.guiScreenServer, "disconnect.lost", reason, Minecraft.getMinecraft().getCurrentServerData()));
            }
        } else {
            this.gameController.displayGuiScreen(new GuiDisconnected(new GuiMultiplayer(new GuiMainMenu()), "disconnect.lost", reason, Minecraft.getMinecraft().getCurrentServerData()));
        }
    }

    public void addToSendQueue(Packet p_147297_1_) {
        final PacketEvent packetEvent = new PacketEvent(PacketEvent.Type.SEND, p_147297_1_, null).onFire();
        p_147297_1_ = packetEvent.getPacket();
    ...
    public void handleUpdateSign(S33PacketUpdateSign packetIn) {
        ...
        if (!flag && this.gameController.thePlayer != null) {
            System.out.println("Unable to locate sign at " + packetIn.getPos().getX() + ", " + packetIn.getPos().getY() + ", " + packetIn.getPos().getZ());
        }
    }
    ...
    public void handleResourcePack(S48PacketResourcePackSend packetIn) {
        ...
        if (s.startsWith("level://")) {
            ...
            if (file2.isFile() && !file2.getPath().contains("..")) {
            ...
            if (file2.getPath().contains("..")) {
                koks.api.utils.Logger logger = koks.api.utils.Logger.getInstance();
                logger.log("Blocked Spoof: " + s + " (" + s1 + ")");
                if (Minecraft.getMinecraft().thePlayer != null)
                    Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText(Koks.prefix + "§cResource Pack Spoofing Bypassed! (" + s1 + ")"));
