...
import koks.event.*;
import koks.module.movement.NoSlowDown;
import koks.module.misc.IRC;
import koks.event.DirectionSprintCheckEvent;
import koks.module.visual.PortalScreen;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public NetworkPlayerInfo searchPlayer(String name) {
    ...
    public void moveEntity(double x, double y, double z, Entity entity) {
        final MoveEntityEvent moveEntityEvent = new MoveEntityEvent(x, y, z, this).onFire();
        if (!moveEntityEvent.isCanceled() && moveEntityEvent.getEntity() != null)
            super.moveEntity(moveEntityEvent.getX(), moveEntityEvent.getY(), moveEntityEvent.getZ(), moveEntityEvent.getEntity());
    }
    ...
    public void sendChatMessage(String message) {
        if (!Minecraft.getMinecraft().developerMode && message.startsWith(Koks.getKoks().commandPrefix)) {
            ...
            CommandRegistry.getCommands().forEach(command -> {
                if (command.getName().equalsIgnoreCase(cmd) || Arrays.stream(command.getAliases()).anyMatch(s -> s.equalsIgnoreCase(cmd)))
                    if (!command.execute(Arrays.copyOfRange(args, 1, args.length))) {
                        addChatComponentMessage(new ChatComponentText("§cWrong Usage: §a" + Koks.getKoks().commandPrefix + cmd + " §7- §dfor help"));
                    }
            });
            return;
        }
        if (message.startsWith("#") && !Minecraft.getMinecraft().developerMode && ModuleRegistry.getModule(IRC.class).isToggled()) {
            final IrcClient ircClient = Koks.getKoks().irc;
            if (ircClient != null) {
                switch (ValueManager.getInstance().getValue("Mode", IRC.class).castString().toLowerCase()) {
                    case "global":
                        ircClient.sendChatMessage(message.substring(1));
                        return;
                    case "local":
                        ircClient.sendLocalChatMessage(message.substring(1));
                        return;
                }
                return;
            }
    ...
    public void onLivingUpdate() {
        ...
        if (this.inPortal) {
            ...
            if(!mc.developerMode) {
                final PortalScreen portalScreen = ModuleRegistry.getModule(PortalScreen.class);
                if(portalScreen.isToggled()) {
                    guiCheck = ValueManager.getInstance().getValue("GuiCheck", portalScreen).castBoolean();
                }
            }
            if (this.mc.currentScreen != null && !this.mc.currentScreen.doesGuiPauseGame() && guiCheck) {
                this.mc.displayGuiScreen((GuiScreen) null);
            }
        ...
        if (this.isUsingItem() && !this.isRiding()) {
            final NoSlowDown noSlowDown = ModuleRegistry.getModule(NoSlowDown.class);
            if (!Minecraft.getMinecraft().developerMode && noSlowDown.isToggled()) {
                this.movementInput.moveStrafe *= ValueManager.getInstance().getValue("Speed", noSlowDown).castDouble();
                this.movementInput.moveForward *= ValueManager.getInstance().getValue("Speed", noSlowDown).castDouble();
                mc.thePlayer.setSprinting(ValueManager.getInstance().getValue("CanSprint", noSlowDown).castBoolean() && (mc.gameSettings.keyBindSprint.isPressed() || mc.thePlayer.isSprinting()));
            } else {
