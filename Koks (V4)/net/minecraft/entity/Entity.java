...
public abstract class Entity implements ICommandSender {
    ...
    public void setAngles(float yaw, float pitch) {
        ...
        if (Minecraft.getMinecraft().developerMode || !ModuleRegistry.getModule(NoPitchLimit.class).isToggled())
    ...
    public void moveEntity(double x, double y, double z) {
        this.moveEntity(x,y,z,this);
    }

    public void moveEntity(double x, double y, double z, Entity entity) {
        if (this.noClip) {
            this.setEntityBoundingBox(entity.getEntityBoundingBox().offset(x, y, z));
            ...
            if (safewalkEvent.isSafe()) {
                ...
                for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(entity, entity.getEntityBoundingBox().offset(x, -1.0D, 0.0D)).isEmpty(); d3 = x) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
                ...
                for (; z != 0.0D && entity.worldObj.getCollidingBoundingBoxes(entity, entity.getEntityBoundingBox().offset(0.0D, -1.0D, z)).isEmpty(); d5 = z) {
                    if (z < d6 && z >= -d6) {
                        z = 0.0D;
                    } else if (z > 0.0D) {
                        z -= d6;
                ...
                for (; x != 0.0D && z != 0.0D && entity.worldObj.getCollidingBoundingBoxes(entity, entity.getEntityBoundingBox().offset(x, -1.0D, z)).isEmpty(); d5 = z) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
            ...
            List<AxisAlignedBB> list1 = entity.worldObj.getCollidingBoundingBoxes(entity, entity.getEntityBoundingBox().addCoord(x, y, z));
            AxisAlignedBB axisalignedbb = entity.getEntityBoundingBox();

            for (AxisAlignedBB axisalignedbb1 : list1) {
                y = axisalignedbb1.calculateYOffset(entity.getEntityBoundingBox(), y);
            }

            entity.setEntityBoundingBox(entity.getEntityBoundingBox().offset(0.0D, y, 0.0D));
            boolean flag1 = entity.onGround || d4 != y && d4 < 0.0D;

            for (AxisAlignedBB axisalignedbb2 : list1) {
                x = axisalignedbb2.calculateXOffset(entity.getEntityBoundingBox(), x);
            }

            entity.setEntityBoundingBox(entity.getEntityBoundingBox().offset(x, 0.0D, 0.0D));

            for (AxisAlignedBB axisalignedbb13 : list1) {
                z = axisalignedbb13.calculateZOffset(entity.getEntityBoundingBox(), z);
            }
            ...
            if (entity.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                double d7 = y;
                double d8 = z;
                AxisAlignedBB axisalignedbb3 = entity.getEntityBoundingBox();
                entity.setEntityBoundingBox(axisalignedbb);
                y = (double) entity.stepHeight;
                List<AxisAlignedBB> list = entity.worldObj.getCollidingBoundingBoxes(entity, entity.getEntityBoundingBox().addCoord(d3, y, d5));
                ...
                for (AxisAlignedBB axisalignedbb12 : list) {
                    y = axisalignedbb12.calculateYOffset(entity.getEntityBoundingBox(), y);
                }
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z) {
                    ...
                    y = d7;
                    z = d8;
                    entity.setEntityBoundingBox(axisalignedbb3);
                }
            }
            ...
            entity.isCollidedHorizontally = d3 != x || d5 != z;
            entity.isCollidedVertically = d4 != y;
            entity.onGround = entity.isCollidedVertically && d4 < 0.0D;
            ...
            Block block1 = entity.worldObj.getBlockState(blockpos).getBlock();
            ...
            entity.updateFallState(y, entity.onGround, block1, blockpos);
            ...
            if (d4 != y) {
                block1.onLanded(entity.worldObj, entity);
            }

            if (entity.canTriggerWalking() && !flag && entity.ridingEntity == null) {
                double d12 = entity.posX - d0;
                double d13 = entity.posY - d1;
                double d14 = entity.posZ - d2;

                if (block1 != Blocks.ladder) {
                    d13 = 0.0D;
                }

                if (block1 != null && entity.onGround) {
                    block1.onEntityCollidedWithBlock(entity.worldObj, blockpos, entity);
                }
                ...
                if (entity.distanceWalkedOnStepModified > (float) entity.nextStepDistance && block1.getMaterial() != Material.air) {
            ...
            if (entity.worldObj.isFlammableWithin(entity.getEntityBoundingBox().contract(0.001D, 0.001D, 0.001D))) {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (f >= 1.0E-4F) {
            ...
            final MoveEvent moveEvent = new MoveEvent(rotationYaw).onFire();

            float yaw = this == Minecraft.getMinecraft().thePlayer ? moveEvent.getYaw() : rotationYaw;
    ...
    public boolean isInvisibleToPlayer(EntityPlayer player) {
        return !player.isSpectator() || (!Minecraft.getMinecraft().developerMode && !ModuleRegistry.getModule(TrueSight.class).isToggled() && this.isInvisible());
    }
