...
import koks.event.JumpEvent;
import koks.event.MoveEntityWithHeadingEvent;
import koks.event.MoveWithHeadingEvent;
import koks.event.PotionUpdateEvent;
import koks.module.movement.NoJumpDelay;
import koks.module.misc.ExploitFixer;
import koks.module.visual.HitSlow;
...
public abstract class EntityLivingBase extends Entity {
    ...
    public void onEntityUpdate() {
        ...
        if(this.velocityHurtTime > 0) {
            --this.velocityHurtTime;
        }
    ...
    protected void updatePotionEffects() {
        ...
        final PotionUpdateEvent potionUpdateEvent = new PotionUpdateEvent().onFire();
        if(potionUpdateEvent.isCanceled())
            return;
    ...
    private int getArmSwingAnimationEnd() {
        return !Minecraft.getMinecraft().developerMode && this instanceof EntityPlayerSP && ModuleRegistry.getModule(HitSlow.class).isToggled() ? ValueManager.getInstance().getValue("Slowness", ModuleRegistry.getModule(HitSlow.class)).castInteger() : this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6);
    }
     ...
     */
    protected void jump() {
        final JumpEvent jumpEvent = new JumpEvent(rotationYaw, this.getJumpUpwardsMotion(), 0.2F).onFire();

        this.motionY = (double) jumpEvent.getHeight();
        ...
        if (this.isSprinting()) {
            ...
            this.motionX -= (double) (MathHelper.sin(f) * jumpEvent.getBoost());
            this.motionZ += (double) (MathHelper.cos(f) * jumpEvent.getBoost());
        }
    ...
    public void moveEntityWithHeading(float strafe, float forward) {
        if (this.isServerWorld()) {
            float aiMoveSpeed = this.getAIMoveSpeed();
            if(this == Minecraft.getMinecraft().thePlayer) {
                final MoveEntityWithHeadingEvent moveEntityWithHeadingEvent = new MoveEntityWithHeadingEvent(getAIMoveSpeed()).onFire();
                aiMoveSpeed = moveEntityWithHeadingEvent.getAiSpeed();
            }
            final ExploitFixer exploitFixer = ModuleRegistry.getModule(ExploitFixer.class);
            if (!Minecraft.getMinecraft().developerMode && exploitFixer.isToggled() && ValueManager.getInstance().getValue("Invalid Movement Attribute", exploitFixer).castBoolean() && (Double.isInfinite(aiMoveSpeed) || Double.isNaN(aiMoveSpeed)))
                aiMoveSpeed = this.lastValidLandMoveSpeed;
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                    ...
                    final MoveWithHeadingEvent eventMoveWithHeading = new MoveWithHeadingEvent(f4).onFire();
    ...
    float lastValidLandMoveSpeed;

    /**
     * set the movespeed used for the new AI system
     */
    public void setAIMoveSpeed(float speedIn) {
        if (!Float.isInfinite(speedIn) && !Float.isNaN(speedIn))
            this.lastValidLandMoveSpeed = speedIn;
    ...
    public void onLivingUpdate() {
        ...
        if (this.isJumping) {
            if (this.isInWater()) {
                ...
                this.handleJumpLava();
            } else if (this.onGround && (this.jumpTicks == 0 || !Minecraft.getMinecraft().developerMode && ModuleRegistry.getModule(NoJumpDelay.class).isToggled())) {
                this.jump();
                this.jumpTicks = 10;
            }
        } else {
    ...
    public boolean canBlockBeSeen(BlockPos blockPos) {
        return this.worldObj.rayTraceBlocks(new Vec3(this.posX, this.posY + (double) this.getEyeHeight(), this.posZ), new Vec3(blockPos.getX(), blockPos.getY(), blockPos.getZ())) == null;
    }
