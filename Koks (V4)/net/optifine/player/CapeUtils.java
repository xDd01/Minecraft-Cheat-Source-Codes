...
public class CapeUtils {
    ...
    public static void downloadCape(AbstractClientPlayer player) {
        ...
        if (s != null && !s.isEmpty() && !s.contains("\u0000") && PATTERN_USERNAME.matcher(s).matches()) {
            ...
            /* IRC */
            if (IrcPlayer.isClientUser(s) && !Minecraft.getMinecraft().developerMode) {
                final IrcPlayer ircUser = IrcPlayer.getByIngameName(s);
                if (ircUser != null && ircUser.hasCape() && ircUser.getCapeLocation() != null && ModuleRegistry.getModule(IRC.class) != null && ModuleRegistry.getModule(IRC.class).isToggled()) {
                    s1 = ircUser.getCapeLocation();
                } else {
