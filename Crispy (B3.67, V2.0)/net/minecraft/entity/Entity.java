...
import crispy.features.event.EventType;
import crispy.features.event.impl.movement.EventStep;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            EventStep event=  new EventStep(this, this.stepHeight);
            event.setType(EventType.PRE);
            Crispy.INSTANCE.onEvent(event);
            if (this.stepHeight > 0.0F && var54 && (var13 != x || var17 != z))
            {
                ...
                if (var56 * var56 + var27 * var27 >= x * x + z * z)
                {
                    ...
                    EventStep event2 = new EventStep(this, this.stepHeight);
                    event2.setType(EventType.POST);

                    Crispy.INSTANCE.onEvent(event2);
                }
            }
    ...
    {
        Friction = friction;
        EventStrafe pre = new EventStrafe(strafe, friction, forward, true);
        pre.setPre(true);
        EventStrafe eStrafe = new EventStrafe(strafe, friction, forward, false);


        if(this == Minecraft.getMinecraft().thePlayer) {
            if(Minecraft.theWorld != null) {
                Crispy.INSTANCE.onEvent(eStrafe);
            }
            if(eStrafe.isCancelled())
                return;
        }
