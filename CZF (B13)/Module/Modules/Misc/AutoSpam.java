...
public class AutoSpam extends Module {
    ...
    private final Mode<Enum> mode = new Mode("Mode", "mode", Lmode.values(), Lmode.Penshen);

    public AutoSpam() {
        super("SprayGod", new String[]{"autol"}, ModuleType.Player);
    ...
    private void onChat(EventChat e) {
        if(this.mode.getValue() == Lmode.Penshen) {
            Random r = new Random();
            String fuck = EMessages[r.nextInt(EMessages.length)];
            if (e.getMessage().contains("被" + mc.thePlayer.getName() + "击杀")
                    ...
                    || e.getMessage().contains("被" + mc.thePlayer.getName() + "扔下了悬崖。")) {
                mc.thePlayer.sendChatMessage("/shout "+e.getMessage().split("被")[0] + " " + fuck);
            }
        }
    }
