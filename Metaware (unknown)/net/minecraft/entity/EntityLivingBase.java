...
public abstract class EntityLivingBase extends Entity
{
    ...
    public String getName(){
        String s = this.getDisplayName().getFormattedText();
        if(s == null){
            return null;
        }else{
            return s;
    ...
    protected void jump()
    {
        final PlayerJumpEvent event = new PlayerJumpEvent(this.getJumpUpwardsMotion());
        Metaware.INSTANCE.getEventBus().post(event);
        if (event.isCancelled()) return;
        PlayerStrafeEvent event1 = new PlayerStrafeEvent(this.rotationYaw, 0.2f, this.getJumpUpwardsMotion());
