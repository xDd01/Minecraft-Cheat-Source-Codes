...
import client.metaware.impl.module.render.Camera;
...
public abstract class GuiContainer extends GuiScreen
{
    ...
    {
        this.translate.interpolate2(this.width, this.height, 3);

        this.drawDefaultBackground();
        int i = this.guiLeft;
        int j = this.guiTop;
        double containerX = this.width / 2 - (translate.getX() / 2);
        double containerY = this.height / 2 - (translate.getY() / 2);

        if(Metaware.INSTANCE.getModuleManager().getModuleByClass(Camera.class).containerAnimation.getValue() && Metaware.INSTANCE.getModuleManager().getModuleByClass(Camera.class).isToggled()){
            GlStateManager.translate(containerX, containerY, 0);
            GlStateManager.scale(translate.getX() / width, translate.getY() / height, 1);
        }
