...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public UpdatePlayerEvent currentEvent;
    ...
    public void moveEntity(double x, double y, double z) {
        MovePlayerEvent event = new MovePlayerEvent(x, y, z);
        Metaware.INSTANCE.getEventBus().post(event);
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        boolean flag1 = this.isSneaking() || Metaware.INSTANCE.getModuleManager().getModuleByClass(Scaffold.class).isToggled();
        ...
        {
            final UpdatePlayerEvent updatePlayerEvent = new UpdatePlayerEvent(UpdatePlayerEvent.EventState.PRE, lastReportedYaw, lastReportedPitch,
                    lastTickPosX, lastTickPosY, lastTickPosZ,
                    posX, getEntityBoundingBox().minY, posZ,
                    rotationYaw, rotationPitch, onGround);
            Metaware.INSTANCE.getEventBus().post(updatePlayerEvent);
            currentEvent = updatePlayerEvent;
            final double eventX = updatePlayerEvent.getPosX();
            final double eventY = updatePlayerEvent.getPosY();
            final double eventZ = updatePlayerEvent.getPosZ();
            final float eventYaw = updatePlayerEvent.getYaw();
            final float eventPitch = updatePlayerEvent.getPitch();
            final boolean eventGround = updatePlayerEvent.isOnGround();
            double d0 = eventX - this.lastReportedPosX;
            double d1 = eventY - this.lastReportedPosY;
            double d2 = eventZ - this.lastReportedPosZ;
            double d3 = (double)(eventYaw - this.lastReportedYaw);
            double d4 = (double)(eventPitch - this.lastReportedPitch);
            boolean flag2 = d0 * d0 + d1 * d1 + d2 * d2 > 9.0E-4D || this.positionUpdateTicks >= 20;
            boolean flag3 = d3 != 0.0D || d4 != 0.0D;
            if (currentEvent.isRotating()) {
                this.rotationYawHead = (float) RenderUtil.interpolate(currentEvent.getPrevYaw(), currentEvent.getYaw(), 0);
                this.renderYawOffset = (float) RenderUtil.interpolate(currentEvent.getPrevYaw(), currentEvent.getYaw(), 0);
            }
            ...
            updatePlayerEvent.setState(UpdatePlayerEvent.EventState.POST);
            Metaware.INSTANCE.getEventBus().post(updatePlayerEvent);
        }
    }
    ...
    {
        if(message.startsWith(Metaware.INSTANCE.getClientInfo().getCommandPrefix())) {
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isUsingItem() && !this.isRiding() && !Metaware.INSTANCE.getModuleManager().getModuleByName("Noslow").isToggled())
        {
    ...
    public void updateHeldTool(BlockPos pos) {
