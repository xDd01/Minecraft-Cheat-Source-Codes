...
public class PlayerControllerMP
{
    ...
    public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn, EntityLivingBase target, C08PacketPlayerBlockPlacement packetPlayerBlockPlacement) {
        if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
            return false;
        } else {
            this.syncCurrentPlayItem();
            if(mc.thePlayer.ticksExisted % 8 == 0) this.netClientHandler.addToSendQueue(packetPlayerBlockPlacement);
    ...
    {
        if(playerIn.equals(mc.thePlayer)) Metaware.INSTANCE.getEventBus().post(new WindowClickEvent(windowId, slotId, mouseButtonClicked, mode));
