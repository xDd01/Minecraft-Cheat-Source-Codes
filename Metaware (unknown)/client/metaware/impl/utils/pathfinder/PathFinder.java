...
public class PathFinder
{
    ...
    private ArrayList<PathPos> getNeighbors(final PathPos pos) {
        ...
        if (pos.getY() < 256 && this.canGoThrough(up.up()) && (flying || onGround || this.canClimbUpAt(pos)) && (flying || this.canClimbUpAt(pos) || this.goal.equals(up) || this.canSafelyStandOn(north) || this.canSafelyStandOn(east) || this.canSafelyStandOn(south) || this.canSafelyStandOn(west))) {
    ...
    protected boolean isPassable(final BlockPos pos) {
        return this.canGoThrough(pos) && this.canGoThrough(pos.up()) && this.canGoAbove(pos.down());
    }

    protected boolean canBeSolid(final BlockPos pos) {
        final Material material = ScaffoldUtils.getMaterial(pos);
        final Block block = ScaffoldUtils.getBlock(pos);
    ...
    private boolean canFallBelow(final PathPos pos) {
        ...
        if (ScaffoldUtils.getBlock(down2) instanceof BlockSlime && this.fallingAllowed) {
            return true;
        }
        ...
        for (int i = 0; i <= (this.fallingAllowed ? 3 : 1); ++i) {
            ...
            final Block prevBlock = ScaffoldUtils.getBlock(prevPos);
    ...
    private boolean canMoveSidewaysInMidairAt(final BlockPos pos) {
        final Block blockFeet = ScaffoldUtils.getBlock(pos);
        ...
        final Block blockHead = ScaffoldUtils.getBlock(pos.up());
    ...
    private float getCost(final BlockPos current, final BlockPos next) {
        ...
        final Material nextMaterial = ScaffoldUtils.getMaterial(next);
        if (nextMaterial == Material.lava) {
            cost *= 4.5395155f;
        }
        if (!this.canFlyAt(next) && ScaffoldUtils.getBlock(next.down()) instanceof BlockSoulSand) {
            cost *= 2.5f;
        }
        return cost;
    }
    ...
    public PathProcessor getInfiniteAuraProcessor(final EntityLivingBase e) {
        return new InfAuraProcessor(this.path, this.creativeFlying, e);
    }
