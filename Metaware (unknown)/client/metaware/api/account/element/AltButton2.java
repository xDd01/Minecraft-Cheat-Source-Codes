...
public class AltButton2 {
    ...
    public AltButton2(float x, float y, float width, float height, int fontSize, int color) {
        this(x, y, width, height, "", fontSize, color);
    }

    public AltButton2(Position position, int fontSize, int color) {
        this(position.x(), position.y(), position.width(), position.height(), "", fontSize, color);
    }

    public AltButton2(Position position, String text, int fontSize, int color) {
        this(position.x(), position.y(), position.width(), position.height(), text, fontSize, color);
    }

    public AltButton2(float x, float y, float width, float height, String text, int fontSize, int color) {
        this.position = new Position(x, y, width, height);
        this.text = text;
        this.color = color;
        this.fontRenderer = Metaware.INSTANCE.getFontManager().defaultFont().size(fontSize);
    }
