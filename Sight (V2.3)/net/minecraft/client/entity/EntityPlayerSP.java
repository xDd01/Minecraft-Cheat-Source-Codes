...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdateWalkingPlayer()
    {
    	...
    	if (mc.gameSettings.thirdPersonView != 0) {
        	this.rotationYawHead = euPre.getYaw();
        	this.serverPitch = euPre.getPitch();
    	} else {
    		this.serverPitch = this.rotationPitch;
        ...
        boolean flag1 = euPre.isSneaking();
        ...
        {
            double d0 = euPre.getX() - this.lastReportedPosX;
            double d1 = euPre.getY() - this.lastReportedPosY;
            ...
            if (this.ridingEntity == null)
            {
                ...
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(euPre.getYaw(), euPre.getPitch(), euPre.isOnGround()));
                }
                else
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer(euPre.isOnGround()));
                }
            }
            else
            {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, euPre.getYaw(), euPre.getPitch(), euPre.isOnGround()));
                flag2 = false;
            }
            ...
            if (flag2)
            {
                this.lastReportedPosX = euPre.getX();
                this.lastReportedPosY = euPre.getY();
                this.lastReportedPosZ = euPre.getZ();
                this.positionUpdateTicks = 0;
            }

            if (flag3)
            {
                this.lastReportedYaw = euPre.getYaw();
                this.lastReportedPitch = euPre.getPitch();
            }
        }
        
    	EventUpdate euPost = new EventUpdate(this.rotationYaw, this.rotationPitch, this.posX, this.getEntityBoundingBox().minY, this.posZ, this.onGround, this.isSneaking(), false);
    	euPost.call();
    }
    ...
    {
    	EventChatSend ec = new EventChatSend(message);
    	ec.call();
    	if (ec.isCancelled()) {
    		return;
    	}
        this.sendQueue.addToSendQueue(new C01PacketChatMessage(ec.getMessage()));
    }
    ...
    {
    	EventMove em = new EventMove(x, y, z);
    	em.call();
    ...
    public static long lastCrit = System.currentTimeMillis();
