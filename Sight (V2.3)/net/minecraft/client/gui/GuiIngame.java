    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        for (Object score1 : arraylist1)
        {
            ...
            if (k == arraylist1.size())
            {
                ...
                if (ServerUtils.isOnHypixel() && s3 != null) {
                	if (s3.contains("DUELS")) {
                		Manager.setGameMode(GAMEMODE.DUELS);
                	}else if (s3.contains("BEDWARS")) {
                		Manager.setGameMode(GAMEMODE.BEDWARS);
                	}else if (s3.contains("SKYWARS")) {
                		Manager.setGameMode(GAMEMODE.SKYWARS);
                	}else if (s3.contains("PIT")) {
                		Manager.setGameMode(GAMEMODE.PIT);
                	}else Manager.setGameMode(GAMEMODE.UNKNOWN);
                } else {
                	Manager.setGameMode(GAMEMODE.UNKNOWN);
                }
