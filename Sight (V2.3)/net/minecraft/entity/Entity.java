...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            EventStep es = new EventStep(this.stepHeight, true);
            es.call();
            
            if (es.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z))
            {
                double d11 = x;
                double d7 = y;
                double d8 = z;
                ...
                if (y != -es.stepHeight && y != -(es.stepHeight - 0.5) && y != -(es.stepHeight - 0.0625) && y != -0.20000004768371582) {
                    if (this == Minecraft.getMinecraft().thePlayer) {
                    	EventStep es2 = new EventStep(this.stepHeight, false);
                    	es2.call();
                    }
                }
    ...
    public float getDistanceToVec3(Vec3 vec)
    {
        float f = (float)(this.posX - vec.getX());
        float f1 = (float)(this.posY - vec.getY());
        float f2 = (float)(this.posZ - vec.getZ());
        return MathHelper.sqrt_float(f * f + f1 * f1 + f2 * f2);
    }
    ...
    public void updateRiderPosition()
    {
        if (this.riddenByEntity != null)
        {
            this.riddenByEntity.setPosition(this.posX, this.posY + this.getMountedYOffset() + this.riddenByEntity.getFrontOffsetY(), this.posZ);
        }
    }
