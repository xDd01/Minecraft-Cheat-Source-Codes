...
public final class VecUtils {
    ...
    public static Vec3 calculateBlockCenter(World world, BlockPos pos) {
        double xDiff = (((double)pos.getX() - 0.5) + ((double)pos.getX() + 0.5)) / 2;
        double yDiff = (((double)pos.getY() - 0.5) + ((double)pos.getY() + 0.5)) / 2;
        double zDiff = (((double)pos.getZ() - 0.5) + ((double)pos.getZ() + 0.5)) / 2;
        if (world.getBlockState(pos).getBlock() instanceof BlockFire) {//look at bottom of fire when putting it out
            yDiff = 0;
        }
        return new Vec3(
                pos.getX() + xDiff,
