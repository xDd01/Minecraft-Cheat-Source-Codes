...
public class GuiTextFieldFlux extends GuiTextField {

	private String info;
	private GlyphPageFontRenderer fluxFont, fluxFont2;

	public GuiTextFieldFlux(int componentId, BasicFontRenderer fontrendererObj, int x, int y, int par5Width,
			int par6Height, String info) {
    ...
    public void drawTextBox()
    {
        if (this.getVisible())
        {
        	if (!isFocused && this.getText().isEmpty()) {
        		RenderUtil.drawRoundedRect(xPosition, yPosition, width, height, 6, Color.decode("#DDDDDD").getRGB());
    			RenderUtil.drawRoundedRect(xPosition + 1, yPosition + 1, width - 2, height - 2, 6, Color.WHITE.getRGB());
    			fluxFont.drawString(info, xPosition + 5, yPosition + height / 2 - fluxFont.getFontHeight() / 2,
    					Color.decode("#DDDDDD").getRGB());
    		} else {
    			RenderUtil.drawRoundedRect(xPosition, yPosition, width, height, 6, Color.decode("#7D7D7D").getRGB());
    			RenderUtil.drawRoundedRect(xPosition + 1, yPosition + 1, width - 2, height - 2, 6, Color.WHITE.getRGB());
    			int width = fluxFont2.getStringWidth(info) + 4;
    			drawRect(xPosition + 7, yPosition, xPosition + 7 + width, yPosition + 1, Color.white.getRGB());
    			fluxFont2.drawCenteredString(info, xPosition + 7 + width / 2, yPosition - fluxFont2.getFontHeight() / 2,
    					Color.decode("#7D7D7D").getRGB(), false);
    		}
            ...
            if (s.length() > 0)
            {
                ...
                j1 = this.fontRendererInstance.drawString(s1, (float)l, (float)i1+1, Color.black.getRGB());
            }
            ...
            {
                j1 = this.fontRendererInstance.drawString(s.substring(j), (float)j1, (float)i1+1, Color.black.getRGB());
            }
