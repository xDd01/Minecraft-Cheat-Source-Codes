...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            ...
            try
            {
            	...
            	if(Client.INSTANCE.moduleManager.getModule("Scaffold").isState()) {
                    float f = this.interpolateRotation(entity.prevRenderYawOffset  - 35, entity.renderYawOffset -   35 , partialTicks);
    ...
    protected boolean setScoreTeamColor(EntityLivingBase entityLivingBaseIn)
    {
        ...
        if (entityLivingBaseIn instanceof EntityPlayer)
        {
            ...
            {
                String s = BasicFontRenderer.getFormatFromString(scoreplayerteam.getColorPrefix());
        ...
        float c1 = (float) (Color.GREEN.getRGB() >> 16 & 255) / 255.0F;
        float c2 = (float) (Color.GREEN.getRGB()  >> 8 & 255) / 255.0F;
        float c = (float) (Color.GREEN.getRGB()  & 255) / 255.0F;
...
//        return true;
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float p_77036_7_)
    {
        ...
        if (flag || flag1)
        {
            ...
            if( Client.INSTANCE.moduleManager.getModule("ESP").isState() && ((DropdownBox) Client.INSTANCE.moduleManager.getModule("ESP").getSetting("ESPModes").getSetting()).curOption == "NewOutline") {

            if (Minecraft.getMinecraft().thePlayer != null &&  entitylivingbaseIn instanceof EntityPlayer) {
				Color colorForOutline =Color.green;
				//	RenderUtil.setColor(new Color(0,172,255).getRGB());
    ...
    public void renderName(T entity, double x, double y, double z)
    {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)})) {
        	try {
            if (!Client.INSTANCE.moduleManager.getModule("NameTags").state) {
