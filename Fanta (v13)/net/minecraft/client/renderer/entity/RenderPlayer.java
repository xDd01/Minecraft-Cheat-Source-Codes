	...
	protected void renderOffsetLivingLabel(AbstractClientPlayer entityIn, double x, double y, double z, String str,
		...
		if (p_177069_10_ < 100.0D) {
			...
			if (scoreobjective != null) {
				...
				this.renderLivingLabel(entityIn, score.getScorePoints() + " " + scoreobjective.getDisplayName(), x, y,
						z, 64);
				y += (double) ((float) this.getBasicFontRendererFromRenderManager().getFontHeight() * 1.15F * p_177069_9_);
				yoffset += 0.5f;
			}
		}

		// Team and friend nametags
		...
		if (ircPlayer != null) {
			...
			if (ircPlayer.getRank() == IrcRank.ADMIN || ircPlayer.getRank() == IrcRank.DEV) {
				s.append("§c" + client + "-Dev §8: §7" + ircPlayer.getIrcNick());
			} else if (ircPlayer.getRank() == IrcRank.MOD) {
				s.append("§5" + client + "-Youtuber §8: §7" + ircPlayer.getIrcNick());
			} else if (ircPlayer.getRank() == IrcRank.VIP) {
				s.append("§6" + client + "-Vip §8: §7" + ircPlayer.getIrcNick());
			} else {
				s.append("§e" + client + "-User §8: §7" + ircPlayer.getIrcNick());
			}
			
			this.renderLivingLabel(entityIn, s.toString(), x, y
					+ 0.25,
					z, 64);
