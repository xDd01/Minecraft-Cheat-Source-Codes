...
import de.fanta.events.listeners.EventLocation;
import de.fanta.events.listeners.EventNoRotate;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
        ...
        final EventLocation eventLocation = new EventLocation(this.gameController.thePlayer.rotationYaw, this.gameController.thePlayer.rotationPitch);
    	Client.INSTANCE.onEvent(eventLocation);
    	float rotYaw = eventLocation.yaw;
    	float rotPitch = eventLocation.pitch;
        ...
        float serverYaw = f;
        float serverPitch = f1;

        final EventNoRotate eventNoRotate = new EventNoRotate(f, f1);
        Client.INSTANCE.onEvent(eventNoRotate);
        Rotations.yaw = serverYaw;        
        Rotations.pitch = serverPitch;
        entityplayer.setPositionAndRotation(d0, d1, d2, eventNoRotate.yaw, eventNoRotate.pitch);
        this.netManager.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, serverYaw, serverPitch, false));
    ...
    public void addToSendQueue(Packet packet) {
        try {
        	EventPacket eventPacket = new EventPacket(EventPacket.Action.SEND, packet);
            eventPacket.setType(EventType.PRE);
            Client.INSTANCE.onEvent(eventPacket);
