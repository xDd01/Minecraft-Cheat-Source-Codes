...
public abstract class EntityPlayer extends EntityLivingBase {
	...
	public void onLivingUpdate() {
		...
		if (Client.INSTANCE.moduleManager.getModule("Regen").isState()) {
		...
		// float f1 = (float) (Math.atan(-this.motionY));

		// if (!Client.INSTANCE.moduleManager.getModule("Triggerbot").isState()) {
					...
					if (flag2) {
						if (i > 0) {
							targetEntity.addVelocity(
									(double) (-MathHelper.sin(Rotations.yaw * (float) Math.PI / 180.0F) * (float) i
											* 0.5F),
									0.1D, (double) (MathHelper.cos(Rotations.yaw * (float) Math.PI / 180.0F) * (float) i
											* 0.5F));
							if (((CheckBox) Client.INSTANCE.moduleManager.getModule("TestAura").getSetting("KeepSprint")
									.getSetting()).state) {
