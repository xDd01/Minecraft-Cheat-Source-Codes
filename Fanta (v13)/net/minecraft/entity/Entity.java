...
public abstract class Entity implements ICommandSender {
	...
	public void moveEntity(double x, double y, double z) {
		if (this.noClip) {
			...
			// && (((DropdownBox)
			// Client.INSTANCE.moduleManager.getModule("Scaffold").getSetting("RotationMode").getSetting()).curOption
			// != "Intave"
			boolean flag = this.onGround && this.isSneaking() && this instanceof EntityPlayer;
			EventEnd end = new EventEnd();
			end.setType(EventType.PRE);

			/*
			 * mc.thePlayer.swingItem(); if
			 * (mc.playerController.onPlayerRightClick(mc.thePlayer, mc.theWorld,
			 * player.inventory.getCurrentItem(), blockpos, facing,
			 * mc.objectMouseOver.hitVec)) {
			 * 
			 * 
			 * 
			 * 
			 * if(Client.INSTANCE.moduleManager.getModule("Scaffold").state){
			 * 
			 * double d6; Minecraft.getMinecraft().thePlayer.onGround = true;
			 * 
			 * for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(this,
			 * this.getEntityBoundingBox().offset(0.0D, -1.0D, 0.0D)).isEmpty(); d3 = x) {
			 * 
			 * 
			 * 
			 * if (x < d6 && x >= -d6) {
			 * 
			 * x = 0.0D; } else if (x > 0.0D) { x -= d6; } else { x += d6; } }
			 * 
			 * for (; z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this,
			 * this.getEntityBoundingBox().offset(0.0D, -1.0D, 0.0D)).isEmpty(); d5 = z) {
			 * 
			 * 
			 * if (z < d6 && z >= -d6) {
			 * 
			 * z = 0.0D; } else if (z > 0.0D) { z -= d6; } else { z += d6; } }
			 * 
			 * for (; x != 0.0D && z != 0.0D &&
			 ...
			 * 
			 * 
			 * if (z < d6 && z >= -d6) { z = 0.0D; } else if (z > 0.0D) { z -= d6; } else {
			 * z += d6; } } }else {
			 * 
			 * 
			 */
		...
		if (f >= 1.0E-4F) {
			...
			try {
				...
				if (Client.INSTANCE.moduleManager.getModule("TestAura").isState()
						&&  TestAura.target != null &&  ((CheckBox) Client.INSTANCE.moduleManager.getModule("TestAura").getSetting("MoveFix")
								.getSetting()).state ||Client.INSTANCE.moduleManager.getModule("TestAura").isState()
						&&  TestAura.target != null &&  ((CheckBox) Client.INSTANCE.moduleManager.getModule("TestAura").getSetting("SilentMoveFix")
								.getSetting()).state ) {
					float f1 = MathHelper.sin(Rotations.yaw * (float) Math.PI / 180.0F);
					float f2 = MathHelper.cos(Rotations.yaw * (float) Math.PI / 180.0F);
	...
	public Vec3 getLook(float partialTicks) {
		...
		if (!(Client.INSTANCE.moduleManager.getModule("AutoEagle").isState())) {
			...
			if (EventPreMotion.getInstance != null)
				return this.getVectorForRotation(EventPreMotion.getInstance.getPitch(),
						EventPreMotion.getInstance.getYaw());
			else
