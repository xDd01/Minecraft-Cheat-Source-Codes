...

{
    ...
    public static EntityLivingBase INSTANCE;
    ...
    {
    if(Client.INSTANCE.moduleManager.getModule("HitSlow").isState()) {
       return (int) (this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : HitSlow.hitSlowdown));
    	}else {
    ...
    protected void jump()
    {
        ...
        {
             {
            if(Client.INSTANCE.moduleManager.getModule("TestAura").state   && TestAura.target != null && ((CheckBox) Client.INSTANCE.moduleManager.getModule("TestAura").getSetting("JumpFix")
					.getSetting()).state ) {
                    float f = Rotations.yaw * 0.017453292F;
