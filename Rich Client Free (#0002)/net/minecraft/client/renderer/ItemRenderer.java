...
public class ItemRenderer
{
    ...
    private void transformFirstPerson(EnumHandSide p_187453_1_, float p_187453_2_)
    {
		...
		float g = (float) Main.settingsManager.getSettingByName(Main.moduleManager.getModule(Animations.class), "Size").getValDouble();
		if (Main.moduleManager.getModule(Animations.class).isToggled()) {
			GlStateManager.translate(Main.settingsManager.getSettingByName(Main.moduleManager.getModule(Animations.class), "Distance").getValDouble(), -0.37F, -0.41999997F);
			...
			if(Main.settingsManager.getSettingByName(Main.moduleManager.getModule(Animations.class), "Item360").getValBoolean()) {
			GlStateManager.rotate(rotate, rotate , 0  , 0);
			rotate++;
			}
    ...
    private void transformSideFirstPerson(EnumHandSide p_187459_1_, float p_187459_2_)
    {
          ...
          event.call();
          if (Main.instance.moduleManager.getModule(ViewModel.class).isToggled()) {
    ...
    public void renderItemInFirstPerson(AbstractClientPlayer p_187457_1_, float p_187457_2_, float p_187457_3_, EnumHand p_187457_4_, float p_187457_5_, ItemStack p_187457_6_, float p_187457_7_)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_))
        {
            ...
            else
            {
				...
				if (Main.moduleManager.getModule(BlockHitAnimation.class).isToggled() && BlockHitAnimation.blocking  && p_187457_4_ == EnumHand.MAIN_HAND) {
                  GlStateManager.rotate((this.mc.gameSettings.mainHand == EnumHandSide.RIGHT) ? 75.0F : -75.0F, 0.0F, 0.0F, 1.0F);
                   GlStateManager.rotate((this.mc.gameSettings.mainHand == EnumHandSide.RIGHT) ? 150.0F : -20.0F, -0.2F, 1.0F, 0.0F);
                 }
    ...
    public void renderOverlays(float partialTicks)
    {
        ...
        if (!this.mc.player.isSpectator())
        {
            ...
            if (this.mc.player.isBurning() && !Main.settingsManager.getSettingByName(Main.moduleManager.getModule(NoRender.class), "NoFireOverlay").getValBoolean() && !Reflector.callBoolean(Reflector.ForgeEventFactory_renderFireOverlay, this.mc.player, partialTicks))
            {
