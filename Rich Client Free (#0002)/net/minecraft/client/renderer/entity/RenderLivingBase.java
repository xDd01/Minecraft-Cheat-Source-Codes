...
import me.rich.module.render.Chams;
...
public abstract class RenderLivingBase<T extends EntityLivingBase> extends Render<T>
{
    ...
    protected void renderModel(T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float ageInTicks,
		...
		if (flag || flag1) {
			...
			if (Main.moduleManager.getModule(Chams.class).isToggled() && entitylivingbaseIn instanceof EntityPlayer) {
                if (entitylivingbaseIn != Minecraft.getMinecraft().player) {
                	GlStateManager.pushMatrix();
    				RenderHelper.setColor(Main.getClientColor().getRGB());
    				GL11.glColor4f((float)((float)Main.getClientColor().getRed() / 255.0f), (float)((float)Main.getClientColor().getGreen() / 255.0f), (float)((float)Main.getClientColor().getBlue() / 255.0f), (float)((float)Main.settingsManager.getSettingByName(Main.moduleManager.getModule(Chams.class) , "Alpha").getValInt() / 255.0f));
