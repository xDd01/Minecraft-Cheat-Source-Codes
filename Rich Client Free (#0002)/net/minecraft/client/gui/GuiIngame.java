...
import me.rich.module.render.ScoreBoard;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
		...
		if (this.mc.gameSettings.thirdPersonView == 0 && itemstack.getItem() == Item.getItemFromBlock(Blocks.PUMPKIN)) {
			if(Main.moduleManager.getModule(NoRender.class).isToggled() && Main.settingsManager.getSettingByName(Main.moduleManager.getModule(NoRender.class), "NoPumpkinOverlay").getValBoolean()) {
		...
		if (this.mc.player.isRidingHorse()) {
			...
			if(Main.moduleManager.getModule(NoRender.class).isToggled() && Main.settingsManager.getSettingByName(Main.moduleManager.getModule(NoRender.class), "NoExpBar").getValBoolean()) {
		...
		if (scoreobjective1 != null) {
			if(!(Main.moduleManager.getModule(ScoreBoard.class).isToggled() && Main.settingsManager.getSettingByName(Main.moduleManager.getModule(ScoreBoard.class), "Delete").getValBoolean())) {
    ...
    private void renderAttackIndicator(float p_184045_1_, ScaledResolution p_184045_2_)
    {
        ...
        {
        	 if ((this.mc.playerController.isSpectator() && this.mc.pointedEntity == null) || Main.moduleManager.getModule(NoClip.class).isToggled())            {
            ...
            {
            	// TODO Crosshair
				EventCrosshair event = new EventCrosshair(p_184045_2_);
				event.call();
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        for (Score score1 : collection)
        {
            ...
            int k = (int) (j1 - j * this.getFontRenderer().FONT_HEIGHT + (Main.moduleManager.getModule(ScoreBoard.class).isToggled() ? Main.settingsManager.getSettingByName(Main.moduleManager.getModule(ScoreBoard.class), "PositionY").getValFloat() : 0));
            int l = scaledRes.getScaledWidth() - 3 + 2;
            drawRect(l1 - 2, k, l, k + this.getFontRenderer().FONT_HEIGHT, new Color(33, 33, 33, 225).getRGB());
            this.getFontRenderer().drawStringWithShadow(s1, l1, k, new Color(60, 61, 60).getRGB());
            this.getFontRenderer().drawStringWithShadow(s2, l - this.getFontRenderer().getStringWidth(s2), k, new Color(60, 61, 60).getRGB());

            if (j == collection.size())
            {
                String s3 = objective.getDisplayName();
                drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - 1, new Color(33, 33, 33, 245).getRGB());
                drawRect(l1 - 2, k - 1, l, k, new Color(33, 33, 33, 245).getRGB());
                me.rich.helpers.render.RenderHelper.drawNewRect(l1 - 2, k - 19f, l, k - 10, new Color(33, 33, 33, 255).getRGB());
                ...
                Fonts.sfui15.drawString("Scoreboard", l1 + 1, k - 16, -1);
            }
        }
    }
