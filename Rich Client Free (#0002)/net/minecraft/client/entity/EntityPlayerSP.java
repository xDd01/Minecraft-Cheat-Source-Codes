...
import me.rich.event.events.EventMoveRelative;
...
import me.rich.module.player.AntiCollision;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate()
    {
        ...
        if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ)))
        {
            ...
            else
            {
                ...
                eventPre.call();
                if (!Main.instance.moduleManager.getModule(FreeCam.class).isToggled()) {
            ...
            if (args[0].equalsIgnoreCase(".config")) {
                try {
                    if (args[1].equals("save")) {
                        Main.config.save();
                        Main.msg("config succesfully saved.", true);
                    }

                } catch (Exception ex) { 
                    Main.msg("error! use '.config save!'", true);
                }
            }
        }
        }
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        if (Main.moduleManager.getModule(AntiCollision.class).isToggled() || Main.moduleManager.getModule(NoClip.class).isToggled() || Main.settingsManager.getSettingByName("Blocks").getValBoolean()  ||  Main.instance.moduleManager.getModule(FreeCam.class).isToggled()) {
            return false;
        }
    ...
    public void onLivingUpdate()
    {
        ...
        {
        	this.movementInput.moveStrafe *= Main.moduleManager.getModule(NoSlowDown.class).isToggled() ? Main.settingsManager.getSettingByName("CustomSpeed").getValDouble() / 100 : 0.2F;
			this.movementInput.moveForward *= Main.moduleManager.getModule(NoSlowDown.class).isToggled() ? Main.settingsManager.getSettingByName("CustomSpeed").getValDouble() / 100 : 0.2F;
    ...
    {
        EventMoveRelative e = new EventMoveRelative(strafe,up,forward,friction, this.rotationYaw);
        e.call();
        if(e.isCancelled())
            return;
        float motionSize = e.strafe * e.strafe + e.up * e.up + e.forward * e.forward;

        if (motionSize >= 1.0E-4F)
        {
            motionSize = MathHelper.sqrt(motionSize);
            ...
            motionSize = e.friction / motionSize;
            e.strafe = e.strafe * motionSize;
            e.up = up * motionSize;
            e.forward = e.forward * motionSize;
            float x = MathHelper.sin(e.yaw * 0.017453292F);
            float z = MathHelper.cos(e.yaw * 0.017453292F);
            this.motionX += (e.strafe * z - e.forward * x);
            this.motionY += e.up;
            this.motionZ += (e.forward * z + e.strafe * x);
        }
    }
