...
public class InventoryHelper{
    ...
    public static int findHotbarPotion() {
        for (int o = 0; o < 9; ++o) {
            ...
            if (item == null || !InventoryHelper.isPotion(item)) continue;
            return o;
        }
        return -1;
    }
    ...
    public static int getUseablePotion() {
        for (int o = 9; o < 36; ++o) {
            ...
            ItemStack item = Minecraft.player.inventoryContainer.getSlot(o).getStack();
            if (!InventoryHelper.isPotion(item)) continue;
            return o;
        }
        return -1;
    }
    ...
    public static void usePotion() {
        ...
        if (item != null) {
            if (InventoryHelper.mc.playerController.processRightClick(Minecraft.player, InventoryHelper.mc.world, hand) == EnumActionResult.SUCCESS) {
                // empty if block
            }
        }
    }
    ...
    public static boolean hotbarHasPotion(Potion effect, boolean splash) {
        for (int index = 0; index <= 8; ++index) {
            ...
            if (stack == null || !InventoryHelper.isPotion(stack, effect, splash)) continue;
            return true;
        }
        return false;
    }
    ...
    public static void swap(int slot, int hotbarNum) {
        InventoryHelper.mc.playerController.windowClick(Minecraft.player.inventoryContainer.windowId, slot, 0, ClickType.PICKUP, Minecraft.player);
        InventoryHelper.mc.playerController.windowClick(Minecraft.player.inventoryContainer.windowId, hotbarNum, 0, ClickType.PICKUP, Minecraft.player);
        InventoryHelper.mc.playerController.windowClick(Minecraft.player.inventoryContainer.windowId, slot, 0, ClickType.PICKUP, Minecraft.player);
