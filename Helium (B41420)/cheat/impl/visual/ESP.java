	...
	public void onRenderOverlay(RenderOverlayEvent renderOverlayEvent) {
		...
		entityLoop: for (EntityLivingBase entity : collectEntities()) {
			...
			if (prop_elements.getValue().get("Name")) {
				...
				Fonts.verdana.drawCenteredString(entity.getName() + " " + Math.round(mc.thePlayer.getDistanceToEntity(entity)) + "m",
						(renderPosX1 + ((renderPosX2 - renderPosX1) / 2)) / scale, ((renderPosY1) / scale) - 9,
						prop_nameColor.getValue().getRGB());
				/*/Fonts.verdana3.drawCenteredStringWithShadow(MathUtils.round(getPlayer().getDistanceToEntity(entity), 2) + " m",
						(renderPosX1 + ((renderPosX2 - renderPosX1) / 2)) / scale, ((renderPosY1) / scale) - 18,
						prop_nameColor.getValue().getRGB());/*/
