...
public class Scaffold extends Cheat {
    ...
    BooleanProperty scfld = new BooleanProperty("Slow Down", "Slows down", null, false);
    ...
    private List<Block> invalidBlocks = Arrays.asList(
            ...
            Blocks.furnace, Blocks.stone_slab, Blocks.wooden_slab, Blocks.stone_slab2, Blocks.web);
    ...
    public Scaffold() {
        ...
        blacklistedBlocks = Arrays.asList(
                ...
                Blocks.sand, Blocks.web, Blocks.cactus, Blocks.dispenser, Blocks.noteblock, Blocks.dropper, Blocks.crafting_table, Blocks.web, Blocks.pumpkin, Blocks.sapling, Blocks.cobblestone_wall, Blocks.oak_fence);
    }

    //We do not need to override the cheat class ondisable/onenable for scaffold, made that mistake too many times
    public void onDisable() {
        mc.timer.timerSpeed = 1.0f;
        mc.gameSettings.keyBindSneak.pressed = false;
        getPlayer().movementInput.sneak = false;
        SpeedUtils.setPlayerSpeed(1.0);
    ...
    public void onEnable() {
        placedelay = 95;
        if (Helium.instance.cheatManager.isCheatEnabled("Speed")) {
            Helium.instance.cheatManager.getCheatRegistry().get("Speed").setState(false, false);
            NotificationManager.postWarning("dasds", "Disabled Speed to prevent flags.");
        }
        mc.timer.timerSpeed = 0.9f;
        SpeedUtils.setPlayerSpeed(0.8);
        //ClientBase.chat("§c<!> YOU WILL BE LAUNCHED ONCE YOU DISABLE SCAFFOLD. BE CAREFUL!");
    ...
    public void onPlayerJump(PlayerJumpEvent playerJumpEvent) {
        ...
        if (mc.thePlayer.isMoving()) {
            if (mc.thePlayer.isSprinting()) {
                ...
                warnthisnibba = warnthisnibba + 1;
                boolean bool = (warnthisnibba == 3);
                if (bool) {
                }
    ...
    private int getBlockCount() {
        ...
        mc.fontRendererObj.drawStringWithShadow(Integer.toString(this.getBlockCount()), (float)(sr.getScaledWidth() / 2 + 1 - this.mc.fontRendererObj.getStringWidth(Integer.toString(this.getBlockCount())) / 2), (float)(sr.getScaledHeight() / 2 + 24) - 40, this.getBlockColor(this.getBlockCount()));
        int blockCount = 0;
        for (int i = 0; i < 45; ++i) {
            if (this.mc.thePlayer.inventoryContainer.getSlot(i).getHasStack()) {
                ...
                if (is.getItem() instanceof ItemBlock) {
                    if (!this.invalidBlocks.contains(((ItemBlock)item).getBlock())) {
