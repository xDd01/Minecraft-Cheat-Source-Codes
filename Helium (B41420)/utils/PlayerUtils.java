...
public class PlayerUtils {
    ...
    public static boolean isInLiquid() {
        ...
        return mc.theWorld.getBlockState(new BlockPos(x, y + mc.thePlayer.getEyeHeight(), z)).getBlock() instanceof BlockLiquid;
    }

    public static void damage() {
        final double offset = 0.055D;
        for (int i = 0; i < (getMaxFallDist() / (offset - 0.005D)) + 1; i++) {
            mc.thePlayer.sendQueue.getNetworkManager().sendPacket(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY + offset, mc.thePlayer.posZ, false));
            mc.thePlayer.sendQueue.getNetworkManager().sendPacket(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY + 0.005D, mc.thePlayer.posZ, false));
        }
    }
    ...
    public static float[] getNeededRotations(EntityLivingBase entityIn) {
        ...
        double d2 = entityIn.posY + entityIn.getEyeHeight() - (mc.thePlayer.getEntityBoundingBox().minY + (mc.thePlayer.getEntityBoundingBox().maxY - mc.thePlayer.getEntityBoundingBox().minY));
    ...
    public static float[] getRotations(EntityLivingBase entityIn, float speed) {
        float yaw = incrementRotation(mc.thePlayer.rotationYaw,
                getNeededRotations(entityIn)[0],
                speed);
        float pitch = incrementRotation(mc.thePlayer.rotationPitch,
