...
public class ChunkUtils
{
    ...
    public static boolean hasEntities(final Chunk p_hasEntities_0_) {
        if (ChunkUtils.fieldHasEntities == null) {
            ...
            ChunkUtils.fieldHasEntities = fildFieldHasEntities(p_hasEntities_0_);
            if (ChunkUtils.fieldHasEntities == null) {
    ...
    private static Field fildFieldHasEntities(final Chunk p_fildFieldHasEntities_0_) {
        try {
            ...
            for (int i = 0; i < afield.length; ++i) {
                ...
                if (field.getType() == Boolean.TYPE) {
                    ...
                    list.add(field);
                    list2.add(field.get(p_fildFieldHasEntities_0_));
                }
            }
            ...
            for (final Object field3 : list) {
                list4.add(((Field)field3).get(p_fildFieldHasEntities_0_));
            }
            ...
            for (int j = 0; j < list.size(); ++j) {
                ...
                if (!obool && obool2) {
                    ...
                    field4.set(p_fildFieldHasEntities_0_, obool3);
                }
            }
            if (list5.size() == 1) {
