...
public final class UScript
{
    ...
    private static int[] getCodesFromLocale(final ULocale locale) {
    ...
    private static int[] findCodeFromLocale(final ULocale locale) {
        final int[] result = getCodesFromLocale(locale);
        if (result != null) {
            return result;
        }
        final ULocale likely = ULocale.addLikelySubtags(locale);
    ...
    public static final int[] getCode(final String nameOrAbbrOrLocale) {
        ...
        if (nameOrAbbrOrLocale.indexOf(95) < 0 && nameOrAbbrOrLocale.indexOf(45) < 0) {
            final int propNum = UCharacter.getPropertyValueEnumNoThrow(4106, nameOrAbbrOrLocale);
            if (propNum != -1) {
                return new int[] { propNum };
            }
            triedCode = true;
        }
        final int[] scripts = findCodeFromLocale(new ULocale(nameOrAbbrOrLocale));
        if (scripts != null) {
            return scripts;
        }
        if (!triedCode) {
            final int propNum2 = UCharacter.getPropertyValueEnumNoThrow(4106, nameOrAbbrOrLocale);
            if (propNum2 != -1) {
    ...
    public static final int getCodeFromName(final String nameOrAbbr) {
        final int propNum = UCharacter.getPropertyValueEnumNoThrow(4106, nameOrAbbr);
