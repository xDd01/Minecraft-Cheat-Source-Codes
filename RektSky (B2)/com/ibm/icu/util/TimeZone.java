...
public abstract class TimeZone implements Serializable, Cloneable, Freezable<TimeZone>
{
    ...
    private static TimeZone getTimeZone(final String id, final int type, final boolean frozen) {
        TimeZone result;
        if (type == 1) {
    ...
    static BasicTimeZone getFrozenICUTimeZone(final String id, final boolean trySystem) {
    ...
    public static TimeZone getDefault() {
        TimeZone tmpDefaultZone = TimeZone.defaultZone;
        if (tmpDefaultZone == null) {
            synchronized (java.util.TimeZone.class) {
                synchronized (TimeZone.class) {
                    tmpDefaultZone = TimeZone.defaultZone;
                    if (tmpDefaultZone == null) {
                        ...
                        TimeZone.defaultZone = tmpDefaultZone;
                    }
                }
            }
        }
    ...
    public static synchronized void setICUDefault(final TimeZone tz) {
        if (tz == null) {
            ...
            TimeZone.defaultZone = ((TimeZone)tz.clone()).freeze();
        }
    }
    ...
    public static String getWindowsID(String id) {
        ...
        final UResourceBundle top = UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt62b", "windowsZones", ICUResourceBundle.ICU_DATA_CLASS_LOADER);
        final UResourceBundle mapTimezones = top.get("mapTimezones");
        final UResourceBundleIterator resitr = mapTimezones.getIterator();
        while (resitr.hasNext()) {
            final UResourceBundle winzone = resitr.next();
            if (winzone.getType() != 2) {
                continue;
            }
            final UResourceBundleIterator rgitr = winzone.getIterator();
            while (rgitr.hasNext()) {
                final UResourceBundle regionalData = rgitr.next();
                if (regionalData.getType() != 0) {
                    continue;
                }
                final String[] split;
                final String[] tzids = split = regionalData.getString().split(" ");
                for (final String tzid : split) {
                    if (tzid.equals(id)) {
    ...
    public static String getIDForWindowsID(final String winid, final String region) {
        ...
        final UResourceBundle mapTimezones = top.get("mapTimezones");
        try {
            final UResourceBundle zones = mapTimezones.get(winid);
            if (region != null) {
                try {
                    id = zones.getString(region);
                    if (id != null) {
                        final int endIdx = id.indexOf(32);
                        if (endIdx > 0) {
    ...
    private static final class ConstantZone extends TimeZone
    {
        ...
        private ConstantZone(final int rawOffset, final String ID) {
            super(ID);
            this.isFrozen = false;
        ...
        @Override
        public TimeZone cloneAsThawed() {
            final ConstantZone tz = (ConstantZone)super.cloneAsThawed();
            tz.isFrozen = false;
            return tz;
        }
    }
