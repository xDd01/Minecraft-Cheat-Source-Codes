...
public final class ULocale implements Serializable, Comparable<ULocale>
{
        ...
        if (cmp == 0) {
            ...
            if (cmp == 0) {
                ...
                if (cmp == 0) {
                    ...
                    if (cmp == 0) {
                        final Iterator<String> thisKwdItr = this.getKeywords();
                        final Iterator<String> otherKwdItr = other.getKeywords();
                        if (thisKwdItr == null) {
                            ...
                            while (cmp == 0 && thisKwdItr.hasNext()) {
                                ...
                                final String thisKey = thisKwdItr.next();
                                final String otherKey = otherKwdItr.next();
                                cmp = thisKey.compareTo(otherKey);
                                if (cmp != 0) {
                                    continue;
                                }
                                final String thisVal = this.getKeywordValue(thisKey);
                                final String otherVal = other.getKeywordValue(otherKey);
                                if (thisVal == null) {
    ...
    public static String getRegionForSupplementalData(final ULocale locale, final boolean inferRegion) {
        ...
        if (region != null && region.length() == 6) {
            final String regionUpper = AsciiUtil.toUpperString(region);
        ...
        if (region.length() == 0 && inferRegion) {
            final ULocale maximized = addLikelySubtags(locale);
    ...
    public static String getName(final String localeID) {
        ...
        return ULocale.nameCache.getInstance(tmpLocaleID, null);
    }
    ...
    public boolean isRightToLeft() {
        ...
        if (script.length() == 0) {
            final String lang = this.getLanguage();
            if (lang.length() == 0) {
                return false;
            }
            final int langIndex = "root-en-es-pt-zh-ja-ko-de-fr-it-ar+he+fa+ru-nl-pl-th-tr-".indexOf(lang);
            if (langIndex >= 0) {
            ...
            final ULocale likely = addLikelySubtags(this);
            script = likely.getScript();
            if (script.length() == 0) {
                return false;
            }
        }
        final int scriptCode = UScript.getCodeFromName(script);
            ...
            for (final Character key : extKeys) {
                ...
                if (ext instanceof UnicodeLocaleExtension) {
                    ...
                    for (final String bcpKey : ukeys) {
                        ...
                        final String lkey = toLegacyKey(bcpKey);
                        final String ltype = toLegacyType(bcpKey, (bcpType.length() == 0) ? "yes" : bcpType);
    ...
    private BaseLocale base() {
        if (this.baseLocale == null) {
            ...
            String region;
            String language;
            String script = language = (region = (variant = ""));
            if (!this.equals(ULocale.ROOT)) {
                final LocaleIDParser lp = new LocaleIDParser(this.localeID);
            ...
            if (kwitr == null) {
                ...
                while (kwitr.hasNext()) {
                    ...
                    if (key.equals("attribute")) {
                        final String[] split;
                        final String[] uattributes = split = this.getKeywordValue(key).split("[-_]");
                        for (final String uattr : split) {
                            try {
                        ...
                        final String bcpKey = toUnicodeLocaleKey(key);
                        final String bcpType = toUnicodeLocaleType(key, this.getKeywordValue(key));
    ...
    static {
        ULocale.nameCache = new SoftCache<String, String, Void>() {
            @Override
            protected String createInstance(final String tmpLocaleID, final Void unused) {
    ...
    private static final class JDKLocaleHelper
    {
        ...
        public static boolean hasLocaleCategories() {
            return JDKLocaleHelper.hasLocaleCategories;
        }
        
        public static ULocale toULocale(final Locale loc) {
            return JDKLocaleHelper.hasScriptsAndUnicodeExtensions ? toULocale7(loc) : toULocale6(loc);
        }
        
        public static Locale toLocale(final ULocale uloc) {
            return JDKLocaleHelper.hasScriptsAndUnicodeExtensions ? toLocale7(uloc) : toLocale6(uloc);
        }
        ...
        public static boolean isOriginalDefaultLocale(final Locale loc) {
            if (JDKLocaleHelper.hasScriptsAndUnicodeExtensions) {
                String script = "";
                try {
                    script = (String)JDKLocaleHelper.mGetScript.invoke(loc, (Object[])null);
                }
        ...
        static {
            JDKLocaleHelper.hasScriptsAndUnicodeExtensions = false;
            JDKLocaleHelper.hasLocaleCategories = false;
            ...
            try {
                ...
                JDKLocaleHelper.hasScriptsAndUnicodeExtensions = true;
            }
            ...
            try {
                ...
                final Class<?>[] classes = declaredClasses = Locale.class.getDeclaredClasses();
                ...
                if (cCategory != null) {
                    ...
                    final?[] enumConstants2;
                    final Object[] enumConstants = (Object[])(enumConstants2 = cCategory.getEnumConstants());
                    for (final Object e : enumConstants2) {
                    ...
                    if (JDKLocaleHelper.eDISPLAY != null && JDKLocaleHelper.eFORMAT != null) {
                        JDKLocaleHelper.hasLocaleCategories = true;
                    }
                }
            }
