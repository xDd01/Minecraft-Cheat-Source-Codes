...
public class VTimeZone extends BasicTimeZone
{
    ...
    public static VTimeZone create(final String tzid) {
        final BasicTimeZone basicTimeZone = TimeZone.getFrozenICUTimeZone(tzid, true);
    ...
    private boolean parse() {
        ...
        final Iterator<TimeZoneRule> iterator2 = rules.iterator();
        while (iterator2.hasNext()) {
            final TimeZoneRule r = iterator2.next();
        ...
        if (!hasTransitions) {
            ...
            if (dstCount > 0) {
                if (finalDstRule == null) {
                    ...
                    writeZonePropsByDOW(w, true, dstName, dstFromOffset, dstToOffset, dstMonth, dstWeekInMonth, dstDayOfWeek, dstStartTime, dstUntilTime);
                    final Date nextStart = finalDstRule.getNextStart(dstUntilTime, dstFromOffset - dstFromDSTSavings, dstFromDSTSavings, false);
            ...
            if (stdCount > 0) {
                if (finalStdRule == null) {
                    ...
                    writeZonePropsByDOW(w, false, stdName, stdFromOffset, stdToOffset, stdMonth, stdWeekInMonth, stdDayOfWeek, stdStartTime, stdUntilTime);
                    final Date nextStart = finalStdRule.getNextStart(stdUntilTime, stdFromOffset - stdFromDSTSavings, stdFromDSTSavings, false);
                    assert nextStart != null;
                    if (nextStart != null) {
