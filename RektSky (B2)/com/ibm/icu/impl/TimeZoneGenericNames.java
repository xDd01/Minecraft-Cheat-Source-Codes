...
public class TimeZoneGenericNames implements Serializable, Freezable<TimeZoneGenericNames>
{
        ...
        if (name == null) {
            ...
            if (name == null) {
                ...
                synchronized (this) {
                    ...
                    if (tmp == null) {
                        final NameInfo info = new NameInfo(canonicalTzID, GenericNameType.LOCATION);
    ...
    private synchronized String formatPattern(final Pattern pat, final String... args) {
        ...
        if (this._patternFormatters[idx] == null) {
            String patText;
            try {
                final ICUResourceBundle bundle = (ICUResourceBundle)UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt62b/zone", this._locale);
    ...
    private String getPartialLocationName(final String tzID, final String mzID, final boolean isLong, final String mzDisplayName) {
        ...
        synchronized (this) {
            ...
            if (tmp == null) {
                final NameInfo info = new NameInfo(tzID.intern(), isLong ? GenericNameType.LONG : GenericNameType.SHORT);
                this._gnamesTrie.put(name, info);
            }
            else {
                name = tmp;
    ...
    private GenericMatchInfo createGenericMatchInfo(final TimeZoneNames.MatchInfo matchInfo) {
        ...
        assert tzID != null;
        final GenericMatchInfo gmatch = new GenericMatchInfo(nameType, tzID, matchInfo.matchLength(), timeType);
        return gmatch;
    }
    ...
    private static class NameInfo
    {
        ...
        NameInfo(final String tzID, final GenericNameType type) {
            this.tzID = tzID;
            this.type = type;
        }
    }
    ...
    public static class GenericMatchInfo
    {
        ...
        private GenericMatchInfo(final GenericNameType nameType, final String tzID, final int matchLength) {
            this(nameType, tzID, matchLength, TimeZoneFormat.TimeType.UNKNOWN);
        }
        
        private GenericMatchInfo(final GenericNameType nameType, final String tzID, final int matchLength, final TimeZoneFormat.TimeType timeType) {
    ...
    private static class GenericNameSearchHandler implements TextTrieMap.ResultHandler<NameInfo>
    {
        ...
        public boolean handlePrefixMatch(final int matchLength, final Iterator<NameInfo> values) {
            while (values.hasNext()) {
                ...
                final GenericMatchInfo matchInfo = new GenericMatchInfo(info.type, info.tzID, matchLength);
