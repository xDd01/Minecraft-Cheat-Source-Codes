...
public abstract class Trie2 implements Iterable<Range>
{
    ...
    public static Trie2 createFromSerialized(final ByteBuffer bytes) throws IOException {
        final ByteOrder outerByteOrder = bytes.order();
        try {
            final UTrie2Header header = new UTrie2Header();
            switch (header.signature = bytes.getInt()) {
                case 1416784178: {
                    break;
                }
                case 845771348: {
                    final boolean isBigEndian = outerByteOrder == ByteOrder.BIG_ENDIAN;
