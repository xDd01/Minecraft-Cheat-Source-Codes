...
public class TextTrieMap<V>
{
    ...
    public Iterator<V> get(final CharSequence text, final int start, final Output output) {
    ...
    private void find(final CharSequence text, final int offset, final ResultHandler<V> handler, final Output output) {
        final CharIterator chitr = new CharIterator(text, offset, this._ignoreCase);
        this.find(this._root, chitr, handler, output);
    }
    
    private synchronized void find(final Node node, final CharIterator chitr, final ResultHandler<V> handler, final Output output) {
        final Iterator<V> values = node.values();
        if (values != null && !handler.handlePrefixMatch(chitr.processedLength(), values)) {
            return;
        }
        final Node nextMatch = node.findMatch(chitr, output);
        if (nextMatch != null) {
            this.find(nextMatch, chitr, handler, output);
        }
    }
    
    public void putLeadCodePoints(final UnicodeSet output) {
    ...
    private class Node
    {
        ...
        public Node findMatch(final CharIterator chitr, final Output output) {
        ...
        public void putLeadCodePoints(final UnicodeSet output) {
            ...
            for (final Node child : this._children) {
                ...
                if (!UCharacter.isHighSurrogate(c0)) {
                    ...
                    for (final Node grandchild : child._children) {
                        final char c2 = grandchild._text[0];
                        final int cp = Character.toCodePoint(c0, c2);
                        output.add(cp);
                    }
                }
            }
        ...
        private boolean matchFollowing(final CharIterator chitr, final Output output) {
