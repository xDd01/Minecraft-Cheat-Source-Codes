...
public class Grego
{
    ...
    public static String timeToString(final long time) {
        final int[] fields = timeToFields(time, null);
        int millis = fields[5];
        final int hour = millis / 3600000;
        millis %= 3600000;
        final int min = millis / 60000;
        millis %= 60000;
        final int sec = millis / 1000;
        millis %= 1000;
        return String.format(null, "%04d-%02d-%02dT%02d:%02d:%02d.%03dZ", fields[0], fields[1] + 1, fields[2], hour, min, sec, millis);
    }
