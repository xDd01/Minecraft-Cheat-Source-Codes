...
public final class UPropertyAliases
{
    ...
    private void load(final ByteBuffer bytes) throws IOException {
        ICUBinary.readHeader(bytes, 1886282093, UPropertyAliases.IS_ACCEPTABLE);
        ...
        bytes.get(this.bytesTries = new byte[numBytes]);
        offset = nextOffset;
    ...
    private UPropertyAliases() throws IOException {
        final ByteBuffer bytes = ICUBinary.getRequiredData("pnames.icu");
        this.load(bytes);
    }
    ...
    public int getPropertyValueEnumNoThrow(final int property, final CharSequence alias) {
        int valueMapIndex = this.findProperty(property);
        if (valueMapIndex == 0) {
            return -1;
        }
