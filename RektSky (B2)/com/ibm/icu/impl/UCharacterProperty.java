...
public final class UCharacterProperty
{
    ...
    public boolean hasBinaryProperty(final int c, final int which) {
        return which >= 0 && 65 > which && this.binProps[which].contains(c);
    }
    ...
    public double getUnicodeNumericValue(final int c) {
        ...
        if (ntv < 828) {
            final int frac20 = ntv - 804;
            final int numerator2 = 2 * (frac20 & 0x3) + 1;
            final int denominator2 = 20 << (frac20 >> 2);
            return numerator2 / (double)denominator2;
        }
        return -1.23456789E8;
    }
    ...
    private UCharacterProperty() throws IOException {
        ...
        final ByteBuffer bytes = ICUBinary.getRequiredData("uprops.icu");
        this.m_unicodeVersion_ = ICUBinary.readHeaderAndDataVersion(bytes, 1431335535, new IsAcceptable());
        final int propertyOffset = bytes.getInt();
        bytes.getInt();
        bytes.getInt();
        final int additionalOffset = bytes.getInt();
        final int additionalVectorsOffset = bytes.getInt();
        this.m_additionalColumnsCount_ = bytes.getInt();
        final int scriptExtensionsOffset = bytes.getInt();
        final int reservedOffset7 = bytes.getInt();
        bytes.getInt();
        bytes.getInt();
