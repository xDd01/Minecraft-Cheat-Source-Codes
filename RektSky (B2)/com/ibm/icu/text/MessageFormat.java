...
public class MessageFormat extends UFormat
{
    ...
    private void format(final int msgStart, final PluralSelectorContext pluralNumber, final Object[] args, final Map<String, Object> argsMap, final AppendableWrapper dest, FieldPosition fp) {
        final String msgString = this.msgPattern.getPatternString();
        int prevIndex = this.msgPattern.getPart(msgStart).getLimit();
        int i = msgStart + 1;
        while (true) {
            ...
            if (type == MessagePattern.Part.Type.REPLACE_NUMBER) {
                ...
                boolean noArg = false;
                Object argId = null;
                final String argName = this.msgPattern.getSubstring(part);
                Object arg;
                if (args != null) {
                ...
                if (noArg) {
                    ...
                    final Number number2 = (Number)arg;
                    final double offset = this.msgPattern.getPluralOffset(i);
                    final PluralSelectorContext context = new PluralSelectorContext(i, argName, number2, offset);
                    final int subMsgStart2 = PluralFormat.findSubMessage(this.msgPattern, i, selector, context, number2.doubleValue());
                    this.formatComplexSubMessage(subMsgStart2, context, args, argsMap, dest);
                }
                else {
                    ...
                    this.formatComplexSubMessage(subMsgStart3, null, args, argsMap, dest);
                }
    ...
    private void formatComplexSubMessage(final int msgStart, final PluralSelectorContext pluralNumber, final Object[] args, final Map<String, Object> argsMap, final AppendableWrapper dest) {
    ...
    private int findFirstPluralNumberArg(final int msgStart, final String argName) {
        int i = msgStart + 1;
        while (true) {
            MessagePattern.Part part = this.msgPattern.getPart(i);
            final MessagePattern.Part.Type type = part.getType();
            ...
            switch (subformatType) {
                case 0: {
                    switch (findKeyword(style, MessageFormat.modifierList)) {
                        ...
                        default: {
                            int i;
                            for (i = 0; PatternProps.isWhiteSpace(style.charAt(i)); ++i) {}
    ...
    private static final class PluralSelectorProvider implements PluralFormat.PluralSelector
    {
        ...
        public String select(final Object ctx, final double number) {
            ...
            final PluralSelectorContext context = (PluralSelectorContext)ctx;
            final int otherIndex = this.msgFormat.findOtherSubMessage(context.startIndex);
            ...
            if (context.numberArgIndex > 0 && this.msgFormat.cachedFormatters != null) {
                context.formatter = this.msgFormat.cachedFormatters.get(context.numberArgIndex);
            }
            ...
            if (context.formatter instanceof DecimalFormat) {
                final PluralRules.IFixedDecimal dec = ((DecimalFormat)context.formatter).getFixedDecimal(number);
