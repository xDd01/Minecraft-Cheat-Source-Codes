...
public class TimeZoneFormat extends UFormat implements Freezable<TimeZoneFormat>, Serializable
{
    ...
    public TimeZone parse(final Style style, final String text, final ParsePosition pos, final EnumSet<ParseOption> options, Output<TimeType> timeType) {
        ...
        final boolean parseTZDBAbbrev = (options == null) ? this.getDefaultParseOptions().contains(ParseOption.TZ_DATABASE_ABBREVIATIONS) : options.contains(ParseOption.TZ_DATABASE_ABBREVIATIONS);
        switch (style) {
            case LOCALIZED_GMT: {
            ...
            case ISO_EXTENDED_LOCAL_FULL: {
                ...
                if (tmpPos.getErrorIndex() == -1 && hasDigitOffset2.value) {
                    pos.setIndex(tmpPos.getIndex());
                    return this.getTimeZoneForOffset(offset);
                }
                break;
            ...
            case SPECIFIC_SHORT: {
                ...
                final Collection<TimeZoneNames.MatchInfo> tzdbNameMatches = this.getTZDBTimeZoneNames().find(text, startIdx, nameTypes);
                if (tzdbNameMatches != null) {
                    TimeZoneNames.MatchInfo tzdbNameMatch = null;
                    for (final TimeZoneNames.MatchInfo match2 : tzdbNameMatches) {
        ...
        if (parsedPos > startIdx) {
            ...
            if (parsedPos < maxPos && ((evaluated & 0x80) == 0x0 || (evaluated & 0x100) == 0x0)) {
                tmpPos.setIndex(startIdx);
                tmpPos.setErrorIndex(-1);
                final Output<Boolean> hasDigitOffset3 = new Output<Boolean>(false);
                final int offset = parseOffsetISO8601(text, tmpPos, false, hasDigitOffset3);
                if (tmpPos.getErrorIndex() == -1) {
                    if (tmpPos.getIndex() == maxPos || hasDigitOffset3.value) {
                        pos.setIndex(tmpPos.getIndex());
                        return this.getTimeZoneForOffset(offset);
                    }
            ...
            if (parsedPos < maxPos && (evaluated & Style.LOCALIZED_GMT.flag) == 0x0) {
                ...
                tmpPos.setErrorIndex(-1);
                final Output<Boolean> hasDigitOffset3 = new Output<Boolean>(false);
                final int offset = this.parseOffsetLocalizedGMT(text, tmpPos, false, hasDigitOffset3);
                if (tmpPos.getErrorIndex() == -1) {
                    if (tmpPos.getIndex() == maxPos || hasDigitOffset3.value) {
                        pos.setIndex(tmpPos.getIndex());
                        return this.getTimeZoneForOffset(offset);
            ...
            if (parsedPos < maxPos && (evaluated & Style.LOCALIZED_GMT_SHORT.flag) == 0x0) {
                ...
                tmpPos.setErrorIndex(-1);
                final Output<Boolean> hasDigitOffset3 = new Output<Boolean>(false);
                final int offset = this.parseOffsetLocalizedGMT(text, tmpPos, true, hasDigitOffset3);
                if (tmpPos.getErrorIndex() == -1) {
                    if (tmpPos.getIndex() == maxPos || hasDigitOffset3.value) {
                        pos.setIndex(tmpPos.getIndex());
                        return this.getTimeZoneForOffset(offset);
            ...
            if (parseAllStyles) {
                if (parsedPos < maxPos) {
                    ...
                    if (specificMatches2 != null) {
                        for (final TimeZoneNames.MatchInfo match3 : specificMatches2) {
                ...
                if (parseTZDBAbbrev && parsedPos < maxPos && (evaluated & Style.SPECIFIC_SHORT.flag) == 0x0) {
                    final Collection<TimeZoneNames.MatchInfo> tzdbNameMatches2 = this.getTZDBTimeZoneNames().find(text, startIdx, TimeZoneFormat.ALL_SIMPLE_NAME_TYPES);
                    TimeZoneNames.MatchInfo tzdbNameMatch2 = null;
                    int matchPos = -1;
                    if (tzdbNameMatches2 != null) {
                        for (final TimeZoneNames.MatchInfo match3 : tzdbNameMatches2) {
                            if (startIdx + match3.matchLength() > matchPos) {
                                tzdbNameMatch2 = match3;
                                matchPos = startIdx + match3.matchLength();
                            }
                        }
                        if (parsedPos < matchPos) {
                            parsedPos = matchPos;
                            parsedID = this.getTimeZoneID(tzdbNameMatch2.tzID(), tzdbNameMatch2.mzID());
                            parsedTimeType = this.getTimeType(tzdbNameMatch2.nameType());
                            parsedOffset = Integer.MAX_VALUE;
                        }
                    }
                }
        ...
        for (int i = 0; i < patternItems.length; ++i) {
            if (patternItems[i] instanceof String) {
                ...
                if (i == 0 && idx < text.length() && !PatternProps.isWhiteSpace(text.codePointAt(idx))) {
                    while (len > 0) {
                        final int cp = patStr.codePointAt(patIdx);
    ...
    private static String parseZoneID(final String text, final ParsePosition pos) {
        ...
        if (TimeZoneFormat.ZONE_ID_TRIE == null) {
            synchronized (TimeZoneFormat.class) {
                if (TimeZoneFormat.ZONE_ID_TRIE == null) {
                    ...
                    final String[] ids = availableIDs = TimeZone.getAvailableIDs();
        ...
        final Iterator<String> itr = TimeZoneFormat.ZONE_ID_TRIE.get(text, pos.getIndex(), trieOutput);
        if (itr != null) {
            resolvedID = itr.next();
    ...
    private static String parseShortZoneID(final String text, final ParsePosition pos) {
        ...
        final TextTrieMap.Output trieOutput = new TextTrieMap.Output();
        final Iterator<String> itr = TimeZoneFormat.SHORT_ZONE_ID_TRIE.get(text, pos.getIndex(), trieOutput);
        if (itr != null) {
            resolvedID = itr.next();
            pos.setIndex(pos.getIndex() + trieOutput.matchLength);
        }
