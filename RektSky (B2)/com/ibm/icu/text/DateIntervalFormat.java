...
public class DateIntervalFormat extends UFormat
{
    ...
    public final synchronized StringBuffer format(final Calendar fromCalendar, final Calendar toCalendar, final StringBuffer appendTo, final FieldPosition pos) {
        ...
        final boolean fromToOnSameDay = field == 9 || field == 10 || field == 12 || field == 13;
        final DateIntervalInfo.PatternInfo intervalPattern = this.fIntervalPatterns.get(DateIntervalInfo.CALENDAR_FIELD_TO_PATTERN_LETTER[field]);
        if (intervalPattern == null) {
            ...
            if (intervalPattern.getSecondPart() != null) {
                ...
                final FieldPosition otherPos = new FieldPosition(pos.getField());
    ...
    private void adjustPosition(final String combiningPattern, final String pat0, final FieldPosition pos0, final String pat1, final FieldPosition pos1, final FieldPosition posResult) {
        int index0 = combiningPattern.indexOf("{0}");
        int index2 = combiningPattern.indexOf("{1}");
        if (index0 < 0 || index2 < 0) {
            return;
        }
        final int placeholderLen = 3;
        if (index0 < index2) {
            if (pos0.getEndIndex() > 0) {
                ...
                index2 += pat0.length() - placeholderLen;
                posResult.setBeginIndex(pos1.getBeginIndex() + index2);
                posResult.setEndIndex(pos1.getEndIndex() + index2);
            }
        }
        else if (pos1.getEndIndex() > 0) {
            posResult.setBeginIndex(pos1.getBeginIndex() + index2);
    ...
    private final StringBuffer fallbackFormat(final Calendar fromCalendar, final Calendar toCalendar, final boolean fromToOnSameDay, final StringBuffer appendTo, final FieldPosition pos) {
        String fullPattern = null;
        final boolean formatDatePlusTimeRange = fromToOnSameDay && this.fDatePattern != null && this.fTimePattern != null;
    ...
    private final StringBuffer fallbackFormat(final Calendar fromCalendar, final Calendar toCalendar, final boolean fromToOnSameDay, final StringBuffer appendTo, final FieldPosition pos, final String fullPattern) {
        final String originalPattern = this.fDateFormat.toPattern();
        this.fDateFormat.applyPattern(fullPattern);
    ...
    public void setTimeZone(final TimeZone zone) {
        final TimeZone zoneToSet = (TimeZone)zone.clone();
        if (this.fDateFormat != null) {
    ...
    private Map<String, DateIntervalInfo.PatternInfo> initializeIntervalPattern(final String fullPattern, final ULocale locale) {
        ...
        final boolean found = this.genSeparateDateTimePtn(normalizedDateSkeleton, normalizedTimeSkeleton, intervalPatterns, dtpng);
        if (!found) {
            if (time.length() != 0 && date.length() == 0) {
    ...
    private String getConcatenationPattern(final ULocale locale) {
        final ICUResourceBundle rb = (ICUResourceBundle)UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt62b", locale);
        final ICUResourceBundle dtPatternsRb = rb.getWithFallback("calendar/gregorian/DateTimePatterns");
        final ICUResourceBundle concatenationPatternRb = (ICUResourceBundle)dtPatternsRb.get(8);
    ...
    private boolean genSeparateDateTimePtn(final String dateSkeleton, final String timeSkeleton, final Map<String, DateIntervalInfo.PatternInfo> intervalPatterns, final DateTimePatternGenerator dtpng) {
        String skeleton;
    ...
    private void concatSingleDate2TimeInterval(final String dtfmt, final String datePattern, final int field, final Map<String, DateIntervalInfo.PatternInfo> intervalPatterns) {
        ...
        if (timeItvPtnInfo != null) {
            ...
            final String pattern = SimpleFormatterImpl.formatRawPattern(dtfmt, 2, 2, timeIntervalPattern, datePattern);
