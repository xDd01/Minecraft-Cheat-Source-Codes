...
class RBBIRuleScanner
{
    ...
    boolean doParseActions(final int action) {
        ...
        switch (action) {
            ...
            case 4: {
                ...
                final int destRules = this.fReverseRule ? 3 : this.fRB.fDefaultTree;
                if (this.fRB.fTreeRoots[destRules] != null) {
                    ...
                    orNode2.fRightChild = thisRule;
                    thisRule.fParent = orNode2;
            ...
            case 15: {
                ...
                if (opt.equals("unquoted_literals")) {
                    this.fRuleSets[3].applyPattern(RBBIRuleScanner.gRuleSet_rule_char_pattern);
                    break;
                }
                this.error(66061);
                break;
    ...
    static String stripRules(final String rules) {
        ...
        for (int idx = 0; idx < rulesLength; idx = rules.offsetByCodePoints(idx, 1)) {
            final int cp = rules.codePointAt(idx);
            final boolean whiteSpace = UCharacter.hasBinaryProperty(cp, 43);
    ...
    void nextChar(final RBBIRuleChar c) {
        ...
        if (this.fQuoteMode) {
            ...
            if (c.fChar == 35) {
                final int commentStart = this.fScanIndex;
                do {
                    c.fChar = this.nextCharLL();
