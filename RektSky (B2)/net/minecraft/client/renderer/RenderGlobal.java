...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener
{
                ...
                if (!classinheritancemultimap.isEmpty()) {
                    for (final Entity entity4 : classinheritancemultimap) {
                        if (!flag || Reflector.callBoolean(entity4, Reflector.ForgeEntity_shouldRenderInPass, i)) {
                            ...
                            if (flag6 || !(entity4 instanceof EntityWitherSkull)) {
                                continue;
                            }
                ...
                if (flag2) {
                    ...
                    if (tileentity3 != null && Reflector.callBoolean(tileentity3, Reflector.ForgeTileEntity_shouldRenderInPass, i) && Reflector.callBoolean(tileentity3, Reflector.ForgeTileEntity_canRenderBreaking, new Object[0])) {
                        ...
                        if (axisalignedbb3 != null) {
                            flag8 = camera.isBoundingBoxInFrustum(axisalignedbb3);
                        }
                    }
                }
                else {
    ...
    public void setupTerrain(final Entity viewEntity, final double partialTicks, ICamera camera, final int frameCount, final boolean playerSpectator) {
        ...
        if (!flag && this.displayListEntitiesDirty) {
            ...
            while (!deque.isEmpty()) {
                ...
                for (final EnumFacing enumfacing3 : aenumfacing) {
                    if ((!flag2 || !renderglobal$containerlocalrenderinformation4.setFacing.contains(enumfacing3.getOpposite())) && (!flag2 || enumfacing2 == null || renderchunk3.getCompiledChunk().isVisible(enumfacing2.getOpposite(), enumfacing3))) {
                        final RenderChunk renderchunk4 = this.func_181562_a(blockpos1, renderchunk3, enumfacing3);
                        if (renderchunk4 != null && renderchunk4.setFrameIndex(frameCount) && camera.isBoundingBoxInFrustum(renderchunk4.boundingBox)) {
                            final ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation5 = new ContainerLocalRenderInformation(renderchunk4, enumfacing3, renderglobal$containerlocalrenderinformation4.counter + 1, null);
        ...
        while (iterator.hasNext()) {
            final ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation6 = iterator.next();
    ...
    public void renderSky(final float partialTicks, final int pass) {
        ...
        if (this.mc.theWorld.provider.getDimensionId() == 1) {
            ...
            float f15 = (float)vec3.yCoord;
            float f16 = (float)vec3.zCoord;
            if (pass != 2) {
                final float f17 = (f14 * 30.0f + f15 * 59.0f + f16 * 11.0f) / 100.0f;
                final float f18 = (f14 * 30.0f + f15 * 70.0f) / 100.0f;
                final float f19 = (f14 * 30.0f + f16 * 70.0f) / 100.0f;
                f14 = f17;
                f15 = f18;
                f16 = f19;
            }
            GlStateManager.color(f14, f15, f16);
            final Tessellator tessellator = Tessellator.getInstance();
            final WorldRenderer worldrenderer = tessellator.getWorldRenderer();
            GlStateManager.depthMask(false);
            GlStateManager.enableFog();
            ...
            if (afloat != null && Config.isSunMoonEnabled()) {
                ...
                float f22 = afloat[2];
                if (pass != 2) {
                    final float f23 = (f20 * 30.0f + f21 * 59.0f + f22 * 11.0f) / 100.0f;
                    final float f24 = (f20 * 30.0f + f21 * 70.0f) / 100.0f;
                    final float f25 = (f20 * 30.0f + f22 * 70.0f) / 100.0f;
                    f20 = f23;
                    f21 = f24;
                    f22 = f25;
                }
                worldrenderer.begin(6, DefaultVertexFormats.POSITION_COLOR);
                worldrenderer.pos(0.0, 100.0, 0.0).color(f20, f21, f22, afloat[3]).endVertex();
                final boolean flag = true;
                for (int i = 0; i <= 16; ++i) {
                    final float f26 = i * 3.1415927f * 2.0f / 16.0f;
                    final float f27 = MathHelper.sin(f26);
                    final float f28 = MathHelper.cos(f26);
                    worldrenderer.pos(f27 * 120.0f, f28 * 120.0f, -f28 * 40.0f * afloat[3]).color(afloat[0], afloat[1], afloat[2], 0.0f).endVertex();
                }
                tessellator.draw();
                GlStateManager.popMatrix();
            ...
            GlStateManager.pushMatrix();
            final float f29 = 1.0f - this.theWorld.getRainStrength(partialTicks);
            GlStateManager.color(1.0f, 1.0f, 1.0f, f29);
            GlStateManager.rotate(-90.0f, 0.0f, 1.0f, 0.0f);
            CustomSky.renderSky(this.theWorld, this.renderEngine, this.theWorld.getCelestialAngle(partialTicks), f29);
            ...
            if (Config.isSunMoonEnabled()) {
                ...
                worldrenderer.pos(-f30, 100.0, -f30).tex(0.0, 0.0).endVertex();
                worldrenderer.pos(f30, 100.0, -f30).tex(1.0, 0.0).endVertex();
                worldrenderer.pos(f30, 100.0, f30).tex(1.0, 1.0).endVertex();
                worldrenderer.pos(-f30, 100.0, f30).tex(0.0, 1.0).endVertex();
                tessellator.draw();
                f30 = 20.0f;
                ...
                final int j = l % 4;
                final int k = l / 4 % 2;
                final float f31 = (j + 0) / 4.0f;
                final float f32 = (k + 0) / 2.0f;
                final float f33 = (j + 1) / 4.0f;
                final float f34 = (k + 1) / 2.0f;
                worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
                worldrenderer.pos(-f30, -100.0, f30).tex(f33, f34).endVertex();
                worldrenderer.pos(f30, -100.0, f30).tex(f31, f34).endVertex();
                worldrenderer.pos(f30, -100.0, -f30).tex(f31, f32).endVertex();
                worldrenderer.pos(-f30, -100.0, -f30).tex(f33, f32).endVertex();
                tessellator.draw();
            }
            GlStateManager.disableTexture2D();
            final float f35 = this.theWorld.getStarBrightness(partialTicks) * f29;
            if (f35 > 0.0f && Config.isStarsEnabled() && !CustomSky.hasSkyLayers(this.theWorld)) {
                GlStateManager.color(f35, f35, f35, f35);
                if (this.vboEnabled) {
            ...
            if (d0 < 0.0) {
                ...
                GlStateManager.popMatrix();
                final float f36 = 1.0f;
                final float f37 = -(float)(d0 + 65.0);
                final float f38 = -1.0f;
                worldrenderer.begin(7, DefaultVertexFormats.POSITION_COLOR);
                worldrenderer.pos(-1.0, f37, 1.0).color(0, 0, 0, 255).endVertex();
                worldrenderer.pos(1.0, f37, 1.0).color(0, 0, 0, 255).endVertex();
            ...
            if (this.theWorld.provider.isSkyColored()) {
                GlStateManager.color(f14 * 0.2f + 0.04f, f15 * 0.2f + 0.04f, f16 * 0.6f + 0.1f);
            }
            else {
                GlStateManager.color(f14, f15, f16);
            }
    ...
    class ContainerLocalRenderInformation
    {
        ...
        ContainerLocalRenderInformation(final RenderGlobal this$0, final RenderChunk p_i4_2_, final EnumFacing p_i4_3_, final int p_i4_4_, final Object p_i4_5_) {
            this(this$0, p_i4_2_, p_i4_3_, p_i4_4_);
        }
    }
}
