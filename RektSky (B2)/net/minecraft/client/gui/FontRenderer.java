...
public class FontRenderer implements IResourceManagerReloadListener
{
    ...
    public FontRenderer(final GameSettings gameSettingsIn, final ResourceLocation location, final TextureManager textureManagerIn, final boolean unicode) {
        ...
        textureManagerIn.bindTexture(this.locationFontTexture = getHdFontLocation(this.locationFontTextureBase));
        for (int i = 0; i < 32; ++i) {
    ...
    private void renderStringAtPos(final String p_78255_1_, final boolean p_78255_2_) {
        for (int i = 0; i < p_78255_1_.length(); ++i) {
            ...
            if (c0 == '§' && i + 1 < p_78255_1_.length()) {
                ...
                final float f1 = this.unicodeFlag ? 0.5f : (1.0f / this.scaleFactor);
    ...
    private void readCustomCharWidths() {
        final String s = this.locationFontTexture.getResourcePath();
        final String s2 = ".png";
        if (s.endsWith(s2)) {
            final String s3 = s.substring(0, s.length() - s2.length()) + ".properties";
            try {
                final ResourceLocation resourcelocation = new ResourceLocation(this.locationFontTexture.getResourceDomain(), s3);
                ...
                for (final Object s4 : ((Hashtable<Object, V>)properties).keySet()) {
                    final String s5 = (String)s4;
                    final String s6 = "width.";
                    if (s5.startsWith(s6)) {
                        final String s7 = s5.substring(s6.length());
                        final int i = Config.parseInt(s7, -1);
                        if (i < 0 || i >= this.charWidth.length) {
                            continue;
                        }
                        final String s8 = properties.getProperty(s5);
                        final float f = Config.parseFloat(s8, -1.0f);
                        if (f < 0.0f) {
                            continue;
                        }
                        this.charWidth[i] = f;
    ...
    private static ResourceLocation getHdFontLocation(final ResourceLocation p_getHdFontLocation_0_) {
