...
public class BlockVine extends Block
{
    ...
    private boolean recheckGrownSides(final World worldIn, final BlockPos pos, IBlockState state) {
        ...
        for (final Object enumfacing0 : EnumFacing.Plane.HORIZONTAL) {
            ...
            final PropertyBool propertybool = getPropertyFor(enumfacing2);
            if (state.getValue((IProperty<Boolean>)propertybool) && !this.canPlaceOn(worldIn.getBlockState(pos.offset(enumfacing2)).getBlock())) {
            ...
            if (enumfacing1 == EnumFacing.UP && pos.getY() < 255 && worldIn.isAirBlock(blockpos1)) {
                if (!flag) {
                    ...
                    for (final Object enumfacing2 : EnumFacing.Plane.HORIZONTAL) {
                        ...
                        if (rand.nextBoolean() || !this.canPlaceOn(worldIn.getBlockState(blockpos1.offset(enumfacing3)).getBlock())) {
                            iblockstate2 = iblockstate2.withProperty((IProperty<Comparable>)getPropertyFor(enumfacing3), false);
                        }
                    }
                ...
                if (!flag) {
                    ...
                    if (block1.blockMaterial == Material.air) {
                        final EnumFacing enumfacing4 = enumfacing1.rotateY();
                        final EnumFacing enumfacing5 = enumfacing1.rotateYCCW();
                        final boolean flag2 = state.getValue((IProperty<Boolean>)getPropertyFor(enumfacing4));
                        final boolean flag3 = state.getValue((IProperty<Boolean>)getPropertyFor(enumfacing5));
                        final BlockPos blockpos3 = blockpos2.offset(enumfacing4);
                        final BlockPos blockpos4 = blockpos2.offset(enumfacing5);
                        if (flag2 && this.canPlaceOn(worldIn.getBlockState(blockpos3).getBlock())) {
                            ...
                            worldIn.setBlockState(blockpos2, this.getDefaultState().withProperty((IProperty<Comparable>)getPropertyFor(enumfacing5), true), 2);
                        }
                        else if (flag2 && worldIn.isAirBlock(blockpos3) && this.canPlaceOn(worldIn.getBlockState(pos.offset(enumfacing4)).getBlock())) {
                        ...
                        else if (flag3 && worldIn.isAirBlock(blockpos4) && this.canPlaceOn(worldIn.getBlockState(pos.offset(enumfacing5)).getBlock())) {
                ...
                if (block2.blockMaterial == Material.air) {
                    ...
                    for (final Object enumfacing6 : EnumFacing.Plane.HORIZONTAL) {
                        final EnumFacing enumfacing7 = (EnumFacing)enumfacing6;
                        if (rand.nextBoolean()) {
                            iblockstate4 = iblockstate4.withProperty((IProperty<Comparable>)getPropertyFor(enumfacing7), false);
                        }
                    }
                    ...
                    for (final Object enumfacing8 : EnumFacing.Plane.HORIZONTAL) {
                        final EnumFacing enumfacing9 = (EnumFacing)enumfacing8;
                        final PropertyBool propertybool = getPropertyFor(enumfacing9);
