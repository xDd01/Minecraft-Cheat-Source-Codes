...
{
    private final Map<LinuxAxisDescriptor, LinuxComponent> component_map;
    ...
    public LinuxEventDevice(final String filename) throws IOException {
        this.component_map = new HashMap<LinuxAxisDescriptor, LinuxComponent>();
        this.key_states = new byte[96];
    ...
    private static final int countComponents(final List<LinuxEventComponent> components, final Class<?> id_type, final boolean relative) {
        int count = 0;
    ...
    private final Controller.Type guessType() throws IOException {
        final List<LinuxEventComponent> components = this.getComponents();
    ...
    private final Rumbler[] enumerateRumblers() {
        final List<Rumbler> rumblers = new ArrayList<Rumbler>();
        try {
            ...
            ControllerEnvironment.log("Failed to enumerate rumblers: " + e.getMessage());
        }
        return rumblers.toArray(new Rumbler[0]);
    }
    ...
    private final void addKeys(final List<LinuxEventComponent> components) throws IOException {
    ...
    private final void addAbsoluteAxes(final List<LinuxEventComponent> components) throws IOException {
    ...
    private final void addRelativeAxes(final List<LinuxEventComponent> components) throws IOException {
    ...
    private final List<LinuxEventComponent> getDeviceComponents() throws IOException {
        final List<LinuxEventComponent> components = new ArrayList<LinuxEventComponent>();
