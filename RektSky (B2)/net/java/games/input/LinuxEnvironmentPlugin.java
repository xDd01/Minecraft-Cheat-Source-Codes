...
public final class LinuxEnvironmentPlugin extends ControllerEnvironment implements Plugin
{
    ...
    static void loadLibrary(final String lib_name) {
        ...
        AccessController.doPrivileged(() -> {
            ...
            try {
                ...
                ControllerEnvironment.log("Failed to load library: " + e.getMessage());
                e.printStackTrace();
    ...
    public LinuxEnvironmentPlugin() {
        ...
        if (this.isSupported()) {
            ...
            ControllerEnvironment.log("Linux plugin claims to have found " + this.controllers.length + " controllers");
            AccessController.doPrivileged(() -> {
    ...
    private static final Component[] createComponents(final List<LinuxEventComponent> event_components, final LinuxEventDevice device) {
        final LinuxEventComponent[][] povs = new LinuxEventComponent[4][2];
        final List<LinuxComponent> components = new ArrayList<LinuxComponent>();
        for (int i = 0; i < event_components.size(); ++i) {
            ...
            if (identifier == Component.Identifier.Axis.POV) {
                ...
                switch (native_code) {
                    ...
                    default: {
                        ControllerEnvironment.log("Unknown POV instance: " + native_code);
                        break;
                    }
                }
            }
    ...
    private static final Controller createControllerFromDevice(final LinuxEventDevice device) throws IOException {
        final List<LinuxEventComponent> event_components = device.getComponents();
    ...
    private final Controller[] enumerateControllers() {
        ...
        final List<Controller> eventControllers = new ArrayList<Controller>();
        final List<Controller> jsControllers = new ArrayList<Controller>();
    ...
    private final void enumerateJoystickControllers(final List<Controller> controllers) {
        ...
        for (int i = 0; i < joystick_device_files.length; ++i) {
            ...
            try {
                ...
                ControllerEnvironment.log("Failed to open device (" + event_file + "): " + e.getMessage());
            }
        }
    }
    ...
    private static File[] listFilesPrivileged(final File dir, final FilenameFilter filter) {
        ...
        return AccessController.doPrivileged(() -> {
            ...
            if (files == null) {
                ControllerEnvironment.log("dir " + dir.getName() + " exists: " + dir.exists() + ", is writable: " + dir.isDirectory());
                files = new File[0];
            }
            else {
                Arrays.sort(files, Comparator.comparing((Function<? super File, ? extends Comparable>)File::getName));
            }
            return files;
        });
    }
    
    private final void enumerateEventControllers(final List<Controller> controllers) {
        final File dev = new File("/dev/input");
        final File[] event_device_files = listFilesPrivileged(dev, (dir, name) -> name.startsWith("event"));
        ...
        for (int i = 0; i < event_device_files.length; ++i) {
            ...
            try {
                ...
                try {
                    ...
                    ControllerEnvironment.log("Failed to create Controller: " + e.getMessage());
                    device.close();
                }
            }
            catch (IOException e2) {
                ControllerEnvironment.log("Failed to open device (" + event_file + "): " + e2.getMessage());
            }
        }
    }
        ...
        public final void run() {
            for (int i = 0; i < LinuxEnvironmentPlugin.this.devices.size(); ++i) {
                try {
                    ...
                    ControllerEnvironment.log("Failed to close device: " + e.getMessage());
                }
            }
        }
    }
