...
final class OSXHIDDevice
{
    ...
    public final Controller.PortType getPortType() {
        final String transport = (String)this.properties.get("Transport");
        if (transport == null) {
    ...
    private final OSXHIDElement createElementFromElementProperties(final Map<String, ?> element_properties) {
    ...
    private final void addElements(final List<OSXHIDElement> elements, final Map<String, ?> properties) {
        final Object[] elements_properties = (Object)properties.get("Elements");
        if (elements_properties == null) {
            return;
        }
        for (int i = 0; i < elements_properties.length; ++i) {
            final Map<String, ?> element_properties = (Map<String, ?>)elements_properties[i];
    ...
    public final List<OSXHIDElement> getElements() {
        final List<OSXHIDElement> elements = new ArrayList<OSXHIDElement>();
        this.addElements(elements, this.properties);
        return elements;
    }
    
    private static final long getLongFromProperties(final Map<String, ?> properties, final String key, final long default_value) {
        final Long long_obj = (Long)properties.get(key);
        if (long_obj == null) {
    ...
    private static final boolean getBooleanFromProperties(final Map<String, ?> properties, final String key, final boolean default_value) {
        return getLongFromProperties(properties, key, default_value ? 1 : 0) != 0L;
    }
    
    private static final int getIntFromProperties(final Map<String, ?> properties, final String key) {
        return (int)getLongFromProperties(properties, key);
    }
    
    private static final long getLongFromProperties(final Map<String, ?> properties, final String key) {
        final Long long_obj = (Long)properties.get(key);
        return long_obj;
    }
    ...
    private static final void dumpMap(final String prefix, final Map<String, ?> map) {
    ...
    private static final native Map<String, ?> nGetDeviceProperties(final long p0) throws IOException;
