...
final class IDirectInputDevice
{
    ...
    private final List<DIDeviceObject> objects;
    private final List<DIEffectInfo> effects;
    private final List<Rumbler> rumblers;
    private final int[] device_state;
    private final Map<DIDeviceObject, DIComponent> object_to_component;
    ...
    public IDirectInputDevice(final DummyWindow window, final long address, final byte[] instance_guid, final byte[] product_guid, final int dev_type, final int dev_subtype, final String instance_name, final String product_name) throws IOException {
        this.objects = new ArrayList<DIDeviceObject>();
        this.effects = new ArrayList<DIEffectInfo>();
        this.rumblers = new ArrayList<Rumbler>();
        this.object_to_component = new HashMap<DIDeviceObject, DIComponent>();
        this.window = window;
        ...
        try {
            ...
            ControllerEnvironment.log("Failed to create rumblers: " + e.getMessage());
        }
    ...
    public final synchronized boolean getNextEvent(final DIDeviceObjectData data) {
        final DIDeviceObjectData next_event = this.queue.get();
        if (next_event == null) {
            return false;
        }
        data.set(next_event);
    ...
    private final boolean getDeviceData(final DataQueue<DIDeviceObjectData> queue) throws IOException {
    ...
    private static final native int nGetDeviceData(final long p0, final int p1, final DataQueue<DIDeviceObjectData> p2, final Object[] p3, final int p4, final int p5);
    ...
    public final synchronized void setBufferSize(final int size) throws IOException {
        ...
        (this.queue = new DataQueue<DIDeviceObjectData>(size, DIDeviceObjectData.class)).position(this.queue.limit());
        this.acquire();
    }
