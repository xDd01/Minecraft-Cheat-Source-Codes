...
public class SoundStore
{
    ...
    public void clear() {
        if (this.sources != null && AL.isCreated()) {
            for (int i = 0; i < this.sourceCount; ++i) {
                final int src = this.sources.get(i);
    ...
    boolean isPaused(final int index) {
        final int state = AL10.alGetSourcei(this.sources.get(index), 4112);
        return state == 4115;
    }
    
    private int findFreeSource() {
        for (int i = 1; i < this.sourceCount; ++i) {
    ...
    void playAsMusic(final int buffer, final float pitch, float gain, final boolean loop) {
        this.paused = false;
        ...
        if (this.soundWorks) {
            ...
            this.getMusicSource();
            AL10.alSourcef(this.sources.get(0), 4106, gain);
