...
public class Graphics
{
    ...
    public static int MODE_ADD_ALPHA;
    public static int MODE_COLOR_MULTIPLY_ALPHA;
    ...
    public void setDrawMode(final int mode) {
        ...
        if (this.currentDrawingMode == Graphics.MODE_ADD_ALPHA) {
            Graphics.GL.glEnable(3042);
            Graphics.GL.glColorMask(true, true, true, true);
            Graphics.GL.glBlendFunc(770, 1);
        }
        if (this.currentDrawingMode == Graphics.MODE_COLOR_MULTIPLY_ALPHA) {
            Graphics.GL.glEnable(3042);
            Graphics.GL.glColorMask(true, true, true, true);
            Graphics.GL.glBlendFunc(769, 771);
        }
        this.postdraw();
    }
    ...
    public void copyArea(final Image target, final int x, final int y) {
        this.copyArea(target, x, y, 0, 0, target.getWidth(), target.getHeight());
    }
    
    public void copyArea(final Image target, final int x, final int y, final int xoff, final int yoff, final int width, final int height) {
        this.predraw();
        final Texture tex = target.getTexture();
        target.bind();
        if (this.isYFlipped()) {
            GL11.glCopyTexSubImage2D(3553, 0, xoff, yoff, x, y, width, height);
        }
        else {
            final int yoff2 = target.getHeight() - height - yoff;
            GL11.glCopyTexSubImage2D(3553, 0, xoff, yoff2, x, this.screenHeight - (y + height), width, height);
    ...
    static {
        ...
        Graphics.MODE_ADD_ALPHA = 7;
        Graphics.MODE_COLOR_MULTIPLY_ALPHA = 8;
        Graphics.currentGraphics = null;
    }
}
