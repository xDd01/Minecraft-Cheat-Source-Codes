...
public class Animation implements Renderable
{
    ...
    public void renderInUse(final int x, final int y, final float rot) {
        if (this.frames.size() == 0) {
            return;
        }
        if (this.autoUpdate) {
        ...
        final Frame frame = this.frames.get(this.currentFrame);
        this.spriteSheet.renderInUse(x, y, rot, frame.x, frame.y);
    }
    ...
    private void nextFrame(final long delta) {
        ...
        while (this.nextChange < 0L && !this.stopped) {
            ...
            if (this.pauseDuration < 0L || this.currentFrame != 0) {
                continue;
            }
            this.nextChange += this.pauseDuration;
        }
    }
    ...
    public void setPauseDuration(final long pauseDuration) {
