...
public class Image implements Renderable
{
    ...
    public static final int getMaxSingleImageSize() {
        ...
        Image.GL.glGetInteger(3379, buffer);
        return buffer.get(0);
    }
    
    public static Image createOffscreenImage(final int width, final int height, final int filter) throws SlickException {
    ...
    public static Image createOffscreenImage(final int width, final int height) throws SlickException {
        return createOffscreenImage(width, height, 9729);
    }
    ...
    public Image(final String ref, final int filter) throws SlickException {
    ...
    public void drawCentered(final float x, final float y) {
        this.draw(x - this.getWidth() / 2.0f, y - this.getHeight() / 2.0f);
    }
    ...
    public void drawEmbedded(final float x, final float y, final float width, final float height, final float rotation) {
        if (rotation == 0.0f) {
            this.drawEmbedded(x, y, width, height);
            return;
        }
        this.init();
        final float scaleX = width / this.width;
        final float scaleY = height / this.height;
        final float cx = this.getCenterOfRotationX() * scaleX;
        final float cy = this.getCenterOfRotationY() * scaleY;
        final float p1x = -cx;
        final float p1y = -cy;
        ...
        final float tx = this.getTextureOffsetX();
        final float ty = this.getTextureOffsetY();
        final float tw = this.getTextureWidth();
        final float th = this.getTextureHeight();
        final float x2 = cos * p1x - sin * p1y + cx;
        final float y2 = sin * p1x + cos * p1y + cy;
        final float x3 = cos * p4x - sin * p4y + cx;
        final float y3 = sin * p4x + cos * p4y + cy;
        final float x4 = cos * p3x - sin * p3y + cx;
        final float y4 = sin * p3x + cos * p3y + cy;
        final float x5 = cos * p2x - sin * p2y + cx;
        final float y5 = sin * p2x + cos * p2y + cy;
        if (this.corners == null) {
            Image.GL.glTexCoord2f(tx, ty);
            Image.GL.glVertex3f(x + x2, y + y2, 0.0f);
            Image.GL.glTexCoord2f(tx, ty + th);
            Image.GL.glVertex3f(x + x3, y + y3, 0.0f);
            Image.GL.glTexCoord2f(tx + tw, ty + th);
            Image.GL.glVertex3f(x + x4, y + y4, 0.0f);
            Image.GL.glTexCoord2f(tx + tw, ty);
            Image.GL.glVertex3f(x + x5, y + y5, 0.0f);
        }
        else {
            this.corners[0].bind();
            Image.GL.glTexCoord2f(tx, ty);
    ...
    public void drawEmbedded(final float x, final float y) {
        this.drawEmbedded(x, y, (float)this.getWidth(), (float)this.getHeight());
    }
    ...
    public void draw(final float x, final float y, final float width, final float height, Color filter) {
        ...
        final float centerX = this.centerX * (width / this.getWidth());
        final float centerY = this.centerY * (height / this.getHeight());
        this.texture.bind();
        Image.GL.glTranslatef(x, y, 0.0f);
        if (this.angle != 0.0f) {
    ...
    public void draw(final float x, final float y, final float x2, final float y2, final float srcx, final float srcy, final float srcx2, final float srcy2, Color filter) {
        ...
        filter.bind();
        this.texture.bind();
        final float centerX = this.centerX * ((x2 - x) / this.getWidth());
        final float centerY = this.centerY * ((y2 - y) / this.getHeight());
        Image.GL.glTranslatef(x, y, 0.0f);
        if (this.angle != 0.0f) {
            Image.GL.glTranslatef(centerX, centerY, 0.0f);
            Image.GL.glRotatef(this.angle, 0.0f, 0.0f, 1.0f);
    ...
    public Image getSubImage(final int x, final int y, final int width, final int height) {
        ...
        sub.width = width;
        sub.height = height;
        sub.centerX = width / 2.0f;
        sub.centerY = height / 2.0f;
        return sub;
    }
    ...
    public void drawEmbedded(float x, float y, float width, float height, final byte transform) {
        final boolean rotate = (transform & 0x1) > 0;
        final boolean flipY = (transform & 0x2) > 0 ^ rotate;
        final boolean flipX = (transform & 0x4) > 0 ^ rotate;
        if (flipX) {
            x += width;
            width *= -1.0f;
        }
