...
public class UnicodeFont implements Font
{
    ...
    public DisplayList drawDisplayList(float x, float y, final CharSequence text, final Color color, final int startIndex, final int endIndex) {
        if (text == null) {
            throw new IllegalArgumentException("text cannot be null.");
        }
        if (text.length() == 0) {
        ...
        x -= this.paddingLeft;
        y -= this.paddingTop;
        final CharSequence displayListKey = text.subSequence(startIndex, endIndex);
        color.bind();
        ...
        final char[] chars = toCharArray(text, 0, endIndex);
        final GlyphVector vector = this.font.layoutGlyphVector(GlyphPage.renderContext, chars, 0, chars.length, 0);
        int maxWidth = 0;
        int totalHeight = 0;
        int lines = 0;
    ...
    @Override
    public void drawString(final float x, final float y, final CharSequence text, final Color color, final int startIndex, final int endIndex) {
    ...
    public int getWidth(final CharSequence text) {
        ...
        final char[] chars = toCharArray(text, 0, text.length());
        final GlyphVector vector = this.font.layoutGlyphVector(GlyphPage.renderContext, chars, 0, chars.length, 0);
        int width = 0;
        int extraX = 0;
        boolean startNewLine = false;
    ...
    static int indexOf(final CharSequence cs, final char chr) {
    ...
    static char[] toCharArray(final CharSequence cs, final int startIndex, final int endIndex) {
        if (startIndex == 0 && endIndex == cs.length() && cs instanceof String) {
            return ((String)cs).toCharArray();
        }
        final int s = endIndex - startIndex;
        final char[] ar = new char[s];
        int x = 0;
    ...
    public int getYOffset(final CharSequence text) {
        if (text == null) {
            throw new IllegalArgumentException("text cannot be null.");
        }
        DisplayList displayList = null;
        ...
        final char[] chars = toCharArray(text, 0, end);
        final GlyphVector vector = this.font.layoutGlyphVector(GlyphPage.renderContext, chars, 0, chars.length, 0);
