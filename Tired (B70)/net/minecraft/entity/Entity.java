...
import de.tired.module.impl.list.movement.Step;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            double d4 = y;
            double d5 = z;
            ScaffoldWalk scaffoldWalk = ModuleManager.getInstance(ScaffoldWalk.class);
            boolean flag = this.onGround && this.isSneaking() && this instanceof EntityPlayer || (scaffoldWalk.isState() && scaffoldWalk.saveWalk.getValue());
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z) {
                    ...
                    final Step step = ModuleManager.getInstance(Step.class);
                    step.enabled = false;
                } else {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        if (this == IHook.MC.thePlayer) {

            float yaw = this.rotationYaw;

            MoveFlyingEvent event = new MoveFlyingEvent(strafe, forward, friction, yaw);
            event.call();
            ...
            EventStrafe event2 = new EventStrafe(strafe, forward, friction);
            event2.call();
    ...
    public Vec3 getLook(float partialTicks) {
        ...
        float pitch = this.rotationPitch, yaw = this.rotationYaw, prevPitch = this.prevRotationPitch, prevYaw = this.prevRotationYaw;
        if (this instanceof EntityPlayerSP) {
            GetLookEvent event = new GetLookEvent(yaw, prevYaw, pitch, prevPitch);
            event.call();
            yaw = event.getYaw();
            pitch = event.getPitch();
            prevPitch = event.getPrevPitch();
            prevYaw = event.getPrevYaw();
