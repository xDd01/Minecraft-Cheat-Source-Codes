...
import de.tired.event.events.JumpEvent;
import de.tired.module.impl.list.visual.SlowerHit;
...
public abstract class EntityLivingBase extends Entity {
    ...
    public void onEntityUpdate() {
        ...
        if(this == Minecraft.getMinecraft().thePlayer) {
            this.prevRotationYawHead = Rotations.yaw;
    ...
    private int getArmSwingAnimationEnd() {
        return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : SlowerHit.getInstance().isState() ? SlowerHit.getInstance().value.getValueInt() : 6);
    }
    ...
    public Vec3 getLook(float partialTicks) {
        float pitch = 0, yaw = 0;
        final ScaffoldWalk scaffoldWalk = Tired.INSTANCE.moduleManager.moduleBy(ScaffoldWalk.class);


        if (!scaffoldWalk.isState()) {
            yaw = rotationYaw;
            pitch = rotationPitch;
        }
