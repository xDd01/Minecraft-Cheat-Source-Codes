...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    protected void updateEntityActionState() {
        ...
        this.rotationPitchHead = Rotations.pitch;
    }
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem) {
        if (droppedItem == null) {
            ...
            if (dropAround) {
                ...
                float f2 = 0.3F;
                entityitem.motionX = (double) (-MathHelper.sin(yaw / 180.0F * (float) Math.PI) * MathHelper.cos(yaw/ 180.0F * (float) Math.PI) * f2);
            ...
            if (!targetEntity.hitByEntity(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (ViaFabric.clientSideVersion != 47) {
                        if (IHook.MC.thePlayer.isSprinting()) {
                            flag = false;
                        }
                    }
                    ...
                    if (flag2) {
                        final VelocityEvent velocityEvent = new VelocityEvent(true, .6, true);
                        float yaw = this.rotationYaw;
                        if(this == Minecraft.getMinecraft().thePlayer)
                            yaw = Rotations.yaw;
                        if (i > 0) {

                            if (velocityEvent.isFlag()) {
                                if (targetEntity instanceof EntityLivingBase) {
                                    ((EntityLivingBase) targetEntity).knockBack(this, (float) i * 0.5F, MathHelper.sin(yaw * 0.017453292F), -MathHelper.cos(yaw * 0.017453292F));
                                } else {
                                    targetEntity.addVelocity(-MathHelper.sin(yaw * 0.017453292F) * (float) i * 0.5F, 0.1D, MathHelper.cos(this.rotationYaw * 0.017453292F) * (float) i * 0.5F);
                                }
                            }
                            ...
                            this.motionX *= velocityEvent.motion;
                            this.motionZ *= velocityEvent.motion;
                            this.setSprinting(velocityEvent.isSprint());
                        }
