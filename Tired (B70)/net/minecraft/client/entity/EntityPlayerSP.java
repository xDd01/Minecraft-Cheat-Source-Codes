...
public class    EntityPlayerSP extends AbstractClientPlayer {
    ...
    public Vec3 getLook(float partialTicks) {
        if ((ScaffoldWalk.getInstance().isState()) || ModuleManager.getInstance(KillAura.class).isState()) {
            return getLook2(partialTicks);
        } else {
            return getVectorForRotation(Rotations.pitch, Rotations.yaw);
        }
    }

    public Vec3 getLook2(float partialTicks) {
        final EventLook eventLook = new EventLook(new float[]{rotationYaw, rotationPitch});
        eventLook.call();

        return getVectorForRotation(eventLook.getRotations()[1], eventLook.getRotations()[0]);
    }
    ...
    public void onUpdate() {
        ...
        final KillAura killAura = ModuleManager.getInstance(KillAura.class);

        if (!killAura.state && killAura.rotationState) {
            mc.thePlayer.rotationYaw = Rotations.yaw - Rotations.yaw % 360 + mc.thePlayer.rotationYaw % 360;
            killAura.rotationState = false;
        }
        final EventClip noClipEvent = new EventClip(noClip);
        noClipEvent.call();
        this.noClip = noClipEvent.isShouldDoNoClip();

        if (Tired.INSTANCE.clickGui != null) {
            ClickGUI.getInstance().unableModule();
        }
        ...
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            if (this.isRiding()) {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook( Rotations.yaw, Rotations.pitch, this.onGround));
    ...
    public void onUpdateWalkingPlayer() {
        ...
        EventPreMotion pre = new EventPreMotion(this.posY, Rotations.yaw, Rotations.pitch, this.onGround);
        pre.call();
    ...
    public void sendChatMessage(String message) {
        if (TiredCore.CORE.onSendChatMessage(message)) {
    ...
    public void onLivingUpdate() {
        ...
        if (this.isUsingItem() && !this.isRiding()) {
            ...
            float strafeMultiplier = 0.2F;
            float forwardMultiplier = 0.2F;

            SlowDownEvent event = new SlowDownEvent(strafeMultiplier, forwardMultiplier);
            event.call();

            if (!event.isCancelled()) {
                strafeMultiplier = event.getStrafeMultiplier();
                forwardMultiplier = event.getForwardMultiplier();

                this.movementInput.moveStrafe *= strafeMultiplier;
                this.movementInput.moveForward *= forwardMultiplier;
