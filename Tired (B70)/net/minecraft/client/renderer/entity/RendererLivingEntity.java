...
import de.tired.event.events.EventRenderEntity;
import de.tired.event.events.EventRenderModel;
import de.tired.interfaces.IHook;
import de.tired.module.impl.list.visual.Chams;
...
public abstract class   RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        EventRenderEntity eventPre = new EventRenderEntity(entity);
        eventPre.call();
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[]{entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)})) {
            ...
            try {
                ...
                if (entity == Minecraft.getMinecraft().thePlayer)
                    f7 = Rotations.beforePitch + (Rotations.pitch - Rotations.beforePitch) * partialTicks;
            ...
            if (!this.renderOutlines) {
                ...
                if (Extension.EXTENSION.getGenerallyProcessor().renderProcessor.isInViewFrustrum(entity)) {
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float scaleFactor) {
        ...
        if (flag || flag1) {
            ...
            EventRenderModel eventModelRender = new EventRenderModel(renderPartialTicks, entitylivingbaseIn, this.mainModel, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_, scaleFactor);

            eventModelRender.call();
            boolean flag2 = Tired.INSTANCE.moduleManager.moduleBy(Chams.class).state && entitylivingbaseIn instanceof EntityPlayer && entitylivingbaseIn != IHook.MC.thePlayer;

            Color chamsColor;
            if (flag2) {
                final Chams chams = Tired.INSTANCE.moduleManager.moduleBy(Chams.class);
                chamsColor = chams.chamsColor.ColorPickerC;
                GL11.glPushMatrix();
                GL11.glEnable(10754);
                ...
                GL11.glColor4f((float) chamsColor.getRed() / 255.0F, (float) chamsColor.getGreen() / 255.0F, (float) chamsColor.getBlue() / 255.0F, (float) chamsColor.getAlpha() / 155.0F);
