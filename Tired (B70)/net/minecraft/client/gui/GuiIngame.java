...
import de.tired.api.util.gui.LayerRenderer;
...
import de.tired.module.impl.HudModule;
import de.tired.module.impl.list.visual.ClickGUI;
import de.tired.module.impl.list.visual.Crossair;
import de.tired.module.impl.list.visual.HotBar;
...
public class GuiIngame extends Gui {
    ...
    private final LayerRenderer layerRenderer = new LayerRenderer();
    ...
    private double animationDown = 0;
    public float float2;
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (this.showCrosshair() && !Tired.INSTANCE.moduleManager.findModuleByClass(Crossair.class).isState()) {
        ...
        if (mc.isSingleplayer()) {
            Tired.INSTANCE.discordRPC.setStage(new String[]{"In Singleplayer", "lmfao"});
        } else {
            Tired.INSTANCE.discordRPC.setStage(new String[]{"Playing on " + mc.getCurrentServerData().serverIP + " as " + mc.thePlayer.getName(), "lmfao"});
        }
        ...
        if (this.mc.gameSettings.keyBindCommand.isKeyDown() && (!this.mc.isIntegratedServerRunning() || this.mc.thePlayer.sendQueue.getPlayerInfoMap().size() > 1 || scoreobjective1 != null)) {
            ...
            overlayPlayerList.translate = new Translate(0, 0);
    ...
    public void renderChatFinal(boolean rect) {
        ScaledResolution scaledresolution = new ScaledResolution(this.mc);
        int j = scaledresolution.getScaledHeight();
        GlStateManager.enableBlend();
        GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
        ...
        this.persistantChatGUI.drawChat(0, 0, this.updateCounter, rect);
        GlStateManager.popMatrix();
    }
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        ...
        Render2DEvent event = new Render2DEvent(partialTicks, sr);
        event.call();

        if (HudModule.getInstance().isState()) {
            layerRenderer.renderLayerWBlur();
            layerRenderer.renderNormalLayer();
        }
        Scoreboard scoreboard = this.mc.theWorld.getScoreboard();
        ScoreObjective scoreobjective = null;
        ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(this.mc.thePlayer.getName());
        ...
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            if (!Tired.INSTANCE.moduleManager.findModuleByClass(HotBar.class).isState() || !Tired.INSTANCE.moduleManager.findModuleByClass(HudModule.class).isState()) {
            ...
            if (Tired.INSTANCE.moduleManager.findModuleByClass(HotBar.class).isState()) {
                ...
                for (int j = 0; j < 9; ++j) {
                    ...
                    int xPos = k;
                    if (Tired.INSTANCE.moduleManager.findModuleByClass(HotBar.class).isState()) {
                        height = (int) (l - Extension.EXTENSION.getGenerallyProcessor().renderProcessor.animationY + 22);
                        xPos--;
                    }
                    this.renderHotbarItem(j, xPos, height, partialTicks, entityplayer);
                }
    ...
    public void renderExpBar(ScaledResolution scaledRes, int x) {
        ...
        if (i > 0) {
            ...
            int l = (int) (scaledRes.getScaledHeight() - 32 + 3 - Extension.EXTENSION.getGenerallyProcessor().renderProcessor.animationY + 22);
        ...
        if (this.mc.thePlayer.experienceLevel > 0) {
            ...
            int i1 = (int) (scaledRes.getScaledHeight() - 31 - 4 - Extension.EXTENSION.getGenerallyProcessor().renderProcessor.animationY + 22);
    ...
    public void renderScoreBoard(boolean rect) {
        ...
        if (scoreobjective1 != null) {
            final ScaledResolution scaledresolution = new ScaledResolution(mc);
            renderScoreboard(scoreobjective1, scaledresolution, rect);
        }

    }


    public void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes, boolean rect) {
        ...
        for (Score score1 : collection) {
            ...
            animationDown = AnimationUtil.getAnimationState(animationDown, 12, 3);

            int k = (int) (j1 - j * FHook.fontRenderer2.FONT_HEIGHT + animationDown);
            int l = 132 - k1 + 2;
            if (rect) {

                Extension.EXTENSION.getGenerallyProcessor().renderProcessor.drawRect(l1 - 2, k, l, k + FHook.fontRenderer2.FONT_HEIGHT, Integer.MIN_VALUE);
            }
            mc.fontRendererObj.drawStringWithShadow(s1, l1, k, -1);
    ...
    private void renderPlayerStats(ScaledResolution scaledRes) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            int j2 = (int) (k1 - (l1 - 1) * i2 - 10 - Extension.EXTENSION.getGenerallyProcessor().renderProcessor.animationY + 22);
            float f2 = f1;
            ...
            for (int i6 = MathHelper.ceiling_float_int((f + f1) / 2.0F) - 1; i6 >= 0; --i6) {
                ...
                int i4 = i1 + i6 % 10 * 8;
                int j4 = (int) (k1 - l3 * i2 - Extension.EXTENSION.getGenerallyProcessor().renderProcessor.animationY + 22);
            ...
            if (entity == null) {
                ...
                if (!Tired.INSTANCE.moduleManager.findModuleByClass(HotBar.class).isState()) {
                    Extension.EXTENSION.getGenerallyProcessor().renderProcessor.animationY = 22;
                }
                for (int k6 = 0; k6 < 10; ++k6) {
                    int j7 = (int) (k1 - Extension.EXTENSION.getGenerallyProcessor().renderProcessor.animationY + 22);
                    int l7 = 16;
                    int k8 = 0;
    ...
    private void renderHotbarItem(int index, int xPos, int yPos, float partialTicks, EntityPlayer player) {
        ...
        if (itemstack != null) {
            ...
            if (HotBar.getInstance().state) {
                this.itemRenderer.renderItemOverlays2(FHook.fontRenderer, itemstack, xPos, yPos);
            } else {
