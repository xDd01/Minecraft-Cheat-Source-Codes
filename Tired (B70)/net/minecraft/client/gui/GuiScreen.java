...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
    ...
    public boolean isOtherRender() {
        return otherRender;
    }

    public void setOtherRender(boolean otherRender) {
        this.otherRender = otherRender;
    }
    ...
    protected List<CheckBoxButton> buttonList2 = Lists.<CheckBoxButton>newArrayList();
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i = 0; i < this.buttonList.size(); ++i) {
            ((GuiButton) this.buttonList.get(i)).drawButton(this.mc, mouseX, mouseY, false);
        }
        ShaderRenderer.startBlur();
        for (int i = 0; i < this.buttonList.size(); ++i) {
            ((GuiButton) this.buttonList.get(i)).drawButton(this.mc, mouseX, mouseY, true);
        }
        ...
        for (CheckBoxButton boxButton : this.buttonList2) {
            boxButton.drawButton();
        }
    ...
    final CopyOnWriteArrayList<GuiButton> buttonCopyOnWriteArrayList = new CopyOnWriteArrayList<>();
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        for (CheckBoxButton boxButton : this.buttonList2) {
            boxButton.mouseClicked(mouseX, mouseY, mouseButton);
        }

        for (CheckBoxButton boxButton : this.buttonList2) {
            if (mouseButton == 0) {
                actionPerformed(boxButton);
            }
        }

        if (mouseButton == 0) {
            buttonCopyOnWriteArrayList.addAll(buttonList);
            this.buttonCopyOnWriteArrayList.forEach(button -> {
            ...
            buttonCopyOnWriteArrayList.clear();
        }
    }
