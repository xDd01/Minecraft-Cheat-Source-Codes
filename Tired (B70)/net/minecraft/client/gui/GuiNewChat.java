...
public class GuiNewChat extends Gui {
    ...
    public void drawChat(int x, int y, int updateCounter, boolean rect) {
                    ...
                    if (chatline != null) {
                        ...
                        if (j1 < 200 || flag) {
                            ...
                            if (l1 > 3) {
                                ...
                                if (rect) {
                                    Extension.EXTENSION.getGenerallyProcessor().renderProcessor.drawRect(i2, j2 - 7, i2 + l + 4, j2 + 2, Integer.MIN_VALUE);;
                                }

                                width = i2 + l + 4;

                                lastY = j2;
                                this.text = this.drawnChatLines.get(i1 + this.scrollPos).getChatComponent().getUnformattedText();
                                double height = PerformanceGui.usingType == UsingType.NORMAL_PERFORMANCE ? y + (float) (j2 - 2) : y + (float) (j2 - 4);
                                FontManager.light.drawStringWithShadow(s, x + (float) i2, (float) height - 6, -1);
                                GlStateManager.resetColor();
                                GlStateManager.disableBlend();
        ...
        if (allowRender) {
            ...
            if (rect) {
                Gui.drawRect(f1 + 1, lastY + 2, width + 2, lastY + 12, Integer.MIN_VALUE);
            }
            FHook.fontRenderer3.drawString("Chat", 3, lastY + 8f, -1);
        }
        if (getChatOpen()) {
            //ScaledResolution sr = new ScaledResolution(mc);
            //notificationCenter.render(12, 36);
        }
