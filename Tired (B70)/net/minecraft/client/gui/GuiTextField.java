...
public class GuiTextField extends Gui {
	 ...
	 * Draws the textbox
	 */
	public void drawTextBox(boolean... glow) {
		if (this.getVisible()) {
			if (this.getEnableBackgroundDrawing()) {

				//  drawRect(this.xPosition - 1, this.yPosition - 1, this.xPosition + this.width + 1, this.yPosition + this.height + 1, -6250336);
				Gui.drawRect(this.xPosition, this.yPosition, this.xPosition + this.width + 1, this.yPosition + this.height + 2, Integer.MIN_VALUE);
			}

			int i = this.isEnabled ? this.enabledColor : this.disabledColor;
			int j = this.cursorPosition - this.lineScrollOffset;
			int k = this.selectionEnd - this.lineScrollOffset;
			StringBuilder s = new StringBuilder(fontRendererInstance.trimStringToWidth(this.text.substring(this.lineScrollOffset), this.getWidth()));
			if (this.isCensored()) {
			...
			animation = AnimationUtil.getAnimationState(animation, this.enableBackgroundDrawing ? this.xPosition + 4 : this.xPosition, 0);
			...
			assert glow[0];
			if (s.length() > 0) {
				String s1 = flag ? s.substring(0, j) : s.toString();
				if (PerformanceGui.usingType == UsingType.NORMAL_PERFORMANCE) {;
					j1 = (int) FHook.fontRenderer.drawString(s1, (float) l, (float) i1 + 3, -1);
				} else {
					j1 = (int) FHook.fontRenderer.drawString(s1, (float) l, (float) i1 + 3, -1);
				}
			}
			...
			if (s.length() > 0 && flag && j < s.length()) {
				j1 = (int) Tired.INSTANCE.fontManager.robotoF.drawStringWithShadow2(s.substring(j), (float) j1, (float) i1, i);
			}

			if (flag1) {
				if (flag2) {
					Gui.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + Tired.INSTANCE.fontManager.notosansF.FONT_HEIGHT, -3092272);
				} else {
					Tired.INSTANCE.fontManager.robotoF.drawStringWithShadow2("_", (float) k1, (float) i1, i);
				}
			}

			FHook.fontRenderer4.drawString("test", 1222, 1222, -1);
			if (k != j) {
				int l1 = l + Tired.INSTANCE.fontManager.robotoF.getStringWidth(s.substring(0, k));
				//      this.drawCursorVertical(k1, i1 - 1, l1 - 1, (int) (i1 + 1 + CheatMain.INSTANCE.fontManager.notosansF.FONT_HEIGHT));
			}
		}
	}
	...
	private void drawCursorVertical(int p_146188_1_, int p_146188_2_, int p_146188_3_, int p_146188_4_) {
		drawRect(p_146188_1_, p_146188_2_, p_146188_3_, p_146188_4_, new Color(20, 20, 20, 120).getRGB());
	}
