...
public class CustomColorizer {
    ...
    public static int getColorMultiplier(BakedQuad p_getColorMultiplier_0_, Block p_getColorMultiplier_1_, IBlockAccess p_getColorMultiplier_2_, BlockPos p_getColorMultiplier_3_, RenderEnv p_getColorMultiplier_4_) {
        ...
        if (blockPalettes != null) {
            ...
            if (aint != null) {
                if (!Config.isSmoothBiomes()) return CustomColorizer.getCustomColor(aint, p_getColorMultiplier_2_, p_getColorMultiplier_3_);
        ...
        int n = CustomColorizer.getCustomColor(aint, p_getColorMultiplier_2_, p_getColorMultiplier_3_);
        return n;
    }
    ...
    private static int getSmoothColorMultiplier(Block p_getSmoothColorMultiplier_0_, IBlockAccess p_getSmoothColorMultiplier_1_, BlockPos p_getSmoothColorMultiplier_2_, int[] p_getSmoothColorMultiplier_3_, int[] p_getSmoothColorMultiplier_4_, int p_getSmoothColorMultiplier_5_, int p_getSmoothColorMultiplier_6_, RenderEnv p_getSmoothColorMultiplier_7_) {
        ...
        while (true) {
            ...
            for (int l1 = j1 - 1; l1 <= j1 + 1; i += i2 >> 16 & 0xFF, j += i2 >> 8 & 0xFF, k += i2 & 0xFF, ++l1) {
    ...
    public static int getFluidColor(Block p_getFluidColor_0_, IBlockAccess p_getFluidColor_1_, BlockPos p_getFluidColor_2_) {
        int n;
        if (p_getFluidColor_0_.getMaterial() != Material.water) {
            n = p_getFluidColor_0_.colorMultiplier(p_getFluidColor_1_, p_getFluidColor_2_);
            return n;
        }
        if (waterColors != null) {
            if (Config.isSmoothBiomes()) {
                n = CustomColorizer.getSmoothColor(waterColors, p_getFluidColor_1_, p_getFluidColor_2_.getX(), p_getFluidColor_2_.getY(), p_getFluidColor_2_.getZ(), 3, 1);
                return n;
            }
            n = CustomColorizer.getCustomColor(waterColors, p_getFluidColor_1_, p_getFluidColor_2_);
            return n;
        }
    ...
    public static void updatePortalFX(EntityFX p_updatePortalFX_0_) {
        if (particlePortalColor < 0) return;
    ...
    public static void updateMyceliumFX(EntityFX p_updateMyceliumFX_0_) {
        if (myceliumParticleColors == null) return;
    ...
    public static void updateReddustFX(EntityFX p_updateReddustFX_0_, IBlockAccess p_updateReddustFX_1_, double p_updateReddustFX_2_, double p_updateReddustFX_4_, double p_updateReddustFX_6_) {
        ...
        int i = CustomColorizer.getRedstoneLevel(iblockstate, 15);
        int j = CustomColorizer.getRedstoneColor(i);
        if (j == -1) return;
        int k = j >> 16 & 0xFF;
        int l = j >> 8 & 0xFF;
        int i1 = j & 0xFF;
    ...
    public static int getRedstoneColor(int p_getRedstoneColor_0_) {
        ...
        if (p_getRedstoneColor_0_ < 0) return -1;
        if (p_getRedstoneColor_0_ > 15) return -1;
        int n = redstoneColors[p_getRedstoneColor_0_] & 0xFFFFFF;
        return n;
    }
    ...
    public static int getLilypadColorMultiplier(IBlockAccess p_getLilypadColorMultiplier_0_, BlockPos p_getLilypadColorMultiplier_1_) {
        int n;
        if (lilyPadColor < 0) {
            n = Blocks.waterlily.colorMultiplier(p_getLilypadColorMultiplier_0_, p_getLilypadColorMultiplier_1_);
            return n;
        }
        n = lilyPadColor;
        return n;
    ...
    public static int getSmoothColor(int[] p_getSmoothColor_0_, IBlockAccess p_getSmoothColor_1_, double p_getSmoothColor_2_, double p_getSmoothColor_4_, double p_getSmoothColor_6_, int p_getSmoothColor_8_, int p_getSmoothColor_9_) {
        ...
        while (true) {
            ...
            for (int j2 = k - l; j2 <= k + l; i1 += k2 >> 16 & 0xFF, j1 += k2 >> 8 & 0xFF, k1 += k2 & 0xFF, ++l1, j2 += p_getSmoothColor_9_) {
                blockposm.setXyz(i2, j, j2);
                k2 = CustomColorizer.getCustomColor(p_getSmoothColor_0_, p_getSmoothColor_1_, blockposm);
            }
            i2 += p_getSmoothColor_9_;
        }
    }
    ...
    public static int getStemColorMultiplier(Block p_getStemColorMultiplier_0_, IBlockAccess p_getStemColorMultiplier_1_, BlockPos p_getStemColorMultiplier_2_, RenderEnv p_getStemColorMultiplier_3_) {
        ...
        if (i < stemColors.length) return stemColors[i];
    ...
    public static Vec3 getWorldFogColor(Vec3 p_getWorldFogColor_0_, WorldClient p_getWorldFogColor_1_, Entity p_getWorldFogColor_2_, float p_getWorldFogColor_3_) {
        ...
        switch (i) {
            case -1: {
                return CustomColorizer.getFogColorNether(p_getWorldFogColor_0_);
            }
            case 0: {
                Minecraft minecraft = Minecraft.getMinecraft();
                return CustomColorizer.getFogColor(p_getWorldFogColor_0_, minecraft.theWorld, p_getWorldFogColor_2_.posX, p_getWorldFogColor_2_.posY + 1.0, p_getWorldFogColor_2_.posZ);
            }
            case 1: {
                return CustomColorizer.getFogColorEnd(p_getWorldFogColor_0_);
            }
        }
    ...
    public static Vec3 getWorldSkyColor(Vec3 p_getWorldSkyColor_0_, WorldClient p_getWorldSkyColor_1_, Entity p_getWorldSkyColor_2_, float p_getWorldSkyColor_3_) {
        ...
        switch (i) {
            case 0: {
                Minecraft minecraft = Minecraft.getMinecraft();
                return CustomColorizer.getSkyColor(p_getWorldSkyColor_0_, minecraft.theWorld, p_getWorldSkyColor_2_.posX, p_getWorldSkyColor_2_.posY + 1.0, p_getWorldSkyColor_2_.posZ);
            }
            case 1: {
                return CustomColorizer.getSkyColorEnd(p_getWorldSkyColor_0_);
            }
        }
