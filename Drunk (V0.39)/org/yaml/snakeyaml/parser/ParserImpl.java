...
implements Parser {
    ...
    public boolean checkEvent(Event.ID choice) {
        ...
        if (!this.currentEvent.is(choice)) return false;
        return true;
    }
    ...
    private VersionTagsTuple processDirectives() {
        ...
        block3: while (this.scanner.checkToken(Token.ID.Directive)) {
            List value;
        ...
        if (yamlVersion == null) {
            if (tagHandles.isEmpty()) return this.directives;
        }
        Iterator<String> i$ = DEFAULT_TAGS.keySet().iterator();
        while (true) {
            if (!i$.hasNext()) {
    ...
    private Event parseNode(boolean block, boolean indentlessSequence) {
        ...
        if (block && this.scanner.checkToken(Token.ID.BlockSequenceStart)) {
            endMark = this.scanner.peekToken().getStartMark();
            event = new SequenceStartEvent(anchor, tag, implicit, startMark, endMark, DumperOptions.FlowStyle.BLOCK);
            this.state = new ParseBlockSequenceFirstEntry();
            return event;
        }
        if (block && this.scanner.checkToken(Token.ID.BlockMappingStart)) {
            endMark = this.scanner.peekToken().getStartMark();
