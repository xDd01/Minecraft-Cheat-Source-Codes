...
extends LegacyBlockItemRewriter<Protocol1_10To1_11> {
            ...
            public void registerMap() {
                ...
                this.handler(new PacketHandler(){
                    ...
                    public void handle(PacketWrapper wrapper) throws Exception {
                        if (!BlockItemPackets1_11.this.isLlama(wrapper.user())) return;
            ...
            public void registerMap() {
                ...
                this.handler(new PacketHandler(){
                    ...
                    public void handle(PacketWrapper wrapper) throws Exception {
                        ...
                        if (wrapper.get(Type.UNSIGNED_BYTE, 0) != 1) return;
    ...
    private boolean isLlama(UserConnection user) {
        ...
        if (tracker.getInventory() == null) return false;
        if (!tracker.getInventory().equals("EntityHorse")) return false;
        ...
        if (!entityData.type().is((EntityType)Entity1_11Types.EntityType.LIAMA)) return false;
        return true;
    }

    private Optional<ChestedHorseStorage> getChestedHorse(UserConnection user) {
        WindowTracker tracker = user.get(WindowTracker.class);
        if (tracker.getInventory() == null) return Optional.empty();
        if (!tracker.getInventory().equals("EntityHorse")) return Optional.empty();
        Object entTracker = user.getEntityTracker(Protocol1_10To1_11.class);
        StoredEntityData entityData = entTracker.entityData(tracker.getEntityId());
