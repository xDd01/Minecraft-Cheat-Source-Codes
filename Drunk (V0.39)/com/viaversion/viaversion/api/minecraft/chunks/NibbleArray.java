...
public class NibbleArray {
    ...
    public NibbleArray(int length) {
        if (length == 0) throw new IllegalArgumentException("Length of nibble array must be a positive number dividable by 2!");
        if (length % 2 != 0) {
    ...
    public NibbleArray(byte[] handle) {
        if (handle.length == 0) throw new IllegalArgumentException("Length of nibble array must be a positive number dividable by 2!");
        if (handle.length % 2 != 0) {
            throw new IllegalArgumentException("Length of nibble array must be a positive number dividable by 2!");
        }
        this.handle = handle;
    }
    ...
    public byte get(int index) {
        ...
        if (index % 2 != 0) return (byte)(value >> 4 & 0xF);
    ...
    public void set(int index, int value) {
        if (index % 2 == 0) {
            this.handle[index /= 2] = (byte)(this.handle[index] & 0xF0 | value & 0xF);
            return;
        }
        this.handle[index /= 2] = (byte)(this.handle[index] & 0xF | (value & 0xF) << 4);
    }
