...
public class DamageSource {
    ...
    public static DamageSource causeFireballDamage(EntityFireball fireball, Entity p_76362_1_) {
        ...
        if (p_76362_1_ == null) {
            damageSource = new EntityDamageSourceIndirect("onFire", fireball, fireball).setFireDamage().setProjectile();
            return damageSource;
        }
        damageSource = new EntityDamageSourceIndirect("fireball", fireball, p_76362_1_).setFireDamage().setProjectile();
        return damageSource;
    }
    ...
    public static DamageSource setExplosionSource(Explosion explosionIn) {
        DamageSource damageSource;
        if (explosionIn != null && explosionIn.getExplosivePlacedBy() != null) {
            damageSource = new EntityDamageSource("explosion.player", explosionIn.getExplosivePlacedBy()).setDifficultyScaled().setExplosion();
            return damageSource;
        }
    ...
    public IChatComponent getDeathMessage(EntityLivingBase p_151519_1_) {
        ...
        String s1 = s + ".player";
        if (entitylivingbase != null && StatCollector.canTranslate(s1)) {
            chatComponentTranslation = new ChatComponentTranslation(s1, p_151519_1_.getDisplayName(), entitylivingbase.getDisplayName());
            return chatComponentTranslation;
        }
        chatComponentTranslation = new ChatComponentTranslation(s, p_151519_1_.getDisplayName());
        return chatComponentTranslation;
    }
    ...
    public boolean isCreativePlayer() {
        ...
        if (!((EntityPlayer)entity).capabilities.isCreativeMode) return false;
        return true;
    }
}
