...
extends BlockLeaves {
    ...
    public int colorMultiplier(IBlockAccess worldIn, BlockPos pos, int renderPass) {
        ...
        if (iblockstate.getBlock() != this) return super.colorMultiplier(worldIn, pos, renderPass);
        BlockPlanks.EnumType blockplanks$enumtype = iblockstate.getValue(VARIANT);
        if (blockplanks$enumtype == BlockPlanks.EnumType.SPRUCE) {
            return ColorizerFoliage.getFoliageColorPine();
        }
        if (blockplanks$enumtype != BlockPlanks.EnumType.BIRCH) return super.colorMultiplier(worldIn, pos, renderPass);
        return ColorizerFoliage.getFoliageColorBirch();
    }

    @Override
    protected void dropApple(World worldIn, BlockPos pos, IBlockState state, int chance) {
        if (state.getValue(VARIANT) != BlockPlanks.EnumType.OAK) return;
    ...
    protected int getSaplingDropChance(IBlockState state) {
        if (state.getValue(VARIANT) == BlockPlanks.EnumType.JUNGLE) {
            return 40;
        }
        int n = super.getSaplingDropChance(state);
        return n;
    }
