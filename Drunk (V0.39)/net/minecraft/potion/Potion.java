...
public class Potion {
    ...
    public void performEffect(EntityLivingBase entityLivingBaseIn, int p_76394_2_) {
        if (this.id == Potion.regeneration.id) {
            if (!(entityLivingBaseIn.getHealth() < entityLivingBaseIn.getMaxHealth())) return;
            entityLivingBaseIn.heal(1.0f);
            return;
        }
        if (this.id == Potion.poison.id) {
            if (!(entityLivingBaseIn.getHealth() > 1.0f)) return;
        ...
        if (this.id == Potion.hunger.id && entityLivingBaseIn instanceof EntityPlayer) {
            ((EntityPlayer)entityLivingBaseIn).addExhaustion(0.025f * (float)(p_76394_2_ + 1));
            return;
        }
        if (this.id == Potion.saturation.id && entityLivingBaseIn instanceof EntityPlayer) {
            if (entityLivingBaseIn.worldObj.isRemote) return;
        ...
        if (!(this.id == Potion.heal.id && !entityLivingBaseIn.isEntityUndead() || this.id == Potion.harm.id && entityLivingBaseIn.isEntityUndead())) {
            if (this.id != Potion.harm.id || entityLivingBaseIn.isEntityUndead()) {
                if (this.id != Potion.heal.id) return;
                if (!entityLivingBaseIn.isEntityUndead()) return;
            }
    ...
    public boolean isReady(int p_76397_1_, int p_76397_2_) {
        if (this.id == Potion.regeneration.id) {
            ...
            if (p_76397_1_ % k != 0) return false;
            return true;
        }
        if (this.id == Potion.poison.id) {
            int j = 25 >> p_76397_2_;
            ...
            if (p_76397_1_ % j != 0) return false;
            return true;
        }
        if (this.id == Potion.wither.id) {
            int i = 40 >> p_76397_2_;
            ...
            if (p_76397_1_ % i != 0) return false;
            return true;
        }
        if (this.id != Potion.hunger.id) return false;
        return true;
    }
    ...
    public boolean hasStatusIcon() {
        if (this.statusIconIndex < 0) return false;
        return true;
    }
    ...
    public void removeAttributesModifiersFromEntity(EntityLivingBase entityLivingBaseIn, BaseAttributeMap p_111187_2_, int amplifier) {
        Iterator<Map.Entry<IAttribute, AttributeModifier>> iterator = this.attributeModifierMap.entrySet().iterator();
        while (iterator.hasNext()) {
            Map.Entry<IAttribute, AttributeModifier> entry = iterator.next();
