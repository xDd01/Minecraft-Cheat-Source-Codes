...
extends EntityAIBase {
    ...
    public EntityAITempt(EntityCreature temptedEntityIn, double speedIn, Item temptItemIn, boolean scaredByPlayerMovementIn) {
        ...
        this.setMutexBits(3);
        if (temptedEntityIn.getNavigator() instanceof PathNavigateGround) return;
    ...
    public boolean shouldExecute() {
        ...
        if (itemstack.getItem() != this.temptItem) return false;
        return true;
    }

    @Override
    public boolean continueExecuting() {
        if (!this.scaredByPlayerMovement) return this.shouldExecute();
        if (this.temptedEntity.getDistanceSqToEntity(this.temptingPlayer) < 36.0) {
            ...
            if (Math.abs((double)this.temptingPlayer.rotationPitch - this.pitch) > 5.0) return false;
            if (Math.abs((double)this.temptingPlayer.rotationYaw - this.yaw) > 5.0) {
                return false;
            }
        } else {
