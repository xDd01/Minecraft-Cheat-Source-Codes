...
extends EntityAIBase {
    ...
    public EntityAIFollowOwner(EntityTameable thePetIn, double followSpeedIn, float minDistIn, float maxDistIn) {
        ...
        this.setMutexBits(3);
        if (thePetIn.getNavigator() instanceof PathNavigateGround) return;
    ...
    public boolean continueExecuting() {
        if (this.petPathfinder.noPath()) return false;
        if (!(this.thePet.getDistanceSqToEntity(this.theOwner) > (double)(this.maxDist * this.maxDist))) return false;
    ...
    public void updateTask() {
        ...
        if (this.thePet.isSitting()) return;
        if (--this.field_75343_h > 0) return;
        this.field_75343_h = 10;
        if (this.petPathfinder.tryMoveToEntityLiving(this.theOwner, this.followSpeed)) return;
        if (this.thePet.getLeashed()) return;
        if (!(this.thePet.getDistanceSqToEntity(this.theOwner) >= 144.0)) return;
