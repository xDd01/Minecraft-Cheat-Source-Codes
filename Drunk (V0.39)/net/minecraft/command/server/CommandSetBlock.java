...
extends CommandBase {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        ...
        if (args.length > 0 && args.length <= 3) {
            list = CommandSetBlock.func_175771_a(args, 0, pos);
            return list;
        }
        if (args.length == 4) {
            list = CommandSetBlock.getListOfStringsMatchingLastWord(args, Block.blockRegistry.getKeys());
            return list;
        }
        if (args.length != 6) return null;
        list = CommandSetBlock.getListOfStringsMatchingLastWord(args, "replace", "destroy", "keep");
        return list;
    }
}
