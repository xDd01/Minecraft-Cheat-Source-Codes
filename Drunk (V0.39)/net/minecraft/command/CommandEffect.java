...
extends CommandBase {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
        List<String> list;
        if (args.length == 1) {
            list = CommandEffect.getListOfStringsMatchingLastWord(args, this.getAllUsernames());
            return list;
        }
        if (args.length == 2) {
            list = CommandEffect.getListOfStringsMatchingLastWord(args, Potion.func_181168_c());
            return list;
        }
        if (args.length != 5) return null;
        list = CommandEffect.getListOfStringsMatchingLastWord(args, "true", "false");
        return list;
    }
