...
extends Item {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (worldIn.getBlockState(pos.up()).getBlock().getMaterial() != Material.air) return false;
        if (block == Blocks.grass) {
            return this.useHoe(stack, playerIn, worldIn, pos, Blocks.farmland.getDefaultState());
        }
        if (block != Blocks.dirt) return false;
        switch (1.$SwitchMap$net$minecraft$block$BlockDirt$DirtType[iblockstate.getValue(BlockDirt.VARIANT).ordinal()]) {
            case 1: {
                return this.useHoe(stack, playerIn, worldIn, pos, Blocks.farmland.getDefaultState());
            }
            case 2: {
