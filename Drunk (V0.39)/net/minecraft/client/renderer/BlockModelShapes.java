...
public class BlockModelShapes {
    ...
    public TextureAtlasSprite getTexture(IBlockState state) {
        ...
        if (ibakedmodel == null || ibakedmodel == this.modelManager.getMissingModel()) {
            if (block == Blocks.wall_sign) return this.modelManager.getTextureMap().getAtlasSprite("minecraft:blocks/planks_oak");
            if (block == Blocks.standing_sign) return this.modelManager.getTextureMap().getAtlasSprite("minecraft:blocks/planks_oak");
            if (block == Blocks.chest) return this.modelManager.getTextureMap().getAtlasSprite("minecraft:blocks/planks_oak");
            if (block == Blocks.trapped_chest) return this.modelManager.getTextureMap().getAtlasSprite("minecraft:blocks/planks_oak");
            if (block == Blocks.standing_banner) return this.modelManager.getTextureMap().getAtlasSprite("minecraft:blocks/planks_oak");
            if (block == Blocks.wall_banner) {
            ...
            if (block == Blocks.flowing_lava) return this.modelManager.getTextureMap().getAtlasSprite("minecraft:blocks/lava_still");
            if (block == Blocks.lava) {
                return this.modelManager.getTextureMap().getAtlasSprite("minecraft:blocks/lava_still");
            }
            if (block == Blocks.flowing_water) return this.modelManager.getTextureMap().getAtlasSprite("minecraft:blocks/water_still");
        ...
        if (ibakedmodel != null) return ibakedmodel.getParticleTexture();
    ...
    public void reloadModels() {
        ...
        Iterator<Map.Entry<IBlockState, ModelResourceLocation>> iterator = this.blockStateMapper.putAllStateModelLocations().entrySet().iterator();
        while (iterator.hasNext()) {
            Map.Entry<IBlockState, ModelResourceLocation> entry = iterator.next();
    ...
    private void registerAllBlocks() {
        ...
        this.registerBlockWithStateMapper(Blocks.quartz_block, new StateMapperBase(){
            ...
            protected ModelResourceLocation getModelResourceLocation(IBlockState state) {
                ...
                switch (8.$SwitchMap$net$minecraft$block$BlockQuartz$EnumType[blockquartz$enumtype.ordinal()]) {
                    default: {
        ...
        this.registerBlockWithStateMapper(Blocks.pumpkin_stem, new StateMapperBase(){
            ...
            protected ModelResourceLocation getModelResourceLocation(IBlockState state) {
                ...
                if (state.getValue(BlockStem.FACING) == EnumFacing.UP) return new ModelResourceLocation((ResourceLocation)Block.blockRegistry.getNameForObject(state.getBlock()), this.getPropertyString(map));
        ...
        this.registerBlockWithStateMapper(Blocks.dirt, new StateMapperBase(){
            ...
            protected ModelResourceLocation getModelResourceLocation(IBlockState state) {
                ...
                if (BlockDirt.DirtType.PODZOL == state.getValue(BlockDirt.VARIANT)) return new ModelResourceLocation(s, this.getPropertyString(map));
