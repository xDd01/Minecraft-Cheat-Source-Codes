...
import drunkclient.beta.NOT.NotificationManager;
...
implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        Vec3 vec33 = null;
        float f = 1.0f;
        List<Entity> list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand(f, f, f), Predicates.and(EntitySelectors.NOT_SPECTATING, new EntityRenderer1(this)));
        double d2 = d1;
    ...
    private void updateLightmap(float partialTicks) {
        ...
        if (CustomColorizer.updateLightmap(worldclient, this.torchFlickerX, this.lightmapColors, Minecraft.thePlayer.isPotionActive(Potion.nightVision))) {
    ...
    public void func_181560_a(float p_181560_1_, long p_181560_2_) {
        ...
        if (!(flag || !this.mc.gameSettings.pauseOnLostSwider || this.mc.gameSettings.touchscreen && Mouse.isButtonDown((int)1))) {
        ...
        if (flag && Minecraft.isRunningOnMac && this.mc.inGameHasSwider && !Mouse.isInsideWindow()) {
            Mouse.setGrabbed((boolean)false);
            Mouse.setCursorPosition((int)(Display.getWidth() / 2), (int)(Display.getHeight() / 2));
            Mouse.setGrabbed((boolean)true);
        }
        if (this.mc.inGameHasSwider && flag) {
            ...
            if (this.mc.gameSettings.smoothCamera) {
                ...
                Minecraft.thePlayer.setAngles(f2, f3 * (float)b0);
            } else {
                this.smoothCamYaw = 0.0f;
                this.smoothCamPitch = 0.0f;
                Minecraft.thePlayer.setAngles(f2, f3 * (float)b0);
    ...
    private void renderWorldDirections(float partialTicks) {
        ...
        if (Minecraft.thePlayer.hasReducedDebug()) return;
    ...
    private void renderCloudsCheck(RenderGlobal renderGlobalIn, float partialTicks, int pass) {
        if (this.mc.gameSettings.renderDistanceChunks < 4) return;
    ...
    private void waitForServerThread() {
        ...
        if (Config.isSmoothWorld() && Config.isSingleProcessor()) {
            if (!this.mc.isIntegratedServerRunning()) return;
    ...
    private void frameInit() {
        ...
        if (this.updatedWorld == world) return;
    ...
    private void frameFinish() {
        ...
        if (i <= this.lastErrorCheckTimeMs + 10000L) return;
