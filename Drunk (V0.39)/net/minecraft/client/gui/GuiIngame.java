...
extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (Minecraft.thePlayer.getSleepTimer() > 0) {
            ...
            l = Minecraft.thePlayer.getSleepTimer();
            f2 = (float)l / 100.0f;
            if (f2 > 1.0f) {
                f2 = 1.0f - (float)(l - 100) / 10.0f;
            }
            k = (int)(220.0f * f2) << 24 | 0x101020;
        ...
        if (this.recordPlayingUpFor > 0) {
            ...
            if (k1 > 8) {
                ...
                this.getFontRenderer().drawString(this.recordPlaying, -this.getFontRenderer().getStringWidth(this.recordPlaying) / 2, -4.0f, i1 + (k1 << 24 & -16777216));
                GlStateManager.disableBlend();
        ...
        if (this.field_175195_w > 0) {
            ...
            if (this.field_175195_w > this.field_175193_B + this.field_175192_A) {
                f1 = (float)(this.field_175199_z + this.field_175192_A + this.field_175193_B) - f4;
        ...
        v0 = scoreobjective1 = scoreobjective != null ? scoreobjective : scoreboard.getObjectiveInDisplaySlot(1);
        ...
        if (this.mc.isIntegratedServerRunning()) {
            ** if (Minecraft.thePlayer.sendQueue.getPlayerInfoMap().size() > 1 || scoreobjective1 != null) goto lbl-1000
        }
        ** GOTO lbl-1000
lbl-1000:
        // 2 sources
...
lbl148:
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
        ...
        float f = this.zLevel;
        this.zLevel = -90.0f;
        this.drawTexturedModalRect(i - 91 - 1 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 22 - 1 - 23, 0, 22, 24, 22);
        this.zLevel = f;
        ...
        while (true) {
            ...
            this.renderHotbarItem(j, k, l - 23, partialTicks, entityplayer);
            ++j;
        }
    }
    ...
    public void renderHorseJumpBar(ScaledResolution p_175186_1_, int p_175186_2_) {
        ...
        this.drawTexturedModalRect(p_175186_2_, j + 20, 0, 84, short1, 5);
        if (i > 0) {
            this.drawTexturedModalRect(p_175186_2_, j + 20, 0, 89, i, 5);
        }
        this.mc.mcProfiler.endSection();
    }
    ...
    public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_) {
        ...
        if (i > 0) {
            int short1 = 182;
            int k = (int)(Minecraft.thePlayer.experience * (float)(short1 + 1));
            int j = p_175176_1_.getScaledHeight() - 32 + 3;
            this.drawTexturedModalRect(p_175176_2_, j - 23, 0, 64, short1, 5);
            if (k > 0) {
                this.drawTexturedModalRect(p_175176_2_, j - 21, 0, 69, k, 5);
            }
        }
        this.mc.mcProfiler.endSection();
        if (Minecraft.thePlayer.experienceLevel <= 0) return;
        this.mc.mcProfiler.startSection("expLevel");
        int j1 = 8453920;
        String s = "" + Minecraft.thePlayer.experienceLevel;
        int i1 = (p_175176_1_.getScaledWidth() - this.getFontRenderer().getStringWidth(s)) / 2;
        int l = p_175176_1_.getScaledHeight() - 31 - 4;
        boolean flag = false;
        this.getFontRenderer().drawString(s, i1 + 1, l + 20, 0);
        this.getFontRenderer().drawString(s, i1 - 1, l + 20, 0);
        this.getFontRenderer().drawString(s, i1, l + 1 + 20, 0);
        this.getFontRenderer().drawString(s, i1, l - 1 + 20, 0);
        this.getFontRenderer().drawString(s, i1, l + 20, j1);
        this.mc.mcProfiler.endSection();
    }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        while (iterator2.hasNext()) {
            ...
            if (k == 1) {
                s1 = s1.replace("&ewww.kokscraft.pl", "&5Drunk Client");
                s1 = "\u00a7cDrunk\u00a74 Client";
            }
            ...
            GuiIngame.drawRect(j - 2, l - this.getFontRenderer().FONT_HEIGHT - 3, i1, l - 10, HUD.color);
            GuiIngame.drawRect(j - 2, l - 1, i1, l, 0x50000000);
            this.getFontRenderer().drawString(s3, j + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2, l - this.getFontRenderer().FONT_HEIGHT, HUD.color);
        }
    }
    ...
    private void renderPlayerStats(ScaledResolution p_180477_1_) {
        ...
        for (int j5 = MathHelper.ceiling_float_int((f + f1) / 2.0f) - 1; j5 >= 0; --j5) {
            ...
            this.drawTexturedModalRect(l3, i4 - 23, 16 + b0 * 9, 10 * b1, 9, 9);
            if (flag) {
                if (j5 * 2 + 1 < j) {
                    this.drawTexturedModalRect(l3, i4 - 23, k5 + 54, 10 * b1, 9, 9);
                }
                if (j5 * 2 + 1 == j) {
                    this.drawTexturedModalRect(l3, i4 - 23, k5 + 63, 9 * b1, 9, 9);
                }
            }
            if (f2 <= 0.0f) {
                if (j5 * 2 + 1 < i) {
                    this.drawTexturedModalRect(l3, i4 - 23, k5 + 36, 9 * b1, 7, 9);
                }
                if (j5 * 2 + 1 != i) continue;
                this.drawTexturedModalRect(l3, i4 - 23, k5 + 45, 9 * b1, 9, 9);
                continue;
            }
            if (f2 == f1 && f1 % 2.0f == 1.0f) {
                this.drawTexturedModalRect(l3, i4 - 23, k5 + 153, 9 * b1, 9, 9);
            } else {
                this.drawTexturedModalRect(l3, i4 - 23, k5 + 144, 9 * b1, 9, 9);
            }
            f2 -= 2.0f;
        }
        ...
        if (entity == null) {
            ...
            for (int l5 = 0; l5 < 10; ++l5) {
                ...
                int k7 = j1 - l5 * 8 - 9;
                this.drawTexturedModalRect(k7, i8 - 23, 16 + b4 * 9, 27, 9, 9);
                if (flag1) {
                    if (l5 * 2 + 1 < l) {
                        this.drawTexturedModalRect(k7, i8 - 23, j6 + 54, 27, 9, 9);
                    }
                    if (l5 * 2 + 1 == l) {
                        this.drawTexturedModalRect(k7, i8 - 23, j6 + 63, 27, 9, 9);
                    }
                }
                if (l5 * 2 + 1 < k) {
                    this.drawTexturedModalRect(k7, i8 - 23, j6 + 36, 27, 9, 9);
                }
                if (l5 * 2 + 1 != k) continue;
                this.drawTexturedModalRect(k7, i8 - 23, j6 + 45, 27, 9, 9);
            }
            ...
            while (l6 > 0) {
                ...
                for (int l4 = 0; l4 < k4; ++l4) {
                    ...
                    int i5 = j1 - l4 * 8 - 9;
                    this.drawTexturedModalRect(i5, j7 + 19, b2 + b3 * 9, 9, 9, 9);
                    if (l4 * 2 + 1 + j4 < l7) {
                        this.drawTexturedModalRect(i5, j7 + 19, b2 + 36, 9, 9, 9);
                    }
                    if (l4 * 2 + 1 + j4 != l7) continue;
                    this.drawTexturedModalRect(i5, j7 + 19, b2 + 45, 9, 9, 9);
                }
                j7 -= 10;
                j4 += 20;
            }
        ...
        if (entityplayer.isInsideOfMaterial(Material.water)) {
            int i6 = Minecraft.thePlayer.getAir();
            ...
            for (int i7 = 0; i7 < j8 + k6; ++i7) {
                if (i7 < j8) {
                    this.drawTexturedModalRect(j1 - i7 * 8 - 9, j2 + 19, 16, 18, 9, 9);
                    continue;
                }
                this.drawTexturedModalRect(j1 - i7 * 8 - 9, j2 + 19, 25, 18, 9, 9);
            }
        }
        this.mc.mcProfiler.endSection();
    }
    ...
    private void renderVignette(float p_180480_1_, ScaledResolution p_180480_2_) {
        if (!Config.isVignetteEnabled()) return;
