...
extends MapGenStructure {
    ...
    public MapGenScatteredFeature(Map<String, String> p_i2061_1_) {
        this();
        Iterator<Map.Entry<String, String>> iterator = p_i2061_1_.entrySet().iterator();
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        if (i != (k += random.nextInt(this.maxDistanceBetweenScatteredFeatures - this.minDistanceBetweenScatteredFeatures))) return false;
        if (j != (l += random.nextInt(this.maxDistanceBetweenScatteredFeatures - this.minDistanceBetweenScatteredFeatures))) return false;
        BiomeGenBase biomegenbase = this.worldObj.getWorldChunkManager().getBiomeGenerator(new BlockPos(i * 16 + 8, 0, j * 16 + 8));
        if (biomegenbase == null) {
            return false;
        }
        Iterator<BiomeGenBase> iterator = biomelist.iterator();
        do {
            if (!iterator.hasNext()) return false;
        } while (biomegenbase != (biomegenbase1 = iterator.next()));
        return true;
    }
    ...
    public boolean func_175798_a(BlockPos p_175798_1_) {
        ...
        if (!(structurestart instanceof Start)) return false;
        if (structurestart.components.isEmpty()) return false;
