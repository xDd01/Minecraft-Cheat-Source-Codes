...
public class StructureMineshaftPieces {
    ...
    private static StructureComponent func_175892_a(List<StructureComponent> listIn, Random rand, int x, int y, int z, EnumFacing facing, int type) {
        ...
        if (i >= 70) {
            ...
            if (structureboundingbox1 == null) return null;
        ...
        if (structureboundingbox2 == null) return null;
    ...
    private static StructureComponent func_175890_b(StructureComponent componentIn, List<StructureComponent> listIn, Random rand, int x, int y, int z, EnumFacing facing, int type) {
        ...
        if (Math.abs(x - componentIn.getBoundingBox().minX) > 80) return null;
        if (Math.abs(z - componentIn.getBoundingBox().minZ) > 80) return null;
    ...
    extends StructureComponent {
        ...
        public void buildComponent(StructureComponent componentIn, List<StructureComponent> listIn, Random rand) {
            ...
            while (k < this.boundingBox.getZSize()) {
                if ((k += rand.nextInt(this.boundingBox.getZSize())) + 3 > this.boundingBox.getZSize()) {
                    return;
                }
    ...
    extends StructureComponent {
        ...
        public void buildComponent(StructureComponent componentIn, List<StructureComponent> listIn, Random rand) {
            int i = this.getComponentType();
            switch (1.$SwitchMap$net$minecraft$util$EnumFacing[this.corridorDirection.ordinal()]) {
                case 1: {
            ...
            if (!this.isMultipleFloors) return;
            if (rand.nextBoolean()) {
    ...
    extends StructureComponent {
        ...
        public Corridor(int type, Random rand, StructureBoundingBox structurebb, EnumFacing facing) {
            ...
            boolean bl = this.hasSpiders = !this.hasRails && rand.nextInt(23) == 0;
        ...
        protected boolean generateChestContents(World worldIn, StructureBoundingBox boundingBoxIn, Random rand, int x, int y, int z, List<WeightedRandomChestContent> listIn, int max) {
            ...
            if (worldIn.getBlockState(blockpos).getBlock().getMaterial() != Material.air) return false;
