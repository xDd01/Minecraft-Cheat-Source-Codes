...
public class FlatGeneratorInfo {
    ...
    public void func_82645_d() {
        int i = 0;
        Iterator<FlatLayerInfo> iterator = this.flatLayers.iterator();
    ...
    public String toString() {
        ...
        stringbuilder.append(";");
        int k = 0;
        Iterator<Map.Entry<String, Map<String, String>>> iterator = this.worldFeatures.entrySet().iterator();
        while (iterator.hasNext()) {
    ...
    private static List<FlatLayerInfo> func_180716_a(int p_180716_0_, String p_180716_1_) {
        if (p_180716_1_ == null) return null;
        if (p_180716_1_.length() < 1) return null;
    ...
    public static FlatGeneratorInfo createFlatGeneratorFromString(String p_82651_0_) {
        ...
        if (i < 0) return FlatGeneratorInfo.getDefaultFlatGenerator();
        if (i > 3) return FlatGeneratorInfo.getDefaultFlatGenerator();
        FlatGeneratorInfo flatgeneratorinfo = new FlatGeneratorInfo();
        int j = astring.length == 1 ? 0 : 1;
        List<FlatLayerInfo> list = FlatGeneratorInfo.func_180716_a(i, astring[j++]);
        if (list == null) return FlatGeneratorInfo.getDefaultFlatGenerator();
        if (list.isEmpty()) return FlatGeneratorInfo.getDefaultFlatGenerator();
        ...
        String[] stringArray = astring1 = astring[j++].toLowerCase().split(",");
        int n = stringArray.length;
        int n2 = 0;
        while (n2 < n) {
            String s = stringArray[n2];
