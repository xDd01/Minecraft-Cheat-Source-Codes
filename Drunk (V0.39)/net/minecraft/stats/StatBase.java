...
public class StatBase {
    ...
    public static IStatType timeStatType = new IStatType(){
        ...
        public String format(int p_75843_1_) {
            ...
            double d4 = d3 / 365.0;
            if (d4 > 0.5) {
                string = decimalFormat.format(d4) + " y";
                return string;
            }
            if (d3 > 0.5) {
                string = decimalFormat.format(d3) + " d";
                return string;
            }
            if (d2 > 0.5) {
                string = decimalFormat.format(d2) + " h";
                return string;
            }
            if (d1 > 0.5) {
                string = decimalFormat.format(d1) + " m";
                return string;
            }
            string = d0 + " s";
            return string;
    ...
    public static IStatType distanceStatType = new IStatType(){
        ...
        public String format(int p_75843_1_) {
            ...
            double d1 = d0 / 1000.0;
            if (d1 > 0.5) {
                string = decimalFormat.format(d1) + " km";
                return string;
            }
            if (d0 > 0.5) {
                string = decimalFormat.format(d0) + " m";
                return string;
            }
            string = p_75843_1_ + " cm";
            return string;
        }
    };
