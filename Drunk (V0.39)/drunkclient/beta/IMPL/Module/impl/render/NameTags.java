...
extends Module {
    ...
    public NameTags() {
        super("NameTags", new String[]{"NameTags", "Tags"}, Type.RENDER, "NameTags");
    }
    ...
    private void onRender(EventRender3D render) {
        ...
        for (EntityLivingBase player2 : NameTags.mc.theWorld.playerEntities) {
    ...
    private void drawNames(EntityPlayer player) {
        ...
        RenderUtil.drawBorderedRect(nw + 6.0f, -1.0f, width, 10.0f, 1.0f, new Color(20, 20, 20, 0).getRGB(), new Color(10, 10, 10, 200).getRGB());
        GlStateManager.disableDepth();
        offset += (float)(this.getWidth(this.getHealth(player)) + this.getWidth(" ") - 1);
        this.drawString(this.getPlayerName(player), w - offset + 2.0f, 0.0f, 0xFFFFFF);
    ...
    private void drawString(String text, float x, float y, int color) {
        NameTags.mc.fontRendererObj.drawStringWithShadow(text, x, y, color);
    }

    private int getWidth(String text) {
        return NameTags.mc.fontRendererObj.getStringWidth(text);
    }

    private void startDrawing(float x, float y, float z, EntityPlayer player) {
        float var10001 = NameTags.mc.gameSettings.thirdPersonView == 2 ? -1.0f : 1.0f;
        double size = Config.zoomMode ? (double)(this.getSize(player) / 10.0f) * 1.6 : (double)(this.getSize(player) / 10.0f) * 4.8;
        GL11.glPushMatrix();
        RenderUtil.startDrawing();
        GL11.glTranslatef((float)x, (float)y, (float)z);
        GL11.glNormal3f((float)0.0f, (float)1.0f, (float)0.0f);
        GL11.glRotatef((float)(-NameTags.mc.getRenderManager().playerViewY), (float)0.0f, (float)1.0f, (float)0.0f);
        GL11.glRotatef((float)NameTags.mc.getRenderManager().playerViewX, (float)var10001, (float)0.0f, (float)0.0f);
    ...
    private float getSize(EntityPlayer player) {
        ...
        float f = Minecraft.thePlayer.getDistanceToEntity(player) / 4.0f;
        return f;
    }
    ...
    private void renderEnchantText(ItemStack stack, int x, int y) {
        ...
        if (stack.getEnchantmentTagList() != null && stack.getEnchantmentTagList().tagCount() >= 6) {
            NameTags.mc.fontRendererObj.drawStringWithShadow("god", x * 2, enchantmentY, 0xFF0000);
            return;
        }
        if (stack.getItem() instanceof ItemArmor) {
            ...
            if (protectionLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("prot" + protectionLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (projectileProtectionLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("proj" + projectileProtectionLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (blastProtectionLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("bp" + blastProtectionLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (fireProtectionLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("frp" + fireProtectionLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (thornsLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("th" + thornsLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (unbreakingLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("unb" + unbreakingLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
        }
        if (stack.getItem() instanceof ItemBow) {
            ...
            if (powerLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("pow" + powerLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (punchLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("pun" + punchLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (flameLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("flame" + flameLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (unbreakingLevel2 > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("unb" + unbreakingLevel2, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
        }
        if (stack.getItem() instanceof ItemSword) {
            ...
            unbreakingLevel2 = EnchantmentHelper.getEnchantmentLevel(Enchantment.unbreaking.effectId, stack);
            if (sharpnessLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("sh" + sharpnessLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (knockbackLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("kb" + knockbackLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (fireAspectLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("fire" + fireAspectLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (unbreakingLevel2 > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("unb" + unbreakingLevel2, x * 2, enchantmentY, -1);
        ...
        if (stack.getItem() instanceof ItemTool) {
            ...
            if (efficiencyLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("eff" + efficiencyLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (fortuneLevel > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("fo" + fortuneLevel, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (silkTouch > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("silk" + silkTouch, x * 2, enchantmentY, -1);
                enchantmentY += 8;
            }
            if (unbreakingLevel22 > 0) {
                NameTags.mc.fontRendererObj.drawStringWithShadow("ub" + unbreakingLevel22, x * 2, enchantmentY, -1);
            }
        }
        if (stack.getItem() != Items.golden_apple) return;
        if (!stack.hasEffect()) return;
        NameTags.mc.fontRendererObj.drawStringWithShadow("god", x * 2, enchantmentY, -1);
    }
}
