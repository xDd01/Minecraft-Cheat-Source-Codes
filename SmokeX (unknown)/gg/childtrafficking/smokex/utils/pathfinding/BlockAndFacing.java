...
public final class BlockAndFacing
{
    private static Minecraft mc;
    private BlockPos currentPos;
    
    private void setBlockAndFacing(final BlockPos var1) {
        if (BlockAndFacing.mc.theWorld.getBlockState(var1.add(0, -1, 0)).getBlock() != Blocks.air) {
            this.currentPos = var1.add(0, -1, 0);
            final EnumFacing currentFacing = EnumFacing.UP;
        }
        else if (BlockAndFacing.mc.theWorld.getBlockState(var1.add(-1, 0, 0)).getBlock() != Blocks.air) {
            this.currentPos = var1.add(-1, 0, 0);
            final EnumFacing currentFacing = EnumFacing.EAST;
        }
        else if (BlockAndFacing.mc.theWorld.getBlockState(var1.add(1, 0, 0)).getBlock() != Blocks.air) {
            this.currentPos = var1.add(1, 0, 0);
            final EnumFacing currentFacing = EnumFacing.WEST;
        }
        else if (BlockAndFacing.mc.theWorld.getBlockState(var1.add(0, 0, -1)).getBlock() != Blocks.air) {
            this.currentPos = var1.add(0, 0, -1);
            final EnumFacing currentFacing = EnumFacing.SOUTH;
        }
        else if (BlockAndFacing.mc.theWorld.getBlockState(var1.add(0, 0, 1)).getBlock() != Blocks.air) {
            this.currentPos = var1.add(0, 0, 1);
            final EnumFacing currentFacing = EnumFacing.NORTH;
        }
        else {
            this.currentPos = null;
            final EnumFacing currentFacing = null;
        }
    }
    
    static {
        BlockAndFacing.mc = Minecraft.getMinecraft();
    }
