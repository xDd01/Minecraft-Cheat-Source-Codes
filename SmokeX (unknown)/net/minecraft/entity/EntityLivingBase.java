...
public abstract class EntityLivingBase extends Entity
{
    ...
    public PotionEffect getActivePotionEffect(final int potionIn) {
    ...
    private float getArmSwingAnimationEnd() {
        return this.isPotionActive(Potion.digSpeed) ? ((float)(6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()))) : ((float)(int)(this.isPotionActive(Potion.digSlowdown) ? ((float)(6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2)) : (ModuleManager.getInstance(AnimationsModule.class).isToggled() ? (ModuleManager.getInstance(AnimationsModule.class).swingSpeed.getValue() * Minecraft.getMinecraft().timer.timerSpeed) : 6.0f)));
    }
    
    public void swingItem() {
        if (!this.isSwingInProgress || this.swingProgressInt >= this.getArmSwingAnimationEnd() / 2.0f || this.swingProgressInt < 0) {
    ...
    protected void updateArmSwingProgress() {
        final float i = this.getArmSwingAnimationEnd();
