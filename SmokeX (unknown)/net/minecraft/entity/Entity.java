...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            if (this.stepHeight > 0.0f && flag2 && (d4 != x || d6 != z)) {
                ...
                final EventStep eventStep = new EventStep(this.stepHeight);
                if (isPlayer) {
                    SmokeXClient.getInstance().getEventDispatcher().dispatch(eventStep);
                }
                final double d8 = x;
                final double d9 = y;
                final double d10 = z;
                ...
                this.setEntityBoundingBox(this.getEntityBoundingBox().offset(0.0, y, 0.0));
                final double blockHeight = eventStep.getStepHeight() + y;
                if (isPlayer && blockHeight <= eventStep.getStepHeight() && blockHeight > 0.5) {
                    eventStep.setHeightStepped(blockHeight);
                    eventStep.setPost();
                    SmokeXClient.getInstance().getEventDispatcher().dispatch(eventStep);
                }
    ...
    public static final Vec3 getVectorForRotation(final float pitch, final float yaw) {
