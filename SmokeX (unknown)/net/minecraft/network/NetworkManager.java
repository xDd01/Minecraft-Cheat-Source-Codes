...
public class NetworkManager extends SimpleChannelInboundHandler<Packet>
{
    ...
    protected void channelRead0(final ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            try {
                final EventReceivePacket eventReceivePacket = new EventReceivePacket(p_channelRead0_2_);
                SmokeXClient.getInstance().getEventDispatcher().dispatch(eventReceivePacket);
                if (eventReceivePacket.cancelled()) {
                    return;
                }
                p_channelRead0_2_ = eventReceivePacket.getPacket();
