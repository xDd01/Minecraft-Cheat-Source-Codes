...
import gg.childtrafficking.smokex.event.events.player.EventDamageEntity;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
            ...
            if (entity instanceof EntityLivingBase && !(entity instanceof EntityPlayerSP)) {
                ...
                if (watchableObject.getDataValueId() == 6 && watchableObject.getObjectType() == 3) {
                    ...
                    if (dif >= 0.0f && newHealth != 10.0f) {
                        SmokeXClient.getInstance().getEventDispatcher().dispatch(new EventDamageEntity(elb, dif));
                    }
                }
            }
    ...
    public void handlePlayerPosLook(final S08PacketPlayerPosLook packetIn) {
        ...
        final NoRotateModule noRotateModule = ModuleManager.getInstance(NoRotateModule.class);
        if (noRotateModule != null && noRotateModule.isToggled() && noRotateModule.modeEnumProperty.getValue() == NoRotateModule.Mode.WATCHDOG) {
            entityplayer.setPosition(d0, d2, d3);
            this.netManager.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, f, f2, false));
            this.netManager.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, (float)(f + Math.random()), (float)(f2 + Math.random()), false));
        }
        else {
    ...
    public void addToSendQueue(final Packet packet) {
        ...
        SmokeXClient.getInstance().getEventDispatcher().dispatch(eventSendPacket);
        if (eventSendPacket.cancelled()) {
            return;
        }
