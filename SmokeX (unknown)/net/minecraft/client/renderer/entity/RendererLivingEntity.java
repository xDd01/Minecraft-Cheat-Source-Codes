    ...
    public void doRender(final T entity, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            try {
                ...
                final boolean showRots = entity instanceof EntityPlayerSP && (playerSP = (EntityPlayerSP)entity).currentEvent.isRotating();
                if (entity instanceof EntityPlayerSP) {
                    playerSP = (EntityPlayerSP)entity;
                }
                float f2;
                float f;
                if (showRots) {
                    final EventUpdate event = playerSP.currentEvent;
                    f = (f2 = this.interpolateRotation(event.getPrevYaw(), event.getYaw(), partialTicks));
                }
                else {
                    f = this.interpolateRotation(entity.prevRenderYawOffset, entity.renderYawOffset, partialTicks);
                    f2 = this.interpolateRotation(entity.prevRotationYawHead, entity.rotationYawHead, partialTicks);
                }
                float f3 = f2 - f;
                ...
                float f5;
                if (showRots) {
                    final EventUpdate event2 = playerSP.currentEvent;
                    f5 = (f6 = this.interpolateRotation(event2.getPrevPitch(), event2.getPitch(), partialTicks));
                }
                else {
                    f5 = (f6 = entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialTicks);
                }
    ...
    public void renderName(final T entity, final double x, final double y, final double z) {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z)) {
            if (this.canRenderName(entity)) {
                final EventRenderNameTag eventRenderNameTag = new EventRenderNameTag(entity);
                SmokeXClient.getInstance().getEventDispatcher().dispatch(eventRenderNameTag);
                if (eventRenderNameTag.cancelled()) {
                    return;
                }
