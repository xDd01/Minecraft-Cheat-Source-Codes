...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext context, Packet packetIn) { 
    	if (this.channel.isOpen()) {
    		try { 
    			ProcessPacketEvent processPacketEvent = new ProcessPacketEvent(packetIn);
    			Sensation.eventBus.publish(processPacketEvent);

                /*if (packetIn instanceof S3FPacketCustomPayload) {
                    Minecraft.getMinecraft().thePlayer.sendQueue.addToSendQueueNoEvent(new C17PacketCustomPayload("MC|Sensation", (new PacketBuffer(Unpooled.buffer())).writeString("vanilla")));
                    //SC|UL62BFQ
                    //CRYSTAL|KZ1LM9TO
                }*/
    			if (processPacketEvent.isCancelled() || packetIn instanceof S48PacketResourcePackSend)
    				return;

    			processPacketEvent.getPacket().processPacket(this.packetListener);
