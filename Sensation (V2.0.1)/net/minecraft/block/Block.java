...
import me.vaziak.sensation.client.impl.visual.XRay;
...
public class Block {
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
    	...
    	if (Sensation.instance.cheatManager.isModuleEnabled("XRay")) {
    		if (XRay.blocks.containsItem(Item.getItemFromBlock(this))) {
    			return true;
    		}
    	}
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity) {
        ...
        BoundingBoxEvent event = new BoundingBoxEvent(this, pos, axisalignedbb, true);
        if (collidingEntity == Minecraft.getMinecraft().thePlayer) {
            Sensation.eventBus.publish(event);
        }
        if ((axisalignedbb = event.getBoundingBox()) == null) return;
    ...
    public EnumWorldBlockLayer getBlockLayer() {
    	if (Sensation.instance.cheatManager.isModuleEnabled("XRay")) {
    			return XRay.blocks.containsItem(Item.getItemFromBlock(this)) ? EnumWorldBlockLayer.SOLID : EnumWorldBlockLayer.TRANSLUCENT;
    	}
