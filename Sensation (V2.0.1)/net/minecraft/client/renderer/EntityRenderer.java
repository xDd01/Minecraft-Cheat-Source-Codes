...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	public void getMouseOver(float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			if (this.mc.playerController.extendedReach()) {
				...
				if (d0 > (Sensation.instance.cheatManager.isModuleEnabled("Kill Aura") ? 4 : 0)) {
					flag = true;
				}
			}
	...
	private void hurtCameraEffect(float partialTicks) {
		if (!Sensation.instance.cheatManager.isModuleEnabled("No Hurt Cam")) {
			...
			if (this.mc.gameSettings.debugCamEnable) {
				...
				for (int i = 0; i < 8; ++i) {
					...
					if (movingobjectposition != null) {
						...
						if (!Sensation.instance.cheatManager.isModuleEnabled("ViewClip") && d7 < d3) {
							d3 = d7;
						}
					}
				}
	...
	private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
		...
		Sensation.eventBus.publish(new EntityRenderEvent(partialTicks));
	...
	private void updateMainMenu(MainMenuGui currentScreen) {
		try {
			String s = null;
			...
			Field[] afield = MainMenuGui.class.getDeclaredFields();
