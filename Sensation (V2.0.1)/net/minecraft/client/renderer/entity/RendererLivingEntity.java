...
import me.vaziak.sensation.client.impl.visual.Chams;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        try
        {
            ...
            if (Sensation.instance.cheatManager.isModuleEnabled("Kill Aura")) {
            	f7 = Minecraft.getMinecraft().thePlayer.getServerSidePitch();
            }
            ...
            else
            {
                ...
                if (valid) {
                	boolean check = (Chams.viewCheck.getValue() && !Minecraft.getMinecraft().thePlayer.canEntityBeSeen(entity)) || !Chams.viewCheck.getValue();
                    
                    boolean shouldFill = Sensation.instance.cheatManager.isModuleEnabled("Chams") && check;

                	 if (shouldFill) {
                        Minecraft mc = Minecraft.getMinecraft();
                        mc.entityRenderer.enableLightmap();
                        if (Chams.mode.getValue().get("Rainbow") || Chams.mode.getValue().get("Fill")) {
                        	if (check) {
                        		Draw.glColor( Chams.mode.getValue().get("Fill") ? Chams.fillColor.getValue().getRGB() : ColorCreator.createRainbowFromOffset(3200, 1, 6));
                        	}
                        }

                        if (Chams.mode.getValue().get("Hurttime")){
                        	//ff5447
                        	
                        	if (entity.hurtTime > 0) {
                        		Draw.glColor(new Color(255, 71, 71, 100).getRGB());
                        	} else {
                        		Draw.glColor(new Color(71, 255, 96, 150).getRGB());
                        	}
                        }
                        if (!Chams.mode.getValue().get("Normal")) {
	                        GL11.glPushMatrix();
        ...
        Sensation.instance.eventBus.publish(ePost);
    }
    ...
    public void renderName(T entity, double x, double y, double z)
    {
        ...
        Sensation.eventBus.publish(renderNametagEvent);
