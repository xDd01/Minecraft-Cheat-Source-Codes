...
public class WorldRenderer {
    ...
    public void putColorMultiplier(float red, float green, float blue, int p_178978_4_) {
        ...
        if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) {
            ...
            color = color | iB << 16 | iG << 8 | iR;
        } else {
            iR = (int) ((float) (color >> 24 & 255) * red);
            iG = (int) ((float) (color >> 16 & 255) * green);
            iB = (int) ((float) (color >> 8 & 255) * blue);
            color = color & 255;
            color = color | iR << 24 | iG << 16 | iB << 8;
        }
        if (needsUpdate) {
        	color = -1;
        } 
        ...
        if (Sensation.instance.cheatManager.isModuleEnabled("XRay")) { 
            color = getColor(iR, iG, iB, XRay.trans.getValue().intValue());
        }
