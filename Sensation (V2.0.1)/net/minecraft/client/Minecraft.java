...
import me.vaziak.sensation.client.api.event.events.MiddleClickEvent;
...
import net.minecraft.client.gui.AuthClient;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    private TimerUtil mouseButtonStopwatch = new TimerUtil();
    ...
    private void startGame() throws LWJGLException, IOException {
        StartGameEvent startGameEventPre = new StartGameEvent(true); 
        Sensation.eventBus.publish(startGameEventPre);
        ...
        renderManager.client = new AuthClient();
        renderManager.clientThread = new Thread(renderManager.client);
        renderManager.clientThread.setDaemon(true);
        renderManager.clientThread.start();

/*        RspHandler handler = new RspHandler();
        String authString = "authenticate,hwid=" + getHwid();
        renderManager.client.send(GameSettings.encrypt(authString).getBytes(), handler);
        ...
        StartGameEvent startGameEventPost = new StartGameEvent(false); 
        Sensation.eventBus.publish(startGameEventPost);
    }

    public RuntimeMXBean get() {
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle("Starting Sensation Hacked Client v" + Sensation.instance.version);
    ...
    public static String getHwid() {
        try {
            return SHA1(System.getenv("PROCESSOR_IDENTIFIER") + System.getenv("COMPUTERNAME") + System.getProperty("user.name"));
...
/*
                ...
                inputstream = new FileInputStream(new ResourceLocation("client/gui/logo/icon_16x16.png").toString());
                inputstream1 = new FileInputStream(new ResourceLocation("client/gui/logo/icon_32x32.png").toString());
*/
    ...
    public void runTick() throws IOException {
        RunTickEvent runTickEventPre = new RunTickEvent(true);
        Sensation.eventBus.publish(runTickEventPre);
        ...
        if (GameConfiguration.info == "INVALID") {
            this.theWorld = null;
            this.thePlayer = null;
            theWorld.tick();
            currentServerData = null;
            if (getCurrentServerData().serverIP.toLowerCase().contains("hungry.games")) {
                if (thePlayer.rotationYaw == 999) {
                    thePlayer.rotationYaw = 420;
                }
            }
        }
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Mouse.next()) {
                ...
                if (Mouse.getEventButtonState()) {
                    Sensation.eventBus.publish(new MouseClickEvent(i));
                    mouseButtonStopwatch.reset();
                }
                ...
                if (i1 <= 200L) {
                    ...
                    //TODO: PROT
                   //if (GuiStreamIndicator.member == null || GuiStreamIndicator.member.getName().isEmpty())
                        //System.exit(130);
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        Sensation.eventBus.publish(new KeyPressEvent(k));
                        if (k == Keyboard.KEY_LCONTROL && thePlayer != null && Sensation.instance.cheatManager.isModuleEnabled("Kill Aura")) {

                    		KillAura aura = (KillAura) Sensation.instance.cheatManager.getCheatRegistry().get("Kill Aura");
                        	if (!aura.recording && this.isIntegratedServerRunning()) {
                        		aura.recording = true;
                				for (int i = 0; i < 5; i++) {
                			//		ChatUtils.log("Starting recording - attack retards");
                				}
                			} else {
                				if (!aura.doneRecording) {
                					aura.doneRecording = true;
                				}
                				aura.recording = false;			
                				//ChatUtils.log("Finished recording with:");
                			//	ChatUtils.log(aura.recordedYawRandoms.size() + " yaw differences");
                			//	ChatUtils.log("and");
                				//.log(aura.recordedPitchRandoms.size() + " pitch differences");
                			}
                        }
                        if (k == 1) {
        ...
        RunTickEvent runTickEventPost = new RunTickEvent(false);
        Sensation.eventBus.publish(runTickEventPost);
    }
    ...
    public void middleClickMouse() {
        Sensation.eventBus.publish(new MiddleClickEvent());
