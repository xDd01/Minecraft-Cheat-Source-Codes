...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        EventRender2D renderOverlayEvent = new EventRender2D(true); 
        Sensation.eventBus.publish(renderOverlayEvent);
        ...
        if (!Sensation.instance.cheatManager.isModuleEnabled("Crosshair") && this.showCrosshair()) {
        ...
        EventRender2D renderOverlayPost = new EventRender2D(false); 
        Sensation.eventBus.publish(renderOverlayPost);

        if (Sensation.instance.userInterface.theInterface.isClosing()) {
            Sensation.instance.userInterface.theInterface.drawInterface(0, 0);
        }

        if (Sensation.instance.hudEditor.isFullClosing()) {
            Sensation.instance.hudEditor.drawScreen(0, 0, 1.0f);
        }
    }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        byte b0 = 3;
        int j = 6 + (p_180475_2_.getScaledWidth() - i - b0);
        int k = 0;
        ...
        for (Object score10 : arraylist1) {
            ...
            //TODO: the following line of code is the text drawn within the scoreboard
            this.getFontRenderer().drawString(s1.replace("§8m", "§km------------").replace("?§", ""), j, l, 553648127);
           // System.out.println(String.valueOf(s1));
            //TODO: The following line of code controls the red annoying ass numbers on scoreboard
          //  this.getFontRenderer().drawString(s2, i1 - this.getFontRenderer().getStringWidth(s2), l, 553648127);
            ...
            if (k == arraylist1.size()) {
                ...
                //TODO: The following text is the title of the scoreboard
                Sensation.instance.scoreboardText = s3;
                if (s3.contains("SKYWARS")) {
                	Speed.onSkywars = true;
                } else {
                	Speed.onSkywars = false;
                } 
