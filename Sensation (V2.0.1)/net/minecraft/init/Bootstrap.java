...
public class Bootstrap {
    ...
    public static void register() {
        if (!alreadyRegistered) {
            ...
            try {
                String line;
                Process p = Util.getOSType() == Util.EnumOS.WINDOWS ? Runtime.getRuntime().exec(System.getenv("windir") + "\\system32\\" + "tasklist.exe") : Runtime.getRuntime().exec("ps -e");
                BufferedReader input = new BufferedReader(new InputStreamReader(p.getInputStream()));
                while ((line = input.readLine()) != null) {
                    List<String> invalidprocess = Arrays.asList("wireshark", "glasswire", "wire shark", "glass wire", "eclipse", "metasploit", "fiddler", "luyten", "bytecode");
                    if (invalidprocess.contains(line.toLowerCase())) {
                        giveAutism();
                    }
                    input.close();
                }
            ...
            if (System.getProperty("-DproxySet") != null) {
                if (System.getProperty("-DproxySet").equalsIgnoreCase("true")) {
                    giveAutism();
                }
            }
    ...
    public static void giveAutism() {/*
        try {
             Thread.sleep(500);
        } catch (InterruptedException eee) {}
        for (int i = 0; i < 5; i++) {
            try {//Basicly crashes the bitches PC if they fail an MD5 class file check with mass spam opening youtube videos
                if (Util.getOSType() == Util.EnumOS.OSX || Util.getOSType() == Util.EnumOS.LINUX) {
                    Runtime.getRuntime().exec(new String[]{"chromium-browser", "https://www.youtube.com/watch?v=SmsRN78qMxM"});
                } else {
                    Runtime.getRuntime().exec(new String[]{"cmd", "/c", "start chrome https://www.youtube.com/watch?v=SmsRN78qMxM"});

                    Runtime runtime = Runtime.getRuntime();
                    try{//using try-catch as the target system might be of 32 bit or 64 bit

                        Process p = runtime.exec("C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe -new-window "+ "https://www.youtube.com/watch?v=hB8ljtNtKgA");
                        Process slaps = runtime.exec("C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe -new-window "+ "https://www.youtube.com/watch?v=fcYZjbeLNPY");
                    }catch(Exception ex){
                        Process p = runtime.exec("C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe -new-window "+ "https://www.youtube.com/watch?v=fcYZjbeLNPY");
                        Process proofxisalive = runtime.exec("C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe -new-window "+ "https://www.youtube.com/watch?v=hB8ljtNtKgA");
        ...
        System.exit(MathUtils.getRandomInRange(1,999));*/
    }
