...
public class Chunk
{
    ...
    public void generateSkylightMap()
    {
        ...
        for (int j = 0; j < 16; ++j)
        {
            for (int k = 0; k < 16; ++k)
            {
                ...
                if (this.worldObj.provider.isNether())
                {
                    int k1 = 15;
                    int i1 = i + 16 - 1;
    ...
    public IBlockState setBlockState(BlockPos pos, IBlockState state)
    {
        ...
        else
        {
            ...
            if (extendedblockstorage == NULL_BLOCK_STORAGE)
            {
                ...
                extendedblockstorage = new ExtendedBlockStorage(j >> 4 << 4, this.worldObj.provider.isNether());
    ...
    public int getLightFor(EnumSkyBlock p_177413_1_, BlockPos pos)
    {
        ...
        {
            return !this.worldObj.provider.isNether() ? 0 : extendedblockstorage.getExtSkylightValue(i, j & 15, k);
        }
        else
        {
    ...
    public int getLightSubtracted(BlockPos pos, int amount)
    {
        ...
        if (extendedblockstorage == NULL_BLOCK_STORAGE)
        {
            return this.worldObj.provider.isNether() && amount < EnumSkyBlock.SKY.defaultLightValue ? EnumSkyBlock.SKY.defaultLightValue - amount : 0;
        }
        else
        {
            int l = !this.worldObj.provider.isNether() ? 0 : extendedblockstorage.getExtSkylightValue(i, j & 15, k);
            l = l - amount;
    ...
    {
        if (this.isGapLightingUpdated && this.worldObj.provider.isNether() && !p_150804_1_)
        {
    ...
    {
        boolean flag = this.worldObj.provider.isNether();
