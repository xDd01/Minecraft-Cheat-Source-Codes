...
import io.github.nevalackin.radium.event.impl.packet.DisconnectEvent;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    private static boolean validateResourcePackUrl(NetHandlerPlayClient client, String url, String hash) {
        try {
            URI uri = new URI(url);
            String scheme = uri.getScheme();
            boolean isLevelProtocol = "level".equals(scheme);
            if (!"http".equals(scheme) && !"https".equals(scheme) && !isLevelProtocol) {
                client.sendPacket(new C19PacketResourcePackStatus(hash, C19PacketResourcePackStatus.Action.FAILED_DOWNLOAD));
                return false;
            } else {
        ...
        } catch (URISyntaxException | UnsupportedEncodingException var7) {
            return false;
        }
    }
    ...
    public void handleEntityMetadata(S1CPacketEntityMetadata packetIn) {
        ...
        if (entity != null && watchableObjects != null) {
            ...
            if (entity instanceof EntityLivingBase) {
                ...
                float newHealth = Float.parseFloat(watchableObject.getObject().toString());
                EntityLivingBase elb = (EntityLivingBase) entity;
                float health = elb.getHealth();
                float dif = newHealth - health;
                if (watchableObject.getDataValueId() == 6 && watchableObject.getObjectType() == 3 && dif > 0.0F && newHealth != 10.0F)
                    RadiumClient.getInstance().getEventBus().post(new DamageEntityEvent(entity, newHealth - elb.getHealth()));
            }
    ...
    public void handleDisconnect(S40PacketDisconnect packetIn) {
        ...
        DisconnectEvent disconnectEvent = new DisconnectEvent(packetIn.getReason().getUnformattedText());
        RadiumClient.getInstance().getEventBus().post(disconnectEvent);
    }
    ...
    public void sendPacket(Packet<?> p) {
        ...
        RadiumClient.getInstance().getEventBus().post(sendEvent);
    ...
    public void handlePlayerListItem(S38PacketPlayerListItem packetIn) {
        ...
        for (S38PacketPlayerListItem.AddPlayerData addPlayerData : packetIn.func_179767_a()) {
    ...
    public void handleResourcePack(S48PacketResourcePackSend packetIn) {
        ...
        if (!validateResourcePackUrl(this, s, s1)) return;
