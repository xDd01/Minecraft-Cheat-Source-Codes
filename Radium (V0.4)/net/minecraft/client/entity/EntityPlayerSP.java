...
import io.github.nevalackin.radium.event.impl.player.*;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public void onUpdatePlayer() {
        ...
        if (this.isCurrentViewEntity()) {
            final UpdatePositionEvent updatePositionEvent = new UpdatePositionEvent(posX, getEntityBoundingBox().minY,
                    posZ, rotationYaw, rotationPitch, lastReportedYaw, lastReportedPitch, onGround);
            ...
            if (riding) {
                if (!cancelled) {
                    ...
                    this.sendQueue.sendPacket(new C0CPacketInput(
                            this.moveStrafing, this.moveForward, this.movementInput.jump, this.movementInput.sneak));
                }
            } else {
                if (!cancelled) {
    ...
    public void setPlayerSPHealth(float health) {
        if (this.hasValidHealth) {
            ...
            if (f <= 0.0F) {
                ...
                if (f < 0.0F) {
                    this.hurtResistantTime = MAX_HURT_RESISTANT_TIME / 2;
                }
            } else {
                this.lastDamage = f;
                this.setHealth(this.getHealth());
                this.hurtResistantTime = MAX_HURT_RESISTANT_TIME;
