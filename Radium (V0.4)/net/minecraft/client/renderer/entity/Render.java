...
public abstract class Render<T extends Entity> implements IEntityRenderer {
    ...
    private void func_180549_a(Block blockIn, double p_180549_2_, double p_180549_4_, double p_180549_6_, BlockPos pos, float p_180549_9_, float p_180549_10_, double p_180549_11_, double p_180549_13_, double p_180549_15_) {
        if (blockIn.isFullCube()) {
            ...
            if (d0 >= 0.0D) {
                ...
                worldrenderer.pos(d1, d3, d4).tex(f, f2).color4f(1.0F, 1.0F, 1.0F, (float) d0).endVertex();
                worldrenderer.pos(d1, d3, d5).tex(f, f3).color4f(1.0F, 1.0F, 1.0F, (float) d0).endVertex();
                worldrenderer.pos(d2, d3, d5).tex(f1, f3).color4f(1.0F, 1.0F, 1.0F, (float) d0).endVertex();
                worldrenderer.pos(d2, d3, d4).tex(f1, f2).color4f(1.0F, 1.0F, 1.0F, (float) d0).endVertex();
            }
        }
    }
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance) {
        ...
        if (d0 <= (double) (maxDistance * maxDistance)) {
            ...
            worldrenderer.pos(-j - 1, -1 + i, 0.0D).color4f(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
            worldrenderer.pos(-j - 1, 8 + i, 0.0D).color4f(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
            worldrenderer.pos(j + 1, 8 + i, 0.0D).color4f(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
            worldrenderer.pos(j + 1, -1 + i, 0.0D).color4f(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
            tessellator.draw();
            GlStateManager.enableTexture2D();
