...
public class OutOfFovArrows extends Feature {

    public OutOfFovArrows() {
        super("OOFArrows", "treygolni4ki.",0, Category.VISUALS);
    ...
    public void onRender2D(EventRender2D event) {
        ...
        for (int i = 0; i < mc.world.playerEntities.size(); i++) {
            ...
            if(!RotationHelper.canSeeEntityAtFov(entity, 90)) {

            int noommom = 100;
            double xOffset = sr.getScaledWidth() / 2F - (noommom / 2.04);
            double yOffset = sr.getScaledHeight() / 2F - (noommom / 1.983);
            double NoomMomPosX = mc.player.posX;
            double NoomMomPosZ = mc.player.posZ;
            double loaddist = 0.2f;
            double pTicks = mc.timer.renderPartialTicks;
            double pos1 = (((entity.posX + (entity.posX - entity.lastTickPosX) * pTicks) - NoomMomPosX) * loaddist);
            double pos2 = (((entity.posZ + (entity.posZ - entity.lastTickPosZ) * pTicks) - NoomMomPosZ) * loaddist);
            ...
            if(!entity.isDead && entity.getHealth() > 0.0f && entity != null && entity != mc.player) {
                if (MathHelper.sqrt(var7 * var7 + var9 * var9) < noommom / 2F - 4) {
                    ...
                    triangle(0, 0, 3F, 3f, Main.getClientColor().getRGB());
