...
extends Component {
    ...
    public void onDraw(int mouseX, int mouseY) {
        ...
        DrawHelper.drawNewRect(x, y + 2, x + width, y + height, parent.dragging ? new Color(25, 25, 25).getRGB() : DrawHelper.getColorWithOpacity(BACKGROUND, 255 - this.opacity).getRGB());
        DrawHelper.drawNewRect(x + 3.5, y + height - 3.5, x + (width - 2), y + height - 1.5, new Color(140, 140, 140).getRGB());
        DrawHelper.drawNewRect(x + 4, y + height - 3, x + (width - 2.5), y + height - 2, new Color(45, 44, 44).getRGB());
        DrawHelper.drawGradientSideways(x + 4, y + height - 3, (float)x + (float)width * this.currentValueAnimate - 2.5f, y + height - 2, Main.getClientColor(y + height / 1.5f - 4.5f, yTotal, 4).getRGB(), Main.getClientColor(y + height / 1.5f - 4.5f, yTotal, 4).getRGB());

        Fonts.neverlose500_14.drawStringWithShadow(option.getName(), (float)x + 3.5f, (float)y + (float)height - 10f, color);
        Fonts.neverlose500_14.drawStringWithShadow(optionValueStr, x + width - Fonts.neverlose500_14.getStringWidth(optionValueStr) - 3, (float)y + (float)height - 10f, color);
    }
