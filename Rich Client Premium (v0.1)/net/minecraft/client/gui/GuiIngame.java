...
import white.floor.features.impl.display.Hotbar;
import white.floor.features.impl.player.NoClip;
import white.floor.features.impl.visuals.NoRender;
import white.floor.features.impl.visuals.ScoreBoard;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if(Main.featureDirector.getModule(NoRender.class).isToggled() && Main.settingsManager.getSettingByName(Main.featureDirector.getModule(NoRender.class), "NoBossBar").getValBoolean()) {
        } else {
        ...
        {
            if(!Main.settingsManager.getSettingByName(Main.featureDirector.getModule(ScoreBoard.class), "Remove").getValBoolean() && !Main.featureDirector.getModule(ScoreBoard.class).isToggled())
    ...
    private void renderAttackIndicator(float p_184045_1_, ScaledResolution p_184045_2_)
    {
        ...
        {
            if ((this.mc.playerController.isSpectator() && this.mc.pointedEntity == null) || Main.featureDirector.getModule(NoClip.class).isToggled())
            {
    ...
    protected void renderHotbar(ScaledResolution sr, float partialTicks) {
        ...
        if (!(mc.currentScreen instanceof GuiChat)) {
            if (Main.featureDirector.getModule(Hotbar.class).isToggled()) {
                drawRect(0, sr.getScaledHeight() - 22 - Feature.deltaTime(), sr.getScaledWidth(), sr.getScaledHeight(), 0x99000000);

                double target = (sr.getScaledWidth() / 2) - 91 + mc.player.inventory.currentItem * 20;
                last = MathHelper.lerp(last, target, 15 * Feature.deltaTime());

                DrawHelper.drawRect((float) last, sr.getScaledHeight() - 22 - Feature.deltaTime(), (float) last + (float) 182 - mc.player.inventory.currentItem * 20 - 20 * (8 - mc.player.inventory.currentItem), sr.getScaledHeight() - Feature.deltaTime(), Main.getClientColor().getRGB());

                GlStateManager.disableBlend();
            } else if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {

                double target = (sr.getScaledWidth() / 2) - 91 + mc.player.inventory.currentItem * 20;
                last = MathHelper.lerp(last, target, 12 * Feature.deltaTime());

                GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                this.mc.getTextureManager().bindTexture(WIDGETS_TEX_PATH);
                EntityPlayer entityplayer = (EntityPlayer) this.mc.getRenderViewEntity();
                float i = sr.getScaledWidth() / 2;
                this.zLevel = -90.0F;
                this.drawTexturedModalRect(i - 91, (float) (sr.getScaledHeight() - 22 - Feature.deltaTime()), 0, 0, 182, 22);
                this.drawTexturedModalRect((float) last - 1, (float) (sr.getScaledHeight() - 22 - 1 - Feature.deltaTime()), 0, 22, 24, 22);
            }
            if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
                ...
                if (Main.featureDirector.getModule(Hotbar.class).isToggled()) {
                    if (!itemstack.func_190926_b()) {
                        if (enumhandside == EnumHandSide.LEFT) {
                            this.drawTexturedModalRect(i - 91 - 29, (float) (sr.getScaledHeight() - 22 - Feature.deltaTime()), 24, 22, 29, 24);
                        } else {
                            this.drawTexturedModalRect(i + 91, (float) (sr.getScaledHeight() - 23 - Feature.deltaTime()), 53, 22, 29, 24);
                        }
                    }
                }
                ...
                for (int l = 0; l < 9; ++l) {
                    ...
                    this.renderHotbarItem(i1, (int) (j1 - Feature.deltaTime() + 1), partialTicks, entityplayer, entityplayer.inventory.mainInventory.get(l));
                }
                ...
                if (!itemstack.func_190926_b()) {
                    ...
                    if (Main.featureDirector.getModule(Hotbar.class).isToggled()) {
                        l1 = (int) (sr.getScaledHeight() - 16 - 2 - Feature.deltaTime());
                    } else {
                        l1 = (int) (sr.getScaledHeight() - 16 - 3 - Feature.deltaTime());
                    }
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        int l1 = scaledRes.getScaledWidth() - i + 15;
        int j = 0;
        ...
        for (Score score1 : collection)
        {
            ...
            int k = (int) (j1 - j * (this.getFontRenderer().FONT_HEIGHT - 1) + (Main.featureDirector.getModule(ScoreBoard.class).isToggled() ? Main.settingsManager.getSettingByName(Main.featureDirector.getModule(ScoreBoard.class), "Position Y").getValDouble() : 5));
            int l = scaledRes.getScaledWidth() - 3 + 2;
            drawRect(l1 - 2, k, l + 1, k + this.getFontRenderer().FONT_HEIGHT - 1, 1342177280);
            this.getFontRenderer().drawString(s1, l1, k, 553648127);

            if(!Main.settingsManager.getSettingByName(Main.featureDirector.getModule(ScoreBoard.class), "Remove Points").getValBoolean() && !Main.featureDirector.getModule(ScoreBoard.class).isToggled()) {
            ...
            if (j == collection.size())
            {
                ...
                Fonts.sfui15.drawString(s3, l1 + i / 2 - Fonts.sfui15.getStringWidth(s3) / 2 - 6, k - Fonts.sfui15.getHeight() - 2, 553648127);
            }
        }
    }
