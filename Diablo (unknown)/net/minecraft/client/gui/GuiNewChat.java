        ...
        if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN) {
            ...
            if (k > 0) {
                ...
                for (int i1 = 0; i1 + this.scrollPos < this.field_146253_i.size() && i1 < i; ++i1) {
                    ...
                    int i2 = 0;
                    int j2 = -i1 * 9;
                    if (!ModuleManager.getModule(CustomChat.class).toggled || !CustomChat.clearChat.isChecked()) {
                        GuiNewChat.drawRect(i2, j2 - 9, i2 + l + 4, j2, l1 / 2 << 24);
                    }
                    String s = chatline.getChatComponent().getFormattedText();
                    GlStateManager.enableBlend();
                    if (ModuleManager.getModule(CustomChat.class).toggled && CustomChat.customFont.isChecked()) {
                        TTFRenderer moduleTitle = new TTFRenderer("Comfortaa", 0, 18);
                        moduleTitle.drawStringWithShadow(s, i2, j2 - 8, 0xFFFFFF + (l1 << 24));
                    } else {
