...
extends Module {
    ...
    public ChestESP() {
        super("ChestESP", "Chest EPS", 0, Category.Render);
    }

    @Subscribe
    public void onUpdate3D(Render3DEvent e) {
        for (TileEntity object : Minecraft.theWorld.getLoadedTileEntityList()) {
            if (!(object instanceof TileEntityChest) && !(object instanceof TileEntityEnderChest)) continue;
            TileEntityChest entity = (TileEntityChest)object;
            double posX = entity.getPos().getX();
            double posY = entity.getPos().getY();
            double posZ = entity.getPos().getZ();
            double finalWidth = 0.5;
            double finalHeight = 0.75;
            AxisAlignedBB axisAlignedBB = new AxisAlignedBB(posX + 0.05, posY + 0.05, posZ + 0.05, posX + 0.95, posY + 0.95, posZ + 0.95);
            ...
            ChestESP.mc.entityRenderer.setupCameraTransform(ChestESP.mc.timer.renderPartialTicks, 0);
            ...
            for (Vector3d vector : vectorList) {
                ...
                if (GLU.gluProject((float)((float)(vector.x - ChestESP.mc.getRenderManager().viewerPosX)), (float)((float)((double)((float)vector.y) - ChestESP.mc.getRenderManager().viewerPosY)), (float)((float)((double)((float)vector.z) - ChestESP.mc.getRenderManager().viewerPosZ)), (FloatBuffer)modelview, (FloatBuffer)projection, (IntBuffer)viewport, (FloatBuffer)otherVec)) {
