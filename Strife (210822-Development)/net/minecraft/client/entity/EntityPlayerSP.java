...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        StrifeClient.INSTANCE.eventBus().post(movePlayerEvent);
        if(movePlayerEvent.cancelled()) return;
        super.moveEntity(movePlayerEvent.x(), movePlayerEvent.y(), movePlayerEvent.z());
    }
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        {
            final UpdatePlayerEvent updatePlayerEvent = new UpdatePlayerEvent(EventState.PRE, lastReportedYaw, lastReportedPitch, 
                                                                            lastTickPosX, lastTickPosY, lastTickPosZ, 
                                                                            posX, getEntityBoundingBox().minY, posZ, 
                                                                            rotationYaw, rotationPitch, onGround);
            killAuraModule.directUpdate(updatePlayerEvent);
            StrifeClient.INSTANCE.eventBus().post(updatePlayerEvent);
            currentEvent = updatePlayerEvent;
            final double eventX = updatePlayerEvent.posX();
            final double eventY = updatePlayerEvent.posY();
            final double eventZ = updatePlayerEvent.posZ();
            final float eventYaw = updatePlayerEvent.yaw();
            final float eventPitch = updatePlayerEvent.pitch();
            final boolean eventGround = updatePlayerEvent.ground();
            ...
            updatePlayerEvent.state(EventState.POST);
            KillAuraModule.instance().directUpdate(updatePlayerEvent);
            StrifeClient.INSTANCE.eventBus().post(updatePlayerEvent);
        }
    }
    ...
    {
        if(message.startsWith(StrifeClient.CHAT_PREFIX)) {
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z)
    {
        ...
        StrifeClient.INSTANCE.eventBus().post(pushOutOfBlocksEvent);
        if (this.noClip || pushOutOfBlocksEvent.cancelled())
        {
            return false;
        }
        else
    ...
    public void currentEvent(UpdatePlayerEvent currentEvent) {
