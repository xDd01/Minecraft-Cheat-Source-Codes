...
import club.async.event.impl.EventLook;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    private int getArmSwingAnimationEnd()
    {
        if(Async.INSTANCE.getModuleManager().isToggled(Animations.class)) {
            return (int) (Async.INSTANCE.getModuleManager().moduleBy(Animations.class).animSpeed.getDouble() * 6);
        }
    ...
    {
        final EventLook eventLook = new EventLook(new float[]{rotationYaw, rotationPitch}, new float[]{prevRotationYaw, prevRotationPitch});
        Event.fire(eventLook);
        final float yaw = eventLook.getRotations()[0], pitch = eventLook.getRotations()[1], prevYaw = eventLook.getPrevRotations()[0], prevPitch = eventLook.getPrevRotations()[1];
