...
import club.async.module.impl.visual.Animations;
...
public class ItemRenderer
{
    ...
    private void transformFirstPersonItem(float equipProgress, float swingProgress)
    {
        ...
        if(Async.INSTANCE.getModuleManager().isToggled(Animations.class)) {
            Animations animations = Async.INSTANCE.getModuleManager().moduleBy(Animations.class);
            GlStateManager.scale(animations.itemSize.getFloat(), animations.itemSize.getFloat(), animations.itemSize.getFloat());
        }
    }
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            if(Async.INSTANCE.getModuleManager().isToggled(Animations.class)) {
                GlStateManager.translate(Async.INSTANCE.getModuleManager().moduleBy(Animations.class).x.getFloat(), Async.INSTANCE.getModuleManager().moduleBy(Animations.class).y.getFloat(), 0);
            }
                    ...
                    switch (enumaction)
                    {
                        ...
                        case BLOCK:
                            if(!Async.INSTANCE.getModuleManager().isToggled(Animations.class)) {
                                ...
                                GlStateManager.translate(animations.animX.getFloat(), animations.animY.getFloat(), 0);

                                float cock = MathHelper.sin(MathHelper.sqrt_float(f1) * (float)Math.PI);
                                float equipProgress;
                                if(animations.equipProgress.get()) {
                                    equipProgress = f * animations.equipProgressMultiplier.getFloat();
                                } else {
                                    equipProgress = 0;
                                }

                                switch (animations.mode.getCurrMode()) {
                                    case "Default":
                                        this.transformFirstPersonItem(f, 0.0F);
                                        this.doBlockTransformations();
                                        break;
                                    ...
                                    case "Exhibition":
                                        ...
                                        GL11.glRotated(-cock * 35, cock / 2, 0, 9);
                                        GL11.glRotated(-cock * 60, 0.8, cock / 2, 0);
                                        this.doBlockTransformations();
                                        break;
                                }
                            }
