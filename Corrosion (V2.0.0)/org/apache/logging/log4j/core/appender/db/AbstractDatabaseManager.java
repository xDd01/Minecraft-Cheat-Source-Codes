    ...
    public final synchronized void connect() {
        if (!this.isConnected()) {
            try {
                ...
                LOGGER.error("Could not connect to database using logging manager [{}].", this.getName(), e2);
            }
        }
    }
    ...
    public final synchronized void disconnect() {
        ...
        if (this.isConnected()) {
            try {
                ...
                LOGGER.warn("Error while disconnecting from database using logging manager [{}].", this.getName(), e2);
            }
            finally {
