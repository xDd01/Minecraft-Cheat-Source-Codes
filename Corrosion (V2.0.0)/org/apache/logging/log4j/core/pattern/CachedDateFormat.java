...
extends DateFormat {
    ...
    public static int findMillisecondStart(long time, String formatted, DateFormat formatter) {
        ...
        for (int i2 = 0; i2 < formatted.length(); ++i2) {
            if (formatted.charAt(i2) == plusMagic.charAt(i2)) continue;
            ...
            if (plusZero.length() == formatted.length() && magicString.regionMatches(0, plusMagic, i2, magicString.length()) && formattedMillis.toString().regionMatches(0, formatted, i2, magicString.length()) && ZERO_STRING.regionMatches(0, plusZero, i2, ZERO_STRING.length())) {
                return i2;
            }
            return -1;
        }
    ...
    @Override
    public Date parse(String s2, ParsePosition pos) {
        return this.formatter.parse(s2, pos);
    }
