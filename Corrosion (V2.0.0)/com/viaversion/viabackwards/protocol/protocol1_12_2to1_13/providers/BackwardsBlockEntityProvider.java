...
implements Provider {
    ...
    public CompoundTag transform(UserConnection user, Position position, CompoundTag tag) throws Exception {
        ...
        String id2 = (String)((Tag)idTag).getValue();
        BackwardsBlockEntityHandler handler = this.handlers.get(id2);
        if (handler == null) {
            if (Via.getManager().isDebug()) {
                ViaBackwards.getPlatform().getLogger().warning("Unhandled BlockEntity " + id2 + " full tag: " + tag);
            }
            return tag;
        }
        ...
        if (blockId == null) {
            if (Via.getManager().isDebug()) {
                ViaBackwards.getPlatform().getLogger().warning("Handled BlockEntity does not have a stored block :( " + id2 + " full tag: " + tag);
            }
            return tag;
        }
    ...
    public CompoundTag transform(UserConnection user, Position position, String id2) throws Exception {
