...
implements Packet<INetHandlerPlayClient> {
    ...
    public S22PacketMultiBlockChange(int p_i45181_1_, short[] crammedPositionsIn, Chunk chunkIn) {
        ...
        for (int i2 = 0; i2 < this.changedBlocks.length; ++i2) {
            this.changedBlocks[i2] = new BlockUpdateData(crammedPositionsIn[i2], chunkIn);
        }
    }
    ...
    public void readPacketData(PacketBuffer buf) throws IOException {
        ...
        for (int i2 = 0; i2 < this.changedBlocks.length; ++i2) {
            this.changedBlocks[i2] = new BlockUpdateData(buf.readShort(), (IBlockState)Block.BLOCK_STATE_IDS.getByValue(buf.readVarIntFromBuffer()));
        }
    }
