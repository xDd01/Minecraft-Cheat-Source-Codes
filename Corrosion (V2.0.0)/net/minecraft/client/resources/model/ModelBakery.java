...
public class ModelBakery {
    ...
    private ModelBlock loadModel(ResourceLocation p_177594_1_) throws IOException {
        ...
        Reader reader;
        String s2 = p_177594_1_.getResourcePath();
        if ("builtin/generated".equals(s2)) {
            return MODEL_GENERATED;
        }
        if ("builtin/compass".equals(s2)) {
            return MODEL_COMPASS;
        }
        if ("builtin/clock".equals(s2)) {
            return MODEL_CLOCK;
        }
        if ("builtin/entity".equals(s2)) {
            return MODEL_ENTITY;
        }
        if (s2.startsWith("builtin/")) {
            String s1 = s2.substring("builtin/".length());
            String s22 = BUILT_IN_MODELS.get(s1);
            if (s22 == null) {
                throw new FileNotFoundException(p_177594_1_.toString());
            }
            reader = new StringReader(s22);
        } else {
    ...
    private void loadItemModels() {
        ...
        for (Item item : Item.itemRegistry) {
            for (String s2 : this.getVariantNames(item)) {
                ...
                this.itemLocations.put(s2, resourcelocation);
                if (this.models.get(resourcelocation) != null) continue;
                try {
                    ModelBlock modelblock = this.loadModel(resourcelocation);
                    this.models.put(resourcelocation, modelblock);
        ...
        for (ResourceLocation resourcelocation : this.itemLocations.values()) {
            ...
            if (this.hasItemModel(modelblock)) {
                for (String s2 : ItemModelGenerator.LAYERS) {
                    ResourceLocation resourcelocation2 = new ResourceLocation(modelblock.resolveTextureName(s2));
