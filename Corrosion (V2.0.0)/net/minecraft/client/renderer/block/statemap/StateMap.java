...
extends StateMapperBase {
    ...
    protected ModelResourceLocation getModelResourceLocation(IBlockState state) {
        ...
        String s2 = this.name == null ? ((ResourceLocation)Block.blockRegistry.getNameForObject(state.getBlock())).toString() : this.name.getName((Comparable)map.remove(this.name));
        if (this.suffix != null) {
            s2 = s2 + this.suffix;
        }
