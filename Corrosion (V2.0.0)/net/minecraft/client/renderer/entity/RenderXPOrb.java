...
extends Render {
    ...
    public void doRender(EntityXPOrb entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        ...
        int i3 = entity.getTextureByXP();
        float f2 = (float)(i3 % 4 * 16 + 0) / 64.0f;
        float f1 = (float)(i3 % 4 * 16 + 16) / 64.0f;
        float f22 = (float)(i3 / 4 * 16 + 0) / 64.0f;
        float f3 = (float)(i3 / 4 * 16 + 16) / 64.0f;
        float f4 = 1.0f;
        float f5 = 0.5f;
        float f6 = 0.25f;
        ...
        worldrenderer.pos(0.0f - f5, 0.0f - f6, 0.0).tex(f2, f3).color(j1, k1, l1, 128).normal(0.0f, 1.0f, 0.0f).endVertex();
        worldrenderer.pos(f4 - f5, 0.0f - f6, 0.0).tex(f1, f3).color(j1, k1, l1, 128).normal(0.0f, 1.0f, 0.0f).endVertex();
        worldrenderer.pos(f4 - f5, 1.0f - f6, 0.0).tex(f1, f22).color(j1, k1, l1, 128).normal(0.0f, 1.0f, 0.0f).endVertex();
        worldrenderer.pos(0.0f - f5, 1.0f - f6, 0.0).tex(f2, f22).color(j1, k1, l1, 128).normal(0.0f, 1.0f, 0.0f).endVertex();
        tessellator.draw();
    ...
    public void doRender(Entity entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        this.doRender((EntityXPOrb)entity, x2, y2, z2, entityYaw, partialTicks);
    }
}
