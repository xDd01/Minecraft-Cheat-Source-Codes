...
implements IResourceManagerReloadListener {
        ...
        for (int i2 = 0; i2 < j2; ++i2) {
            ...
            if (flag && bakedquad.hasTintIndex()) {
                ...
                if (Config.isCustomColors()) {
                    k2 = CustomColors.getColorFromItemStack(stack, bakedquad.getTintIndex(), k2);
                }
    ...
    public void renderItemOverlays(FontRenderer fr2, ItemStack stack, int xPosition, int yPosition) {
        this.renderItemOverlayIntoGUI(fr2, stack, xPosition, yPosition, null);
    }

    public void renderItemOverlayIntoGUI(FontRenderer fr2, ItemStack stack, int xPosition, int yPosition, String text) {
        if (stack != null) {
            if (stack.stackSize != 1 || text != null) {
                ...
                GlStateManager.disableBlend();
                fr2.drawStringWithShadow(s2, xPosition + 19 - 2 - fr2.getStringWidth(s2), yPosition + 6 + 3, 0xFFFFFF);
                GlStateManager.enableLighting();
            ...
            if (flag) {
                int i2 = (int)Math.round(13.0 - (double)stack.getItemDamage() * 13.0 / (double)stack.getMaxDamage());
                int j2 = (int)Math.round(255.0 - (double)stack.getItemDamage() * 255.0 / (double)stack.getMaxDamage());
                if (Reflector.ForgeItem_getDurabilityForDisplay.exists()) {
                    ...
                    i2 = (int)Math.round(13.0 - d0 * 13.0);
                    j2 = (int)Math.round(255.0 - d0 * 255.0);
                }
                GlStateManager.disableLighting();
                GlStateManager.disableDepth();
                ...
                this.func_181565_a(worldrenderer, xPosition + 2, yPosition + 13, 12, 1, (255 - j2) / 4, 64, 0, 255);
                this.func_181565_a(worldrenderer, xPosition + 2, yPosition + 13, i2, 1, 255 - j2, j2, 0, 255);
    ...
    public static void forgeHooksClient_putQuadColor(WorldRenderer p_forgeHooksClient_putQuadColor_0_, BakedQuad p_forgeHooksClient_putQuadColor_1_, int p_forgeHooksClient_putQuadColor_2_) {
        float f2 = p_forgeHooksClient_putQuadColor_2_ & 0xFF;
        float f1 = p_forgeHooksClient_putQuadColor_2_ >>> 8 & 0xFF;
        float f22 = p_forgeHooksClient_putQuadColor_2_ >>> 16 & 0xFF;
        ...
        for (int j2 = 0; j2 < 4; ++j2) {
            ...
            float f6 = k2 >>> 16 & 0xFF;
            float f7 = k2 >>> 24 & 0xFF;
            int l2 = Math.min(255, (int)(f2 * f4 / 255.0f));
            int i1 = Math.min(255, (int)(f1 * f5 / 255.0f));
            int j1 = Math.min(255, (int)(f22 * f6 / 255.0f));
            int k1 = Math.min(255, (int)(f3 * f7 / 255.0f));
            p_forgeHooksClient_putQuadColor_0_.putColorRGBA(p_forgeHooksClient_putQuadColor_0_.getColorIndex(4 - j2), l2, i1, j1, k1);
        }
    }
}
