...
public class ActiveRenderInfo {
    ...
    public static void updateRenderInfo(EntityPlayer entityplayerIn, boolean p_74583_1_) {
        ...
        rotationX = MathHelper.cos(f3 * (float)Math.PI / 180.0f) * (float)(1 - i2 * 2);
        rotationZ = MathHelper.sin(f3 * (float)Math.PI / 180.0f) * (float)(1 - i2 * 2);
        rotationYZ = -rotationZ * MathHelper.sin(f22 * (float)Math.PI / 180.0f) * (float)(1 - i2 * 2);
        rotationXY = rotationX * MathHelper.sin(f22 * (float)Math.PI / 180.0f) * (float)(1 - i2 * 2);
        rotationXZ = MathHelper.cos(f22 * (float)Math.PI / 180.0f);
    }
    ...
    public static Block getBlockAtEntityViewpoint(World worldIn, Entity p_180786_1_, float p_180786_2_) {
        ...
        if (block.getMaterial().isLiquid()) {
            ...
            if (vec3.yCoord >= (double)(f1 = (float)(blockpos.getY() + 1) - f2)) {
