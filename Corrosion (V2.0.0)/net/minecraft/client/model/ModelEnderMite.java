...
extends ModelBase {
    ...
    public ModelEnderMite() {
        float f2 = -3.5f;
        for (int i2 = 0; i2 < this.field_178713_d.length; ++i2) {
            ...
            if (i2 >= this.field_178713_d.length - 1) continue;
    ...
    public void setRotationAngles(float p_78087_1_, float p_78087_2_, float p_78087_3_, float p_78087_4_, float p_78087_5_, float p_78087_6_, Entity entityIn) {
        for (int i2 = 0; i2 < this.field_178713_d.length; ++i2) {
            this.field_178713_d[i2].rotateAngleY = MathHelper.cos(p_78087_3_ * 0.9f + (float)i2 * 0.15f * (float)Math.PI) * (float)Math.PI * 0.01f * (float)(1 + Math.abs(i2 - 2));
            this.field_178713_d[i2].rotationPointX = MathHelper.sin(p_78087_3_ * 0.9f + (float)i2 * 0.15f * (float)Math.PI) * (float)Math.PI * 0.1f * (float)Math.abs(i2 - 2);
        }
    }
}
