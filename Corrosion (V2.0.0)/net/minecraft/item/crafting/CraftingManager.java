...
public class CraftingManager {
    ...
    public ShapedRecipes addRecipe(ItemStack stack, Object ... recipeComponents) {
        ...
        int j2 = 0;
        int k2 = 0;
        if (recipeComponents[i2] instanceof String[]) {
            String[] astring = (String[])recipeComponents[i2++];
            for (int l2 = 0; l2 < astring.length; ++l2) {
                String s22 = astring[l2];
                ++k2;
                j2 = s22.length();
                s2 = s2 + s22;
            }
        } else {
            while (recipeComponents[i2] instanceof String) {
                String s1 = (String)recipeComponents[i2++];
                ++k2;
                j2 = s1.length();
                s2 = s2 + s1;
            }
        ...
        while (i2 < recipeComponents.length) {
            Character character = (Character)recipeComponents[i2];
            ItemStack itemstack = null;
            if (recipeComponents[i2 + 1] instanceof Item) {
                itemstack = new ItemStack((Item)recipeComponents[i2 + 1]);
            } else if (recipeComponents[i2 + 1] instanceof Block) {
                itemstack = new ItemStack((Block)recipeComponents[i2 + 1], 1, Short.MAX_VALUE);
            } else if (recipeComponents[i2 + 1] instanceof ItemStack) {
                itemstack = (ItemStack)recipeComponents[i2 + 1];
            }
    ...
    public ItemStack[] func_180303_b(InventoryCrafting p_180303_1_, World worldIn) {
        ...
        for (int i2 = 0; i2 < aitemstack.length; ++i2) {
            aitemstack[i2] = p_180303_1_.getStackInSlot(i2);
        }
        return aitemstack;
    }
