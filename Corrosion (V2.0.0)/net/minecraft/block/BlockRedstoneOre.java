...
extends Block {
    ...
    private void spawnParticles(World worldIn, BlockPos pos) {
        ...
        for (int i2 = 0; i2 < 6; ++i2) {
            ...
            if (i2 == 0 && !worldIn.getBlockState(pos.up()).getBlock().isOpaqueCube()) {
                d2 = (double)pos.getY() + d0 + 1.0;
            }
            if (i2 == 1 && !worldIn.getBlockState(pos.down()).getBlock().isOpaqueCube()) {
                d2 = (double)pos.getY() - d0;
            }
            if (i2 == 2 && !worldIn.getBlockState(pos.south()).getBlock().isOpaqueCube()) {
                d3 = (double)pos.getZ() + d0 + 1.0;
            }
            if (i2 == 3 && !worldIn.getBlockState(pos.north()).getBlock().isOpaqueCube()) {
                d3 = (double)pos.getZ() - d0;
            }
            if (i2 == 4 && !worldIn.getBlockState(pos.east()).getBlock().isOpaqueCube()) {
                d1 = (double)pos.getX() + d0 + 1.0;
            }
            if (i2 == 5 && !worldIn.getBlockState(pos.west()).getBlock().isOpaqueCube()) {
