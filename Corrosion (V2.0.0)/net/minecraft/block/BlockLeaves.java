...
extends BlockLeavesBase {
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (worldIn.isAreaLoaded(new BlockPos(k2 - j2, (l2 = pos.getY()) - j2, (i1 = pos.getZ()) - j2), new BlockPos(k2 + j2, l2 + j2, i1 + j2))) {
            for (int j1 = -i2; j1 <= i2; ++j1) {
                for (int k1 = -i2; k1 <= i2; ++k1) {
                    for (int l1 = -i2; l1 <= i2; ++l1) {
        ...
        if (!worldIn.isRemote && state.getValue(CHECK_DECAY).booleanValue() && state.getValue(DECAYABLE).booleanValue()) {
            ...
            if (worldIn.isAreaLoaded(new BlockPos(k2 - j2, l3 - j2, i1 - j2), new BlockPos(k2 + j2, l3 + j2, i1 + j2))) {
                ...
                for (int i22 = -i2; i22 <= i2; ++i22) {
                    for (int j22 = -i2; j22 <= i2; ++j22) {
                        for (int k22 = -i2; k22 <= i2; ++k22) {
                            Block block = worldIn.getBlockState(blockpos$mutableblockpos.func_181079_c(k2 + i22, l3 + j22, i1 + k22)).getBlock();
                            if (block != Blocks.log && block != Blocks.log2) {
                                if (block.getMaterial() == Material.leaves) {
                                    this.surroundings[(i22 + l1) * k1 + (j22 + l1) * j1 + k22 + l1] = -2;
                                    continue;
                                }
                                this.surroundings[(i22 + l1) * k1 + (j22 + l1) * j1 + k22 + l1] = -1;
                                continue;
                            }
                            this.surroundings[(i22 + l1) * k1 + (j22 + l1) * j1 + k22 + l1] = 0;
                        }
                    }
                }
                for (int i3 = 1; i3 <= 4; ++i3) {
                    for (int j3 = -i2; j3 <= i2; ++j3) {
                        for (int k3 = -i2; k3 <= i2; ++k3) {
                            for (int l32 = -i2; l32 <= i2; ++l32) {
                                if (this.surroundings[(j3 + l1) * k1 + (k3 + l1) * j1 + l32 + l1] != i3 - 1) continue;
                                ...
                                if (this.surroundings[(j3 + l1) * k1 + (k3 + l1) * j1 + l32 + l1 + 1] != -2) continue;
                                this.surroundings[(j3 + l1) * k1 + (k3 + l1) * j1 + l32 + l1 + 1] = i3;
                            }
                        }
                    }
