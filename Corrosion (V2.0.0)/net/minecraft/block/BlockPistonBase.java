...
extends Block {
    ...
    private boolean doMove(World worldIn, BlockPos pos, EnumFacing direction, boolean extending) {
        ...
        int i2 = list.size() + list1.size();
        Block[] ablock = new Block[i2];
        EnumFacing enumfacing = extending ? direction : direction.getOpposite();
        for (int j2 = list1.size() - 1; j2 >= 0; --j2) {
            BlockPos blockpos = list1.get(j2);
            Block block = worldIn.getBlockState(blockpos).getBlock();
        ...
        for (int k2 = list.size() - 1; k2 >= 0; --k2) {
            BlockPos blockpos2 = list.get(k2);
        ...
        for (int l2 = list1.size() - 1; l2 >= 0; --l2) {
            worldIn.notifyNeighborsOfStateChange(list1.get(l2), ablock[i2++]);
        }
        for (int i1 = list.size() - 1; i1 >= 0; --i1) {
            worldIn.notifyNeighborsOfStateChange(list.get(i1), ablock[i2++]);
        }
        if (extending) {
