...
public class BlockPistonStructureHelper {
    ...
    private boolean func_177251_a(BlockPos origin) {
        ...
        int i2 = 1;
        if (i2 + this.toMove.size() > 12) {
            return false;
        }
        while (block == Blocks.slime_block && (block = this.world.getBlockState(blockpos = origin.offset(this.moveDirection.getOpposite(), i2)).getBlock()).getMaterial() != Material.air && BlockPistonBase.canPush(block, this.world, blockpos, this.moveDirection, false) && !blockpos.equals(this.pistonPos)) {
            if (++i2 + this.toMove.size() <= 12) continue;
            return false;
        }
        int i1 = 0;
        for (int j2 = i2 - 1; j2 >= 0; --j2) {
            this.toMove.add(origin.offset(this.moveDirection.getOpposite(), j2));
            ++i1;
        }
        int j1 = 1;
        while (true) {
            BlockPos blockpos1;
            int k2;
            if ((k2 = this.toMove.indexOf(blockpos1 = origin.offset(this.moveDirection, j1))) > -1) {
                this.func_177255_a(i1, k2);
                for (int l2 = 0; l2 <= k2 + i1; ++l2) {
                    BlockPos blockpos2 = this.toMove.get(l2);
