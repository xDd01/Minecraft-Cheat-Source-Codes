...
public abstract class ServerConfigurationManager {
    ...
    public void transferEntityToWorld(Entity entityIn, int p_82448_2_, WorldServer p_82448_3_, WorldServer p_82448_4_) {
        ...
        if (p_82448_2_ != 1) {
            ...
            if (entityIn.isEntityAlive()) {
                entityIn.setLocationAndAngles(d0, entityIn.posY, d1, entityIn.rotationYaw, entityIn.rotationPitch);
                p_82448_4_.getDefaultTeleporter().placeInPortal(entityIn, f2);
    ...
    public String func_181058_b(boolean p_181058_1_) {
        ...
        for (int i2 = 0; i2 < list.size(); ++i2) {
            ...
            s2 = s2 + ((EntityPlayerMP)list.get(i2)).getName();
            if (!p_181058_1_) continue;
            s2 = s2 + " (" + ((EntityPlayerMP)list.get(i2)).getUniqueID().toString() + ")";
        }
        return s2;
    }
    ...
    public void sendToAllNearExcept(EntityPlayer p_148543_1_, double x2, double y2, double z2, double radius, int dimension, Packet p_148543_11_) {
        for (int i2 = 0; i2 < this.playerEntityList.size(); ++i2) {
            ...
            double d0;
            EntityPlayerMP entityplayermp = this.playerEntityList.get(i2);
            if (entityplayermp == p_148543_1_ || entityplayermp.dimension != dimension || !((d0 = x2 - entityplayermp.posX) * d0 + (d1 = y2 - entityplayermp.posY) * d1 + (d2 = z2 - entityplayermp.posZ) * d2 < radius * radius)) continue;
