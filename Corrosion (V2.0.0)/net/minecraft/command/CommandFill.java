    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        ...
        block24: {
            block19: {
                ...
                int i2 = 0;
                if (args.length >= 8) {
                    i2 = CommandFill.parseInt(args[7], 0, 15);
                }
                ...
                j2 = (blockpos3.getX() - blockpos2.getX() + 1) * (blockpos3.getY() - blockpos2.getY() + 1) * (blockpos3.getZ() - blockpos2.getZ() + 1);
                if (j2 > 32768) {
                    throw new CommandException("commands.fill.tooManyBlocks", j2, 32768);
                }
                if (blockpos2.getY() < 0 || blockpos3.getY() >= 256) break block19;
                World world = sender.getEntityWorld();
                for (int k2 = blockpos2.getZ(); k2 < blockpos3.getZ() + 16; k2 += 16) {
                    for (int l2 = blockpos2.getX(); l2 < blockpos3.getX() + 16; l2 += 16) {
                        if (world.isBlockLoaded(new BlockPos(l2, blockpos3.getY() - blockpos2.getY(), k2))) continue;
                ...
                if (args.length >= 10 && block.hasTileEntity()) {
                    String s2 = CommandFill.getChatComponentFromNthArg(sender, args, 9).getUnformattedText();
                    try {
                ...
                for (int i1 = blockpos2.getZ(); i1 <= blockpos3.getZ(); ++i1) {
                    for (int j1 = blockpos2.getY(); j1 <= blockpos3.getY(); ++j1) {
                        for (int k1 = blockpos2.getX(); k1 <= blockpos3.getX(); ++k1) {
                            ...
                            if (!world.setBlockState(blockpos4, iblockstate1 = block.getStateFromMeta(i2), 2)) continue;
                            list.add(blockpos4);
                            ++j2;
        ...
        sender.setCommandStat(CommandResultStats.Type.AFFECTED_BLOCKS, j2);
        CommandFill.notifyOperators(sender, (ICommand)this, "commands.fill.success", j2);
    }
