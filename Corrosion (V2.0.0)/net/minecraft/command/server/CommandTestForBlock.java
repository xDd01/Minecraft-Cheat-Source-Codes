...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        ...
        int i2 = -1;
        if (args.length >= 5) {
            i2 = CommandTestForBlock.parseInt(args[4], -1, 15);
        }
        ...
        if (args.length >= 6 && block.hasTileEntity()) {
            String s2 = CommandTestForBlock.getChatComponentFromNthArg(sender, args, 5).getUnformattedText();
            try {
        ...
        if (i2 > -1 && (j2 = iblockstate.getBlock().getMetaFromState(iblockstate)) != i2) {
            throw new CommandException("commands.testforblock.failed.data", blockpos.getX(), blockpos.getY(), blockpos.getZ(), j2, i2);
        }
        if (flag) {
