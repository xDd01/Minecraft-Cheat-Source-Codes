...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        ...
        int i2 = 0;
        if (args.length >= 5) {
            i2 = CommandSetBlock.parseInt(args[4], 0, 15);
        }
        ...
        if (args.length >= 7 && block.hasTileEntity()) {
            String s2 = CommandSetBlock.getChatComponentFromNthArg(sender, args, 6).getUnformattedText();
            try {
        ...
        if (!world.setBlockState(blockpos, iblockstate = block.getStateFromMeta(i2), 2)) {
            throw new CommandException("commands.setblock.noChange", new Object[0]);
        }
