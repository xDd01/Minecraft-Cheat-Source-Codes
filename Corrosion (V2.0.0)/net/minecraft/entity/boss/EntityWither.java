...
IRangedAttackMob {
    ...
    public void onLivingUpdate() {
        ...
        for (int i2 = 0; i2 < 2; ++i2) {
            this.field_82218_g[i2] = this.field_82221_e[i2];
            this.field_82217_f[i2] = this.field_82220_d[i2];
        }
        for (int j2 = 0; j2 < 2; ++j2) {
            int k2 = this.getWatchedTargetId(j2 + 1);
            Entity entity1 = null;
            if (k2 > 0) {
                entity1 = this.worldObj.getEntityByID(k2);
            }
            if (entity1 != null) {
                double d11 = this.func_82214_u(j2 + 1);
                double d12 = this.func_82208_v(j2 + 1);
                double d13 = this.func_82213_w(j2 + 1);
                ...
                float f2 = (float)(MathHelper.func_181159_b(d8, d6) * 180.0 / Math.PI) - 90.0f;
                float f1 = (float)(-(MathHelper.func_181159_b(d7, d9) * 180.0 / Math.PI));
                this.field_82220_d[j2] = this.func_82204_b(this.field_82220_d[j2], f1, 40.0f);
                this.field_82221_e[j2] = this.func_82204_b(this.field_82221_e[j2], f2, 10.0f);
                continue;
            }
            this.field_82221_e[j2] = this.func_82204_b(this.field_82221_e[j2], this.renderYawOffset, 10.0f);
        }
        boolean flag = this.isArmored();
        for (int l2 = 0; l2 < 3; ++l2) {
            double d10 = this.func_82214_u(l2);
            double d2 = this.func_82208_v(l2);
            double d4 = this.func_82213_w(l2);
    ...
    protected void updateAITasks() {
        if (this.getInvulTime() > 0) {
            ...
            block0: for (int i2 = 1; i2 < 3; ++i2) {
                int k1;
                if (this.ticksExisted < this.field_82223_h[i2 - 1]) continue;
                this.field_82223_h[i2 - 1] = this.ticksExisted + 10 + this.rand.nextInt(10);
                if (this.worldObj.getDifficulty() == EnumDifficulty.NORMAL || this.worldObj.getDifficulty() == EnumDifficulty.HARD) {
                    int j3 = i2 - 1;
                    int k3 = this.field_82224_i[i2 - 1];
                    this.field_82224_i[j3] = this.field_82224_i[i2 - 1] + 1;
                    if (k3 > 15) {
                        float f2 = 10.0f;
                        float f1 = 5.0f;
                        double d0 = MathHelper.getRandomDoubleInRange(this.rand, this.posX - (double)f2, this.posX + (double)f2);
                        double d1 = MathHelper.getRandomDoubleInRange(this.rand, this.posY - (double)f1, this.posY + (double)f1);
                        double d2 = MathHelper.getRandomDoubleInRange(this.rand, this.posZ - (double)f2, this.posZ + (double)f2);
                        this.launchWitherSkullToCoords(i2 + 1, d0, d1, d2, true);
                        this.field_82224_i[i2 - 1] = 0;
                    }
                }
                if ((k1 = this.getWatchedTargetId(i2)) > 0) {
                    ...
                    if (entity != null && entity.isEntityAlive() && this.getDistanceSqToEntity(entity) <= 900.0 && this.canEntityBeSeen(entity)) {
                        ...
                        this.field_82223_h[i2 - 1] = this.ticksExisted + 40 + this.rand.nextInt(20);
                        this.field_82224_i[i2 - 1] = 0;
                        continue;
                    }
                    this.updateWatchedTargetId(i2, 0);
    ...
    private double func_82214_u(int p_82214_1_) {
        ...
        float f2 = (this.renderYawOffset + (float)(180 * (p_82214_1_ - 1))) / 180.0f * (float)Math.PI;
    ...
    private double func_82213_w(int p_82213_1_) {
        ...
        float f2 = (this.renderYawOffset + (float)(180 * (p_82213_1_ - 1))) / 180.0f * (float)Math.PI;
        float f1 = MathHelper.sin(f2);
        return this.posZ + (double)f1 * 1.3;
    }

    private float func_82204_b(float p_82204_1_, float p_82204_2_, float p_82204_3_) {
        float f2 = MathHelper.wrapAngleTo180_float(p_82204_2_ - p_82204_1_);
        if (f2 > p_82204_3_) {
            f2 = p_82204_3_;
        }
        if (f2 < -p_82204_3_) {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (source != DamageSource.drown && !(source.getEntity() instanceof EntityWither)) {
            ...
            int i2 = 0;
            while (i2 < this.field_82224_i.length) {
                int n2 = i2++;
