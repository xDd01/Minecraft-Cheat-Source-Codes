...
IMob {
    ...
    public double[] getMovementOffsets(int p_70974_1_, float p_70974_2_) {
        ...
        int i2 = this.ringBufferIndex - p_70974_1_ * 1 & 0x3F;
        int j2 = this.ringBufferIndex - p_70974_1_ * 1 - 1 & 0x3F;
        double[] adouble = new double[3];
        double d0 = this.ringBuffer[i2][0];
        double d1 = MathHelper.wrapAngleTo180_double(this.ringBuffer[j2][0] - d0);
    ...
    public void onLivingUpdate() {
        if (this.worldObj.isRemote) {
            float f2 = MathHelper.cos(this.animTime * (float)Math.PI * 2.0f);
            float f1 = MathHelper.cos(this.prevAnimTime * (float)Math.PI * 2.0f);
            if (f1 <= -0.3f && f2 >= -0.3f && !this.isSilent()) {
        ...
        if (this.getHealth() <= 0.0f) {
            ...
            if (this.isAIDisabled()) {
                ...
                for (int j2 = 0; j2 < 3; ++j2) {
                    ...
                    double[] adouble2 = this.getMovementOffsets(12 + j2 * 2, 1.0f);
    ...
    private void updateDragonEnderCrystal() {
        ...
        if (this.rand.nextInt(10) == 0) {
            float f2 = 32.0f;
            List<EntityEnderCrystal> list = this.worldObj.getEntitiesWithinAABB(EntityEnderCrystal.class, this.getEntityBoundingBox().expand(f2, f2, f2));
    ...
    private boolean destroyBlocksInAABB(AxisAlignedBB p_70972_1_) {
        int i2 = MathHelper.floor_double(p_70972_1_.minX);
        int j2 = MathHelper.floor_double(p_70972_1_.minY);
        int k2 = MathHelper.floor_double(p_70972_1_.minZ);
        int l2 = MathHelper.floor_double(p_70972_1_.maxX);
    ...
    public boolean attackEntityFromPart(EntityDragonPart dragonPart, DamageSource source, float p_70965_3_) {
        ...
        this.targetZ = this.posZ - (double)(f22 * 5.0f) + (double)((this.rand.nextFloat() - 0.5f) * 2.0f);
        this.target = null;
    ...
    protected void onDeathUpdate() {
        ...
        if (this.deathTicks >= 180 && this.deathTicks <= 200) {
            ...
            this.worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, this.posX + (double)f2, this.posY + 2.0 + (double)f1, this.posZ + (double)f22, 0.0, 0.0, 0.0, new int[0]);
        }
        boolean flag = this.worldObj.getGameRules().getBoolean("doMobLoot");
        if (!this.worldObj.isRemote) {
            if (this.deathTicks > 150 && this.deathTicks % 5 == 0 && flag) {
                int k2;
                for (int i2 = 1000; i2 > 0; i2 -= k2) {
                    k2 = EntityXPOrb.getXPSplit(i2);
                    this.worldObj.spawnEntityInWorld(new EntityXPOrb(this.worldObj, this.posX, this.posY, this.posZ, k2));
                }
            }
        ...
        if (this.deathTicks == 200 && !this.worldObj.isRemote) {
            if (flag) {
                int l2;
                for (int j2 = 2000; j2 > 0; j2 -= l2) {
                    l2 = EntityXPOrb.getXPSplit(j2);
                    this.worldObj.spawnEntityInWorld(new EntityXPOrb(this.worldObj, this.posX, this.posY, this.posZ, l2));
                }
            }
        ...
        for (int j2 = -1; j2 <= 32; ++j2) {
            for (int k2 = -4; k2 <= 4; ++k2) {
                for (int l2 = -4; l2 <= 4; ++l2) {
                    double d2 = k2 * k2 + l2 * l2;
                    if (!(d2 <= 12.25)) continue;
                    BlockPos blockpos = pos.add(k2, j2, l2);
                    if (j2 < 0) {
                        if (!(d2 <= 6.25)) continue;
