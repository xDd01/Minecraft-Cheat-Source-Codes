...
public class EntityList {
    ...
    private static void addMapping(Class<? extends Entity> entityClass, String entityName, int id2) {
        if (stringToClassMapping.containsKey(entityName)) {
            throw new IllegalArgumentException("ID is already registered: " + entityName);
        }
        if (idToClassMapping.containsKey(id2)) {
            throw new IllegalArgumentException("ID is already registered: " + id2);
        }
        if (id2 == 0) {
            throw new IllegalArgumentException("Cannot register to reserved id: " + id2);
        }
        if (entityClass == null) {
            throw new IllegalArgumentException("Cannot register null clazz for id: " + id2);
        }
        stringToClassMapping.put(entityName, entityClass);
        classToStringMapping.put(entityClass, entityName);
        idToClassMapping.put(id2, entityClass);
        classToIDMapping.put(entityClass, id2);
        stringToIDMapping.put(entityName, id2);
    }
    ...
    public static class EntityEggInfo {
        ...
        public EntityEggInfo(int id2, int baseColor, int spotColor) {
            this.spawnedID = id2;
