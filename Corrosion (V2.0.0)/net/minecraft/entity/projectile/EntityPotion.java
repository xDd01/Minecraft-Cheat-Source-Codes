...
extends EntityThrowable {
    ...
    public EntityPotion(World worldIn, double x2, double y2, double z2, int p_i1791_8_) {
        this(worldIn, x2, y2, z2, new ItemStack(Items.potionitem, 1, p_i1791_8_));
    }
            ...
            if (list != null && !list.isEmpty() && !(list1 = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, axisalignedbb = this.getEntityBoundingBox().expand(4.0, 2.0, 4.0))).isEmpty()) {
                for (EntityLivingBase entitylivingbase : list1) {
                    ...
                    for (PotionEffect potioneffect : list) {
                        int i2 = potioneffect.getPotionID();
                        if (Potion.potionTypes[i2].isInstant()) {
                            Potion.potionTypes[i2].affectEntity(this, this.getThrower(), entitylivingbase, potioneffect.getAmplifier(), d1);
                            continue;
                        }
                        int j2 = (int)(d1 * (double)potioneffect.getDuration() + 0.5);
                        if (j2 <= 20) continue;
                        entitylivingbase.addPotionEffect(new PotionEffect(i2, j2, potioneffect.getAmplifier()));
                    }
                }
            }
