...
extends Entity {
    ...
    public EntityFishHook(World worldIn, double x2, double y2, double z2, EntityPlayer anglerIn) {
        this(worldIn);
    ...
    public EntityFishHook(World worldIn, EntityPlayer fishingPlayer) {
        ...
        float f2 = 0.4f;
        this.motionX = -MathHelper.sin(this.rotationYaw / 180.0f * (float)Math.PI) * MathHelper.cos(this.rotationPitch / 180.0f * (float)Math.PI) * f2;
        this.motionZ = MathHelper.cos(this.rotationYaw / 180.0f * (float)Math.PI) * MathHelper.cos(this.rotationPitch / 180.0f * (float)Math.PI) * f2;
        this.motionY = -MathHelper.sin(this.rotationPitch / 180.0f * (float)Math.PI) * f2;
        ...
        if (this.fishPosRotationIncrements > 0) {
            ...
            if (!this.inGround) {
                ...
                for (int k2 = 0; k2 < j2; ++k2) {
                    ...
                    double d4 = axisalignedbb1.minY + d3 * (double)k2 / (double)j2;
                    double d5 = axisalignedbb1.minY + d3 * (double)(k2 + 1) / (double)j2;
    ...
    private ItemStack getFishingResult() {
        float f2 = this.worldObj.rand.nextFloat();
        int i2 = EnchantmentHelper.getLuckOfSeaModifier(this.angler);
        int j2 = EnchantmentHelper.getLureModifier(this.angler);
        float f1 = 0.1f - (float)i2 * 0.025f - (float)j2 * 0.01f;
        float f22 = 0.05f + (float)i2 * 0.01f - (float)j2 * 0.01f;
        f1 = MathHelper.clamp_float(f1, 0.0f, 1.0f);
        f22 = MathHelper.clamp_float(f22, 0.0f, 1.0f);
        if (f2 < f1) {
