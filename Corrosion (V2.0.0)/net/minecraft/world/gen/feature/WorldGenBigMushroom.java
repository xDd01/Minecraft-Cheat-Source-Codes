            ...
            for (int j2 = position.getY(); j2 <= position.getY() + 1 + i2; ++j2) {
                ...
                for (int l2 = position.getX() - k2; l2 <= position.getX() + k2 && flag; ++l2) {
                    for (int i1 = position.getZ() - k2; i1 <= position.getZ() + k2 && flag; ++i1) {
                        if (j2 >= 0 && j2 < 256) {
                            Block block = worldIn.getBlockState(blockpos$mutableblockpos.func_181079_c(l2, j2, i1)).getBlock();
            ...
            for (int l2 = k2; l2 <= position.getY() + i2; ++l2) {
                ...
                for (int l1 = k3; l1 <= l3; ++l1) {
                    for (int i22 = j1; i22 <= k1; ++i22) {
                        ...
                        if (this.mushroomType == Blocks.brown_mushroom_block || l2 < position.getY() + i2) {
                        ...
                        if (position.getY() < position.getY() + i2 - 1 && blockhugemushroom$enumtype == BlockHugeMushroom.EnumType.ALL_INSIDE || worldIn.getBlockState(blockpos = new BlockPos(l1, l2, i22)).getBlock().isFullBlock()) continue;
