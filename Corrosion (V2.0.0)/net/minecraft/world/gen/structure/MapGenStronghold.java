...
extends MapGenStructure {
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        if (!this.ranBiomeCheck) {
            ...
            int i2 = 1;
            for (int j2 = 0; j2 < this.structureCoords.length; ++j2) {
                double d1 = (1.25 * (double)i2 + random.nextDouble()) * this.field_82671_h * (double)i2;
                int k2 = (int)Math.round(Math.cos(d0) * d1);
                int l2 = (int)Math.round(Math.sin(d0) * d1);
                BlockPos blockpos = this.worldObj.getWorldChunkManager().findBiomePosition((k2 << 4) + 8, (l2 << 4) + 8, 112, this.field_151546_e, random);
                if (blockpos != null) {
                    k2 = blockpos.getX() >> 4;
                    l2 = blockpos.getZ() >> 4;
                }
                this.structureCoords[j2] = new ChunkCoordIntPair(k2, l2);
                d0 += Math.PI * 2 * (double)i2 / (double)this.field_82672_i;
                if (j2 != this.field_82672_i) continue;
                i2 += 2 + random.nextInt(5);
    ...
    extends StructureStart {
        ...
        public Start(World worldIn, Random p_i2067_2_, int p_i2067_3_, int p_i2067_4_) {
            ...
            while (!list.isEmpty()) {
                int i2 = p_i2067_2_.nextInt(list.size());
