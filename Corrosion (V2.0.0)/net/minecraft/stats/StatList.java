...
public class StatList {
    ...
    private static void initCraftableStats() {
        ...
        for (Item item : set) {
            ...
            String s2 = StatList.func_180204_a(item);
            if (s2 == null) continue;
            StatList.objectCraftStats[i2] = new StatCrafting("stat.craftItem.", s2, new ChatComponentTranslation("stat.craftItem", new ItemStack(item).getChatComponent()), item).registerStat();
        }
    ...
    private static void initMiningStats() {
        for (Block block : Block.blockRegistry) {
            ...
            StatList.mineBlockStatArray[i2] = new StatCrafting("stat.mineBlock.", s2, new ChatComponentTranslation("stat.mineBlock", new ItemStack(block).getChatComponent()), item).registerStat();
            objectMineStats.add((StatCrafting)mineBlockStatArray[i2]);
        }
    ...
    private static void initStats() {
        for (Item item : Item.itemRegistry) {
            ...
            String s2 = StatList.func_180204_a(item);
            if (s2 == null) continue;
            StatList.objectUseStats[i2] = new StatCrafting("stat.useItem.", s2, new ChatComponentTranslation("stat.useItem", new ItemStack(item).getChatComponent()), item).registerStat();
            if (item instanceof ItemBlock) continue;
            itemStats.add((StatCrafting)objectUseStats[i2]);
        }
    ...
    private static void initItemDepleteStats() {
        for (Item item : Item.itemRegistry) {
            ...
            StatList.objectBreakStats[i2] = new StatCrafting("stat.breakItem.", s2, new ChatComponentTranslation("stat.breakItem", new ItemStack(item).getChatComponent()), item).registerStat();
        }
    ...
    private static void mergeStatBases(StatBase[] statBaseIn, Block p_151180_1_, Block p_151180_2_) {
        int i2 = Block.getIdFromBlock(p_151180_1_);
        int j2 = Block.getIdFromBlock(p_151180_2_);
        if (statBaseIn[i2] != null && statBaseIn[j2] == null) {
            statBaseIn[j2] = statBaseIn[i2];
        } else {
            allStats.remove(statBaseIn[i2]);
            objectMineStats.remove(statBaseIn[i2]);
            generalStats.remove(statBaseIn[i2]);
            statBaseIn[i2] = statBaseIn[j2];
        }
    }

    public static StatBase getStatKillEntity(EntityList.EntityEggInfo eggInfo) {
        String s2 = EntityList.getStringFromID(eggInfo.spawnedID);
        return s2 == null ? null : new StatBase("stat.killEntity." + s2, new ChatComponentTranslation("stat.entityKill", new ChatComponentTranslation("entity." + s2 + ".name", new Object[0]))).registerStat();
    }

    public static StatBase getStatEntityKilledBy(EntityList.EntityEggInfo eggInfo) {
        String s2 = EntityList.getStringFromID(eggInfo.spawnedID);
        return s2 == null ? null : new StatBase("stat.entityKilledBy." + s2, new ChatComponentTranslation("stat.entityKilledBy", new ChatComponentTranslation("entity." + s2 + ".name", new Object[0]))).registerStat();
    }
